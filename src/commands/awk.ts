	...
	run: ({ authUser, args, stdin, cwd, shell }) => {
		...
		if (fileArg) {
			...
			try {
				...
				return {
					stderr: `awk: ${fileArg}: No such file or directory`,
					exitCode: 1,
				};
			}
		}
		...
		function evalExpr(
			...
			if (/^\$\d+$/.test(expr)) {
				return fields[Number.parseInt(expr.slice(1), 10) - 1] ?? "";
			}
			if (/^\$/.test(expr)) {
			...
			if (substrM) {
				...
				const len2 =
					parts2[2] === undefined
						? undefined
						: numVal(evalExpr(parts2[2].trim(), vars, fields, nr, nf));
				return len2 === undefined
					? s.slice(Math.max(0, start))
					: s.slice(Math.max(0, start), start + len2);
			}
			...
			if (ternM) {
				...
				return numVal(cond) !== 0 || (typeof cond === "string" && cond !== "")
					...
					: evalExpr((ternM[3] as string).trim(), vars, fields, nr, nf);
			}
			...
			if (concatM) {
				return (
					strVal(evalExpr(concatM[1] as string, vars, fields, nr, nf)) +
					strVal(evalExpr(concatM[2] as string, vars, fields, nr, nf))
				);
			}
		...
		function execStmt(
			...
			if (splitM) {
				...
				const fs2 = parts2[2]
					? strVal(evalExpr(parts2[2].trim(), vars, fields, nr, nf))
					: sep;
			...
			if (incM) {
				vars[incM[1] as string] =
					numVal(vars[incM[1] as string]) + (incM[2] === "++" ? 1 : -1);
				return "ok";
			}
			...
			if (assignM) {
				vars[assignM[1] as string] = evalExpr(
					assignM[2] as string,
					vars,
					fields,
					nr,
		...
		function sprintfAWK(
			...
			const fmtArgs = parts2
				.slice(1)
				.map((p) => evalExpr(p.trim(), vars, fields, nr, nf));
			let ai = 0;
			return fmt.replace(
				/%(-?\d*\.?\d*)?([diouxXeEfgGsq%])/g,
				(_, spec, type2) => {
		...
		// Split top-level { } blocks respecting nesting and strings
		{
			...
			while (j < progTrim.length) {
				// Skip whitespace
				while (j < progTrim.length && /\s/.test(progTrim.charAt(j))) {
					j++;
				}
		...
		if (clauses.length === 0) {
			clauses.push({
				pattern: "",
				action: progTrim.replace(/[{}]/g, "").trim(),
			});
		}
		...
		function matchClause(
			...
			if (fieldMatch) {
				...
				try {
					return new RegExp(fieldMatch[2] as string).test(fv);
				} catch {
					return false;
				}
			}
