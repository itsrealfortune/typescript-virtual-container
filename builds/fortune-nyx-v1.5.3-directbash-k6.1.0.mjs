#!/usr/bin/env node
import{readFile as Oo,unlink as To,writeFile as Fo}from"node:fs/promises";import*as Ls from"node:path";import{basename as Ro}from"node:path";import{stdin as de,stdout as ae}from"node:process";import{createInterface as Do}from"node:readline";var en={name:"adduser",description:"Add a new user",category:"users",params:["<username>"],run:({authUser:e,shell:t,args:n})=>{if(e!=="root")return{stderr:`adduser: permission denied
...
`)||void 0,exitCode:0}}},rn={name:"unalias",description:"Remove alias definitions",category:"shell",params:["<name...> | -a"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};if(v(e,["-a"])){for(let n of Object.keys(t.vars))n.startsWith("__alias_")&&delete t.vars[n];return{exitCode:0}}for(let n of e)delete t.vars[`__alias_${n}`];return{exitCode:0}}};import*as fe from"node:path";var Vs=["/.virtual-env-js/.auth","/etc/htpasswd"];function I(e,t,n){if(!t||t.trim()==="")return e;if(t.startsWith("~")){let r=n??"/root";return fe.posix.normalize(`${r}${t.slice(1)}`)}return t.startsWith("/")?fe.posix.normalize(t):fe.posix.normalize(fe.posix.join(e,t))}function zs(e){let t=e.startsWith("/")?fe.posix.normalize(e):fe.posix.normalize(`/${e}`);return Vs.some(n=>t===n||t.startsWith(`${n}/`))}function L(e,t,n){if(e!=="root"&&zs(t))throw new Error(`${n}: permission denied: ${t}`)}function sn(e){let n=(e.split("?")[0]?.split("#")[0]??e).split("/").filter(Boolean).pop();return n&&n.length>0?n:"index.html"}function Bs(e,t){let n=Array.from({length:e.length+1},()=>Array(t.length+1).fill(0));for(let r=0;r<=e.length;r+=1)n[r][0]=r;for(let r=0;r<=t.length;r+=1)n[0][r]=r;for(let r=1;r<=e.length;r+=1)for(let i=1;i<=t.length;i+=1){let s=e[r-1]===t[i-1]?0:1;n[r][i]=Math.min(n[r-1][i]+1,n[r][i-1]+1,n[r-1][i-1]+s)}return n[e.length][t.length]}function on(e,t,n){let r=I(t,n);if(e.exists(r))return r;let i=fe.posix.dirname(r),s=fe.posix.basename(r),o=e.list(i),a=o.filter(c=>c.toLowerCase()===s.toLowerCase());if(a.length===1)return fe.posix.join(i,a[0]);let l=o.filter(c=>Bs(c.toLowerCase(),s.toLowerCase())<=1);return l.length===1?fe.posix.join(i,l[0]):r}function Te(e){return e.packageManager}var an={name:"apt",aliases:["apt-get"],description:"Package manager",category:"package",params:["<install|remove|update|upgrade|search|show|list> [pkg...]"],run:({args:e,shell:t,authUser:n})=>{let r=Te(t);if(!r)return{stderr:"apt: package manager not initialised",exitCode:1};let i=e[0]?.toLowerCase(),s=e.slice(1),o=v(s,["-q","--quiet","-qq"]),a=v(s,["--purge"]),l=s.filter(u=>!u.startsWith("-"));if(["install","remove","purge","upgrade","update"].includes(i??"")&&n!=="root")return{stderr:`E: Could not open lock file /var/lib/dpkg/lock-frontend - open (13: Permission denied)
...
`),exitCode:0}}default:return{stderr:`apt-cache: unknown command '${r??""}'`,exitCode:1}}}};var cn={name:"awk",description:"Pattern scanning and processing language",category:"text",params:["[-F <sep>] '<program>' [file]"],run:({authUser:e,args:t,stdin:n,cwd:r,shell:i})=>{let s=ue(t,["-F"])??" ",o=t.filter(w=>!w.startsWith("-")&&w!==s),a=o[0],l=o[1];if(!a)return{stderr:"awk: no program",exitCode:1};let c=n??"";if(l){let w=I(r,l);try{L(e,w,"awk"),c=i.vfs.readFile(w)}catch{return{stderr:`awk: ${l}: No such file or directory`,exitCode:1}}}let u=c.split(`
...
`)+(p.length>0?`
`:""),exitCode:0}}};var un={name:"base64",description:"Encode/decode base64",category:"text",params:["[-d] [file]"],run:({args:e,stdin:t})=>{let n=v(e,["-d","--decode"]),r=t??"";if(n)try{return{stdout:Buffer.from(r.trim(),"base64").toString("utf8"),exitCode:0}}catch{return{stderr:"base64: invalid input",exitCode:1}}return{stdout:Buffer.from(r).toString("base64"),exitCode:0}}};var dn={name:"cat",description:"Concatenate and print files",category:"files",params:["[-n] [-b] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=v(r,["-n","--number"]),o=v(r,["-b","--number-nonblank"]),a=r.filter(m=>!m.startsWith("-"));if(a.length===0&&i!==void 0)return{stdout:i,exitCode:0};if(a.length===0)return{stderr:"cat: missing file operand",exitCode:1};let l=[];for(let m of a){let p=on(t.vfs,n,m);L(e,p,"cat"),l.push(t.vfs.readFile(p))}let c=l.join("");if(!s&&!o)return{stdout:c,exitCode:0};let u=1;return{stdout:c.split(`
`).map(m=>o&&m.trim()===""?m:`${String(u++).padStart(6)}	${m}`).join(`
`),exitCode:0}}};async function mn(e,t,n,r,i,s,o){let a={exitCode:0},l=[],c=i,u=0;for(;u<e.length;){let m=e[u];if(a=await js(m.pipeline,t,n,r,c,s,o),o.lastExitCode=a.exitCode??0,a.nextCwd&&(a.exitCode??0)===0&&(c=a.nextCwd),a.stdout&&l.push(a.stdout),a.closeSession||a.switchUser)return{...a,stdout:l.join("")||a.stdout};let p=m.op;if(!(!p||p===";")){if(p==="&&"){if((a.exitCode??0)!==0)for(;u<e.length&&e[u]?.op==="&&";)u++}else if(p==="||"&&(a.exitCode??0)===0)for(;u<e.length&&e[u]?.op==="||";)u++}u++}let d=l.join("");return{...a,stdout:d||a.stdout,nextCwd:c!==i?c:void 0}}async function js(e,t,n,r,i,s,o){if(!e.isValid)return{stderr:e.error||"Syntax error",exitCode:1};if(e.commands.length===0)return{exitCode:0};let a=o??{vars:{},lastExitCode:0};return e.commands.length===1?Ws(e.commands[0],t,n,r,i,s,a):Hs(e.commands,t,n,r,i,s,a)}async function Ws(e,t,n,r,i,s,o){let a;if(e.inputFile){let c=I(i,e.inputFile);try{a=s.vfs.readFile(c)}catch{return{stderr:`${e.inputFile}: No such file or directory`,exitCode:1}}}let l=await Re(e.name,e.args,t,n,r,i,s,a,o);if(e.outputFile){let c=I(i,e.outputFile),u=l.stdout||"";try{if(e.appendOutput){let d=(()=>{try{return s.vfs.readFile(c)}catch{return""}})();s.writeFileAsUser(t,c,d+u)}else s.writeFileAsUser(t,c,u);return{...l,stdout:""}}catch{return{...l,stderr:`Failed to write to ${e.outputFile}`,exitCode:1}}}return l}async function Hs(e,t,n,r,i,s,o){let a="",l=0;for(let c=0;c<e.length;c++){let u=e[c];if(c===0&&u.inputFile){let m=I(i,u.inputFile);try{a=s.vfs.readFile(m)}catch{return{stderr:`${u.inputFile}: No such file or directory`,exitCode:1}}}let d=await Re(u.name,u.args,t,n,r,i,s,a,o);if(l=d.exitCode??0,c===e.length-1&&u.outputFile){let m=I(i,u.outputFile),p=d.stdout||"";try{if(u.appendOutput){let g=(()=>{try{return s.vfs.readFile(m)}catch{return""}})();s.writeFileAsUser(t,m,g+p)}else s.writeFileAsUser(t,m,p);a=""}catch{return{stderr:`Failed to write to ${u.outputFile}`,exitCode:1}}}else a=d.stdout||"";if(d.stderr&&l!==0)return{stderr:d.stderr,exitCode:l};if(d.closeSession||d.switchUser)return d}return{stdout:a,exitCode:l}}function Be(e){let t=[],n="",r=!1,i="",s=0;for(;s<e.length;){let o=e[s],a=e[s+1];if((o==='"'||o==="'")&&!r){r=!0,i=o,s++;continue}if(r&&o===i){r=!1,i="",s++;continue}if(r){n+=o,s++;continue}if(o===" "){n&&(t.push(n),n=""),s++;continue}if(!r&&o==="2"&&a===">"){let l=e.slice(s+1);if(l.startsWith(">>&1")||l.startsWith(">> &1")){n&&(t.push(n),n=""),t.push("2>>&1"),s+=5;continue}if(l.startsWith(">&1")){n&&(t.push(n),n=""),t.push("2>&1"),s+=4;continue}if(l.startsWith(">>")){n&&(t.push(n),n=""),t.push("2>>"),s+=3;continue}if(l.startsWith(">")){n&&(t.push(n),n=""),t.push("2>"),s+=2;continue}}if((o===">"||o==="<")&&!r){n&&(t.push(n),n=""),o===">"&&a===">"?(t.push(">>"),s+=2):(t.push(o),s++);continue}n+=o,s++}return n&&t.push(n),t}function pn(e){let t=e.trim();if(!t)return{statements:[],isValid:!0};try{return{statements:qs(t),isValid:!0}}catch(n){return{statements:[],isValid:!1,error:n.message}}}function qs(e){let t=Gs(e),n=[];for(let r of t){let s={pipeline:{commands:Ys(r.text.trim()),isValid:!0}};r.op&&(s.op=r.op),n.push(s)}return n}function Gs(e){let t=[],n="",r=0,i=!1,s="",o=0,a=l=>{n.trim()&&t.push({text:n,op:l}),n=""};for(;o<e.length;){let l=e[o],c=e.slice(o,o+2);if((l==='"'||l==="'")&&!i){i=!0,s=l,n+=l,o++;continue}if(i&&l===s){i=!1,n+=l,o++;continue}if(i){n+=l,o++;continue}if(l==="("){r++,n+=l,o++;continue}if(l===")"){r--,n+=l,o++;continue}if(r>0){n+=l,o++;continue}if(c==="&&"){a("&&"),o+=2;continue}if(c==="||"){a("||"),o+=2;continue}if(l===";"){a(";"),o++;continue}n+=l,o++}return a(),t}function Ys(e){return Ks(e).map(Js)}function Ks(e){let t=[],n="",r=!1,i="";for(let o=0;o<e.length;o++){let a=e[o];if((a==='"'||a==="'")&&!r){r=!0,i=a,n+=a;continue}if(r&&a===i){r=!1,n+=a;continue}if(r){n+=a;continue}if(a==="|"&&e[o+1]!=="|"){if(!n.trim())throw new Error("Syntax error near unexpected token '|'");t.push(n.trim()),n=""}else n+=a}let s=n.trim();if(!s&&t.length>0)throw new Error("Syntax error near unexpected token '|'");return s&&t.push(s),t}function Js(e){let t=Be(e);if(t.length===0)return{name:"",args:[]};let n=[],r,i,s=!1,o=0,a,l=!1,c=!1;for(;o<t.length;){let m=t[o];if(m==="<"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after <");r=t[o],o++}else if(m===">>"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after >>");i=t[o],s=!0,o++}else if(m===">"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after >");i=t[o],s=!1,o++}else if(m==="2>&1")c=!0,o++;else if(m==="2>>"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after 2>>");a=t[o],l=!0,o++}else if(m==="2>"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after 2>");a=t[o],l=!1,o++}else n.push(m),o++}let u=n[0]??"";return{name:/^([A-Za-z_][A-Za-z0-9_]*)=(.*)$/.test(u)?u:u.toLowerCase(),args:n.slice(1),inputFile:r,outputFile:i,appendOutput:s,stderrFile:a,stderrAppend:l,stderrToStdout:c}}function Zs(e,t){let n=[],r=0;for(;r<e.length;){let i=e[r];if(/\s/.test(i)){r++;continue}if(i==="+"){n.push({type:"plus"}),r++;continue}if(i==="-"){n.push({type:"minus"}),r++;continue}if(i==="*"){if(e[r+1]==="*"){n.push({type:"pow"}),r+=2;continue}n.push({type:"mul"}),r++;continue}if(i==="/"){n.push({type:"div"}),r++;continue}if(i==="%"){n.push({type:"mod"}),r++;continue}if(i==="("){n.push({type:"lparen"}),r++;continue}if(i===")"){n.push({type:"rparen"}),r++;continue}if(/\d/.test(i)){let s=r+1;for(;s<e.length&&/\d/.test(e[s]);)s++;n.push({type:"number",value:Number(e.slice(r,s))}),r=s;continue}if(/[A-Za-z_]/.test(i)){let s=r+1;for(;s<e.length&&/[A-Za-z0-9_]/.test(e[s]);)s++;let o=e.slice(r,s),a=t[o],l=a===void 0||a===""?0:Number(a);n.push({type:"number",value:Number.isFinite(l)?l:0}),r=s;continue}return[]}return n}function At(e,t){let n=e.trim();if(n.length===0||n.length>1024)return NaN;let r=Zs(n,t);if(r.length===0)return NaN;let i=0,s=()=>r[i],o=()=>r[i++],a=()=>{let p=o();if(!p)return NaN;if(p.type==="number")return p.value;if(p.type==="lparen"){let g=d();return r[i]?.type!=="rparen"?NaN:(i++,g)}return NaN},l=()=>{let p=s();return p?.type==="plus"?(o(),l()):p?.type==="minus"?(o(),-l()):a()},c=()=>{let p=l();for(;s()?.type==="pow";){o();let g=l();p=p**g}return p},u=()=>{let p=c();for(;;){let g=s();if(g?.type==="mul"){o(),p*=c();continue}if(g?.type==="div"){o();let S=c();p=S===0?NaN:p/S;continue}if(g?.type==="mod"){o();let S=c();p=S===0?NaN:p%S;continue}return p}},d=()=>{let p=u();for(;;){let g=s();if(g?.type==="plus"){o(),p+=u();continue}if(g?.type==="minus"){o(),p-=u();continue}return p}},m=d();return!Number.isFinite(m)||i!==r.length?NaN:Math.trunc(m)}function Qs(e,t){let n=[],r=0;for(;r<e.length;){let i=e.indexOf("'",r);if(i===-1){n.push(t(e.slice(r)));break}n.push(t(e.slice(r,i)));let s=e.indexOf("'",i+1);if(s===-1){n.push(e.slice(i));break}n.push(e.slice(i,s+1)),r=s+1}return n.join("")}function nt(e){function r(i,s){if(s>8)return[i];let o=0,a=-1;for(let l=0;l<i.length;l++){let c=i[l];if(c==="{"&&i[l-1]!=="$")o===0&&(a=l),o++;else if(c==="}"&&(o--,o===0&&a!==-1)){let u=i.slice(0,a),d=i.slice(a+1,l),m=i.slice(l+1),p=d.match(/^(-?\d+)\.\.(-?\d+)(?:\.\.-?(\d+))?$/)||d.match(/^([a-z])\.\.([a-z])$/);if(p){let P=[];if(/\d/.test(p[1])){let w=parseInt(p[1],10),A=parseInt(p[2],10),N=p[3]?parseInt(p[3],10):1,b=w<=A?N:-N;for(let E=w;w<=A?E<=A:E>=A;E+=b)P.push(String(E))}else{let w=p[1].charCodeAt(0),A=p[2].charCodeAt(0),N=w<=A?1:-1;for(let b=w;w<=A?b<=A:b>=A;b+=N)P.push(String.fromCharCode(b))}let x=P.map(w=>`${u}${w}${m}`),_=[];for(let w of x)if(_.push(...r(w,s+1)),_.length>256)return[i];return _}let g=[],S="",C=0;for(let P of d)P==="{"?(C++,S+=P):P==="}"?(C--,S+=P):P===","&&C===0?(g.push(S),S=""):S+=P;if(g.push(S),g.length>1){let P=[];for(let x of g)if(P.push(...r(`${u}${x}${m}`,s+1)),P.length>256)return[i];return P}break}}return[i]}return r(e,0)}function Xs(e,t){let n="",r=0;for(;r<e.length;){if(e[r]==="$"&&e[r+1]==="("&&e[r+2]==="("){let i=r+3,s=0;for(;i<e.length;){let o=e[i];if(o==="(")s++;else if(o===")"){if(s>0)s--;else if(e[i+1]===")"){let a=e.slice(r+3,i),l=At(a,t);n+=Number.isNaN(l)?"0":String(l),r=i+2;break}}i++}if(i>=e.length){n+=e.slice(r);break}continue}n+=e[r],r++}return n}function tt(e,t,n=0,r){let i=r??t.HOME??"/home/user";return Qs(e,s=>{let o=s;return o=o.replace(/(^|[\s:])~(\/|$)/g,(a,l,c)=>`${l}${i}${c}`),o=o.replace(/\$\?/g,String(n)),o=o.replace(/\$\$/g,"1"),o=o.replace(/\$#/g,"0"),o=Xs(o,t),o=o.replace(/\$\{#([A-Za-z_][A-Za-z0-9_]*)\}/g,(a,l)=>String((t[l]??"").length)),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*):-([^}]*)\}/g,(a,l,c)=>t[l]!==void 0&&t[l]!==""?t[l]:c),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*):=([^}]*)\}/g,(a,l,c)=>((t[l]===void 0||t[l]==="")&&(t[l]=c),t[l])),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*):\+([^}]*)\}/g,(a,l,c)=>t[l]!==void 0&&t[l]!==""?c:""),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g,(a,l)=>t[l]??""),o=o.replace(/\$([A-Za-z_][A-Za-z0-9_]*|\d+)/g,(a,l)=>t[l]??""),o})}async function rt(e,t,n,r){let i="__shellExpandDepth",o=Number(t[i]??"0");if(o>=8)return tt(e,t,n);t[i]=String(o+1);try{if(e.includes("$(")){let a="",l=!1,c=0;for(;c<e.length;){let u=e[c];if(u==="'"&&!l){l=!0,a+=u,c++;continue}if(u==="'"&&l){l=!1,a+=u,c++;continue}if(!l&&u==="$"&&e[c+1]==="("){if(e[c+2]==="("){a+=u,c++;continue}let d=0,m=c+1;for(;m<e.length;){if(e[m]==="(")d++;else if(e[m]===")"&&(d--,d===0))break;m++}let p=e.slice(c+2,m).trim(),g=(await r(p)).replace(/\n$/,"");a+=g,c=m+1;continue}a+=u,c++}e=a}return tt(e,t,n)}finally{o<=0?delete t[i]:t[i]=String(o)}}function q(e){return e==="root"?"/root":`/home/${e}`}function Fe(e,t){return{vars:{PATH:"/usr/local/bin:/usr/bin:/bin",HOME:q(e),USER:e,LOGNAME:e,SHELL:"/bin/sh",TERM:"xterm-256color",HOSTNAME:t,PS1:"\\u@\\h:\\w\\$ "},lastExitCode:0}}function fn(e,t,n,r){if(e.startsWith("/")){if(!n.vfs.exists(e))return null;try{let s=n.vfs.stat(e);return s.type!=="file"||!(s.mode&73)||(e.startsWith("/sbin/")||e.startsWith("/usr/sbin/"))&&r!=="root"?null:e}catch{return null}}let i=(t.vars.PATH??"/usr/local/bin:/usr/bin:/bin").split(":");for(let s of i){if((s==="/sbin"||s==="/usr/sbin")&&r!=="root")continue;let o=`${s}/${e}`;if(n.vfs.exists(o))try{let a=n.vfs.stat(o);if(a.type!=="file"||!(a.mode&73))continue;return o}catch{}}return null}var st=8,ke=0;async function Re(e,t,n,r,i,s,o,a,l){if(ke++,ke>st)return ke--,{stderr:`${e}: maximum call depth (${st}) exceeded`,exitCode:126};try{return await ei(e,t,n,r,i,s,o,a,l)}finally{ke--}}async function ei(e,t,n,r,i,s,o,a,l){let c=/^([A-Za-z_][A-Za-z0-9_]*)=(.*)$/,u=[e,...t],d=0;for(;d<u.length&&c.test(u[d]);)d+=1;if(d>0){let g=u.slice(0,d).map(P=>P.match(c)),S=u.slice(d),C=[];for(let[,P,x]of g)C.push([P,l.vars[P]]),l.vars[P]=x;if(S.length===0)return{exitCode:0};try{return await Re(S[0],S.slice(1),n,r,i,s,o,a,l)}finally{for(let[P,x]of C)x===void 0?delete l.vars[P]:l.vars[P]=x}}let m=l.vars[`__alias_${e}`];if(m)return Z(`${m} ${t.join(" ")}`,n,r,i,s,o,a,l);let p=he(e);if(!p){let g=fn(e,l,o,n);if(g){let S=o.vfs.readFile(g),C=S.match(/exec\s+builtin\s+(\S+)/);if(C){let x=he(C[1]);return x?await x.run({authUser:n,hostname:r,activeSessions:o.users.listActiveSessions(),rawInput:[e,...t].join(" "),mode:i,args:t,stdin:a,cwd:s,shell:o,env:l}):{stderr:`${e}: exec builtin '${C[1]}' not found`,exitCode:127}}let P=he("sh");if(P)return await P.run({authUser:n,hostname:r,activeSessions:o.users.listActiveSessions(),rawInput:`sh -c ${JSON.stringify(S)}`,mode:i,args:["-c",S,"--",...t],stdin:a,cwd:s,shell:o,env:l})}return{stderr:`${e}: command not found`,exitCode:127}}try{return await p.run({authUser:n,hostname:r,activeSessions:o.users.listActiveSessions(),rawInput:[e,...t].join(" "),mode:i,args:t,stdin:a,cwd:s,shell:o,env:l})}catch(g){return{stderr:g instanceof Error?g.message:"Command failed",exitCode:1}}}async function Z(e,t,n,r,i,s,o,a){let l=e.trim();if(l.length===0)return{exitCode:0};let c=a??Fe(t,n);if(ke++,ke>st)return ke--,{stderr:`${l.split(" ")[0]}: maximum call depth (${st}) exceeded`,exitCode:126};try{let d=Be(l)[0]?.toLowerCase()??"",m=c.vars[`__alias_${d}`],p=m?l.replace(d,m):l,g=/\bfor\s+\w+\s+in\b/.test(p)||/\bwhile\s+/.test(p)||/\bif\s+/.test(p)||/\w+\s*\(\s*\)\s*\{/.test(p)||/\bfunction\s+\w+/.test(p)||/\(\(\s*.+\s*\)\)/.test(p),S=/(?<![|&])[|](?![|])/.test(p)||p.includes(">")||p.includes("<")||p.includes("&&")||p.includes("||")||p.includes(";");if(g&&d!=="sh"&&d!=="bash"||S){if(g&&d!=="sh"&&d!=="bash"){let b=he("sh");if(b)return await b.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:p,mode:r,args:["-c",p],stdin:void 0,cwd:i,shell:s,env:c})}let N=pn(p);if(!N.isValid)return{stderr:N.error||"Syntax error",exitCode:1};try{return await mn(N.statements,t,n,r,i,s,c)}catch(b){return{stderr:b instanceof Error?b.message:"Execution failed",exitCode:1}}}let C=await rt(p,c.vars,c.lastExitCode,N=>Z(N,t,n,r,i,s,void 0,c).then(b=>b.stdout??"")),P=Be(C.trim());if(P.length===0)return{exitCode:0};if(/^([A-Za-z_][A-Za-z0-9_]*)=(.*)$/.test(P[0]))return Re(P[0],P.slice(1),t,n,r,i,s,o,c);let _=P[0]?.toLowerCase()??"",w=P.slice(1).flatMap(nt),A=he(_);if(!A){let N=fn(_,c,s,t);if(N){let b=s.vfs.readFile(N),E=b.match(/exec\s+builtin\s+(\S+)/);if(E){let k=E[1],M=he(k);if(M)return await M.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:[_,...w].join(" "),mode:r,args:w,stdin:o,cwd:i,shell:s,env:c})}let T=he("sh");if(T)return await T.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:`sh -c ${JSON.stringify(b)}`,mode:r,args:["-c",b,"--",...w],stdin:o,cwd:i,shell:s,env:c})}return{stderr:`${_}: command not found`,exitCode:127}}try{return await A.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:C,mode:r,args:w,stdin:o,cwd:i,shell:s,env:c})}catch(N){return{stderr:N instanceof Error?N.message:"Command failed",exitCode:1}}}finally{ke--}}var hn={name:"cd",description:"Change directory",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=I(n,r[0]??"~",q(e));return L(e,i,"cd"),t.vfs.stat(i).type!=="directory"?{stderr:`cd: not a directory: ${i}`,exitCode:1}:{nextCwd:i,exitCode:0}}};function ti(e,t){let n=/^([ugoa]*)([+\-=])([rwx]*)$/,r=t.split(","),i=e;for(let s of r){let o=s.trim().match(n);if(!o)return null;let[,a="a",l,c=""]=o,u=a===""||a==="a"?["u","g","o"]:a.split(""),d={u:{r:256,w:128,x:64},g:{r:32,w:16,x:8},o:{r:4,w:2,x:1}};for(let m of u)for(let p of c.split("")){let g=d[m]?.[p];if(g!==void 0){if(l==="+")i|=g;else if(l==="-")i&=~g;else if(l==="="){let S=Object.values(d[m]??{}).reduce((C,P)=>C|P,0);i=i&~S|g}}}}return i}var gn={name:"chmod",description:"Change file permissions",category:"files",params:["<mode> <file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let[i,s]=r;if(!i||!s)return{stderr:"chmod: missing operand",exitCode:1};let o=I(n,s);try{if(L(e,o,"chmod"),!t.vfs.exists(o))return{stderr:`chmod: ${s}: No such file or directory`,exitCode:1};let a,l=parseInt(i,8);if(!Number.isNaN(l)&&/^[0-7]+$/.test(i))a=l;else{let c=t.vfs.stat(o).mode,u=ti(c,i);if(u===null)return{stderr:`chmod: invalid mode: ${i}`,exitCode:1};a=u}return t.vfs.chmod(o,a),{exitCode:0}}catch(a){return{stderr:`chmod: ${a instanceof Error?a.message:String(a)}`,exitCode:1}}}};var yn={name:"clear",description:"Clear the terminal screen",category:"shell",params:[],run:()=>({clearScreen:!0,stdout:"",exitCode:0})};var Sn={name:"cp",description:"Copy files or directories",category:"files",params:["[-r] <source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=v(r,["-r","-R","--recursive"]),s=r.filter(u=>!u.startsWith("-")),[o,a]=s;if(!o||!a)return{stderr:"cp: missing operand",exitCode:1};let l=I(n,o),c=I(n,a);try{if(L(e,l,"cp"),L(e,c,"cp"),!t.vfs.exists(l))return{stderr:`cp: ${o}: No such file or directory`,exitCode:1};if(t.vfs.stat(l).type==="directory"){if(!i)return{stderr:`cp: ${o}: is a directory (use -r)`,exitCode:1};let d=(p,g)=>{t.vfs.mkdir(g,493);for(let S of t.vfs.list(p)){let C=`${p}/${S}`,P=`${g}/${S}`;if(t.vfs.stat(C).type==="directory")d(C,P);else{let _=t.vfs.readFileRaw(C);t.writeFileAsUser(e,P,_)}}},m=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c;d(l,m)}else{let d=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c,m=t.vfs.readFileRaw(l);t.writeFileAsUser(e,d,m)}return{exitCode:0}}catch(u){return{stderr:`cp: ${u instanceof Error?u.message:String(u)}`,exitCode:1}}}};var bn={name:"curl",description:"Transfer data from or to a server (pure fetch)",category:"network",params:["[options] <url>"],run:async({authUser:e,cwd:t,args:n,shell:r})=>{let{flagsWithValues:i,positionals:s}=le(n,{flagsWithValue:["-o","--output","-X","--request","-d","--data","-H","--header","-u","--user"]});if(v(n,["--help","-h"]))return{stdout:["Usage: curl [options] <url>","  -o, --output <file>    Write to file","  -X, --request <method> HTTP method","  -d, --data <data>      POST data","  -H, --header <hdr>     Extra header","  -s, --silent           Silent mode","  -I, --head             Fetch headers only","  -L, --location         Follow redirects","  -v, --verbose          Verbose"].join(`
...
Type the username to confirm: `,mode:"confirm",onPassword:s},exitCode:0}}};var Pn={name:"df",description:"Report filesystem disk space usage",category:"system",params:["[-h]"],run:({shell:e})=>{let n=(e.vfs.getUsageBytes()/1024).toFixed(0),r="1048576",i=String(Number(r)-Number(n)),s=Math.round(Number(n)/Number(r)*100),o="Filesystem     1K-blocks    Used Available Use% Mounted on",a=`virtual-fs     ${r.padStart(9)} ${n.padStart(7)} ${i.padStart(9)} ${s}% /`;return{stdout:`${o}
${a}`,exitCode:0}}};var $n={name:"diff",description:"Compare files line by line",category:"text",params:["<file1> <file2>"],run:({shell:e,cwd:t,args:n})=>{let[r,i]=n;if(!r||!i)return{stderr:"diff: missing operand",exitCode:1};let s=I(t,r),o=I(t,i),a,l;try{a=e.vfs.readFile(s).split(`
...
`),exitCode:c.length>0?1:0}}};var kn={name:"dpkg",description:"Debian package manager low-level tool",category:"package",params:["[-l] [-s pkg] [-L pkg] [-i pkg] [--remove pkg]"],run:({args:e,authUser:t,shell:n})=>{let r=Te(n);if(!r)return{stderr:"dpkg: package manager not initialised",exitCode:1};let i=v(e,["-l","--list"]),s=v(e,["-s","--status"]),o=v(e,["-L","--listfiles"]),a=v(e,["-r","--remove"]),l=v(e,["-P","--purge"]),{positionals:c}=le(e,{flags:["-l","--list","-s","--status","-L","--listfiles","-r","--remove","-P","--purge"]});if(i){let u=r.listInstalled();if(u.length===0)return{stdout:["Desired=Unknown/Install/Remove/Purge/Hold","|Status=Not/Inst/Conf-files/Unpacked/halF-conf/Half-inst/trig-aWait/Trig-pend","|/ Err?=(none)/Reinst-required (Status,Err: uppercase=bad)","||/ Name           Version         Architecture Description","+++-==============-===============-============-========================================","(no packages installed)"].join(`
...
`)||"(no packages match)",exitCode:0}}return{stderr:"dpkg-query: need a flag (-l, -W)",exitCode:1}}};var Nn={name:"du",description:"Estimate file space usage",category:"system",params:["[-h] [-s] [path]"],run:({shell:e,cwd:t,args:n})=>{let r=v(n,["-h"]),i=v(n,["-s"]),s=n.find(u=>!u.startsWith("-"))??".",o=I(t,s),a=u=>r?`${(u/1024).toFixed(1)}K`:String(Math.ceil(u/1024));if(!e.vfs.exists(o))return{stderr:`du: ${s}: No such file or directory`,exitCode:1};if(i||e.vfs.stat(o).type==="file")return{stdout:`${a(e.vfs.getUsageBytes(o))}	${s}`,exitCode:0};let l=[],c=(u,d)=>{let m=0;for(let p of e.vfs.list(u)){let g=`${u}/${p}`,S=`${d}/${p}`,C=e.vfs.stat(g);C.type==="directory"?m+=c(g,S):(m+=C.size,i||l.push(`${a(C.size)}	${S}`))}return l.push(`${a(m)}	${d}`),m};return c(o,s),{stdout:l.join(`
`),exitCode:0}}};function ni(e){return e.replace(/\\n/g,`
`).replace(/\\t/g,"	").replace(/\\r/g,"\r").replace(/\\\\/g,"\\").replace(/\\a/g,"\x07").replace(/\\b/g,"\b").replace(/\\f/g,"\f").replace(/\\v/g,"\v").replace(/\\0(\d{1,3})/g,(t,n)=>String.fromCharCode(parseInt(n,8)))}var En={name:"echo",description:"Display text",category:"shell",params:["[-n] [-e] [text...]"],run:({args:e,stdin:t,env:n})=>{let{flags:r,positionals:i}=le(e,{flags:["-n","-e","-E"]}),s=r.has("-n"),o=r.has("-e"),a=i.length>0?i.join(" "):t??"",l=tt(a,n?.vars??{},n?.lastExitCode??0),c=o?ni(l):l;return{stdout:s?c:`${c}
`,exitCode:0}}};var In={name:"env",description:"Print environment variables",category:"shell",params:[],run:({env:e,authUser:t})=>{let n={...e.vars,USER:t,HOME:`/home/${t}`};return{stdout:Object.entries(n).map(([r,i])=>`${r}=${i}`).join(`
`),exitCode:0}}};var An={name:"exit",aliases:["bye"],description:"Exit the shell session",category:"shell",params:["[code]"],run:({args:e})=>({closeSession:!0,exitCode:parseInt(e[0]??"0",10)||0})};var _n={name:"export",description:"Set shell environment variable",category:"shell",params:["[VAR=value]"],run:({args:e,env:t})=>{if(e.length===0||e.length===1&&e[0]==="-p"){let n=Object.entries(t.vars).filter(([r])=>r&&/^[A-Za-z_][A-Za-z0-9_]*$/.test(r)).map(([r,i])=>`declare -x ${r}="${i}"`).join(`
`);return{stdout:n?`${n}
`:"",exitCode:0}}for(let n of e.filter(r=>r!=="-p"))if(n.includes("=")){let r=n.indexOf("="),i=n.slice(0,r),s=n.slice(r+1);t.vars[i]=s}return{exitCode:0}}};var On={name:"find",description:"Search for files",category:"files",params:["[path] [-name <pattern>] [-type f|d]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=ue(r,["-name"]),s=ue(r,["-type"]),a=r.filter(m=>!m.startsWith("-")&&m!==i&&m!==s)[0]??".",l=I(n,a);try{if(L(e,l,"find"),!t.vfs.exists(l))return{stderr:`find: ${a}: No such file or directory`,exitCode:1}}catch(m){return{stderr:`find: ${m instanceof Error?m.message:String(m)}`,exitCode:1}}let c=i?new RegExp(`^${i.replace(/\./g,"\\.").replace(/\*/g,".*").replace(/\?/g,".")}$`):null,u=[],d=(m,p)=>{let g=t.vfs.stat(m),S=!s||s==="f"&&g.type==="file"||s==="d"&&g.type==="directory",C=!c||c.test(m.split("/").pop()??"");if(S&&C&&u.push(p),g.type==="directory")for(let P of t.vfs.list(m)){let x=`${m}/${P}`,_=`${p}/${P}`;d(x,_)}};return d(l,a),{stdout:u.join(`
`),exitCode:0}}};import*as it from"node:os";var Tn={name:"free",description:"Display amount of free and used memory",category:"system",params:["[-h] [-m] [-g]"],run:({args:e})=>{let t=v(e,["-h","--human"]),n=v(e,["-m"]),r=v(e,["-g"]),i=it.totalmem(),s=it.freemem(),o=i-s,a=Math.floor(i*.02),l=Math.floor(i*.05),c=Math.floor(s*.95),u=Math.floor(i*.5),d=S=>t?S>=1024*1024*1024?`${(S/(1024*1024*1024)).toFixed(1)}G`:S>=1024*1024?`${(S/(1024*1024)).toFixed(1)}M`:`${(S/1024).toFixed(1)}K`:String(Math.floor(r?S/(1024*1024*1024):n?S/(1024*1024):S/1024)),m="               total        used        free      shared  buff/cache   available",p=`Mem:  ${d(i).padStart(12)} ${d(o).padStart(11)} ${d(s).padStart(11)} ${d(a).padStart(11)} ${d(l).padStart(11)} ${d(c).padStart(11)}`,g=`Swap: ${d(u).padStart(12)} ${d(0).padStart(11)} ${d(u).padStart(11)}`;return{stdout:[m,p,g].join(`
`),exitCode:0}}};var Fn={name:"grep",description:"Search text patterns",category:"text",params:["[-i] [-v] [-n] [-r] <pattern> [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let{flags:s,positionals:o}=le(r,{flags:["-i","-v","-n","-r","-c","-l","-L","-q","--quiet","--silent"]}),a=s.has("-i"),l=s.has("-v"),c=s.has("-n"),u=s.has("-r"),d=s.has("-c"),m=s.has("-l"),p=s.has("-q")||s.has("--quiet")||s.has("--silent"),g=o[0],S=o.slice(1);if(!g)return{stderr:"grep: no pattern specified",exitCode:1};let C;try{let w=a?"mi":"m";C=new RegExp(g,w)}catch{return{stderr:`grep: invalid regex: ${g}`,exitCode:1}}let P=(w,A="")=>{let N=w.split(`
`),b=[];for(let E=0;E<N.length;E++){let T=N[E]??"",k=C.test(T);if(l?!k:k){let F=c?`${E+1}:`:"";b.push(`${A}${F}${T}`)}}return b},x=w=>{if(!t.vfs.exists(w))return[];if(t.vfs.stat(w).type==="file")return[w];if(!u)return[];let N=[],b=E=>{for(let T of t.vfs.list(E)){let k=`${E}/${T}`;t.vfs.stat(k).type==="file"?N.push(k):b(k)}};return b(w),N},_=[];if(S.length===0){if(!i)return{stdout:"",exitCode:1};let w=P(i);if(d)return{stdout:`${w.length}
`,exitCode:w.length>0?0:1};if(p)return{exitCode:w.length>0?0:1};_.push(...w)}else{let w=S.flatMap(A=>{let N=I(n,A);return x(N).map(b=>({file:A,path:b}))});for(let{file:A,path:N}of w)try{L(e,N,"grep");let b=t.vfs.readFile(N),E=w.length>1?`${A}:`:"",T=P(b,E);d?_.push(w.length>1?`${A}:${T.length}`:String(T.length)):m?T.length>0&&_.push(A):_.push(...T)}catch{return{stderr:`grep: ${A}: No such file or directory`,exitCode:1}}}return{stdout:_.length>0?`${_.join(`
`)}
`:"",exitCode:_.length>0?0:1}}};var Rn={name:"groups",description:"Print group memberships",category:"system",params:["[user]"],run:({authUser:e,shell:t,args:n})=>{let r=n[0]??e;return{stdout:t.users.isSudoer(r)?`${r} sudo root`:r,exitCode:0}}};var Dn={name:"gzip",description:"Compress files",category:"archive",params:["[-k] [-d] <file>"],run:({shell:e,cwd:t,args:n})=>{if(!e.packageManager.isInstalled("gzip"))return{stderr:`bash: gzip: command not found
...
`,exitCode:1};let o=e.vfs.readFile(s),a=s.slice(0,-3);return e.vfs.writeFile(a,o),r||e.vfs.remove(s),{exitCode:0}}};var Un={name:"head",description:"Output first lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=ue(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
`),p=m.slice(0,a);return p.join(`
`)+(d.endsWith(`
`)&&p.length===m.slice(0,a).length?`
`:"")};if(l.length===0)return{stdout:c(i??""),exitCode:0};let u=[];for(let d of l){let m=I(n,d);try{L(e,m,"head"),u.push(c(t.vfs.readFile(m)))}catch{return{stderr:`head: ${d}: No such file or directory`,exitCode:1}}}return{stdout:u.join(`
`),exitCode:0}}};var Vn=["navigation","files","text","archive","system","package","network","shell","users","misc"],Bn={navigation:"Navigation",files:"Files & Filesystem",text:"Text Processing",archive:"Archive & Compression",system:"System",package:"Package Management",network:"Network",shell:"Shell & Scripting",users:"Users & Permissions",misc:"Miscellaneous"},jn="\x1B[1m",Se="\x1B[0m",ri="\x1B[36m",si="\x1B[33m",je="\x1B[2m",ii="\x1B[32m";function zn(e,t){return e.length>=t?e:e+" ".repeat(t-e.length)}function oi(e){let t=e.aliases?.length?` ${je}(${e.aliases.join(", ")})${Se}`:"";return`  ${ri}${zn(e.name,16)}${Se}${t}${zn("",(e.aliases?.length,0))} ${e.description??""}`}function ai(e){let t={};for(let s of e){let o=s.category??"misc";t[o]||(t[o]=[]),t[o].push(s)}let n=[`${jn}Available commands${Se}`,`${je}Type 'help <command>' for detailed usage.${Se}`,""],r=[...Vn.filter(s=>t[s]),...Object.keys(t).filter(s=>!Vn.includes(s)).sort()];for(let s of r){let o=t[s];if(!o?.length)continue;n.push(`${si}${Bn[s]??s}${Se}`);let a=[...o].sort((l,c)=>l.name.localeCompare(c.name));for(let l of a)n.push(oi(l));n.push("")}let i=e.length;return n.push(`${je}${i} commands available.${Se}`),n.join(`
`)}function li(e){let t=[];if(t.push(`${jn}${e.name}${Se} \u2014 ${e.description??"no description"}`),e.aliases?.length&&t.push(`${je}Aliases: ${e.aliases.join(", ")}${Se}`),t.push(""),t.push(`${ii}Usage:${Se}`),e.params.length)for(let r of e.params)t.push(`  ${e.name} ${r}`);else t.push(`  ${e.name}`);let n=Bn[e.category??"misc"]??e.category??"misc";return t.push(""),t.push(`${je}Category: ${n}${Se}`),t.join(`
`)}function Wn(e){return{name:"help",description:"List all commands, or show usage for a specific command",category:"shell",params:["[command]"],run:({args:t})=>{let n=It();if(t[0]){let r=t[0].toLowerCase(),i=n.find(s=>s.name===r||s.aliases?.includes(r));return i?{stdout:li(i),exitCode:0}:{stderr:`help: no help entry for '${t[0]}'`,exitCode:1}}return{stdout:ai(n),exitCode:0}}}}var Hn={name:"history",description:"Display command history",category:"shell",params:["[n]"],run:({args:e,shell:t,authUser:n})=>{let r=`/home/${n}/.bash_history`;if(!t.vfs.exists(r))return{stdout:"",exitCode:0};let s=t.vfs.readFile(r).split(`
...
`),exitCode:0}}};var qn={name:"hostname",description:"Print hostname",category:"system",params:[],run:({hostname:e})=>({stdout:e,exitCode:0})};var Gn={name:"htop",description:"System monitor",category:"system",params:[],run:({mode:e})=>e==="exec"?{stderr:"htop: interactive terminal required",exitCode:1}:{openHtop:!0,exitCode:0}};var Yn={name:"id",description:"Print user identity",category:"system",params:["[user]"],run:({authUser:e,shell:t,args:n})=>{let r=n[0]??e,i=r==="root"?0:1e3,s=i,a=t.users.isSudoer(r)?`${s}(${r}),0(root)`:`${s}(${r})`;return{stdout:`uid=${i}(${r}) gid=${s}(${r}) groups=${a}`,exitCode:0}}};var Kn={name:"kill",description:"Send signal to process",category:"system",params:["[-9] <pid>"],run:({args:e})=>e.find(n=>!n.startsWith("-"))?{stdout:"",exitCode:0}:{stderr:"kill: no pid specified",exitCode:1}};var Jn={name:"ln",description:"Create links",category:"files",params:["[-s] <target> <link_name>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=v(r,["-s","--symbolic"]),s=r.filter(u=>!u.startsWith("-")),[o,a]=s;if(!o||!a)return{stderr:"ln: missing operand",exitCode:1};let l=I(n,a),c=i?o:I(n,o);try{if(L(e,l,"ln"),i)t.vfs.symlink(c,l);else{let u=I(n,o);if(L(e,u,"ln"),!t.vfs.exists(u))return{stderr:`ln: ${o}: No such file or directory`,exitCode:1};let d=t.vfs.readFile(u);t.writeFileAsUser(e,l,d)}return{exitCode:0}}catch(u){return{stderr:`ln: ${u instanceof Error?u.message:String(u)}`,exitCode:1}}}},Zn={name:"readlink",description:"Print resolved path of symbolic link",category:"files",params:["[-f] <path>"],run:({shell:e,cwd:t,args:n})=>{let r=n.includes("-f")||n.includes("-e"),i=n.find(a=>!a.startsWith("-"));if(!i)return{stderr:`readlink: missing operand
...
`,exitCode:0}}};var Xn={name:"stat",description:"Display file status",category:"files",params:["[-c <format>] <file>"],run:({shell:e,cwd:t,args:n})=>{let r=n.findIndex(P=>P==="-c"||P==="--format"),i=r!==-1?n[r+1]:void 0,s=n.find(P=>!P.startsWith("-")&&P!==i);if(!s)return{stderr:`stat: missing operand
...
`)}
`,exitCode:0}}};var ci="\x1B[0m",ui="\x1B[1;34m",di="\x1B[1;36m",mi="\x1B[1;32m",pi="",fi="\x1B[30;42m",hi="\x1B[37;44m",gi="\x1B[34;42m";function De(e,t){return t?`${t}${e}${ci}`:e}function Ot(e,t,n){if(n)return di;if(t==="directory"){let r=!!(e&512),i=!!(e&2);return r&&i?fi:r?hi:i?gi:ui}return e&73?mi:pi}function er(e,t,n){let r;n?r="l":t==="directory"?r="d":r="-";let i=c=>e&c?"r":"-",s=c=>e&c?"w":"-",o=(()=>{let c=!!(e&64);return e&2048?c?"s":"S":c?"x":"-"})(),a=(()=>{let c=!!(e&8);return e&1024?c?"s":"S":c?"x":"-"})(),l=(()=>{let c=!!(e&1);return t==="directory"&&e&512?c?"t":"T":c?"x":"-"})();return`${r}${i(256)}${s(128)}${o}${i(32)}${s(16)}${a}${i(4)}${s(2)}${l}`}var yi=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function _t(e){let t=new Date,n=4320*3600*1e3,r=Math.abs(t.getTime()-e.getTime())<n,i=String(e.getDate()).padStart(2," "),s=yi[e.getMonth()]??"";if(r){let o=String(e.getHours()).padStart(2,"0"),a=String(e.getMinutes()).padStart(2,"0");return`${i} ${s.padEnd(3)} ${o}:${a}`}return`${i} ${s.padEnd(3)} ${e.getFullYear()}`}function ot(e,t){try{return e.readFile(t)}catch{return"?"}}function Si(e,t,n){let r=t==="/"?"":t;return n.map(i=>{let s=`${r}/${i}`,o=e.isSymlink(s),a;try{a=e.stat(s)}catch{return i}let l=Ot(a.mode,a.type,o);return De(i,l)}).join("  ")}function bi(e,t,n){let r=t==="/"?"":t,i=n.map(d=>{let m=`${r}/${d}`,p=e.isSymlink(m),g;try{g=e.stat(m)}catch{return{perms:"----------",nlink:"1",size:"0",date:_t(new Date),label:d}}let S=p?41471:g.mode,C=er(S,g.type,p),P=g.type==="directory"?String((g.childrenCount??0)+2):"1",x=p?ot(e,m).length:g.type==="file"?g.size??0:(g.childrenCount??0)*4096,_=String(x),w=_t(g.updatedAt),A=Ot(S,g.type,p),N=p?`${De(d,A)} -> ${ot(e,m)}`:De(d,A);return{perms:C,nlink:P,size:_,date:w,label:N}}),s=Math.max(...i.map(d=>d.nlink.length)),o=Math.max(...i.map(d=>d.size.length)),a="root",l="root",c=n.length*8,u=i.map(d=>`${d.perms} ${d.nlink.padStart(s)} ${a} ${l} ${d.size.padStart(o)} ${d.date} ${d.label}`);return`total ${c}
${u.join(`
`)}`}var tr={name:"ls",description:"List directory contents",category:"navigation",params:["[-la] [path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=v(r,["-l","--long","-la","-al"]),s=v(r,["-a","--all","-la","-al"]),o=$e(r,0,{flags:["-l","--long","-a","--all","-la","-al"]}),a=I(n,o??n);if(L(e,a,"ls"),t.vfs.exists(a)){let u=t.vfs.stat(a),d=t.vfs.isSymlink(a);if(u.type==="file"||d){let m=a.split("/").pop()??a,p=Ot(d?41471:u.mode,u.type,d);if(i){let g=d?41471:u.mode,S=d?ot(t.vfs,a).length:u.size??0,C=er(g,u.type,d),P=d?`${De(m,p)} -> ${ot(t.vfs,a)}`:De(m,p);return{stdout:`${C} 1 root root ${S} ${_t(u.updatedAt)} ${P}
`,exitCode:0}}return{stdout:`${De(m,p)}
`,exitCode:0}}}let l=t.vfs.list(a).filter(u=>s||!u.startsWith("."));return{stdout:`${i?bi(t.vfs,a,l):Si(t.vfs,a,l)}
`,exitCode:0}}};var nr={name:"lsb_release",description:"Print distribution-specific information",category:"system",params:["[-a] [-i] [-d] [-r] [-c]"],run:({args:e,shell:t})=>{let n=t.properties?.os??"Fortune GNU/Linux x64",r="nyx",i="1.0";try{let d=t.vfs.readFile("/etc/os-release");for(let m of d.split(`
...
`),exitCode:0}}};var rr={adduser:`ADDUSER(8)                User Commands                ADDUSER(8)
...
DESCRIPTION
       Read items from stdin and execute COMMAND with those items as arguments.`};var xi={gunzip:"gzip"},sr={name:"man",description:"Interface to the system reference manuals",category:"shell",params:["<command>"],run:async({args:e,shell:t})=>{let n=e[0];if(!n)return{stderr:"What manual page do you want?",exitCode:1};let r=`/usr/share/man/man1/${n}.1`;if(t.vfs.exists(r))return{stdout:t.vfs.readFile(r),exitCode:0};let i=n.toLowerCase(),s=xi[i]??i,o=rr[s]??null;return o?{stdout:o,exitCode:0}:{stderr:`No manual entry for ${n}`,exitCode:16}}};var ir={name:"mkdir",description:"Make directories",category:"files",params:["<dir>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"mkdir: missing operand",exitCode:1};for(let i=0;i<r.length;i++){let s=$e(r,i);if(!s)return{stderr:"mkdir: missing operand",exitCode:1};let o=I(n,s);L(e,o,"mkdir"),t.vfs.mkdir(o)}return{exitCode:0}}};var or={name:"mv",description:"Move or rename files",category:"files",params:["<source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r.filter(c=>!c.startsWith("-")),[s,o]=i;if(!s||!o)return{stderr:"mv: missing operand",exitCode:1};let a=I(n,s),l=I(n,o);try{if(L(e,a,"mv"),L(e,l,"mv"),!t.vfs.exists(a))return{stderr:`mv: ${s}: No such file or directory`,exitCode:1};let c=t.vfs.exists(l)&&t.vfs.stat(l).type==="directory"?`${l}/${s.split("/").pop()}`:l;return t.vfs.move(a,c),{exitCode:0}}catch(c){return{stderr:`mv: ${c instanceof Error?c.message:String(c)}`,exitCode:1}}}};import*as ar from"node:path";var lr={name:"nano",description:"Text editor",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r[0];if(!i)return{stderr:"nano: missing file operand",exitCode:1};let s=I(n,i);L(e,s,"nano");let o=t.vfs.exists(s)?t.vfs.readFile(s):"",a=ar.posix.basename(s)||"buffer",l=`/tmp/sshmimic-nano-${Date.now()}-${a}.tmp`;return{openEditor:{targetPath:s,tempPath:l,initialContent:o},exitCode:0}}};import{existsSync as hr,readdirSync as wi,readFileSync as Tt}from"node:fs";import*as ie from"node:os";import*as gr from"node:path";function vi(e){let t=Math.max(1,Math.floor(e/60)),n=Math.floor(t/1440),r=Math.floor(t%1440/60),i=t%60,s=[];return n>0&&s.push(`${n} day${n>1?"s":""}`),r>0&&s.push(`${r} hour${r>1?"s":""}`),(i>0||s.length===0)&&s.push(`${i} min${i>1?"s":""}`),s.join(", ")}function cr(e){return`\x1B[${e}m   \x1B[0m`}function Ci(){let e=[40,41,42,43,44,45,46,47].map(cr).join(""),t=[100,101,102,103,104,105,106,107].map(cr).join("");return[e,t]}function ur(e,t,n){if(e.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s=n<=1?0:t/(n-1),o=Math.round(r.r+(i.r-r.r)*s),a=Math.round(r.g+(i.g-r.g)*s),l=Math.round(r.b+(i.b-r.b)*s);return`\x1B[38;2;${o};${a};${l}m${e}\x1B[0m`}function Pi(e){if(e.trim().length===0)return e;let t=e.indexOf(":");if(t===-1)return e.includes("@")?dr(e):e;let n=e.substring(0,t+1),r=e.substring(t+1);return dr(n)+r}function dr(e){let t=new RegExp("\x1B\\[[\\d;]*m","g"),n=e.replace(t,"");if(n.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s="";for(let o=0;o<n.length;o+=1){let a=n.length<=1?0:o/(n.length-1),l=Math.round(r.r+(i.r-r.r)*a),c=Math.round(r.g+(i.g-r.g)*a),u=Math.round(r.b+(i.b-r.b)*a);s+=`\x1B[38;2;${l};${c};${u}m${n[o]}\x1B[0m`}return s}function mr(e){return Math.max(0,Math.round(e/(1024*1024)))}function pr(){try{let e=Tt("/etc/os-release","utf8");for(let t of e.split(`
`)){if(!t.startsWith("PRETTY_NAME="))continue;return t.slice(12).trim().replace(/^"|"$/g,"")}}catch{return}}function fr(e){try{let t=Tt(e,"utf8").split(`
`)[0]?.trim();return!t||t.length===0?void 0:t}catch{return}}function $i(e){let t=fr("/sys/devices/virtual/dmi/id/sys_vendor"),n=fr("/sys/devices/virtual/dmi/id/product_name");return t&&n?`${t} ${n}`:n||e}function ki(){let e=["/var/lib/dpkg/status","/usr/local/var/lib/dpkg/status"];for(let t of e)if(hr(t))try{return Tt(t,"utf8").match(/^Package:\s+/gm)?.length??0}catch{}}function Mi(){let e=["/snap","/var/lib/snapd/snaps"];for(let t of e)if(hr(t))try{return wi(t,{withFileTypes:!0}).filter(i=>i.isDirectory()).length}catch{}}function Ni(){let e=ki(),t=Mi();return e!==void 0&&t!==void 0?`${e} (dpkg), ${t} (snap)`:e!==void 0?`${e} (dpkg)`:t!==void 0?`${t} (snap)`:"n/a"}function Ei(){let e=ie.cpus();if(e.length===0)return"unknown";let t=e[0];if(!t)return"unknown";let n=(t.speed/1e3).toFixed(2);return`${t.model} (${e.length}) @ ${n}GHz`}function Ii(e){return!e||e.trim().length===0?"unknown":gr.posix.basename(e.trim())}function Ai(e){let t=ie.totalmem(),n=ie.freemem(),r=Math.max(0,t-n),i=e.shellProps,s=process.uptime();return e.uptimeSeconds===void 0&&(e.uptimeSeconds=Math.round(s)),{user:e.user,host:e.host,osName:i?.os??e.osName??`${pr()??ie.type()} ${ie.arch()}`,kernel:i?.kernel??e.kernel??ie.release(),uptimeSeconds:e.uptimeSeconds??ie.uptime(),packages:e.packages??Ni(),shell:Ii(e.shell),shellProps:e.shellProps??{kernel:e.kernel??ie.release(),os:e.osName??`${pr()??ie.type()} ${ie.arch()}`,arch:ie.arch()},resolution:e.resolution??"n/a (ssh)",terminal:e.terminal??"unknown",cpu:e.cpu??Ei(),gpu:e.gpu??"n/a",memoryUsedMiB:e.memoryUsedMiB??mr(r),memoryTotalMiB:e.memoryTotalMiB??mr(t)}}function yr(e){let t=Ai(e),n=vi(t.uptimeSeconds),r=Ci(),i=["                               .. .:.    "," .::..                       ..     ..   ",".    ....                  ...       ..  ",":       ....             .:.          .. ",":           .:.:........:.            .. ",":                                     .. ",".                                      : ",".                                      : ","..                                     : "," :.                                   .. "," ..                                   .. "," :-.                                  :: ","  .:.                                 :. ","   ..:                               ... ","   ..:                               :.. ","  :...                              :....","   ..                                ....","   .                                  .. ","  .:.                                 .: ","  :.                                  .. "," ::.                                 ..  ",".....                          ..:...    ","...:.                         ..         ",".:...:.       ::.           ..           ","  ... ..:::::..  ..:::::::..             "],s=[`${t.user}@${t.host}`,"-------------------------",`OS: ${t.osName}`,`Host: ${$i(t.host)}`,`Kernel: ${t.kernel}`,`Uptime: ${n}`,`Packages: ${t.packages}`,`Shell: ${t.shell}`,`Resolution: ${t.resolution}`,`Terminal: ${t.terminal}`,`CPU: ${t.cpu}`,`GPU: ${t.gpu}`,`Memory: ${t.memoryUsedMiB}MiB / ${t.memoryTotalMiB}MiB`,"",r[0],r[1]],o=Math.max(i.length,s.length),a=[];for(let l=0;l<o;l+=1){let c=i[l]??"",u=s[l]??"";if(u.length>0){let d=ur(c.padEnd(31," "),l,i.length),m=Pi(u);a.push(`${d}  ${m}`);continue}a.push(ur(c,l,i.length))}return a.join(`
`)}var Sr={name:"neofetch",description:"System info display",category:"system",params:["[--off]"],run:({args:e,authUser:t,hostname:n,shell:r,env:i})=>r.packageManager.isInstalled("neofetch")?v(e,"--help")?{stdout:"Usage: neofetch [--off]",exitCode:0}:v(e,"--off")?{stdout:`${t}@${n}`,exitCode:0}:{stdout:yr({user:t,host:n,shell:i.vars.SHELL,shellProps:r.properties,terminal:i.vars.TERM,uptimeSeconds:Math.floor((Date.now()-r.startTime)/1e3),packages:`${r.packageManager?.installedCount()??0} (dpkg)`}),exitCode:0}:{stderr:`bash: neofetch: command not found
Hint: install it with: apt install neofetch
`,exitCode:127}};import br from"node:vm";var at="v18.19.0",xr={node:at,npm:"9.2.0",v8:"10.2.154.26-node.22"};function _i(e,t){let n={version:at,versions:xr,platform:"linux",arch:"x64",env:{NODE_ENV:"production",HOME:"/root",PATH:"/usr/local/bin:/usr/bin:/bin"},argv:["node"],stdout:{write:s=>(e.push(s),!0)},stderr:{write:s=>(t.push(s),!0)},exit:(s=0)=>{throw new lt(s)},cwd:()=>"/root",hrtime:()=>[0,0]},r={log:(...s)=>e.push(s.map(be).join(" ")),error:(...s)=>t.push(s.map(be).join(" ")),warn:(...s)=>t.push(s.map(be).join(" ")),info:(...s)=>e.push(s.map(be).join(" ")),dir:s=>e.push(be(s))},i=s=>{switch(s){case"path":return{join:(...o)=>o.join("/").replace(/\/+/g,"/"),resolve:(...o)=>`/${o.join("/").replace(/^\/+/,"")}`,dirname:o=>o.split("/").slice(0,-1).join("/")||"/",basename:o=>o.split("/").pop()??"",extname:o=>{let a=o.split("/").pop()??"",l=a.lastIndexOf(".");return l>0?a.slice(l):""},sep:"/",delimiter:":"};case"os":return{platform:()=>"linux",arch:()=>"x64",type:()=>"Linux",hostname:()=>"fortune-vm",homedir:()=>"/root",tmpdir:()=>"/tmp",EOL:`
`};case"util":return{format:(...o)=>o.map(be).join(" "),inspect:o=>be(o)};case"fs":case"fs/promises":throw new Error(`Cannot require '${s}': filesystem access not available in virtual runtime`);case"child_process":case"net":case"http":case"https":throw new Error(`Cannot require '${s}': not available in virtual runtime`);default:throw new Error(`Cannot find module '${s}'`)}};return i.resolve=s=>{throw new Error(`Cannot resolve '${s}'`)},i.cache={},i.extensions={},br.createContext({console:r,process:n,require:i,Math,JSON,Object,Array,String,Number,Boolean,Symbol,Date,RegExp,Error,TypeError,RangeError,SyntaxError,Promise,Map,Set,WeakMap,WeakSet,parseInt,parseFloat,isNaN,isFinite,encodeURIComponent,decodeURIComponent,encodeURI,decodeURI,setTimeout:()=>{},clearTimeout:()=>{},setInterval:()=>{},clearInterval:()=>{},queueMicrotask:()=>{},globalThis:void 0,undefined:void 0,Infinity:1/0,NaN:NaN})}var lt=class{constructor(t){this.code=t}code};function be(e){if(e===null)return"null";if(e===void 0)return"undefined";if(typeof e=="string")return e;if(typeof e=="function")return`[Function: ${e.name||"(anonymous)"}]`;if(Array.isArray(e))return`[ ${e.map(be).join(", ")} ]`;if(e instanceof Error)return`${e.name}: ${e.message}`;if(typeof e=="object")try{return`{ ${Object.entries(e).map(([n,r])=>`${n}: ${be(r)}`).join(", ")} }`}catch{return"[Object]"}return String(e)}function ct(e){let t=[],n=[],r=_i(t,n),i=0;try{let s=br.runInContext(e,r,{timeout:5e3});s!==void 0&&t.length===0&&t.push(be(s))}catch(s){s instanceof lt?i=s.code:s instanceof Error?(n.push(`${s.name}: ${s.message}`),i=1):(n.push(String(s)),i=1)}return{stdout:t.length?`${t.join(`
`)}
`:"",stderr:n.length?`${n.join(`
`)}
`:"",exitCode:i}}function Oi(e){let t=e.trim();return!t.includes(`
`)&&!t.startsWith("const ")&&!t.startsWith("let ")&&!t.startsWith("var ")&&!t.startsWith("function ")&&!t.startsWith("class ")&&!t.startsWith("if ")&&!t.startsWith("for ")&&!t.startsWith("while ")&&!t.startsWith("import ")&&!t.startsWith("//")?ct(t):ct(`(async () => { ${e} })()`)}var wr={name:"node",description:"JavaScript runtime (virtual)",category:"system",params:["[--version] [-e <expr>] [-p <expr>] [file]"],run:({args:e,shell:t,cwd:n})=>{if(!t.packageManager.isInstalled("nodejs"))return{stderr:`bash: node: command not found
Hint: install it with: apt install nodejs
`,exitCode:127};if(v(e,["--version","-v"]))return{stdout:`${at}
`,exitCode:0};if(v(e,["--versions"]))return{stdout:`${JSON.stringify(xr,null,2)}
...
`,exitCode:1};let a=t.vfs.readFile(o),{stdout:l,stderr:c,exitCode:u}=Oi(a);return{stdout:l||void 0,stderr:c||void 0,exitCode:u}}return{stdout:[`Welcome to Node.js ${at}.`,'Type ".exit" to exit the REPL.',"> "].join(`
`),exitCode:0}}};var ut="9.2.0",Ti="18.19.0",vr={name:"npm",description:"Node.js package manager (virtual)",category:"system",params:["<command> [args]"],run:({args:e,shell:t})=>{if(!t.packageManager.isInstalled("npm"))return{stderr:`bash: npm: command not found
Hint: install it with: apt install npm
`,exitCode:127};if(v(e,["--version","-v"]))return{stdout:`${ut}
`,exitCode:0};let n=e[0]?.toLowerCase();switch(n){case"version":case"-version":return{stdout:`{ npm: '${ut}', node: '${Ti}', v8: '10.2.154.26' }
...
`,exitCode:1}}}},Cr={name:"npx",description:"Node.js package runner (virtual)",category:"system",params:["<package> [args]"],run:({args:e,shell:t})=>t.packageManager.isInstalled("npm")?v(e,["--version"])?{stdout:`${ut}
...
`),exitCode:0}}};var Nr={name:"pwd",description:"Print working directory",category:"navigation",params:[],run:({cwd:e})=>({stdout:e,exitCode:0})};var Ri="Python 3.11.2";var dt="3.11.2 (default, Mar 13 2023, 12:18:29) [GCC 12.2.0]",y={__pytype__:"none"};function J(e=[]){return{__pytype__:"dict",data:new Map(e)}}function Ft(e,t,n=1){return{__pytype__:"range",start:e,stop:t,step:n}}function Y(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="dict"}function Ue(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="range"}function xe(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="func"}function Rt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="class"}function We(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="instance"}function Pe(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="none"}function ne(e){return e===null||Pe(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?`'${e.replace(/'/g,"\\'")}'`:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:Y(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Ue(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:xe(e)?`<function ${e.name} at 0x...>`:Rt(e)?`<class '${e.name}'>`:We(e)?`<${e.cls.name} object at 0x...>`:String(e)}function R(e){return e===null||Pe(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?e:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:Y(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Ue(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:ne(e)}function me(e){return e===null||Pe(e)?!1:typeof e=="boolean"?e:typeof e=="number"?e!==0:typeof e=="string"||Array.isArray(e)?e.length>0:Y(e)?e.data.size>0:Ue(e)?Ir(e)>0:!0}function Ir(e){if(e.step===0)return 0;let t=Math.ceil((e.stop-e.start)/e.step);return Math.max(0,t)}function Di(e){let t=[];for(let n=e.start;(e.step>0?n<e.stop:n>e.stop)&&(t.push(n),!(t.length>1e4));n+=e.step);return t}function te(e){if(Array.isArray(e))return e;if(typeof e=="string")return[...e];if(Ue(e))return Di(e);if(Y(e))return[...e.data.keys()];throw new K("TypeError",`'${Ae(e)}' object is not iterable`)}function Ae(e){return e===null||Pe(e)?"NoneType":typeof e=="boolean"?"bool":typeof e=="number"?Number.isInteger(e)?"int":"float":typeof e=="string"?"str":Array.isArray(e)?"list":Y(e)?"dict":Ue(e)?"range":xe(e)?"function":Rt(e)?"type":We(e)?e.cls.name:"object"}var K=class{constructor(t,n){this.type=t;this.message=n}type;message;toString(){return`${this.type}: ${this.message}`}},Le=class{constructor(t){this.value=t}value},He=class{},qe=class{},Ge=class{constructor(t){this.code=t}code};function Li(e){let t=new Map,n=J([["sep","/"],["linesep",`
`],["curdir","."],["pardir",".."]]);return n.__methods__={getcwd:()=>e,getenv:r=>typeof r=="string"?process.env[r]??y:y,path:J([["join",y],["exists",y],["dirname",y],["basename",y]]),listdir:()=>[]},t.set("__builtins__",y),t.set("__name__","__main__"),t.set("__cwd__",e),t}function Ui(e){let t=J([["sep","/"],["curdir","."]]),n=J([["sep","/"],["linesep",`
`],["name","posix"]]);return n._cwd=e,t._cwd=e,n.path=t,n}function Vi(){return J([["version",dt],["version_info",J([["major",3],["minor",11],["micro",2]].map(([e,t])=>[e,t]))],["platform","linux"],["executable","/usr/bin/python3"],["prefix","/usr"],["path",["/usr/lib/python3.11","/usr/lib/python3.11/lib-dynload"]],["argv",[""]],["maxsize",9007199254740991]])}function zi(){return J([["pi",Math.PI],["e",Math.E],["tau",Math.PI*2],["inf",1/0],["nan",NaN],["sqrt",y],["floor",y],["ceil",y],["log",y],["pow",y],["sin",y],["cos",y],["tan",y],["fabs",y],["factorial",y]])}function Bi(){return J([["dumps",y],["loads",y]])}function ji(){return J([["match",y],["search",y],["findall",y],["sub",y],["split",y],["compile",y]])}var Er={os:Ui,sys:()=>Vi(),math:()=>zi(),json:()=>Bi(),re:()=>ji(),random:()=>J([["random",y],["randint",y],["choice",y],["shuffle",y]]),time:()=>J([["time",y],["sleep",y],["ctime",y]]),datetime:()=>J([["datetime",y],["date",y],["timedelta",y]]),collections:()=>J([["Counter",y],["defaultdict",y],["OrderedDict",y]]),itertools:()=>J([["chain",y],["product",y],["combinations",y],["permutations",y]]),functools:()=>J([["reduce",y],["partial",y],["lru_cache",y]]),string:()=>J([["ascii_letters","abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"],["digits","0123456789"],["punctuation","!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"]])},mt=class{constructor(t){this.cwd=t}cwd;output=[];stderr=[];modules=new Map;getOutput(){return this.output.join(`
`)+(this.output.length?`
...
`).replace(/\\t/g,"	"),a=new mt(n),{stdout:l,stderr:c,exitCode:u}=a.run(o);return{stdout:l||void 0,stderr:c||void 0,exitCode:u}}let i=e.find(s=>!s.startsWith("-"));if(i){let s=I(n,i);if(!t.vfs.exists(s))return{stderr:`python3: can't open file '${i}': [Errno 2] No such file or directory
...
>>> `,exitCode:0}}};var _r={name:"read",description:"Read a line from stdin into variables",category:"shell",params:["[-r] [-p prompt] <var...>"],run:({args:e,stdin:t,env:n})=>{let r=e.indexOf("-p"),i=e.filter((a,l)=>a!=="-r"&&a!=="-p"&&e[l-1]!=="-p"),s=(t??"").split(`
`)[0]??"",o=v(e,["-r"])?s:s.replace(/\\(?:\r?\n|.)/g,a=>a[1]===`
`||a[1]==="\r"?"":a[1]);if(!n)return{exitCode:0};if(i.length===0)n.vars.REPLY=o;else if(i.length===1)n.vars[i[0]]=o;else{let a=o.split(/\s+/);for(let l=0;l<i.length;l++)n.vars[i[l]]=l<i.length-1?a[l]??"":a.slice(l).join(" ")}return{exitCode:0}}};var Or={name:"rm",description:"Remove files or directories",category:"files",params:["[-r|-rf] <path>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"rm: missing operand",exitCode:1};let i=v(r,["-r","-rf","-fr"]),s=[];for(let o=0;;o+=1){let a=$e(r,o,{flags:["-r","-rf","-fr"]});if(!a)break;s.push(a)}if(s.length===0)return{stderr:"rm: missing operand",exitCode:1};for(let o of s){let a=I(n,o);L(e,a,"rm"),t.vfs.remove(a,{recursive:i})}return{exitCode:0}}};var Tr={name:"sed",description:"Stream editor for filtering and transforming text",category:"text",params:["-e <expr> [file]","s/pattern/replace/[g]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=v(r,["-i"]),o=ue(r,["-e"])??r.find(C=>!C.startsWith("-")),a=r.filter(C=>!C.startsWith("-")&&C!==o).pop();if(!o)return{stderr:"sed: no expression",exitCode:1};let l=i??"";if(a){let C=I(n,a);try{l=t.vfs.readFile(C)}catch{return{stderr:`sed: ${a}: No such file or directory`,exitCode:1}}}let c=o.match(/^s([^a-zA-Z0-9])(.+?)\1(.*?)\1([gi]*)$/);if(!c)return{stderr:`sed: unrecognized command: ${o}`,exitCode:1};let[,,u,d,m]=c,p=(m??"").includes("i")?"gi":(m??"").includes("g")?"g":"",g;try{g=new RegExp(u,p||"")}catch{return{stderr:`sed: invalid regex: ${u}`,exitCode:1}}let S=((m??"").includes("g")||p.includes("g"),l.replace(g,d??""));if(s&&a){let C=I(n,a);return t.writeFileAsUser(e,C,S),{exitCode:0}}return{stdout:S,exitCode:0}}};var Fr={name:"set",description:"Display or set shell variables",category:"shell",params:["[VAR=value]"],run:({args:e,env:t})=>{if(e.length===0)return{stdout:Object.entries(t.vars).map(([r,i])=>`${r}=${i}`).join(`
`),exitCode:0};for(let n of e)if(n.includes("=")){let r=n.indexOf("=");t.vars[n.slice(0,r)]=n.slice(r+1)}return{exitCode:0}}};async function Lt(e,t,n,r){return rt(e,t,n,i=>Z(i,r.authUser,r.hostname,r.mode,r.cwd,r.shell,void 0,r.env).then(s=>s.stdout??""))}function Me(e){let t=[],n=0;for(;n<e.length;){let r=e[n].trim();if(!r||r.startsWith("#")){n++;continue}let i=r.match(/^(?:function\s+)?(\w+)\s*\(\s*\)\s*\{(.+)\}\s*$/),s=i??(r.match(/^(?:function\s+)?(\w+)\s*\(\s*\)\s*\{?\s*$/)||r.match(/^function\s+(\w+)\s*\{?\s*$/));if(s){let a=s[1],l=[];if(i){l.push(...i[2].split(";").map(c=>c.trim()).filter(Boolean)),t.push({type:"func",name:a,body:l}),n++;continue}for(n++;n<e.length&&e[n]?.trim()!=="}"&&n<e.length+1;){let c=e[n].trim().replace(/^do\s+/,"");c&&c!=="{"&&l.push(c),n++}n++,t.push({type:"func",name:a,body:l});continue}let o=r.match(/^\(\(\s*(.+?)\s*\)\)$/);if(o){t.push({type:"arith",expr:o[1]}),n++;continue}if(r.startsWith("if ")||r==="if"){let a=r.replace(/^if\s+/,"").replace(/;\s*then\s*$/,"").trim(),l=[],c=[],u=[],d="then",m="";for(n++;n<e.length&&e[n]?.trim()!=="fi";){let p=e[n].trim();p.startsWith("elif ")?(d="elif",m=p.replace(/^elif\s+/,"").replace(/;\s*then\s*$/,"").trim(),c.push({cond:m,body:[]})):p==="else"?d="else":p!=="then"&&(d==="then"?l.push(p):d==="elif"&&c.length>0?c[c.length-1].body.push(p):u.push(p)),n++}t.push({type:"if",cond:a,then_:l,elif:c,else_:u})}else if(r.startsWith("for ")){let a=r.match(/^for\s+(\w+)\s+in\s+(.+?)(?:\s*;\s*do)?$/);if(a){let l=[];for(n++;n<e.length&&e[n]?.trim()!=="done";){let c=e[n].trim().replace(/^do\s+/,"");c&&c!=="do"&&l.push(c),n++}t.push({type:"for",var:a[1],list:a[2],body:l})}else t.push({type:"cmd",line:r})}else if(r.startsWith("while ")){let a=r.replace(/^while\s+/,"").replace(/;\s*do\s*$/,"").trim(),l=[];for(n++;n<e.length&&e[n]?.trim()!=="done";){let c=e[n].trim().replace(/^do\s+/,"");c&&c!=="do"&&l.push(c),n++}t.push({type:"while",cond:a,body:l})}else t.push({type:"cmd",line:r});n++}return t}async function Dt(e,t){let n=await Lt(e,t.env.vars,t.env.lastExitCode,t),r=n.match(/^\[?\s*(.+?)\s*\]?$/);if(r){let s=r[1],o=s.match(/^-([fdeznr])\s+(.+)$/);if(o){let[,c,u]=o,d=I(t.cwd,u);if(c==="f")return t.shell.vfs.exists(d)&&t.shell.vfs.stat(d).type==="file";if(c==="d")return t.shell.vfs.exists(d)&&t.shell.vfs.stat(d).type==="directory";if(c==="e")return t.shell.vfs.exists(d);if(c==="z")return(u??"").length===0;if(c==="n")return(u??"").length>0}let a=s.match(/^"?([^"]*)"?\s*(==|!=|=|<|>)\s*"?([^"]*)"?$/);if(a){let[,c,u,d]=a;if(u==="=="||u==="=")return c===d;if(u==="!=")return c!==d}let l=s.match(/^(\S+)\s+(-eq|-ne|-lt|-le|-gt|-ge)\s+(\S+)$/);if(l){let[,c,u,d]=l,m=Number(c),p=Number(d);if(u==="-eq")return m===p;if(u==="-ne")return m!==p;if(u==="-lt")return m<p;if(u==="-le")return m<=p;if(u==="-gt")return m>p;if(u==="-ge")return m>=p}}return((await Z(n,t.authUser,t.hostname,t.mode,t.cwd,t.shell,void 0,t.env)).exitCode??0)===0}async function Ne(e,t){let n={exitCode:0},r="";for(let i of e)if(i.type==="cmd"){let s=await Lt(i.line,t.env.vars,t.env.lastExitCode,t),o=/^([A-Za-z_][A-Za-z0-9_]*)=(.*)/,a=s.trim().split(/\s+/);if(a.length>0&&o.test(a[0])&&a.every(u=>o.test(u))){for(let u of a){let d=u.match(o);t.env.vars[d[1]]=d[2]}t.env.lastExitCode=0;continue}let l=await(async()=>{let c=s.trim().split(/\s+/)[0]??"",u=t.env.vars[`__func_${c}`];if(u){let d=s.trim().split(/\s+/).slice(1),m={...t.env.vars};d.forEach((S,C)=>{t.env.vars[String(C+1)]=S}),t.env.vars[0]=c;let p=u.split(`
`),g=await Ne(Me(p),t);for(let S=1;S<=d.length;S++)delete t.env.vars[String(S)];return Object.assign(t.env.vars,{...m,...t.env.vars}),g}return Z(s,t.authUser,t.hostname,t.mode,t.cwd,t.shell,void 0,t.env)})();if(t.env.lastExitCode=l.exitCode??0,l.stdout&&(r+=`${l.stdout}
`),l.stderr)return{...l,stdout:r.trim()};n=l}else if(i.type==="if"){let s=!1;if(await Dt(i.cond,t)){let o=await Ne(Me(i.then_),t);o.stdout&&(r+=`${o.stdout}
`),s=!0}else{for(let o of i.elif)if(await Dt(o.cond,t)){let a=await Ne(Me(o.body),t);a.stdout&&(r+=`${a.stdout}
`),s=!0;break}if(!s&&i.else_.length>0){let o=await Ne(Me(i.else_),t);o.stdout&&(r+=`${o.stdout}
`)}}}else if(i.type==="func")t.env.vars[`__func_${i.name}`]=i.body.join(`
`);else if(i.type==="arith"){let s=i.expr.trim(),o=s.match(/^(\w+)\s*(\+\+|--)$/);if(o){let a=parseInt(t.env.vars[o[1]]??"0",10);t.env.vars[o[1]]=String(o[2]==="++"?a+1:a-1)}else{let a=s.match(/^(\w+)\s*([+\-*/])=\s*(.+)$/);if(a){let l=parseInt(t.env.vars[a[1]]??"0",10),c=parseInt(a[3],10),u={"+":l+c,"-":l-c,"*":l*c,"/":Math.floor(l/c)};t.env.vars[a[1]]=String(u[a[2]]??l)}else{let l=At(s,t.env.vars);Number.isNaN(l)||(t.env.lastExitCode=l===0?1:0)}}}else if(i.type==="for"){let o=(await Lt(i.list,t.env.vars,t.env.lastExitCode,t)).trim().split(/\s+/).flatMap(nt);for(let a of o){t.env.vars[i.var]=a;let l=await Ne(Me(i.body),t);if(l.stdout&&(r+=`${l.stdout}
`),l.closeSession)return l}}else if(i.type==="while"){let s=0;for(;s<1e3&&await Dt(i.cond,t);){let o=await Ne(Me(i.body),t);if(o.stdout&&(r+=`${o.stdout}
`),o.closeSession)return o;s++}}return{...n,stdout:r.trim()||n.stdout}}function Rr(e){let t=[],n="",r=0,i=!1,s=!1,o=0;for(;o<e.length;){let l=e[o];if(!i&&!s){if(l==="'"){i=!0,n+=l,o++;continue}if(l==='"'){s=!0,n+=l,o++;continue}if(l==="{"){r++,n+=l,o++;continue}if(l==="}"){if(r--,n+=l,o++,r===0){let c=n.trim();for(c&&t.push(c),n="";o<e.length&&(e[o]===";"||e[o]===" ");)o++}continue}if(r===0&&(l===";"||l===`
`)){let c=n.trim();c&&!c.startsWith("#")&&t.push(c),n="",o++;continue}}else i&&l==="'"?i=!1:s&&l==='"'&&(s=!1);n+=l,o++}let a=n.trim();return a&&!a.startsWith("#")&&t.push(a),t}var Dr={name:"sh",aliases:["bash"],description:"Execute shell script or command",category:"shell",params:["-c <script>","[<file>]"],run:async e=>{let{args:t,shell:n,cwd:r}=e;if(v(t,"-c")){let s=t[t.indexOf("-c")+1]??"";if(!s)return{stderr:"sh: -c requires a script",exitCode:1};let o=Rr(s),a=Me(o);return Ne(a,e)}let i=t[0];if(i){let s=I(r,i);if(!n.vfs.exists(s))return{stderr:`sh: ${i}: No such file or directory`,exitCode:1};let o=n.vfs.readFile(s),a=Rr(o),l=Me(a);return Ne(l,e)}return{stderr:"sh: invalid usage. Use: sh -c 'cmd' or sh <file>",exitCode:1}}};var Lr={name:"shift",description:"Shift positional parameters",category:"shell",params:["[n]"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};let n=parseInt(e[0]??"1",10)||1,r=t.vars.__argv?.split("\0").filter(Boolean)??[];t.vars.__argv=r.slice(n).join("\0");let i=r.slice(n);for(let s=1;s<=9;s++)t.vars[String(s)]=i[s-1]??"";return{exitCode:0}}},Ur={name:"trap",description:"Trap signals and events",category:"shell",params:["[action] [signal...]"],run:({args:e,env:t})=>{if(!t||e.length===0)return{exitCode:0};let n=e[0]??"",r=e.slice(1);for(let i of r)t.vars[`__trap_${i.toUpperCase()}`]=n;return{exitCode:0}}},Vr={name:"return",description:"Return from a shell function",category:"shell",params:["[n]"],run:({args:e,env:t})=>{let n=parseInt(e[0]??"0",10);return t&&(t.lastExitCode=n),{exitCode:n}}};var zr={name:"sleep",description:"Delay execution",category:"system",params:["<seconds>"],run:async({args:e})=>{let t=parseFloat(e[0]??"1");return Number.isNaN(t)||t<0?{stderr:"sleep: invalid time",exitCode:1}:(await new Promise(n=>setTimeout(n,t*1e3)),{exitCode:0})}};var Br={name:"sort",description:"Sort lines of text",category:"text",params:["[-r] [-n] [-u] [-k <col>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=v(r,["-r"]),o=v(r,["-n"]),a=v(r,["-u"]),l=r.filter(g=>!g.startsWith("-")),d=[...(l.length>0?l.map(g=>{try{return L(e,I(n,g),"sort"),t.vfs.readFile(I(n,g))}catch{return""}}).join(`
`):i??"").split(`
`).filter(Boolean)].sort((g,S)=>o?Number(g)-Number(S):g.localeCompare(S)),m=s?d.reverse():d;return{stdout:(a?[...new Set(m)]:m).join(`
`),exitCode:0}}};var jr={name:"source",aliases:["."],description:"Execute commands from a file in the current shell environment",category:"shell",params:["<file> [args...]"],run:async({args:e,authUser:t,hostname:n,cwd:r,shell:i,env:s})=>{let o=e[0];if(!o)return{stderr:"source: missing filename",exitCode:1};let a=I(r,o);if(!i.vfs.exists(a))return{stderr:`source: ${o}: No such file or directory`,exitCode:1};let l=i.vfs.readFile(a),c=0;for(let u of l.split(`
`)){let d=u.trim();if(!d||d.startsWith("#"))continue;let m=await Z(d,t,n,"shell",r,i,void 0,s);if(c=m.exitCode??0,m.closeSession||m.switchUser)return m}return{exitCode:c}}};var Wr={name:"su",description:"Switch user",category:"users",params:["[-] [-c <cmd>] [username]"],run:async({authUser:e,shell:t,args:n,hostname:r,mode:i,cwd:s})=>{let o=n.includes("-")||n.includes("-l")||n.includes("--login"),a=n.indexOf("-c"),l=a!==-1?n[a+1]:void 0,u=n.filter((d,m)=>m!==a&&m!==a+1).filter(d=>d!=="-"&&d!=="-l"&&d!=="--login").find(d=>!d.startsWith("-"))??"root";return t.users.listUsers().includes(u)?e==="root"?l?Z(l,u,r,i,o?`/home/${u}`:s,t):{switchUser:u,nextCwd:o?`/home/${u}`:void 0,exitCode:0}:t.users.isSudoer(e)?{sudoChallenge:{username:u,targetUser:u,commandLine:l??null,loginShell:o,prompt:"Password: "},exitCode:0}:{stderr:`su: permission denied
`,exitCode:1}:{stderr:`su: user '${u}' does not exist
`,exitCode:1}}};function Wi(e){let{flags:t,flagsWithValues:n,positionals:r}=le(e,{flags:["-i","-S"],flagsWithValue:["-u","--user"]}),i=t.has("-i"),s=n.get("-u")||n.get("--user")||"root",o=r.length>0?r.join(" "):null;return{targetUser:s,loginShell:i,commandLine:o}}var Hr={name:"sudo",description:"Execute as superuser",category:"users",params:["<command...>"],run:async({authUser:e,hostname:t,mode:n,cwd:r,shell:i,args:s})=>{let{targetUser:o,loginShell:a,commandLine:l}=Wi(s);if(e!=="root"&&!i.users.isSudoer(e))return{stderr:"sudo: permission denied",exitCode:1};let c=o||"root",u=`[sudo] password for ${e}: `;return e==="root"?!l&&a?{switchUser:c,nextCwd:`/home/${c}`,exitCode:0}:l?Z(l,c,t,n,a?`/home/${c}`:r,i):{stderr:"sudo: missing command",exitCode:1}:{sudoChallenge:{username:e,targetUser:c,commandLine:l,loginShell:a,prompt:u},exitCode:0}}};var qr={name:"tail",description:"Output last lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=ue(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
`),p=d.endsWith(`
`),g=p?m.slice(0,-1):m;return g.slice(Math.max(0,g.length-a)).join(`
`)+(p?`
`:"")};if(l.length===0)return{stdout:c(i??""),exitCode:0};let u=[];for(let d of l){let m=I(n,d);try{L(e,m,"tail"),u.push(c(t.vfs.readFile(m)))}catch{return{stderr:`tail: ${d}: No such file or directory`,exitCode:1}}}return{stdout:u.join(`
`),exitCode:0}}};var Gr={name:"tar",description:"Archive utility",category:"archive",params:["[-czf|-xzf|-tf] <archive> [files...]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=[],s=!1;for(let m of r)if(/^-[a-zA-Z]{2,}$/.test(m))for(let p of m.slice(1))i.push(`-${p}`);else if(!s&&/^[cxtdru]{1,}[a-zA-Z]*$/.test(m)&&!m.includes("/")&&!m.startsWith("-")){s=!0;for(let p of m)i.push(`-${p}`)}else i.push(m);let o=i.includes("-c"),a=i.includes("-x"),l=i.includes("-t"),c=i.indexOf("-f"),u=c!==-1?i[c+1]:i.find(m=>m.endsWith(".tar")||m.endsWith(".tar.gz")||m.endsWith(".tgz"));if(!o&&!a&&!l)return{stderr:`tar: must specify -c, -x, or -t
`,exitCode:1};if(!u)return{stderr:`tar: no archive specified
`,exitCode:1};let d=I(n,u);if(o){let m=new Set;c!==-1&&m.add(c+1);let p=i.filter((S,C)=>!S.startsWith("-")&&S!==u&&!m.has(C)),g={};for(let S of p){let C=I(n,S);try{if(t.vfs.stat(C).type==="file")g[S]=t.vfs.readFile(C);else{let x=(_,w)=>{for(let A of t.vfs.list(_)){let N=`${_}/${A}`,b=`${w}/${A}`;t.vfs.stat(N).type==="file"?g[b]=t.vfs.readFile(N):x(N,b)}};x(C,S)}}catch{return{stderr:`tar: ${S}: No such file or directory`,exitCode:1}}}return t.writeFileAsUser(e,d,JSON.stringify(g)),{exitCode:0}}if(l||a){let m;try{m=JSON.parse(t.vfs.readFile(d))}catch{return{stderr:`tar: ${u}: cannot open archive`,exitCode:1}}if(l)return{stdout:Object.keys(m).join(`
`),exitCode:0};for(let[p,g]of Object.entries(m))t.writeFileAsUser(e,I(n,p),g);return{exitCode:0}}return{stderr:"tar: must specify -c, -x, or -t",exitCode:1}}};var Yr={name:"tee",description:"Read stdin, write to stdout and files",category:"text",params:["[-a] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=v(r,["-a"]),o=r.filter(l=>!l.startsWith("-")),a=i??"";for(let l of o){let c=I(n,l);if(s){let u=(()=>{try{return t.vfs.readFile(c)}catch{return""}})();t.writeFileAsUser(e,c,u+a)}else t.writeFileAsUser(e,c,a)}return{stdout:a,exitCode:0}}};function Ve(e,t,n){if(e[e.length-1]==="]"&&(e=e.slice(0,-1)),e[0]==="["&&(e=e.slice(1)),e.length===0)return!1;if(e[0]==="!")return!Ve(e.slice(1),t,n);let r=e.indexOf("-a");if(r!==-1)return Ve(e.slice(0,r),t,n)&&Ve(e.slice(r+1),t,n);let i=e.indexOf("-o");if(i!==-1)return Ve(e.slice(0,i),t,n)||Ve(e.slice(i+1),t,n);if(e.length===2){let[s,o=""]=e,l=(c=>c.startsWith("/")?c:`${n}/${c}`.replace(/\/+/g,"/"))(o);switch(s){case"-e":return t.vfs.exists(l);case"-f":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file";case"-d":return t.vfs.exists(l)&&t.vfs.stat(l).type==="directory";case"-r":return t.vfs.exists(l);case"-w":return t.vfs.exists(l);case"-x":return t.vfs.exists(l)&&!!(t.vfs.stat(l).mode&73);case"-s":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file"&&t.vfs.stat(l).size>0;case"-z":return o.length===0;case"-n":return o.length>0;case"-L":return t.vfs.isSymlink(l)}}if(e.length===3){let[s="",o,a=""]=e,l=Number(s),c=Number(a);switch(o){case"=":case"==":return s===a;case"!=":return s!==a;case"<":return s<a;case">":return s>a;case"-eq":return l===c;case"-ne":return l!==c;case"-lt":return l<c;case"-le":return l<=c;case"-gt":return l>c;case"-ge":return l>=c}}return e.length===1?(e[0]??"").length>0:!1}var Kr={name:"test",aliases:["["],description:"Evaluate conditional expression",category:"shell",params:["<expression>"],run:({args:e,shell:t,cwd:n})=>{try{return{exitCode:Ve([...e],t,n)?0:1}}catch{return{stderr:"test: malformed expression",exitCode:2}}}};var Jr={name:"touch",description:"Create or update files",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"touch: missing file operand",exitCode:1};for(let i of r){let s=I(n,i);L(e,s,"touch"),t.vfs.exists(s)||t.writeFileAsUser(e,s,"")}return{exitCode:0}}};function Hi(e){return e.replace(/\\n/g,`
`).replace(/\\t/g,"	").replace(/\\r/g,"\r").replace(/\\\\/g,"\\")}function Zr(e){let t=[],n=Hi(e),r=0;for(;r<n.length;){if(r+2<n.length&&n[r+1]==="-"){let i=n.charCodeAt(r),s=n.charCodeAt(r+2);if(i<=s){for(let o=i;o<=s;o++)t.push(String.fromCharCode(o));r+=3;continue}}t.push(n[r]),r++}return t}var Qr={name:"tr",description:"Translate or delete characters",category:"text",params:["[-d] [-s] <set1> [set2]"],run:({args:e,stdin:t})=>{let n=v(e,["-d"]),r=v(e,["-s"]),i=e.filter(l=>!l.startsWith("-")),s=Zr(i[0]??""),o=Zr(i[1]??""),a=t??"";if(n){let l=new Set(s);a=[...a].filter(c=>!l.has(c)).join("")}else if(o.length>0){let l=new Map;for(let c=0;c<s.length;c++)l.set(s[c],o[c]??o[o.length-1]??"");a=[...a].map(c=>l.get(c)??c).join("")}if(r&&o.length>0){let l=new Set(o);a=a.replace(/(.)\1+/g,(c,u)=>l.has(u)?u:c)}return{stdout:a,exitCode:0}}};var Xr={name:"tree",description:"Display directory tree",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=I(n,$e(r,0)??n);return L(e,i,"tree"),{stdout:t.vfs.tree(i),exitCode:0}}};var es={name:"true",description:"Return success exit code",category:"shell",params:[],run:()=>({exitCode:0})},ts={name:"false",description:"Return failure exit code",category:"shell",params:[],run:()=>({exitCode:1})};var ns={name:"type",description:"Describe how a command would be interpreted",category:"shell",params:["<command...>"],run:({args:e,shell:t,env:n})=>{if(e.length===0)return{stderr:"type: missing argument",exitCode:1};let r=(n?.vars?.PATH??"/usr/local/bin:/usr/bin:/bin").split(":"),i=[],s=0;for(let o of e){if(he(o)){i.push(`${o} is a shell builtin`);continue}let a=!1;for(let l of r){let c=`${l}/${o}`;if(t.vfs.exists(c)){i.push(`${o} is ${c}`),a=!0;break}}a||(i.push(`${o}: not found`),s=1)}return{stdout:i.join(`
`),exitCode:s}}};var rs={name:"uname",description:"Print system information",category:"system",params:["[-a] [-s] [-r] [-m]"],run:({shell:e,args:t})=>{let n=v(t,["-a"]),r="Linux",i=e.properties?.kernel??"5.15.0",s=e.properties?.arch??"x86_64",o=e.hostname;return n?{stdout:`${r} ${o} ${i} #1 SMP ${s} GNU/Linux`,exitCode:0}:v(t,["-r"])?{stdout:i,exitCode:0}:v(t,["-m"])?{stdout:s,exitCode:0}:{stdout:r,exitCode:0}}};var ss={name:"uniq",description:"Report or filter out repeated lines",category:"text",params:["[-c] [-d] [-u] [file]"],run:({args:e,stdin:t})=>{let n=v(e,["-c"]),r=v(e,["-d"]),i=v(e,["-u"]),s=(t??"").split(`
...
`)||void 0,exitCode:0}):{stdout:S,exitCode:0}}};var cs={name:"which",description:"Locate a command in PATH",category:"shell",params:["<command...>"],run:({args:e,shell:t,env:n})=>{if(e.length===0)return{stderr:"which: missing argument",exitCode:1};let r=(n?.vars?.PATH??"/usr/local/bin:/usr/bin:/bin").split(":"),i=[],s=!1;for(let o of e){let a=!1;for(let l of r){let c=`${l}/${o}`;if(t.vfs.exists(c)&&t.vfs.stat(c).type==="file"){i.push(c),a=!0;break}}a||(s=!0)}return i.length===0?{exitCode:1}:{stdout:i.join(`
`),exitCode:s?1:0}}};function pt(e){let t=e.toLocaleString("en-US",{weekday:"short"}),n=e.toLocaleString("en-US",{month:"short"}),r=e.getDate().toString().padStart(2,"0"),i=e.getHours().toString().padStart(2,"0"),s=e.getMinutes().toString().padStart(2,"0"),o=e.getSeconds().toString().padStart(2,"0"),a=e.getFullYear();return`${t} ${n} ${r} ${i}:${s}:${o} ${a}`}var us={name:"who",description:"Show active sessions",category:"system",params:[],run:({shell:e})=>({stdout:e.users.listActiveSessions().map(n=>{let r=new Date(n.startedAt),i=Number.isNaN(r.getTime())?n.startedAt:pt(r);return`${n.username} ${n.tty} ${i} (${n.remoteAddress||"unknown"})`}).join(`
`),exitCode:0})};var ds={name:"whoami",description:"Print current user",category:"system",params:[],run:({authUser:e})=>({stdout:e,exitCode:0})};var ms={name:"xargs",description:"Build and execute command lines from stdin",category:"text",params:["[command] [args...]"],run:async({authUser:e,hostname:t,mode:n,cwd:r,args:i,stdin:s,shell:o,env:a})=>{let l=i[0]??"echo",c=i.slice(1),u=(s??"").trim().split(/\s+/).filter(Boolean);if(u.length===0)return{exitCode:0};let d=[l,...c,...u].join(" ");return Z(d,e,t,n,r,o,void 0,a)}};var qi=[Nr,hn,tr,Xr,dn,Jr,Or,ir,Sn,or,Jn,Zn,gn,Qn,Xn,On,Fn,Tr,cn,Br,ss,as,Un,qr,xn,Qr,Yr,ms,$n,Gr,Dn,Ln,un,ds,us,qn,Yn,Rn,rs,Mr,Kn,Pn,Nn,wn,zr,$r,En,In,_n,Fr,is,Dr,yn,An,lr,Gn,bn,ls,en,Pr,Cn,Hr,Wr,Sr,an,ln,kn,Mn,cs,ns,sr,nn,rn,Kr,jr,Hn,kr,_r,vn,Lr,Ur,Vr,es,ts,vr,Cr,wr,Ar,os,Tn,nr],ps=[],Ye=new Map,ft=null,Gi=Wn(()=>Vt().map(e=>e.name));function Ut(){Ye.clear();for(let e of Vt()){Ye.set(e.name,e);for(let t of e.aliases??[])Ye.set(t,e)}ft=Array.from(Ye.keys()).sort()}function Vt(){return[...qi,...ps,Gi]}function Nt(e){let t={...e,name:e.name.trim().toLowerCase(),aliases:e.aliases?.map(r=>r.trim().toLowerCase())};if([t.name,...t.aliases??[]].some(r=>r.length===0||/\s/.test(r)))throw new Error("Command names must be non-empty and contain no spaces");ps.push(t),Ut()}function Et(e,t,n){return{name:e,params:t,run:n}}function ze(){return ft||Ut(),ft}function It(){return Vt()}function he(e){return ft||Ut(),Ye.get(e.toLowerCase())}import{spawn as Ki}from"node:child_process";import{readFile as Yi}from"node:fs/promises";import*as ht from"node:path";function zt(e){return`'${e.replace(/'/g,"'\\''")}'`}function Ke(e){return e.replace(/\r\n/g,`
`).replace(/\r/g,`
`).replace(/\n/g,`\r
`)}function fs(e,t){let n=Number.isFinite(t.cols)&&t.cols>0?Math.floor(t.cols):80,r=Number.isFinite(t.rows)&&t.rows>0?Math.floor(t.rows):24;return`stty cols ${n} rows ${r} 2>/dev/null; ${e}`}function gt(e,t){return!t||t.trim()===""||t==="."?e:t.startsWith("/")?ht.posix.normalize(t):ht.posix.normalize(ht.posix.join(e,t))}async function hs(e){try{let n=(await Yi(`/proc/${e}/task/${e}/children`,"utf8")).trim().split(/\s+/).filter(Boolean).map(i=>Number.parseInt(i,10)).filter(i=>Number.isInteger(i)&&i>0),r=await Promise.all(n.map(i=>hs(i)));return[...n,...r.flat()]}catch{return[]}}async function gs(e=process.pid){let t=await hs(e),n=Array.from(new Set(t)).sort((r,i)=>r-i);return n.length===0?null:n.join(",")}function ys(e,t,n){let r=fs(e,t),i=Ki("script",["-qfec",r,"/dev/null"],{stdio:["pipe","pipe","pipe"],env:{...process.env,TERM:process.env.TERM??"xterm-256color"}});return i.stdout.on("data",s=>{n.write(s.toString("utf8"))}),i.stderr.on("data",s=>{n.write(s.toString("utf8"))}),i}function yt(e,t,n){return ys(`nano -- ${zt(e)}`,t,n)}function Ss(e,t,n){return ys(`htop -p ${zt(e)}`,t,n)}function St(e,t,n){let r=[`Linux ${e} ${t.kernel} ${t.arch}`,"","The programs included with the Fortune GNU/Linux system are free software;","the exact distribution terms for each program are described in the","individual files in /usr/share/doc/*/copyright.","","Fortune GNU/Linux comes with ABSOLUTELY NO WARRANTY, to the extent","permitted by applicable law."];if(n){let i=new Date(n.at),s=Number.isNaN(i.getTime())?n.at:pt(i);r.push(`Last login: ${s} from ${n.from||"unknown"}`)}return r.push(""),`${r.map(i=>`${i}\r
`).join("")}`}function bt(e,t,n){let r=e==="root",i=r?"\x1B[31;1m":"\x1B[35;1m",s="\x1B[37;1m",o="\x1B[34;1m",a="\x1B[0m";return`${s}[${i}${e}${s}@${o}${t}${a} ${n}${s}]${a}${r?"#":"$"} `}import{EventEmitter as Eo}from"node:events";import*as we from"node:os";import{EventEmitter as to}from"node:events";import*as B from"node:fs";import*as ce from"node:path";import{gunzipSync as Yt,gzipSync as $s}from"node:zlib";var Ht=Buffer.from([86,70,83,33]),Ji=1,Bt=1,bs=2,jt=class{chunks=[];write(t){this.chunks.push(t)}writeUint8(t){let n=Buffer.allocUnsafe(1);n.writeUInt8(t,0),this.chunks.push(n)}writeUint16(t){let n=Buffer.allocUnsafe(2);n.writeUInt16LE(t,0),this.chunks.push(n)}writeUint32(t){let n=Buffer.allocUnsafe(4);n.writeUInt32LE(t,0),this.chunks.push(n)}writeFloat64(t){let n=Buffer.allocUnsafe(8);n.writeDoubleBE(t,0),this.chunks.push(n)}writeString(t){let n=Buffer.from(t,"utf8");this.writeUint16(n.length),this.chunks.push(n)}writeBytes(t){this.writeUint32(t.length),this.chunks.push(t)}toBuffer(){return Buffer.concat(this.chunks)}};function xs(e,t){if(t.type==="file"){let n=t;e.writeUint8(Bt),e.writeString(n.name),e.writeUint32(n.mode),e.writeFloat64(n.createdAt),e.writeFloat64(n.updatedAt),e.writeUint8(n.compressed?1:0),e.writeBytes(n.content)}else if(t.type==="stub"){let n=t;e.writeUint8(Bt),e.writeString(n.name),e.writeUint32(n.mode),e.writeFloat64(n.createdAt),e.writeFloat64(n.updatedAt),e.writeUint8(0),e.writeBytes(Buffer.from(n.stubContent,"utf8"))}else{let n=t;e.writeUint8(bs),e.writeString(n.name),e.writeUint32(n.mode),e.writeFloat64(n.createdAt),e.writeFloat64(n.updatedAt);let r=Object.values(n.children);e.writeUint32(r.length);for(let i of r)xs(e,i)}}function qt(e){let t=new jt;return t.write(Ht),t.writeUint8(Ji),xs(t,e),t.toBuffer()}var Wt=class{constructor(t){this.buf=t}buf;pos=0;readUint8(){return this.buf.readUInt8(this.pos++)}readUint16(){let t=this.buf.readUInt16LE(this.pos);return this.pos+=2,t}readUint32(){let t=this.buf.readUInt32LE(this.pos);return this.pos+=4,t}readFloat64(){let t=this.buf.readDoubleBE(this.pos);return this.pos+=8,t}readString(){let t=this.readUint16(),n=this.buf.toString("utf8",this.pos,this.pos+t);return this.pos+=t,n}readBytes(){let t=this.readUint32(),n=this.buf.slice(this.pos,this.pos+t);return this.pos+=t,n}remaining(){return this.buf.length-this.pos}};function ws(e){let t=e.readUint8(),n=e.readString(),r=e.readUint32(),i=e.readFloat64(),s=e.readFloat64();if(t===Bt){let o=e.readUint8()===1,a=e.readBytes();return{type:"file",name:n,mode:r,createdAt:i,updatedAt:s,compressed:o,content:a}}if(t===bs){let o=e.readUint32(),a=Object.create(null);for(let l=0;l<o;l++){let c=ws(e);a[c.name]=c}return{type:"directory",name:n,mode:r,createdAt:i,updatedAt:s,children:a,_childCount:o}}throw new Error(`[VFS binary] Unknown node type: 0x${t.toString(16)}`)}function Ee(e){if(e.length<5)throw new Error("[VFS binary] Buffer too short");if(!e.slice(0,4).equals(Ht))throw new Error("[VFS binary] Invalid magic \u2014 not a VFS binary snapshot");let n=new Wt(e);for(let i=0;i<5;i++)n.readUint8();let r=ws(n);if(r.type!=="directory")throw new Error("[VFS binary] Root node must be a directory");return r}function vs(e){return e.length>=4&&e.slice(0,4).equals(Ht)}import*as X from"node:fs";var W={WRITE:1,MKDIR:2,REMOVE:3,CHMOD:4,MOVE:5,SYMLINK:6},Je="utf8";function Zi(e,t,n){let r=Buffer.from(n,Je);return e.writeUInt16LE(r.length,t),r.copy(e,t+2),2+r.length}function Qi(e){let t=Buffer.from(e.path,Je),n=0;e.op===W.WRITE?n=4+(e.content?.length??0)+4:e.op===W.MKDIR?n=4:e.op===W.REMOVE?n=0:e.op===W.CHMOD?n=4:(e.op===W.MOVE||e.op===W.SYMLINK)&&(n=2+Buffer.byteLength(e.dest??"",Je));let r=3+t.length+n,i=Buffer.allocUnsafe(r),s=0;if(i.writeUInt8(e.op,s++),i.writeUInt16LE(t.length,s),s+=2,t.copy(i,s),s+=t.length,e.op===W.WRITE){let o=e.content??Buffer.alloc(0);i.writeUInt32LE(o.length,s),s+=4,o.copy(i,s),s+=o.length,i.writeUInt32LE(e.mode??420,s),s+=4}else e.op===W.MKDIR?(i.writeUInt32LE(e.mode??493,s),s+=4):e.op===W.CHMOD?(i.writeUInt32LE(e.mode??420,s),s+=4):(e.op===W.MOVE||e.op===W.SYMLINK)&&(s+=Zi(i,s,e.dest??""));return i}function Xi(e){let t=[],n=0;try{for(;n<e.length&&!(n+3>e.length);){let r=e.readUInt8(n++),i=e.readUInt16LE(n);if(n+=2,n+i>e.length)break;let s=e.subarray(n,n+i).toString(Je);if(n+=i,r===W.WRITE){if(n+4>e.length)break;let o=e.readUInt32LE(n);if(n+=4,n+o+4>e.length)break;let a=Buffer.from(e.subarray(n,n+o));n+=o;let l=e.readUInt32LE(n);n+=4,t.push({op:r,path:s,content:a,mode:l})}else if(r===W.MKDIR){if(n+4>e.length)break;let o=e.readUInt32LE(n);n+=4,t.push({op:r,path:s,mode:o})}else if(r===W.REMOVE)t.push({op:r,path:s});else if(r===W.CHMOD){if(n+4>e.length)break;let o=e.readUInt32LE(n);n+=4,t.push({op:r,path:s,mode:o})}else if(r===W.MOVE||r===W.SYMLINK){if(n+2>e.length)break;let o=e.readUInt16LE(n);if(n+=2,n+o>e.length)break;let a=e.subarray(n,n+o).toString(Je);n+=o,t.push({op:r,path:s,dest:a})}else break}}catch{}return t}function Cs(e,t){let n=Qi(t);if(X.existsSync(e)){let r=X.openSync(e,X.constants.O_WRONLY|X.constants.O_CREAT|X.constants.O_APPEND);try{X.writeSync(r,n)}finally{X.closeSync(r)}}else X.existsSync(".vfs")||X.mkdirSync(".vfs"),X.writeFileSync(e,n)}function Gt(e){if(!X.existsSync(e))return[];let t=X.readFileSync(e);return t.length===0?[]:Xi(t)}function Ps(e){X.existsSync(e)&&X.unlinkSync(e)}import*as xt from"node:path";function H(e){if(!e||e.trim()==="")return"/";let t=xt.posix.normalize(e.startsWith("/")?e:`/${e}`);return t===""?"/":t}function eo(e){return e.split("/").filter(Boolean)}function oe(e,t){let n=H(t);if(n==="/")return e;let r=eo(n),i=e;for(let s of r){if(i.type!=="directory")throw new Error(`Path '${n}' does not exist.`);let o=i.children[s];if(!o)throw new Error(`Path '${n}' does not exist.`);i=o}return i}function _e(e,t,n,r){let i=H(t);if(i==="/")throw new Error("Root path has no parent directory.");let s=xt.posix.dirname(i),o=xt.posix.basename(i);if(!o)throw new Error(`Invalid path '${t}'.`);n&&r(s);let a=oe(e,s);if(a.type!=="directory")throw new Error(`Parent path '${s}' is not a directory.`);return{parent:a,name:o}}var Kt=class e extends to{root;mode;snapshotFile;journalFile;evictionThreshold;flushAfterNWrites;_writesSinceFlush=0;_flushTimer=null;_dirty=!1;mounts=new Map;static isBrowser=typeof process>"u"||typeof process.versions?.node>"u";constructor(t={}){if(super(),this.mode=t.mode??"memory",this.mode==="fs"){if(!t.snapshotPath)throw new Error('VirtualFileSystem: "snapshotPath" is required when mode is "fs".');this.snapshotFile=ce.resolve(t.snapshotPath,"vfs-snapshot.vfsb"),this.journalFile=ce.resolve(t.snapshotPath,"vfs-journal.bin"),this.evictionThreshold=t.evictionThresholdBytes??64*1024,this.flushAfterNWrites=t.flushAfterNWrites??500;let n=t.flushIntervalMs??1e3;n>0&&(this._flushTimer=setInterval(()=>{this._dirty&&this._autoFlush()},n),typeof this._flushTimer=="object"&&this._flushTimer!==null&&"unref"in this._flushTimer&&this._flushTimer.unref())}else this.snapshotFile=null,this.journalFile=null,this.evictionThreshold=0,this.flushAfterNWrites=0;this.root=this.makeDir("",493)}makeDir(t,n){let r=Date.now();return{type:"directory",name:t,mode:n,createdAt:r,updatedAt:r,children:Object.create(null),_childCount:0}}makeFile(t,n,r,i){let s=Date.now();return{type:"file",name:t,content:n,mode:r,compressed:i,createdAt:s,updatedAt:s}}makeStub(t,n,r){let i=Date.now();return{type:"stub",name:t,stubContent:n,mode:r,createdAt:i,updatedAt:i}}writeStub(t,n,r=420){let i=H(t),{parent:s,name:o}=_e(this.root,i,!0,l=>this.mkdirRecursive(l,493)),a=s.children[o];if(a?.type==="directory")throw new Error(`Cannot write stub '${i}': path is a directory.`);a?.type!=="file"&&(a||s._childCount++,s.children[o]=this.makeStub(o,n,r))}mkdirRecursive(t,n){let r=H(t);if(r==="/")return;let i=r.split("/").filter(Boolean),s=this.root,o="";for(let a of i){o+=`/${a}`;let l=s.children[a];if(!l)l=this.makeDir(a,n),s.children[a]=l,s._childCount++,this.emit("dir:create",{path:o,mode:n}),this._journal({op:W.MKDIR,path:o,mode:n});else if(l.type!=="directory")throw new Error(`Cannot create directory '${o}': path is a file.`);s=l}}async restoreMirror(){if(!(this.mode!=="fs"||!this.snapshotFile)){if(!B.existsSync(this.snapshotFile)){if(this.journalFile){let t=Gt(this.journalFile);t.length>0&&this._replayJournal(t)}return}try{let t=B.readFileSync(this.snapshotFile);if(vs(t))this.root=Ee(t);else{let n=JSON.parse(t.toString("utf8"));this.root=this.deserializeDir(n.root,""),console.info("[VirtualFileSystem] Migrating legacy JSON snapshot to binary format.")}if(this.emit("snapshot:restore",{path:this.snapshotFile}),this.journalFile){let n=Gt(this.journalFile);n.length>0&&this._replayJournal(n)}}catch(t){console.warn(`[VirtualFileSystem] Could not restore snapshot from ${this.snapshotFile}:`,t instanceof Error?t.message:String(t))}}}async flushMirror(){if(this.mode!=="fs"||!this.snapshotFile){this.emit("mirror:flush");return}let t=ce.dirname(this.snapshotFile);B.mkdirSync(t,{recursive:!0});let n=this.root,r=qt(n);B.writeFileSync(this.snapshotFile,r),this.journalFile&&Ps(this.journalFile),this._dirty=!1,this._writesSinceFlush=0,this.emit("mirror:flush",{path:this.snapshotFile}),this.evictLargeFiles()}getMode(){return this.mode}getSnapshotPath(){return this.snapshotFile}async _autoFlush(){this._dirty&&await this.flushMirror()}_markDirty(){this._dirty=!0,this.flushAfterNWrites>0&&(this._writesSinceFlush++,this._writesSinceFlush>=this.flushAfterNWrites&&(this._writesSinceFlush=0,this._autoFlush()))}async stopAutoFlush(){this._flushTimer!==null&&(clearInterval(this._flushTimer),this._flushTimer=null),this._dirty&&await this.flushMirror()}importRootTree(t){let n=this._replayMode;this._replayMode=!0;try{this.root=t}finally{this._replayMode=n}}mergeRootTree(t){let n=this._replayMode;this._replayMode=!0;try{this._mergeDir(this.root,t)}finally{this._replayMode=n}}_mergeDir(t,n){for(let[r,i]of Object.entries(n.children)){let s=t.children[r];i.type==="directory"?s?s.type==="directory"&&this._mergeDir(s,i):(t.children[r]=i,t._childCount++):s||(t.children[r]=i,t._childCount++)}}encodeBinary(){return qt(this.root)}releaseTree(){this.root=this.makeDir("",493)}_replayMode=!1;_journal(t){this.journalFile&&!this._replayMode&&(Cs(this.journalFile,t),this._markDirty())}_replayJournal(t){this._replayMode=!0;try{for(let n of t)try{n.op===W.WRITE?this.writeFile(n.path,n.content??Buffer.alloc(0),{mode:n.mode}):n.op===W.MKDIR?this.mkdir(n.path,n.mode):n.op===W.REMOVE?this.exists(n.path)&&this.remove(n.path,{recursive:!0}):n.op===W.CHMOD?this.exists(n.path)&&this.chmod(n.path,n.mode??420):n.op===W.MOVE?this.exists(n.path)&&n.dest&&this.move(n.path,n.dest):n.op===W.SYMLINK&&n.dest&&this.symlink(n.dest,n.path)}catch{}}finally{this._replayMode=!1}}evictLargeFiles(){!this.snapshotFile||this.evictionThreshold===0||B.existsSync(this.snapshotFile)&&this._evictDir(this.root)}_evictDir(t){for(let n of Object.values(t.children))if(n.type==="directory")this._evictDir(n);else if(n.type==="file"&&!n.evicted){let r=n.compressed?n.size??n.content.length*2:n.content.length;r>this.evictionThreshold&&(n.size=r,n.content=Buffer.alloc(0),n.evicted=!0)}}_reloadEvicted(t,n){if(!(!t.evicted||!this.snapshotFile)&&B.existsSync(this.snapshotFile))try{let r=B.readFileSync(this.snapshotFile),i=Ee(r),s=n.split("/").filter(Boolean),o=i;for(let a of s){if(o.type!=="directory")return;let l=o.children[a];if(!l)return;o=l}o.type==="file"&&(t.content=o.content,t.compressed=o.compressed,t.evicted=void 0)}catch{}}mount(t,n,{readOnly:r=!0}={}){if(e.isBrowser)return;let i=H(t),s=ce.resolve(n);if(!B.existsSync(s))throw new Error(`VirtualFileSystem.mount: host path does not exist: "${s}"`);if(!B.statSync(s).isDirectory())throw new Error(`VirtualFileSystem.mount: host path is not a directory: "${s}"`);this.mkdir(i),this.mounts.set(i,{hostPath:s,readOnly:r}),this.emit("mount",{vPath:i,hostPath:s,readOnly:r})}unmount(t){let n=H(t);this.mounts.delete(n)&&this.emit("unmount",{vPath:n})}getMounts(){return[...this.mounts.entries()].map(([t,n])=>({vPath:t,...n}))}resolveMount(t){let n=H(t),r=[...this.mounts.entries()].sort(([i],[s])=>s.length-i.length);for(let[i,s]of r)if(n===i||n.startsWith(`${i}/`)){let o=n.slice(i.length).replace(/^\//,""),a=o?ce.join(s.hostPath,o):s.hostPath;return{hostPath:s.hostPath,readOnly:s.readOnly,relPath:o,fullHostPath:a}}return null}mkdir(t,n=493){let r=H(t),i=(()=>{try{return oe(this.root,r)}catch{return null}})();if(i&&i.type!=="directory")throw new Error(`Cannot create directory '${r}': path is a file.`);this.mkdirRecursive(r,n)}writeFile(t,n,r={}){let i=this.resolveMount(t);if(i){if(i.readOnly)throw new Error(`EROFS: read-only file system, open '${i.fullHostPath}'`);let p=ce.dirname(i.fullHostPath);B.existsSync(p)||B.mkdirSync(p,{recursive:!0}),B.writeFileSync(i.fullHostPath,Buffer.isBuffer(n)?n:Buffer.from(n,"utf8"));return}let s=H(t),{parent:o,name:a}=_e(this.root,s,!0,p=>this.mkdirRecursive(p,493)),l=o.children[a];if(l?.type==="directory")throw new Error(`Cannot write file '${s}': path is a directory.`);let c=Buffer.isBuffer(n)?n:Buffer.from(n,"utf8"),u=r.compress??!1,d=u?$s(c):c,m=r.mode??420;if(l&&l.type==="file"){let p=l;p.content=d,p.compressed=u,p.mode=m,p.updatedAt=Date.now()}else l||o._childCount++,o.children[a]=this.makeFile(a,d,m,u);this.emit("file:write",{path:s,size:d.length}),this._journal({op:W.WRITE,path:s,content:c,mode:m})}readFile(t){let n=this.resolveMount(t);if(n){if(!B.existsSync(n.fullHostPath))throw new Error(`ENOENT: no such file or directory, open '${n.fullHostPath}'`);return B.readFileSync(n.fullHostPath,"utf8")}let r=H(t),i=oe(this.root,r);if(i.type==="stub")return this.emit("file:read",{path:r,size:i.stubContent.length}),i.stubContent;if(i.type!=="file")throw new Error(`Cannot read '${t}': not a file.`);let s=i;s.evicted&&this._reloadEvicted(s,r);let o=s.compressed?Yt(s.content):s.content;return this.emit("file:read",{path:r,size:o.length}),o.toString("utf8")}readFileRaw(t){let n=this.resolveMount(t);if(n){if(!B.existsSync(n.fullHostPath))throw new Error(`ENOENT: no such file or directory, open '${n.fullHostPath}'`);return B.readFileSync(n.fullHostPath)}let r=H(t),i=oe(this.root,r);if(i.type==="stub"){let a=Buffer.from(i.stubContent,"utf8");return this.emit("file:read",{path:r,size:a.length}),a}if(i.type!=="file")throw new Error(`Cannot read '${t}': not a file.`);let s=i;s.evicted&&this._reloadEvicted(s,r);let o=s.compressed?Yt(s.content):s.content;return this.emit("file:read",{path:r,size:o.length}),o}exists(t){let n=this.resolveMount(t);if(n)return B.existsSync(n.fullHostPath);try{return oe(this.root,H(t)),!0}catch{return!1}}chmod(t,n){let r=H(t);oe(this.root,r).mode=n,this._journal({op:W.CHMOD,path:r,mode:n})}stat(t){let n=this.resolveMount(t);if(n){if(!B.existsSync(n.fullHostPath))throw new Error(`ENOENT: stat '${n.fullHostPath}'`);let a=B.statSync(n.fullHostPath),l=n.relPath.split("/").pop()??n.fullHostPath.split("/").pop()??"",c=a.mtime;return a.isDirectory()?{type:"directory",name:l,path:H(t),mode:493,createdAt:a.birthtime,updatedAt:c,childrenCount:B.readdirSync(n.fullHostPath).length}:{type:"file",name:l,path:H(t),mode:n.readOnly?292:420,createdAt:a.birthtime,updatedAt:c,compressed:!1,size:a.size}}let r=H(t),i=oe(this.root,r),s=r==="/"?"":ce.posix.basename(r);if(i.type==="stub"){let a=i;return{type:"file",name:s,path:r,mode:a.mode,createdAt:new Date(a.createdAt),updatedAt:new Date(a.updatedAt),compressed:!1,size:a.stubContent.length}}if(i.type==="file"){let a=i;return{type:"file",name:s,path:r,mode:a.mode,createdAt:new Date(a.createdAt),updatedAt:new Date(a.updatedAt),compressed:a.compressed,size:a.evicted?a.size??0:a.content.length}}let o=i;return{type:"directory",name:s,path:r,mode:o.mode,createdAt:new Date(o.createdAt),updatedAt:new Date(o.updatedAt),childrenCount:o._childCount}}list(t="/"){let n=this.resolveMount(t);if(n){if(!B.existsSync(n.fullHostPath))return[];try{return B.readdirSync(n.fullHostPath).sort()}catch{return[]}}let r=H(t),i=oe(this.root,r);if(i.type!=="directory")throw new Error(`Cannot list '${t}': not a directory.`);return Object.keys(i.children).sort()}tree(t="/"){let n=H(t),r=oe(this.root,n);if(r.type!=="directory")throw new Error(`Cannot render tree for '${t}': not a directory.`);let i=t==="/"?"/":ce.posix.basename(n);return this.renderTreeLines(r,i)}renderTreeLines(t,n){let r=[n],i=Object.keys(t.children).sort();for(let s=0;s<i.length;s++){let o=i[s],a=t.children[o],l=s===i.length-1,c=l?"\u2514\u2500\u2500 ":"\u251C\u2500\u2500 ",u=l?"    ":"\u2502   ";if(r.push(`${c}${o}`),a.type==="directory"){let d=this.renderTreeLines(a,"").split(`
`).slice(1).map(m=>`${u}${m}`);r.push(...d)}}return r.join(`
`)}getUsageBytes(t="/"){return this.computeUsage(oe(this.root,H(t)))}computeUsage(t){if(t.type==="file")return t.content.length;if(t.type==="stub")return t.stubContent.length;let n=0;for(let r of Object.values(t.children))n+=this.computeUsage(r);return n}compressFile(t){let n=oe(this.root,H(t));if(n.type!=="file")throw new Error(`Cannot compress '${t}': not a file.`);let r=n;r.compressed||(r.content=$s(r.content),r.compressed=!0,r.updatedAt=Date.now())}decompressFile(t){let n=oe(this.root,H(t));if(n.type!=="file")throw new Error(`Cannot decompress '${t}': not a file.`);let r=n;r.compressed&&(r.content=Yt(r.content),r.compressed=!1,r.updatedAt=Date.now())}symlink(t,n){let r=H(n),i=t.startsWith("/")?H(t):t,{parent:s,name:o}=_e(this.root,r,!0,l=>this.mkdirRecursive(l,493)),a={type:"file",name:o,content:Buffer.from(i,"utf8"),mode:41471,compressed:!1,createdAt:Date.now(),updatedAt:Date.now()};s.children[o]=a,s._childCount++,this._journal({op:W.SYMLINK,path:r,dest:i}),this.emit("symlink:create",{link:r,target:i})}isSymlink(t){try{let n=oe(this.root,H(t));return n.type==="file"&&n.mode===41471}catch{return!1}}resolveSymlink(t,n=8){let r=H(t);for(let i=0;i<n;i++){try{let s=oe(this.root,r);if(s.type==="file"&&s.mode===41471){let o=s.content.toString("utf8");r=o.startsWith("/")?o:H(ce.posix.join(ce.posix.dirname(r),o));continue}}catch{break}return r}throw new Error(`Too many levels of symbolic links: ${t}`)}remove(t,n={}){let r=this.resolveMount(t);if(r){if(r.readOnly)throw new Error(`EROFS: read-only file system, unlink '${r.fullHostPath}'`);if(!B.existsSync(r.fullHostPath))throw new Error(`ENOENT: no such file or directory, unlink '${r.fullHostPath}'`);B.statSync(r.fullHostPath).isDirectory()?B.rmSync(r.fullHostPath,{recursive:n.recursive??!1}):B.unlinkSync(r.fullHostPath);return}let i=H(t);if(i==="/")throw new Error("Cannot remove root directory.");let s=oe(this.root,i);if(s.type==="directory"){let l=s;if(!n.recursive&&l._childCount>0)throw new Error(`Directory '${i}' is not empty. Use recursive option.`)}let{parent:o,name:a}=_e(this.root,i,!1,()=>{});delete o.children[a],o._childCount--,this.emit("node:remove",{path:i}),this._journal({op:W.REMOVE,path:i})}move(t,n){let r=H(t),i=H(n);if(r==="/"||i==="/")throw new Error("Cannot move root directory.");let s=oe(this.root,r);if(this.exists(i))throw new Error(`Destination '${i}' already exists.`);this.mkdirRecursive(ce.posix.dirname(i),493);let{parent:o,name:a}=_e(this.root,i,!1,()=>{}),{parent:l,name:c}=_e(this.root,r,!1,()=>{});delete l.children[c],l._childCount--,s.name=a,o.children[a]=s,o._childCount++,this._journal({op:W.MOVE,path:r,dest:i})}toSnapshot(){return{root:this.serializeDir(this.root)}}serializeDir(t){let n=[];for(let r of Object.values(t.children))r.type==="stub"?n.push({type:"file",name:r.name,mode:r.mode,createdAt:new Date(r.createdAt).toISOString(),updatedAt:new Date(r.updatedAt).toISOString(),compressed:!1,contentBase64:Buffer.from(r.stubContent,"utf8").toString("base64")}):r.type==="file"?n.push(this.serializeFile(r)):n.push(this.serializeDir(r));return{type:"directory",name:t.name,mode:t.mode,createdAt:new Date(t.createdAt).toISOString(),updatedAt:new Date(t.updatedAt).toISOString(),children:n}}serializeFile(t){return{type:"file",name:t.name,mode:t.mode,createdAt:new Date(t.createdAt).toISOString(),updatedAt:new Date(t.updatedAt).toISOString(),compressed:t.compressed,contentBase64:t.content.toString("base64")}}static fromSnapshot(t){let n=new e;return n.root=n.deserializeDir(t.root,""),n}importSnapshot(t){this.root=this.deserializeDir(t.root,""),this.emit("snapshot:import")}deserializeDir(t,n){let r={type:"directory",name:n,mode:t.mode,createdAt:Date.parse(t.createdAt),updatedAt:Date.parse(t.updatedAt),children:Object.create(null),_childCount:0};for(let i of t.children){if(i.type==="file"){let s=i;r.children[s.name]={type:"file",name:s.name,mode:s.mode,createdAt:Date.parse(s.createdAt),updatedAt:Date.parse(s.updatedAt),compressed:s.compressed,content:Buffer.from(s.contentBase64,"base64")}}else{let s=this.deserializeDir(i,i.name);r.children[i.name]=s}r._childCount++}return r}},wt=Kt;function h(e,t,n=493){e.exists(t)||e.mkdir(t,n)}function f(e,t,n,r=420){e.writeStub(t,n,r)}function $(e,t,n){e.writeFile(t,n)}function no(e){let t=2166136261;for(let n=0;n<e.length;n++)t^=e.charCodeAt(n),t=Math.imul(t,16777619);return t>>>0}function ro(e,t,n){h(e,"/etc"),f(e,"/etc/os-release",`${['NAME="Fortune GNU/Linux"',`PRETTY_NAME="${n.os}"`,"ID=fortune","ID_LIKE=debian",'HOME_URL="https://github.com/itsrealfortune/typescript-virtual-container"',"VERSION_CODENAME=nyx",'VERSION_ID="24.04"',"FORTUNE_CODENAME=nyx"].join(`
`)}
...
`),$(e,"/proc/hostname",`${n}
`);let P=(Math.random()*.3).toFixed(2),x=1+i.length;$(e,"/proc/loadavg",`${P} ${P} ${P} ${x}/${x} 1
...
`)}
`),Ms(e,1,"root","pts/0","/sbin/init",new Date(r).toISOString(),{});for(let N of i){let b=ks(N.tty);Ms(e,b,N.username,N.tty,"bash",N.startedAt,{USER:N.username,HOME:`/home/${N.username}`,TERM:"xterm-256color",SHELL:"/bin/bash",LANG:"en_US.UTF-8",PATH:"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",LOGNAME:N.username})}let w=i.length>0?ks(i[i.length-1].tty):1;try{e.remove("/proc/self")}catch{}let A=`/proc/${w}`;if(h(e,"/proc/self"),h(e,"/proc/self/fd"),h(e,"/proc/self/fdinfo"),h(e,"/proc/self/net"),e.exists(A))for(let N of e.list(A)){let b=`${A}/${N}`,E=`/proc/self/${N}`;try{e.stat(b).type==="file"&&$(e,E,e.readFile(b))}catch{}}else $(e,"/proc/self/cmdline","bash\0"),$(e,"/proc/self/comm","bash"),$(e,"/proc/self/status",`Name:	bash
State:	S (sleeping)
Pid:	1
PPid:	0
`),$(e,"/proc/self/environ",""),$(e,"/proc/self/cwd","/root\0"),$(e,"/proc/self/exe","/bin/bash\0")}function io(e,t,n){h(e,"/sys"),h(e,"/sys/devices"),h(e,"/sys/devices/virtual"),h(e,"/sys/devices/system"),h(e,"/sys/devices/system/cpu"),h(e,"/sys/devices/system/cpu/cpu0"),f(e,"/sys/devices/system/cpu/cpu0/online",`1
...
`);let r=no(t),i=r.toString(16).padStart(8,"0");f(e,"/sys/class/net/eth0/address",`52:54:00:${i.slice(0,2)}:${i.slice(2,4)}:${i.slice(4,6)}
...
`),f(e,"/sys/kernel/osrelease",`${n.kernel}
`),f(e,"/sys/kernel/ostype",`Linux
`)}function oo(e){h(e,"/dev"),f(e,"/dev/null","",438),f(e,"/dev/zero","",438),f(e,"/dev/full","",438),f(e,"/dev/random","",292),f(e,"/dev/urandom","",292),f(e,"/dev/mem","",416),f(e,"/dev/port","",416),f(e,"/dev/kmsg","",432),f(e,"/dev/hwrng","",432),f(e,"/dev/fuse","",432),f(e,"/dev/autofs","",432),f(e,"/dev/userfaultfd","",432),f(e,"/dev/cpu_dma_latency","",432),f(e,"/dev/ptp0","",432),f(e,"/dev/snapshot","",432),f(e,"/dev/console","",384),f(e,"/dev/tty","",438),f(e,"/dev/ttyS0","",432),f(e,"/dev/ptmx","",438);for(let t=0;t<=63;t++)f(e,`/dev/tty${t}`,"",400);f(e,"/dev/vcs","",400),f(e,"/dev/vcs1","",400),f(e,"/dev/vcsa","",400),f(e,"/dev/vcsa1","",400),f(e,"/dev/vcsu","",400),f(e,"/dev/vcsu1","",400);for(let t=0;t<8;t++)f(e,`/dev/loop${t}`,"",432);h(e,"/dev/loop-control"),f(e,"/dev/vda","",432),f(e,"/dev/vdb","",432),f(e,"/dev/vdc","",432),f(e,"/dev/vdd","",432),h(e,"/dev/net"),f(e,"/dev/net/tun","",432),h(e,"/dev/pts"),h(e,"/dev/shm"),h(e,"/dev/cpu"),f(e,"/dev/stdin","",438),f(e,"/dev/stdout","",438),f(e,"/dev/stderr","",438),h(e,"/dev/fd"),f(e,"/dev/vga_arbiter","",432),f(e,"/dev/vsock","",432)}function ao(e){h(e,"/usr"),h(e,"/usr/bin"),h(e,"/usr/sbin"),h(e,"/usr/local"),h(e,"/usr/local/bin"),h(e,"/usr/local/lib"),h(e,"/usr/local/share"),h(e,"/usr/local/include"),h(e,"/usr/local/sbin"),h(e,"/usr/share"),h(e,"/usr/share/doc"),h(e,"/usr/share/man"),h(e,"/usr/share/man/man1"),h(e,"/usr/share/man/man5"),h(e,"/usr/share/man/man8"),h(e,"/usr/share/common-licenses"),h(e,"/usr/share/ca-certificates"),h(e,"/usr/share/zoneinfo"),h(e,"/usr/lib"),h(e,"/usr/lib/x86_64-linux-gnu"),h(e,"/usr/lib/python3"),h(e,"/usr/lib/python3/dist-packages"),h(e,"/usr/lib/python3.12"),h(e,"/usr/lib/jvm"),h(e,"/usr/lib/jvm/java-21-openjdk-amd64"),h(e,"/usr/lib/jvm/java-21-openjdk-amd64/bin"),h(e,"/usr/lib/node_modules"),h(e,"/usr/lib/node_modules/npm"),h(e,"/usr/include"),h(e,"/usr/src");let t=["sh","bash","ls","cat","echo","grep","find","sort","head","tail","cut","tr","sed","awk","wc","tee","tar","gzip","gunzip","touch","mkdir","rm","mv","cp","chmod","ln","pwd","env","date","sleep","id","whoami","hostname","uname","ps","kill","df","du","curl","wget","nano","diff","uniq","xargs","base64"];for(let r of t)f(e,`/usr/bin/${r}`,`#!/bin/sh
exec builtin ${r} "$@"
...
`;function co(e){h(e,"/var"),h(e,"/var/log"),h(e,"/var/log/apt"),h(e,"/var/log/journal"),h(e,"/var/log/private"),h(e,"/var/tmp"),h(e,"/var/cache"),h(e,"/var/cache/apt"),h(e,"/var/cache/apt/archives"),h(e,"/var/cache/apt/archives/partial"),h(e,"/var/cache/debconf"),h(e,"/var/cache/ldconfig"),h(e,"/var/cache/fontconfig"),h(e,"/var/cache/PackageKit"),h(e,"/var/lib"),h(e,"/var/lib/apt"),h(e,"/var/lib/apt/lists"),h(e,"/var/lib/apt/lists/partial"),h(e,"/var/lib/dpkg"),h(e,"/var/lib/dpkg/info"),h(e,"/var/lib/dpkg/updates"),h(e,"/var/lib/dpkg/alternatives"),h(e,"/var/lib/misc"),h(e,"/var/lib/systemd"),h(e,"/var/lib/systemd/coredump"),h(e,"/var/lib/pam"),h(e,"/var/lib/git"),h(e,"/var/lib/PackageKit"),h(e,"/var/lib/python"),h(e,"/var/spool"),h(e,"/var/spool/cron"),h(e,"/var/spool/mail"),h(e,"/var/mail"),h(e,"/var/backups"),h(e,"/var/www"),f(e,"/var/lib/dpkg/status",lo),f(e,"/var/lib/dpkg/available",""),f(e,"/var/lib/dpkg/lock",""),f(e,"/var/lib/dpkg/lock-frontend",""),f(e,"/var/lib/apt/lists/lock",""),f(e,"/var/cache/apt/pkgcache.bin",""),f(e,"/var/cache/apt/srcpkgcache.bin",""),f(e,"/var/log/syslog",`${new Date().toUTCString()}  kernel: Virtual container started
`),f(e,"/var/log/auth.log",""),f(e,"/var/log/kern.log",""),f(e,"/var/log/dpkg.log",""),f(e,"/var/log/apt/history.log",""),f(e,"/var/log/apt/term.log",""),f(e,"/var/log/faillog",""),f(e,"/var/log/lastlog",""),f(e,"/var/log/wtmp",""),f(e,"/var/log/btmp",""),f(e,"/var/log/alternatives.log",""),h(e,"/run"),h(e,"/run/lock"),h(e,"/run/lock/subsys"),h(e,"/run/systemd"),h(e,"/run/systemd/ask-password"),h(e,"/run/systemd/sessions"),h(e,"/run/systemd/users"),h(e,"/run/user"),h(e,"/run/dbus"),h(e,"/run/adduser"),f(e,"/run/utmp",""),f(e,"/run/dbus/system_bus_socket","")}function uo(e){e.exists("/bin")||e.symlink("/usr/bin","/bin"),e.exists("/sbin")||e.symlink("/usr/sbin","/sbin"),e.exists("/var/run")||e.symlink("/run","/var/run"),h(e,"/lib"),h(e,"/lib64"),h(e,"/lib/x86_64-linux-gnu"),h(e,"/lib/modules"),e.exists("/lib64/ld-linux-x86-64.so.2")||f(e,"/lib64/ld-linux-x86-64.so.2","",493)}function mo(e){h(e,"/tmp",1023),h(e,"/tmp/node-compile-cache",1023)}function po(e){h(e,"/root",448),h(e,"/root/.ssh",448),h(e,"/root/.config",493),h(e,"/root/.config/pip",493),h(e,"/root/.local",493),h(e,"/root/.local/share",493),f(e,"/root/.bashrc",`${["# root .bashrc","export PS1='\\[\\033[0;31m\\]\\u@\\h\\[\\033[0m\\]:\\[\\033[0;34m\\]\\w\\[\\033[0m\\]# '","export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin","export LANG=en_US.UTF-8","alias ll='ls -la'","alias la='ls -A'","alias l='ls -CF'"].join(`
`)}
`),f(e,"/root/.profile",`[ -f ~/.bashrc ] && . ~/.bashrc
`),f(e,"/root/.bash_logout",`# ~/.bash_logout
`),f(e,"/root/.config/pip/pip.conf",`[global]
break-system-packages = true
`)}function fo(e,t){h(e,"/opt"),h(e,"/opt/rclone"),h(e,"/srv"),h(e,"/mnt"),h(e,"/media"),h(e,"/boot"),h(e,"/boot/grub"),f(e,"/boot/grub/grub.cfg",`${["# GRUB configuration (virtual)","set default=0","set timeout=0","",'menuentry "Fortune GNU/Linux" {',`  linux   /vmlinuz-${t.kernel} root=/dev/vda rw console=ttyS0`,`  initrd  /initrd.img-${t.kernel}`,"}"].join(`
`)}
...
CONFIG_VIRTIO_NET=y
CONFIG_KVM_GUEST=y
`,420),e.exists("/vmlinuz")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz"),e.exists("/vmlinuz.old")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz.old"),e.exists("/initrd.img")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img"),e.exists("/initrd.img.old")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img.old"),h(e,"/lost+found",448),h(e,"/home")}var Ns=new Map;function ho(e,t){return`${e}|${t.kernel}|${t.os}|${t.arch}`}function go(e,t){let n=ho(e,t),r=Ns.get(n);if(r)return r;let i=new wt({mode:"memory"});ro(i,e,t),io(i,e,t),oo(i),ao(i),co(i),uo(i),mo(i),fo(i,t),so(i,t);let s=i.encodeBinary();return Ns.set(n,s),s}function Es(e,t,n,r,i,s=[]){let o=go(n,r);e.getMode()==="fs"&&e.exists("/home")?e.mergeRootTree(Ee(o)):e.importRootTree(Ee(o)),po(e),vt(e,r,n,i,s),Jt(e,t)}function Is(e){return e==="1"||e==="true"}function As(){return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}function yo(){return Is(process.env.DEV_MODE)||Is(process.env.RENDER_PERF)}function Ct(e){let t=yo();if(!t)return{enabled:t,mark:()=>{},done:()=>{}};let n=As(),r=s=>{let o=As()-n;console.log(`[perf][${e}] ${s}: ${o.toFixed(1)}ms`)};return{enabled:t,mark:r,done:(s="done")=>{r(s)}}}var Zt=[{name:"vim",version:"2:9.0.1378-2",section:"editors",description:"Vi IMproved - enhanced vi editor",shortDesc:"Vi IMproved",installedSizeKb:3812,files:[{path:"/usr/share/doc/vim/README",content:`Vim editor \u2014 virtual package.
...
`:"",384)||s,s&&await this.vfs.flushMirror()}writeIfChanged(n,r,i){return this.vfs.exists(n)&&this.vfs.readFile(n)===r?(this.vfs.chmod(n,i),!1):(this.vfs.writeFile(n,r,{mode:i}),!0)}createRecord(n,r){let i=_s("sha256").update(n).update(":").update(r).digest("hex"),s=e.recordCache.get(i);if(s)return s;let o=So(16).toString("hex"),a={username:n,salt:o,passwordHash:this.hashPassword(r,o)};return e.recordCache.set(i,a),a}hasPassword(n){re.mark("hasPassword");let r=this.users.get(n);if(!r)return!1;let i=this.hashPassword("",r.salt);return r.passwordHash===i?!1:!!r.passwordHash}hashPassword(n,r=""){return e.fastPasswordHash?_s("sha256").update(r).update(n).digest("hex"):xo(n,r||"",32).toString("hex")}validateUsername(n){if(!n||n.trim()==="")throw new Error("invalid username");if(!/^[a-z_][a-z0-9_-]{0,31}$/i.test(n))throw new Error("invalid username")}validatePassword(n){if(!n||n.trim()==="")throw new Error("invalid password")}authorizedKeys=new Map;addAuthorizedKey(n,r,i){re.mark("addAuthorizedKey");let s=this.authorizedKeys.get(n)??[];s.push({algo:r,data:i}),this.authorizedKeys.set(n,s),this.emit("key:add",{username:n,algo:r})}removeAuthorizedKeys(n){this.authorizedKeys.delete(n),this.emit("key:remove",{username:n})}getAuthorizedKeys(n){return this.authorizedKeys.get(n)??[]}};function Os(e){let t=Ts.posix.normalize(e);return t.startsWith("/")?t:`/${t}`}import{EventEmitter as Po}from"node:events";var kt=class extends Po{vfs;idleThresholdMs;checkIntervalMs;_state="active";_lastActivity=Date.now();_frozenBuffer=null;_checkTimer=null;constructor(t,n={}){super(),this.vfs=t,this.idleThresholdMs=n.idleThresholdMs??6e4,this.checkIntervalMs=n.checkIntervalMs??15e3}start(){this._checkTimer||(this._lastActivity=Date.now(),this._checkTimer=setInterval(()=>this._check(),this.checkIntervalMs),typeof this._checkTimer=="object"&&this._checkTimer!==null&&"unref"in this._checkTimer&&this._checkTimer.unref())}async stop(){this._checkTimer&&(clearInterval(this._checkTimer),this._checkTimer=null),this._state==="frozen"&&this._thaw()}ping(){this._lastActivity=Date.now(),this._state==="frozen"&&this._thaw()}get state(){return this._state}get idleMs(){return Date.now()-this._lastActivity}_check(){this._state!=="frozen"&&Date.now()-this._lastActivity>=this.idleThresholdMs&&this._freeze()}async _freeze(){this._state!=="frozen"&&(await this.vfs.stopAutoFlush(),this._frozenBuffer=this.vfs.encodeBinary(),this.vfs.releaseTree(),this._state="frozen",this.emit("freeze"))}_thaw(){if(this._state!=="frozen"||!this._frozenBuffer)return;let t=Ee(this._frozenBuffer);this.vfs.importRootTree(t),this._frozenBuffer=null,this._state="active",this.emit("thaw")}};import{readFile as $o,unlink as ko,writeFile as Mo}from"node:fs/promises";import*as Qt from"node:path";function Fs(e,t,n,r,i,s="unknown",o={cols:80,rows:24},a){let l="",c=0,u=No(a.vfs,n),d=null,m="",p=q(n),g=Fe(n,r),S=null,C=null,P=()=>{let O=q(n),U=p===O?"~":Qt.posix.basename(p)||"/";return bt(n,r,U)},x=Array.from(new Set(ze())).sort();console.log(`[${i}] Shell started for user '${n}' at ${s}`),(async()=>{let O=`${q(n)}/.bashrc`;if(a.vfs.exists(O))try{let U=a.vfs.readFile(O);for(let z of U.split(`
`)){let D=z.trim();!D||D.startsWith("#")||await Z(D,n,r,"shell",p,a,void 0,g)}}catch{}})();function _(){let O=P();t.write(`\r${O}${l}\x1B[K`);let U=l.length-c;U>0&&t.write(`\x1B[${U}D`)}function w(){t.write("\r\x1B[K")}function A(O){C={...O,buffer:""},w(),t.write(O.prompt)}async function N(O){if(!C)return;let U=C;if(C=null,!O){t.write(`\r
Sorry, try again.\r
`),_();return}if(!U.commandLine){n=U.targetUser,U.loginShell&&(p=q(n)),a.users.updateSession(i,n,s),t.write(`\r
`),_();return}let z=U.loginShell?q(U.targetUser):p,D=await Promise.resolve(Z(U.commandLine,U.targetUser,r,"shell",z,a));if(t.write(`\r
`),D.openEditor){await E(D.openEditor.targetPath,D.openEditor.initialContent,D.openEditor.tempPath);return}if(D.openHtop){await T();return}D.clearScreen&&t.write("\x1B[2J\x1B[H"),D.stdout&&t.write(`${Ke(D.stdout)}\r
`),D.stderr&&t.write(`${Ke(D.stderr)}\r
`),D.switchUser?(n=D.switchUser,p=D.nextCwd??q(n),a.users.updateSession(i,n,s)):D.nextCwd&&(p=D.nextCwd),_()}async function b(){if(!S)return;let O=S;if(O.kind==="nano"){try{let U=await $o(O.tempPath,"utf8");a.writeFileAsUser(n,O.targetPath,U)}catch{}await ko(O.tempPath).catch(()=>{})}S=null,l="",c=0,t.write(`\r
`),_()}async function E(O,U,z){a.vfs.exists(O)&&await Mo(z,U,"utf8");let D=yt(z,o,t);D.on("error",se=>{t.write(`nano: ${se.message}\r
`),b()}),D.on("close",()=>{b()}),S={kind:"nano",targetPath:O,tempPath:z,process:D}}async function T(){let O=await gs();if(!O){t.write(`htop: no child_process processes to display\r
`);return}let U=Ss(O,o,t);U.on("error",z=>{t.write(`htop: ${z.message}\r
`),b()}),U.on("close",()=>{b()}),S={kind:"htop",targetPath:"",tempPath:"",process:U}}function k(O){l=O,c=l.length,_()}function M(O){l=`${l.slice(0,c)}${O}${l.slice(c)}`,c+=O.length,_()}function F(O,U){let z=U;for(;z>0&&!/\s/.test(O[z-1]);)z-=1;let D=U;for(;D<O.length&&!/\s/.test(O[D]);)D+=1;return{start:z,end:D}}function Q(O){let U=O.lastIndexOf("/"),z=U>=0?O.slice(0,U+1):"",D=U>=0?O.slice(U+1):O,se=gt(p,z||".");try{return a.vfs.list(se).filter(V=>!V.startsWith(".")).filter(V=>V.startsWith(D)).map(V=>{let ye=Qt.posix.join(se,V),Ie=a.vfs.stat(ye).type==="directory"?"/":"";return`${z}${V}${Ie}`}).sort()}catch{return[]}}function G(){let{start:O,end:U}=F(l,c),z=l.slice(O,c);if(z.length===0)return;let se=l.slice(0,O).trim().length===0?x.filter(pe=>pe.startsWith(z)):[],V=Q(z),ye=Array.from(new Set([...se,...V])).sort();if(ye.length!==0){if(ye.length===1){let pe=ye[0],Ie=pe.endsWith("/")?"":" ";l=`${l.slice(0,O)}${pe}${Ie}${l.slice(U)}`,c=O+pe.length+Ie.length,_();return}t.write(`\r
`),t.write(`${ye.join("  ")}\r
`),_()}}function ee(O){if(O.length===0)return;u.push(O),u.length>500&&(u=u.slice(u.length-500));let U=u.length>0?`${u.join(`
`)}
`:"";a.vfs.writeFile(`${q(n)}/.bash_history`,U)}function ve(){let O=`${q(n)}/.lastlog.json`;if(!a.vfs.exists(O))return null;try{return JSON.parse(a.vfs.readFile(O))}catch{return null}}function ge(O){let U=`${q(n)}/.lastlog`;a.vfs.writeFile(U,JSON.stringify({at:O,from:s}))}function Ce(){let O=ve(),U=new Date().toISOString();t.write(St(r,e,O)),ge(U)}Ce(),_(),t.on("data",async O=>{if(S){S.process.stdin.write(O);return}if(C){let z=O.toString("utf8");for(let D=0;D<z.length;D+=1){let se=z[D];if(se===""){C=null,t.write(`^C\r
`),_();return}if(se==="\x7F"||se==="\b"){C.buffer=C.buffer.slice(0,-1);continue}if(se==="\r"||se===`
`){let V=C.buffer;if(C.buffer="",C.onPassword){let{result:pe,nextPrompt:Ie}=await C.onPassword(V,a);t.write(`\r
`),pe!==null?(C=null,pe.stdout&&t.write(pe.stdout.replace(/\n/g,`\r
`)),pe.stderr&&t.write(pe.stderr.replace(/\n/g,`\r
`)),_()):(Ie&&(C.prompt=Ie),t.write(C.prompt));return}let ye=a.users.verifyPassword(C.username,V);await N(ye);return}se>=" "&&(C.buffer+=se)}return}let U=O.toString("utf8");for(let z=0;z<U.length;z+=1){let D=U[z];if(D===""){l="",c=0,d=null,m="",t.write(`bye\r
`),ee("bye"),t.write(`logout\r
`),t.exit(0),t.end();return}if(D==="	"){G();continue}if(D==="\x1B"){let se=U[z+1],V=U[z+2],ye=U[z+3];if(se==="["&&V){if(V==="A"){z+=2,u.length>0&&(d===null?(m=l,d=u.length-1):d>0&&(d-=1),k(u[d]??""));continue}if(V==="B"){z+=2,d!==null&&(d<u.length-1?(d+=1,k(u[d]??"")):(d=null,k(m)));continue}if(V==="C"){z+=2,c<l.length&&(c+=1,t.write("\x1B[C"));continue}if(V==="D"){z+=2,c>0&&(c-=1,t.write("\x1B[D"));continue}if(V==="3"&&ye==="~"){z+=3,c<l.length&&(l=`${l.slice(0,c)}${l.slice(c+1)}`,_());continue}}}if(D===""){l="",c=0,d=null,m="",t.write(`^C\r
`),_();continue}if(D==="\r"||D===`
`){let se=l.trim();if(l="",c=0,d=null,m="",t.write(`\r
`),se.length>0){let V=await Promise.resolve(Z(se,n,r,"shell",p,a,void 0,g));if(ee(se),V.openEditor){await E(V.openEditor.targetPath,V.openEditor.initialContent,V.openEditor.tempPath);return}if(V.openHtop){await T();return}if(V.sudoChallenge){A(V.sudoChallenge);return}if(V.clearScreen&&t.write("\x1B[2J\x1B[H"),V.stdout&&t.write(`${Ke(V.stdout)}\r
`),V.stderr&&t.write(`${Ke(V.stderr)}\r
`),V.closeSession){t.write(`logout\r
`),t.exit(V.exitCode??0),t.end();return}V.nextCwd&&(p=V.nextCwd),V.switchUser&&(n=V.switchUser,p=V.nextCwd??q(n),a.users.updateSession(i,n,s),l="",c=0)}_();continue}if(D==="\x7F"||D==="\b"){c>0&&(l=`${l.slice(0,c-1)}${l.slice(c)}`,c-=1,_());continue}M(D)}}),t.on("close",()=>{S&&(S.process.kill("SIGTERM"),S=null)})}function No(e,t){let n=`${q(t)}/.bash_history`;return e.exists(n)?e.readFile(n).split(`
`).map(i=>i.trim()).filter(i=>i.length>0):(e.writeFile(n,""),[])}function Io(e){return typeof e=="object"&&e!==null&&"vfsInstance"in e&&Rs(e.vfsInstance)}function Rs(e){if(typeof e!="object"||e===null)return!1;let t=e;return typeof t.restoreMirror=="function"&&typeof t.flushMirror=="function"&&typeof t.writeFile=="function"&&typeof t.readFile=="function"&&typeof t.mkdir=="function"&&typeof t.exists=="function"&&typeof t.stat=="function"&&typeof t.list=="function"&&typeof t.remove=="function"&&typeof t.copy=="function"&&typeof t.move=="function"&&typeof t.touch=="function"}var Ao={kernel:"1.0.0+itsrealfortune+1-amd64",os:"Fortune GNU/Linux x64",arch:"x86_64"},Ze=Ct("VirtualShell");function _o(){let e=process.env.SSH_MIMIC_AUTO_SUDO_NEW_USERS;return e?!["0","false","no","off"].includes(e.toLowerCase()):!0}var Mt=class extends Eo{vfs;users;packageManager;hostname;properties;startTime;_idle=null;initialized;constructor(t,n,r){super(),Ze.mark("constructor"),this.hostname=t,this.properties=n||Ao,this.startTime=Date.now(),Rs(r)?this.vfs=r:Io(r)?this.vfs=r.vfsInstance:this.vfs=new wt(r??{}),this.users=new $t(this.vfs,_o()),this.packageManager=new Pt(this.vfs,this.users);let i=this.vfs,s=this.users,o=this.packageManager,a=this.properties,l=this.hostname,c=this.startTime;this.initialized=(async()=>{await i.restoreMirror(),await s.initialize(),Es(i,s,l,a,c),o.load(),this.emit("initialized")})()}async ensureInitialized(){Ze.mark("ensureInitialized"),await this.initialized}addCommand(t,n,r){let i=t.trim().toLowerCase();if(i.length===0||/\s/.test(i))throw new Error("Command name must be non-empty and contain no spaces");Nt(Et(i,n,r))}executeCommand(t,n,r){Ze.mark("executeCommand"),this._idle?.ping();let i=Z(t,n,this.hostname,"shell",r,this);return this.emit("command",{command:t,user:n,cwd:r}),i}startInteractiveSession(t,n,r,i,s){Ze.mark("startInteractiveSession"),this._idle?.ping(),this.emit("session:start",{user:n,sessionId:r,remoteAddress:i}),Fs(this.properties,t,n,this.hostname,r,i,s,this),this.refreshProcSessions()}refreshProcFs(){vt(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}mount(t,n,r={}){this.vfs.mount(t,n,r)}unmount(t){this.vfs.unmount(t)}getMounts(){return this.vfs.getMounts()}refreshProcSessions(){vt(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}syncPasswd(){Jt(this.vfs,this.users)}getVfs(){return this?.vfs??null}getUsers(){return this?.users??null}getHostname(){return this?.hostname}writeFileAsUser(t,n,r){Ze.mark("writeFileAsUser"),this.users.assertWriteWithinQuota(t,n,r),this.vfs.writeFile(n,r)}enableIdleManagement(t){this._idle||(this._idle=new kt(this.vfs,t),this._idle.on("freeze",()=>this.emit("shell:freeze")),this._idle.on("thaw",()=>this.emit("shell:thaw")),this._idle.start())}async disableIdleManagement(){this._idle&&(await this._idle.stop(),this._idle=null)}get idleState(){return this._idle?.state??"active"}get idleMs(){return this._idle?.idleMs??0}pingIdle(){this._idle?.ping()}};var Oe=process.env.SSH_MIMIC_HOSTNAME??"typescript-vm",Xt=process.argv.slice(2);console.clear();function Lo(){for(let e=0;e<Xt.length;e+=1){let t=Xt[e];if(t==="--user"){let n=Xt[e+1];if(!n||n.startsWith("--"))throw new Error("self-standalone: --user requires a value");return n}if(t?.startsWith("--user="))return t.slice(7)||"root"}return"root"}var Uo=Lo(),j=new Mt(Oe,void 0,{mode:"fs",snapshotPath:".vfs"});function Vo(e){let t=`/home/${e}/.lastlog`;if(!j.vfs.exists(t))return null;try{return JSON.parse(j.vfs.readFile(t))}catch{return null}}function zo(e,t){j.vfs.writeFile(`/home/${e}/.lastlog`,JSON.stringify({at:new Date().toISOString(),from:t}))}async function Qe(){await j.vfs.stopAutoFlush()}function Bo(e){let t=`${q(e)}/.bash_history`;return j.vfs.exists(t)?j.vfs.readFile(t).split(`
`).map(n=>n.trim()).filter(n=>n.length>0):(j.vfs.writeFile(t,""),[])}function jo(e,t){let n=e.length>0?`${e.join(`
`)}
`:"";j.vfs.writeFile(`${q(t)}/.bash_history`,n)}function Wo(e,t,n){let r=n.lastIndexOf("/"),i=r>=0?n.slice(0,r+1):"",s=r>=0?n.slice(r+1):n,o=gt(t,i||".");try{return e.list(o).filter(a=>!a.startsWith(".")&&a.startsWith(s)).map(a=>{let l=Ls.posix.join(o,a),c=e.stat(l);return`${i}${a}${c.type==="directory"?"/":""}`}).sort()}catch{return[]}}function Ho(e){let t=Array.from(new Set(ze())).sort();return(n,r)=>{let{cwd:i}=e(),s=n.split(/\s+/).at(-1)??"",a=n.trimStart()===s?t.filter(u=>u.startsWith(s)):[],l=Wo(j.vfs,i,s),c=Array.from(new Set([...a,...l])).sort();r(null,[c,s])}}function Xe(e,t){return new Promise(n=>{if(!de.isTTY||!ae.isTTY){e.question(t,n);return}let r=!!de.isRaw,i="",s=()=>{de.off("data",a),r||de.setRawMode(!1)},o=l=>{s(),ae.write(`
`),n(l)},a=l=>{let c=l.toString("utf8");for(let u=0;u<c.length;u+=1){let d=c[u];if(d==="\r"||d===`
`){o(i);return}if(d==="\x7F"||d==="\b"){i=i.slice(0,-1);continue}d>=" "&&(i+=d)}};e.pause(),ae.write(t),r||de.setRawMode(!0),de.resume(),de.on("data",a)})}function qo(e,t,n,r){let i=e,s=t;return n.switchUser?(i=n.switchUser,s=n.nextCwd??q(i),r.vars.USER=i,r.vars.LOGNAME=i,r.vars.HOME=q(i),r.vars.PWD=s):n.nextCwd&&(s=n.nextCwd,r.vars.PWD=s),{authUser:i,cwd:s}}j.addCommand("demo",[],()=>({stdout:"This is a demo command. It does nothing useful.",exitCode:0}));async function Go(){await j.ensureInitialized();let e=Uo.trim()||"root";j.users.getPasswordHash(e)===null&&(process.stderr.write(`self-standalone: user '${e}' does not exist
`),process.exit(1));let t=e==="root"?"/root":q(e);j.vfs.exists(t)||j.vfs.mkdir(t,e==="root"?448:493);let n=`${t}/README.txt`;j.vfs.exists(n)||(j.vfs.writeFile(n,`Welcome to ${Oe}
`),await j.vfs.stopAutoFlush());let r=Fe(e,Oe),i=e,s=q(i);r.vars.PWD=s;let o="localhost",a={cols:ae.columns??80,rows:ae.rows??24},l=Bo(i),c=Do({input:de,output:ae,terminal:!0,completer:Ho(()=>({cwd:s}))}),u=c;u.history=[...l].reverse();async function d(x,_,w){j.vfs.exists(x)&&await Fo(w,_,"utf8"),c.pause();let A=yt(w,a,{write:ae.write.bind(ae),exit:()=>{},end:()=>{}}),N=!!de.isRaw,b=E=>{A.stdin.write(E)};de.resume(),N||de.setRawMode(!0),de.on("data",b),await new Promise(E=>{let T=()=>{de.off("data",b),N||de.setRawMode(!1),c.resume()};A.on("error",k=>{T(),ae.write(`nano: ${k.message}\r
`),E()}),A.on("close",async()=>{T(),c.write("",{ctrl:!0,name:"u"});try{let k=await Oo(w,"utf8");j.writeFileAsUser(i,x,k),await Qe()}catch{}await To(w).catch(()=>{}),ae.write(`\r
`),E()})})}async function m(x){if(x.onPassword){let N=x.prompt;for(;;){let b=await Xe(c,N),E=await x.onPassword(b,j);if(E.result===null){N=E.nextPrompt??N;continue}await g(E.result);return}}let _=await Xe(c,x.prompt);if(!j.users.verifyPassword(x.username,_)){process.stderr.write(`Sorry, try again.
`);return}if(!x.commandLine){i=x.targetUser,s=q(i),r.vars.USER=i,r.vars.LOGNAME=i,r.vars.HOME=q(i),r.vars.PWD=s;return}let w=x.loginShell?q(x.targetUser):s,A=await Z(x.commandLine,x.targetUser,Oe,"shell",w,j,void 0,r);await g(A)}async function p(x){let _=await Xe(c,x.prompt);if(x.confirmPrompt&&await Xe(c,x.confirmPrompt)!==_){process.stderr.write(`passwords do not match
...
`)?x.stderr:`${x.stderr}
`);let _=qo(i,s,x,r);i=_.authUser,s=_.cwd,x.closeSession&&(await Qe(),c.close(),process.exit(x.exitCode??0))}let S=()=>{let x=s===q(i)?"~":Ro(s)||"/";return bt(i,Oe,x)},C=()=>{c.setPrompt(S()),c.prompt()};if(process.env.USER!=="root"&&j.users.hasPassword(i)){let x=await Xe(c,`Password for ${i}: `);j.users.verifyPassword(i,x)||(process.stderr.write(`self-standalone: authentication failed
`),process.exit(1))}ae.write(St(Oe,j.properties,Vo(i))),zo(i,o),await Qe();let P=!1;c.on("line",async x=>{if(P)return;P=!0,c.pause(),x.trim().length>0&&(l.push(x),l.length>500&&(l=l.slice(l.length-500)),jo(l,i),u.history=[...l].reverse());let w=await Z(x,i,Oe,"shell",s,j,void 0,r);await g(w),await Qe(),P=!1,c.resume(),C()}),c.on("SIGINT",()=>{ae.write(`^C
`),c.write("",{ctrl:!0,name:"u"}),C()}),c.on("close",()=>{Qe().then(()=>{console.log(""),process.exit(0)})}),C()}Go().catch(e=>{console.error("Failed to start readline SSH emulation:",e),process.exit(1)});var Ds=!1;async function Yo(e){if(!Ds){Ds=!0,process.stdout.write(`
[${e}] Saving VFS...
`);try{await j.vfs.stopAutoFlush()}catch{}process.exit(0)}}process.on("SIGTERM",()=>{Yo("SIGTERM")});process.on("beforeExit",()=>{j.vfs.stopAutoFlush()});process.on("uncaughtException",e=>{console.error("Uncaught exception:",e)});process.on("unhandledRejection",(e,t)=>{console.error("Unhandled rejection at:",t,"error:",e)});
