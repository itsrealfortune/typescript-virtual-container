...
class Interpreter {
	...
	pyEval(expr: string, scope: Scope): PyVal {
		...
		if (/^0o[0-7]+$/.test(expr)) {
			return Number.parseInt(expr.slice(2), 8);
		}
		...
		if (notMatch) {
			return !pyBool(this.pyEval(notMatch[1] as string, scope));
		}
		...
		if (callMatch) {
			...
			const callArgs = (
				argsStr?.trim() ? Interpreter._splitArgs(argsStr) : []
			).map((a) => this.pyEval(a, scope));
		...
		if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(expr)) {
			if (scope.has(expr)) {
				return scope.get(expr) as PyVal;
			}
	...
	private _callMethod(obj: PyVal, method: string, args: PyVal[]): PyVal {
		// String methods
		if (typeof obj === "string") {
			switch (method) {
				case "upper":
			...
			switch (method) {
				...
				case "update": {
					if (isPyDict(args[0] ?? NONE)) {
						for (const [k, v] of (args[0] as PyDict).data) {
							obj.data.set(k, v);
						}
					}
					return NONE;
		...
		if (isPyDict(obj)) {
			...
			if (method in mathFns) {
				const fn = mathFns[method] as (...a: number[]) => number;
	...
	private _callBuiltin(name: string, args: PyVal[], scope: Scope): PyVal {
		...
		if (scope.has(name)) {
			...
			if (isPyFunc(fn)) {
				return this._callFunc(fn, args, scope);
			}
			if (isPyClass(fn)) {
				return this._instantiate(fn as PyClass, args);
			}
			return fn;
		}
		...
		switch (name) {
			...
			case "range": {
				if (args.length === 1) {
					return pyRange(0, args[0] as number);
				}
				if (args.length === 2) {
	...
	private _collectBlock(
		...
		for (let i = startIdx; i < lines.length; i++) {
			...
			if (Interpreter._getIndent(l) <= baseIndent) {
				break;
			}
	...
	private _execStatement(lines: string[], idx: number, scope: Scope): number {
		...
		if (delMatch) {
			scope.delete((delMatch[1] as string).trim());
			return idx + 1;
		}
		...
		if (importMatch) {
			...
			const factory = MODULE_FACTORIES[modName as string];
			if (factory) {
				const mod = factory(this.cwd);
				this._modules.set(modName as string, mod);
				scope.set(alias ?? (modName as string), mod);
			}
			return idx + 1;
		}
		...
		if (defMatch) {
			...
			const params = (paramsStr as string)
				.split(",")
				.map((p) => p.trim())
				.filter(Boolean);
		...
		if (classMatch) {
			...
			scope.set(className as string, cls);
			return idx + 1 + body.length;
		}
		...
		if (line.startsWith("if ") && line.endsWith(":")) {
			...
			while (j < lines.length) {
				...
				const elt = el.trim();
				if (Interpreter._getIndent(el) !== indent) {
					break;
				}
				...
				if (elifMatch) {
					...
					if (pyBool(this.pyEval(elifMatch[1] as string, scope))) {
		...
		if (forMatch) {
			...
			const iterable = pyIter(this.pyEval((iterExpr as string).trim(), scope));
			const body = this._collectBlock(lines, idx + 1, indent);
			...
			for (const item of iterable) {
				// Unpack
				if ((target as string).includes(",")) {
					const targets = (target as string).split(",").map((t) => t.trim());
					const items = Array.isArray(item) ? item : [item];
					targets.forEach((t, i) => scope.set(t, items[i] ?? NONE));
				} else {
					scope.set((target as string).trim(), item);
				}
				try {
					this._runBlockInScope(body, scope);
				} catch (e) {
			...
			if (!broken && elseBody.length) {
				this._runBlockInScope(elseBody, scope);
			}
			return afterIdx;
		}
		...
		if (whileMatch) {
			const cond = whileMatch[1] as string;
			const body = this._collectBlock(lines, idx + 1, indent);
			let iterations = 0;
		...
		if (line === "try:") {
			...
			try {
				...
				if (finallyBody.length) {
					this._runBlockInScope(finallyBody, scope);
				}
			}
			return j;
		}
		...
		if (withMatch) {
			const body = this._collectBlock(lines, idx + 1, indent);
			scope.set(withMatch[2] as string, NONE); // stub: just set to None
			this._runBlockInScope(body, scope);
			return idx + 1 + body.length;
		}
		...
		if (augMatch) {
			...
			const lhs = scope.get(name as string) ?? 0;
			const rhs = this.pyEval(rhsExpr as string, scope);
			let result: PyVal;
			switch (op) {
				case "+=":
					result =
			...
			scope.set(name as string, result);
			return idx + 1;
		}
		...
		if (attrAssignMatch) {
			...
			if (dotIdx !== -1) {
				const objExpr = (attrAssignMatch[1] as string).slice(0, dotIdx);
		...
		if (multiAssignMatch) {
			...
			const targets = (line.split("=")[0] as string)
				.split(",")
				.map((s) => s.trim());
		...
		if (assignMatch) {
			...
			scope.set(name as string, this.pyEval(rhs as string, scope));
			return idx + 1;
		}
