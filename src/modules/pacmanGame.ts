...
export class PacmanGame {
	...
	private _gridRow(r: number): Cell[] {
		...
		if (row === undefined) {
			throw new Error(`PacmanGame: row ${r} out of range`);
		}
		return row;
	}
	...
	private _ghost(i: number): Ghost {
		...
		if (g === undefined) {
			throw new Error(`PacmanGame: ghost ${i} not found`);
		}
		return g;
	}
	...
	private readonly _modeSchedule = [
		56,
		160,
		56,
		160,
	...
	private _gameTick(): void {
		...
		if (this._globalMode !== "fright") {
			...
			if (
				this._globalModeTick >= (this._modeSchedule[this._modeIdx] as number)
			) {
	...
	private _ghostTarget(g: Ghost): [number, number] {
		...
		switch (g.name) {
			...
			case "Pinky": {
				...
				if (this._pacDir === 3) {
					tc = this._pacC - 4; // NES bug: facing up → also goes left
				}
				return [tr, tc];
			}
			...
			case "Inky": {
				...
				if (this._pacDir === 3) {
					pc = this._pacC - 2; // NES bug mirror
				}
	...
	private _moveGhost(g: Ghost): void {
		...
		if (g.mode === "fright") {
			...
			if (walkable.length > 0) {
				chosen =
					walkable[Math.floor(Math.random() * walkable.length)] ?? chosen;
			}
		} else {
		...
		if (this._isWalkable(nr, nc, true)) {
			g.r = nr;
			g.c = nc;
		}
	}
	...
	private _buildLines(): string[] {
		...
		// Message overlay
		if (this._msg) {
			lines[18] = `        ${C.yellow}${C.blink}${this._msg}${C.r}`;
		}
