...
function encodeEntry(e: JournalEntry): Buffer {
	...
	buf.writeUInt16LE(pathBuf.length, off);
	off += 2;
	pathBuf.copy(buf, off);
	off += pathBuf.length;

	if (e.op === JournalOp.WRITE) {
		const c = e.content ?? Buffer.alloc(0);
		buf.writeUInt32LE(c.length, off);
		off += 4;
		c.copy(buf, off);
		off += c.length;
		buf.writeUInt32LE(e.mode ?? 0o644, off);
		off += 4;
	} else if (e.op === JournalOp.MKDIR) {
		buf.writeUInt32LE(e.mode ?? 0o755, off);
		off += 4;
	} else if (e.op === JournalOp.CHMOD) {
		buf.writeUInt32LE(e.mode ?? 0o644, off);
		off += 4;
...
function decodeJournal(buf: Buffer): JournalEntry[] {
	...
	try {
		while (off < buf.length) {
			...
			const pathLen = buf.readUInt16LE(off);
			off += 2;
			if (off + pathLen > buf.length) {
				break;
			}
			const path = buf.subarray(off, off + pathLen).toString(ENC);
			off += pathLen;
			...
			if (op === JournalOp.WRITE) {
				...
				off += 4;
				if (off + cLen + 4 > buf.length) {
					break;
				}
				const content = Buffer.from(buf.subarray(off, off + cLen));
				off += cLen;
				...
				const dLen = buf.readUInt16LE(off);
				off += 2;
				if (off + dLen > buf.length) {
					break;
				}
