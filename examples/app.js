...
`)||void 0,exitCode:0}}},Mn={name:"unalias",description:"Remove alias definitions",category:"shell",params:["<name...> | -a"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};if(C(e,["-a"])){for(let n of Object.keys(t.vars))n.startsWith("__alias_")&&delete t.vars[n];return{exitCode:0}}for(let n of e)delete t.vars[`__alias_${n}`];return{exitCode:0}}};function qt(){throw new Error("child_process.spawn not supported in browser")}var z={basename(e){let t=e.split("/").filter(Boolean);return t.length?t[t.length-1]:""},dirname(e){if(!e)return".";let t=e.split("/").filter(Boolean);return t.pop(),t.length?"/"+t.join("/"):"/"},join(...e){return e.join("/").replace(/\/+/g,"/")},resolve(...e){let t=e.join("/");return t.startsWith("/")?t:"/"+t},normalize(e){let t=e.split("/"),n=[];for(let r of t)r===".."?n.pop():r&&r!=="."&&n.push(r);return(e.startsWith("/")?"/":"")+n.join("/")||"."}};function wt(e){return z.dirname(e)}function nt(...e){return z.resolve(...e)}function In(...e){return e.join("/").replace(/\/+/g,"/")}var vi=["/.virtual-env-js/.auth","/etc/htpasswd"];function M(e,t,n){if(!t||t.trim()==="")return e;if(t.startsWith("~")){let r=n??"/root";return z.normalize(`${r}${t.slice(1)}`)}return t.startsWith("/")?z.normalize(t):z.normalize(z.join(e,t))}function Ci(e){let t=e.startsWith("/")?z.normalize(e):z.normalize(`/${e}`);return vi.some(n=>t===n||t.startsWith(`${n}/`))}function V(e,t,n){if(e!=="root"&&Ci(t))throw new Error(`${n}: permission denied: ${t}`)}function Nn(e){let n=(e.split("?")[0]?.split("#")[0]??e).split("/").filter(Boolean).pop();return n&&n.length>0?n:"index.html"}function Pi(e,t){let n=Array.from({length:e.length+1},()=>Array(t.length+1).fill(0));for(let r=0;r<=e.length;r+=1)n[r][0]=r;for(let r=0;r<=t.length;r+=1)n[0][r]=r;for(let r=1;r<=e.length;r+=1)for(let i=1;i<=t.length;i+=1){let s=e[r-1]===t[i-1]?0:1;n[r][i]=Math.min(n[r-1][i]+1,n[r][i-1]+1,n[r-1][i-1]+s)}return n[e.length][t.length]}function An(e,t,n){let r=M(t,n);if(e.exists(r))return r;let i=z.dirname(r),s=z.basename(r),o=e.list(i),a=o.filter(c=>c.toLowerCase()===s.toLowerCase());if(a.length===1)return z.join(i,a[0]);let l=o.filter(c=>Pi(c.toLowerCase(),s.toLowerCase())<=1);return l.length===1?z.join(i,l[0]):r}function We(e){return e.packageManager}var _n={name:"apt",aliases:["apt-get"],description:"Package manager",category:"package",params:["<install|remove|update|upgrade|search|show|list> [pkg...]"],run:({args:e,shell:t,authUser:n})=>{let r=We(t);if(!r)return{stderr:"apt: package manager not initialised",exitCode:1};let i=e[0]?.toLowerCase(),s=e.slice(1),o=C(s,["-q","--quiet","-qq"]),a=C(s,["--purge"]),l=s.filter(u=>!u.startsWith("-"));if(["install","remove","purge","upgrade","update"].includes(i??"")&&n!=="root")return{stderr:`E: Could not open lock file /var/lib/dpkg/lock-frontend - open (13: Permission denied)
...
`),exitCode:0}}default:return{stderr:`apt-cache: unknown command '${r??""}'`,exitCode:1}}}};var Fn={name:"awk",description:"Pattern scanning and processing language",category:"text",params:["[-F <sep>] '<program>' [file]"],run:({authUser:e,args:t,stdin:n,cwd:r,shell:i})=>{let s=ae(t,["-F"])??" ",o=t.filter(P=>!P.startsWith("-")&&P!==s),a=o[0],l=o[1];if(!a)return{stderr:"awk: no program",exitCode:1};let c=n??"";if(l){let P=M(r,l);try{V(e,P,"awk"),c=i.vfs.readFile(P)}catch{return{stderr:`awk: ${l}: No such file or directory`,exitCode:1}}}let u=c.split(`
`);u[u.length-1]===""&&u.pop();let d=[],m=a.trim();if(!m.startsWith("{")&&!m.includes("{"))d.push({pattern:m,action:"print $0"});else{let P=/([^{]*)\{([^}]*)\}/g,N=P.exec(m);for(;N!==null;)d.push({pattern:N[1].trim(),action:N[2].trim()}),N=P.exec(m);d.length===0&&d.push({pattern:"",action:m.replace(/[{}]/g,"").trim()})}let p=[],g=d.find(P=>P.pattern==="BEGIN"),S=d.find(P=>P.pattern==="END"),v=d.filter(P=>P.pattern!=="BEGIN"&&P.pattern!=="END");function E(P){return s===" "?P.trim().split(/\s+/).filter(Boolean):P.split(s)}function F(P,N,I){let x=E(N),A=x.length,T=w=>{if(w=w.trim(),w==="NR")return String(I);if(w==="NF")return String(A);if(w==="$0")return N;if(w==="$NF")return x[A-1]??"";if(/^\$\d+$/.test(w))return x[parseInt(w.slice(1),10)-1]??"";let D=w.replace(/\bNR\b/g,String(I)).replace(/\bNF\b/g,String(A));if(/^[\d\s+\-*/()]+$/.test(D))try{return String(Function(`"use strict"; return (${D});`)())}catch{}return w.replace(/"/g,"")},k=P.split(";").map(w=>w.trim()).filter(Boolean);for(let w of k)if(w==="print"||w==="print $0")p.push(N);else if(w.startsWith("print ")){let D=w.slice(6).split(/\s*,\s*/);p.push(D.map(T).join("	"))}}function _(P,N,I){if(!P||P==="1")return!0;let x=P.match(/^NR\s*([=!<>]=?|==)\s*(\d+)$/);if(x){let k=x[1],w=parseInt(x[2],10);switch(k){case"==":return I===w;case"!=":return I!==w;case">":return I>w;case">=":return I>=w;case"<":return I<w;case"<=":return I<=w}}let A=P.match(/^NR%(\d+)==(\d+)$/);if(A)return I%parseInt(A[1],10)===parseInt(A[2],10);if(P.startsWith("/")&&P.endsWith("/"))try{return new RegExp(P.slice(1,-1)).test(N)}catch{return!1}let T=P.match(/^\$(\d+)~\/(.*)\/$/);if(T){let w=E(N)[parseInt(T[1],10)-1]??"";try{return new RegExp(T[2]).test(w)}catch{return!1}}return!1}g&&F(g.action,"",0);for(let P=1;P<=u.length;P++){let N=u[P-1];for(let I of v)_(I.pattern,N,P)&&F(I.action,N,P)}return S&&F(S.action,"",u.length+1),{stdout:p.join(`
`)+(p.length>0?`
`:""),exitCode:0}}};var Tn={name:"base64",description:"Encode/decode base64",category:"text",params:["[-d] [file]"],run:({args:e,stdin:t})=>{let n=C(e,["-d","--decode"]),r=t??"";if(n)try{return{stdout:Buffer.from(r.trim(),"base64").toString("utf8"),exitCode:0}}catch{return{stderr:"base64: invalid input",exitCode:1}}return{stdout:Buffer.from(r).toString("base64"),exitCode:0}}};var Dn={name:"cat",description:"Concatenate and print files",category:"files",params:["[-n] [-b] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-n","--number"]),o=C(r,["-b","--number-nonblank"]),a=r.filter(m=>!m.startsWith("-"));if(a.length===0&&i!==void 0)return{stdout:i,exitCode:0};if(a.length===0)return{stderr:"cat: missing file operand",exitCode:1};let l=[];for(let m of a){let p=An(t.vfs,n,m);V(e,p,"cat"),l.push(t.vfs.readFile(p))}let c=l.join("");if(!s&&!o)return{stdout:c,exitCode:0};let u=1;return{stdout:c.split(`
`).map(m=>o&&m.trim()===""?m:`${String(u++).padStart(6)}	${m}`).join(`
`),exitCode:0}}};var Rn={name:"cd",description:"Change directory",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=M(n,r[0]??"~",Y(e));return V(e,i,"cd"),t.vfs.stat(i).type!=="directory"?{stderr:`cd: not a directory: ${i}`,exitCode:1}:{nextCwd:i,exitCode:0}}};function Ei(e,t){let n=/^([ugoa]*)([+\-=])([rwx]*)$/,r=t.split(","),i=e;for(let s of r){let o=s.trim().match(n);if(!o)return null;let[,a="a",l,c=""]=o,u=a===""||a==="a"?["u","g","o"]:a.split(""),d={u:{r:256,w:128,x:64},g:{r:32,w:16,x:8},o:{r:4,w:2,x:1}};for(let m of u)for(let p of c.split("")){let g=d[m]?.[p];if(g!==void 0){if(l==="+")i|=g;else if(l==="-")i&=~g;else if(l==="="){let S=Object.values(d[m]??{}).reduce((v,E)=>v|E,0);i=i&~S|g}}}}return i}var Ln={name:"chmod",description:"Change file permissions",category:"files",params:["<mode> <file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let[i,s]=r;if(!i||!s)return{stderr:"chmod: missing operand",exitCode:1};let o=M(n,s);try{if(V(e,o,"chmod"),!t.vfs.exists(o))return{stderr:`chmod: ${s}: No such file or directory`,exitCode:1};let a,l=parseInt(i,8);if(!Number.isNaN(l)&&/^[0-7]+$/.test(i))a=l;else{let c=t.vfs.stat(o).mode,u=Ei(c,i);if(u===null)return{stderr:`chmod: invalid mode: ${i}`,exitCode:1};a=u}return t.vfs.chmod(o,a),{exitCode:0}}catch(a){return{stderr:`chmod: ${a instanceof Error?a.message:String(a)}`,exitCode:1}}}};var Vn={name:"clear",description:"Clear the terminal screen",category:"shell",params:[],run:()=>({clearScreen:!0,stdout:"",exitCode:0})};var Un={name:"cp",description:"Copy files or directories",category:"files",params:["[-r] <source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-r","-R","--recursive"]),s=r.filter(u=>!u.startsWith("-")),[o,a]=s;if(!o||!a)return{stderr:"cp: missing operand",exitCode:1};let l=M(n,o),c=M(n,a);try{if(V(e,l,"cp"),V(e,c,"cp"),!t.vfs.exists(l))return{stderr:`cp: ${o}: No such file or directory`,exitCode:1};if(t.vfs.stat(l).type==="directory"){if(!i)return{stderr:`cp: ${o}: is a directory (use -r)`,exitCode:1};let d=(p,g)=>{t.vfs.mkdir(g,493);for(let S of t.vfs.list(p)){let v=`${p}/${S}`,E=`${g}/${S}`;if(t.vfs.stat(v).type==="directory")d(v,E);else{let _=t.vfs.readFileRaw(v);t.writeFileAsUser(e,E,_)}}},m=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c;d(l,m)}else{let d=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c,m=t.vfs.readFileRaw(l);t.writeFileAsUser(e,d,m)}return{exitCode:0}}catch(u){return{stderr:`cp: ${u instanceof Error?u.message:String(u)}`,exitCode:1}}}};var Bn={name:"curl",description:"Transfer data from or to a server (pure fetch)",category:"network",params:["[options] <url>"],run:async({authUser:e,cwd:t,args:n,shell:r})=>{let{flagsWithValues:i,positionals:s}=le(n,{flagsWithValue:["-o","--output","-X","--request","-d","--data","-H","--header","-u","--user"]});if(C(n,["--help","-h"]))return{stdout:["Usage: curl [options] <url>","  -o, --output <file>    Write to file","  -X, --request <method> HTTP method","  -d, --data <data>      POST data","  -H, --header <hdr>     Extra header","  -s, --silent           Silent mode","  -I, --head             Fetch headers only","  -L, --location         Follow redirects","  -v, --verbose          Verbose"].join(`
`),exitCode:0};let o=s[0];if(!o)return{stderr:"curl: no URL specified",exitCode:1};let a=i.get("-o")??i.get("--output")??null,l=(i.get("-X")??i.get("--request")??"GET").toUpperCase(),c=i.get("-d")??i.get("--data")??null,u=i.get("-H")??i.get("--header")??null,d=C(n,["-s","--silent"]),m=C(n,["-I","--head"]),p=C(n,["-L","--location"]),g=C(n,["-v","--verbose"]),S={"User-Agent":"curl/7.88.1"};if(u){let N=u.indexOf(":");N!==-1&&(S[u.slice(0,N).trim()]=u.slice(N+1).trim())}let v=c&&l==="GET"?"POST":l,E={method:v,headers:S,redirect:p?"follow":"manual"};c&&(S["Content-Type"]??="application/x-www-form-urlencoded",E.body=c);let F=[];g&&(F.push(`* Trying ${o}...`,"* Connected"),F.push(`> ${v} / HTTP/1.1`,`> Host: ${new URL(o).host}`));let _;try{let N=o.startsWith("http://")||o.startsWith("https://")?o:`http://${o}`;_=await fetch(N,E)}catch(N){return{stderr:`curl: (6) Could not resolve host: ${N instanceof Error?N.message:String(N)}`,exitCode:6}}if(g&&F.push(`< HTTP/1.1 ${_.status} ${_.statusText}`),m){let N=[`HTTP/1.1 ${_.status} ${_.statusText}`];for(let[I,x]of _.headers.entries())N.push(`${I}: ${x}`);return{stdout:`${N.join(`\r
`)}\r
`,exitCode:0}}let P;try{P=await _.text()}catch{return{stderr:"curl: failed to read response body",exitCode:1}}if(a){let N=M(t,a);return V(e,N,"curl"),r.writeFileAsUser(e,N,P),d||F.push(`  % Total    % Received
100 ${P.length}  100 ${P.length}`),{stderr:F.join(`
`)||void 0,exitCode:_.ok?0:22}}return{stdout:P,stderr:F.length>0?F.join(`
`):void 0,exitCode:_.ok?0:22}}};var zn={name:"cut",description:"Remove sections from lines",category:"text",params:["-d <delim> -f <fields> [file]"],run:({args:e,stdin:t})=>{let n=ae(e,["-d"])??"	",i=(ae(e,["-f"])??"1").split(",").map(a=>{let[l,c]=a.split("-").map(Number);return c!==void 0?{from:(l??1)-1,to:c-1}:{from:(l??1)-1,to:(l??1)-1}});return{stdout:(t??"").split(`
`).map(a=>{let l=a.split(n),c=[];for(let u of i)for(let d=u.from;d<=Math.min(u.to,l.length-1);d++)c.push(l[d]??"");return c.join(n)}).join(`
`),exitCode:0}}};var Hn={name:"date",description:"Print current date and time",category:"system",params:["[+format]"],run:({args:e})=>{let t=new Date,n=e[0];return n?.startsWith("+")?{stdout:n.slice(1).replace("%Y",String(t.getFullYear())).replace("%m",String(t.getMonth()+1).padStart(2,"0")).replace("%d",String(t.getDate()).padStart(2,"0")).replace("%H",String(t.getHours()).padStart(2,"0")).replace("%M",String(t.getMinutes()).padStart(2,"0")).replace("%S",String(t.getSeconds()).padStart(2,"0")).replace("%s",String(Math.floor(t.getTime()/1e3))),exitCode:0}:{stdout:t.toString(),exitCode:0}}};var jn={name:"declare",aliases:["local","typeset"],description:"Declare variables and give them attributes",category:"shell",params:["[-i] [-r] [-x] [-a] [name[=value]...]"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};let n=C(e,["-i"]),r=C(e,["-r"]),i=C(e,["-x"]);if(e.filter(a=>!a.startsWith("-")).length===0)return{stdout:Object.entries(t.vars).map(([l,c])=>`declare -- ${l}="${c}"`).join(`
`),exitCode:0};let o=e.filter(a=>!a.startsWith("-"));for(let a of o){let l=a.indexOf("=");if(l===-1)a in t.vars||(t.vars[a]="");else{let c=a.slice(0,l),u=a.slice(l+1);if(n){let d=parseInt(u,10);u=Number.isNaN(d)?"0":String(d)}t.vars[c]=u}}return{exitCode:0}}};var Wn={name:"deluser",description:"Delete a user",category:"users",params:["[-f] <username>"],run:async({authUser:e,args:t,shell:n})=>{if(e!=="root")return{stderr:`deluser: permission denied
...
Type the username to confirm: `,mode:"confirm",onPassword:s},exitCode:0}}};var Kn={name:"df",description:"Report filesystem disk space usage",category:"system",params:["[-h]"],run:({shell:e})=>{let n=(e.vfs.getUsageBytes()/1024).toFixed(0),r="1048576",i=String(Number(r)-Number(n)),s=Math.round(Number(n)/Number(r)*100),o="Filesystem     1K-blocks    Used Available Use% Mounted on",a=`virtual-fs     ${r.padStart(9)} ${n.padStart(7)} ${i.padStart(9)} ${s}% /`;return{stdout:`${o}
${a}`,exitCode:0}}};var qn={name:"diff",description:"Compare files line by line",category:"text",params:["<file1> <file2>"],run:({shell:e,cwd:t,args:n})=>{let[r,i]=n;if(!r||!i)return{stderr:"diff: missing operand",exitCode:1};let s=M(t,r),o=M(t,i),a,l;try{a=e.vfs.readFile(s).split(`
...
`)||"(no packages match)",exitCode:0}}return{stderr:"dpkg-query: need a flag (-l, -W)",exitCode:1}}};var Jn={name:"du",description:"Estimate file space usage",category:"system",params:["[-h] [-s] [path]"],run:({shell:e,cwd:t,args:n})=>{let r=C(n,["-h"]),i=C(n,["-s"]),s=n.find(u=>!u.startsWith("-"))??".",o=M(t,s),a=u=>r?`${(u/1024).toFixed(1)}K`:String(Math.ceil(u/1024));if(!e.vfs.exists(o))return{stderr:`du: ${s}: No such file or directory`,exitCode:1};if(i||e.vfs.stat(o).type==="file")return{stdout:`${a(e.vfs.getUsageBytes(o))}	${s}`,exitCode:0};let l=[],c=(u,d)=>{let m=0;for(let p of e.vfs.list(u)){let g=`${u}/${p}`,S=`${d}/${p}`,v=e.vfs.stat(g);v.type==="directory"?m+=c(g,S):(m+=v.size,i||l.push(`${a(v.size)}	${S}`))}return l.push(`${a(m)}	${d}`),m};return c(o,s),{stdout:l.join(`
`),exitCode:0}}};function $i(e,t){let n=[],r=0;for(;r<e.length;){let i=e[r];if(/\s/.test(i)){r++;continue}if(i==="+"){n.push({type:"plus"}),r++;continue}if(i==="-"){n.push({type:"minus"}),r++;continue}if(i==="*"){if(e[r+1]==="*"){n.push({type:"pow"}),r+=2;continue}n.push({type:"mul"}),r++;continue}if(i==="/"){n.push({type:"div"}),r++;continue}if(i==="%"){n.push({type:"mod"}),r++;continue}if(i==="("){n.push({type:"lparen"}),r++;continue}if(i===")"){n.push({type:"rparen"}),r++;continue}if(/\d/.test(i)){let s=r+1;for(;s<e.length&&/\d/.test(e[s]);)s++;n.push({type:"number",value:Number(e.slice(r,s))}),r=s;continue}if(/[A-Za-z_]/.test(i)){let s=r+1;for(;s<e.length&&/[A-Za-z0-9_]/.test(e[s]);)s++;let o=e.slice(r,s),a=t[o],l=a===void 0||a===""?0:Number(a);n.push({type:"number",value:Number.isFinite(l)?l:0}),r=s;continue}return[]}return n}function Gt(e,t){let n=e.trim();if(n.length===0||n.length>1024)return NaN;let r=$i(n,t);if(r.length===0)return NaN;let i=0,s=()=>r[i],o=()=>r[i++],a=()=>{let p=o();if(!p)return NaN;if(p.type==="number")return p.value;if(p.type==="lparen"){let g=d();return r[i]?.type!=="rparen"?NaN:(i++,g)}return NaN},l=()=>{let p=s();return p?.type==="plus"?(o(),l()):p?.type==="minus"?(o(),-l()):a()},c=()=>{let p=l();for(;s()?.type==="pow";){o();let g=l();p=p**g}return p},u=()=>{let p=c();for(;;){let g=s();if(g?.type==="mul"){o(),p*=c();continue}if(g?.type==="div"){o();let S=c();p=S===0?NaN:p/S;continue}if(g?.type==="mod"){o();let S=c();p=S===0?NaN:p%S;continue}return p}},d=()=>{let p=u();for(;;){let g=s();if(g?.type==="plus"){o(),p+=u();continue}if(g?.type==="minus"){o(),p-=u();continue}return p}},m=d();return!Number.isFinite(m)||i!==r.length?NaN:Math.trunc(m)}function ki(e,t){let n=[],r=0;for(;r<e.length;){let i=e.indexOf("'",r);if(i===-1){n.push(t(e.slice(r)));break}n.push(t(e.slice(r,i)));let s=e.indexOf("'",i+1);if(s===-1){n.push(e.slice(i));break}n.push(e.slice(i,s+1)),r=s+1}return n.join("")}function Ct(e){function r(i,s){if(s>8)return[i];let o=0,a=-1;for(let l=0;l<i.length;l++){let c=i[l];if(c==="{"&&i[l-1]!=="$")o===0&&(a=l),o++;else if(c==="}"&&(o--,o===0&&a!==-1)){let u=i.slice(0,a),d=i.slice(a+1,l),m=i.slice(l+1),p=d.match(/^(-?\d+)\.\.(-?\d+)(?:\.\.-?(\d+))?$/)||d.match(/^([a-z])\.\.([a-z])$/);if(p){let E=[];if(/\d/.test(p[1])){let P=parseInt(p[1],10),N=parseInt(p[2],10),I=p[3]?parseInt(p[3],10):1,x=P<=N?I:-I;for(let A=P;P<=N?A<=N:A>=N;A+=x)E.push(String(A))}else{let P=p[1].charCodeAt(0),N=p[2].charCodeAt(0),I=P<=N?1:-1;for(let x=P;P<=N?x<=N:x>=N;x+=I)E.push(String.fromCharCode(x))}let F=E.map(P=>`${u}${P}${m}`),_=[];for(let P of F)if(_.push(...r(P,s+1)),_.length>256)return[i];return _}let g=[],S="",v=0;for(let E of d)E==="{"?(v++,S+=E):E==="}"?(v--,S+=E):E===","&&v===0?(g.push(S),S=""):S+=E;if(g.push(S),g.length>1){let E=[];for(let F of g)if(E.push(...r(`${u}${F}${m}`,s+1)),E.length>256)return[i];return E}break}}return[i]}return r(e,0)}function Mi(e,t){let n="",r=0;for(;r<e.length;){if(e[r]==="$"&&e[r+1]==="("&&e[r+2]==="("){let i=r+3,s=0;for(;i<e.length;){let o=e[i];if(o==="(")s++;else if(o===")"){if(s>0)s--;else if(e[i+1]===")"){let a=e.slice(r+3,i),l=Gt(a,t);n+=Number.isNaN(l)?"0":String(l),r=i+2;break}}i++}if(i>=e.length){n+=e.slice(r);break}continue}n+=e[r],r++}return n}function vt(e,t,n=0,r){let i=r??t.HOME??"/home/user";return ki(e,s=>{let o=s;return o=o.replace(/(^|[\s:])~(\/|$)/g,(a,l,c)=>`${l}${i}${c}`),o=o.replace(/\$\?/g,String(n)),o=o.replace(/\$\$/g,"1"),o=o.replace(/\$#/g,"0"),o=Mi(o,t),o=o.replace(/\$\{#([A-Za-z_][A-Za-z0-9_]*)\}/g,(a,l)=>String((t[l]??"").length)),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*):-([^}]*)\}/g,(a,l,c)=>t[l]!==void 0&&t[l]!==""?t[l]:c),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*):=([^}]*)\}/g,(a,l,c)=>((t[l]===void 0||t[l]==="")&&(t[l]=c),t[l])),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*):\+([^}]*)\}/g,(a,l,c)=>t[l]!==void 0&&t[l]!==""?c:""),o=o.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g,(a,l)=>t[l]??""),o=o.replace(/\$([A-Za-z_][A-Za-z0-9_]*|\d+)/g,(a,l)=>t[l]??""),o})}async function Pt(e,t,n,r){let i="__shellExpandDepth",o=Number(t[i]??"0");if(o>=8)return vt(e,t,n);t[i]=String(o+1);try{if(e.includes("$(")){let a="",l=!1,c=0;for(;c<e.length;){let u=e[c];if(u==="'"&&!l){l=!0,a+=u,c++;continue}if(u==="'"&&l){l=!1,a+=u,c++;continue}if(!l&&u==="$"&&e[c+1]==="("){if(e[c+2]==="("){a+=u,c++;continue}let d=0,m=c+1;for(;m<e.length;){if(e[m]==="(")d++;else if(e[m]===")"&&(d--,d===0))break;m++}let p=e.slice(c+2,m).trim(),g=(await r(p)).replace(/\n$/,"");a+=g,c=m+1;continue}a+=u,c++}e=a}return vt(e,t,n)}finally{o<=0?delete t[i]:t[i]=String(o)}}function Ii(e){return e.replace(/\\n/g,`
`).replace(/\\t/g,"	").replace(/\\r/g,"\r").replace(/\\\\/g,"\\").replace(/\\a/g,"\x07").replace(/\\b/g,"\b").replace(/\\f/g,"\f").replace(/\\v/g,"\v").replace(/\\0(\d{1,3})/g,(t,n)=>String.fromCharCode(parseInt(n,8)))}var Zn={name:"echo",description:"Display text",category:"shell",params:["[-n] [-e] [text...]"],run:({args:e,stdin:t,env:n})=>{let{flags:r,positionals:i}=le(e,{flags:["-n","-e","-E"]}),s=r.has("-n"),o=r.has("-e"),a=i.length>0?i.join(" "):t??"",l=vt(a,n?.vars??{},n?.lastExitCode??0),c=o?Ii(l):l;return{stdout:s?c:`${c}
`,exitCode:0}}};var Qn={name:"env",description:"Print environment variables",category:"shell",params:[],run:({env:e,authUser:t})=>{let n={...e.vars,USER:t,HOME:`/home/${t}`};return{stdout:Object.entries(n).map(([r,i])=>`${r}=${i}`).join(`
`),exitCode:0}}};var Xn={name:"exit",aliases:["bye"],description:"Exit the shell session",category:"shell",params:["[code]"],run:({args:e})=>({closeSession:!0,exitCode:parseInt(e[0]??"0",10)||0})};var er={name:"export",description:"Set shell environment variable",category:"shell",params:["[VAR=value]"],run:({args:e,env:t})=>{if(e.length===0||e.length===1&&e[0]==="-p"){let n=Object.entries(t.vars).filter(([r])=>r&&/^[A-Za-z_][A-Za-z0-9_]*$/.test(r)).map(([r,i])=>`declare -x ${r}="${i}"`).join(`
`);return{stdout:n?`${n}
`:"",exitCode:0}}for(let n of e.filter(r=>r!=="-p"))if(n.includes("=")){let r=n.indexOf("="),i=n.slice(0,r),s=n.slice(r+1);t.vars[i]=s}return{exitCode:0}}};var tr={name:"find",description:"Search for files",category:"files",params:["[path] [-name <pattern>] [-type f|d]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=ae(r,["-name"]),s=ae(r,["-type"]),a=r.filter(m=>!m.startsWith("-")&&m!==i&&m!==s)[0]??".",l=M(n,a);try{if(V(e,l,"find"),!t.vfs.exists(l))return{stderr:`find: ${a}: No such file or directory`,exitCode:1}}catch(m){return{stderr:`find: ${m instanceof Error?m.message:String(m)}`,exitCode:1}}let c=i?new RegExp(`^${i.replace(/\./g,"\\.").replace(/\*/g,".*").replace(/\?/g,".")}$`):null,u=[],d=(m,p)=>{let g=t.vfs.stat(m),S=!s||s==="f"&&g.type==="file"||s==="d"&&g.type==="directory",v=!c||c.test(m.split("/").pop()??"");if(S&&v&&u.push(p),g.type==="directory")for(let E of t.vfs.list(m)){let F=`${m}/${E}`,_=`${p}/${E}`;d(F,_)}};return d(l,a),{stdout:u.join(`
`),exitCode:0}}};function we(){try{return navigator?.deviceMemory?navigator.deviceMemory*1024*1024:1024*1024*1024}catch{return 1024*1024*1024}}function Re(){return Math.floor(we()*.5)}function Et(){return[{model:"web-cpu",speed:1e3}]}function Yt(){return"web"}function $t(){return"x86_64"}function Jt(){return"web-release"}function nr(){return Math.floor(performance.now()/1e3)}var rr={name:"free",description:"Display amount of free and used memory",category:"system",params:["[-h] [-m] [-g]"],run:({args:e})=>{let t=C(e,["-h","--human"]),n=C(e,["-m"]),r=C(e,["-g"]),i=we(),s=Re(),o=i-s,a=Math.floor(i*.02),l=Math.floor(i*.05),c=Math.floor(s*.95),u=Math.floor(i*.5),d=S=>t?S>=1024*1024*1024?`${(S/(1024*1024*1024)).toFixed(1)}G`:S>=1024*1024?`${(S/(1024*1024)).toFixed(1)}M`:`${(S/1024).toFixed(1)}K`:String(Math.floor(r?S/(1024*1024*1024):n?S/(1024*1024):S/1024)),m="               total        used        free      shared  buff/cache   available",p=`Mem:  ${d(i).padStart(12)} ${d(o).padStart(11)} ${d(s).padStart(11)} ${d(a).padStart(11)} ${d(l).padStart(11)} ${d(c).padStart(11)}`,g=`Swap: ${d(u).padStart(12)} ${d(0).padStart(11)} ${d(u).padStart(11)}`;return{stdout:[m,p,g].join(`
`),exitCode:0}}};var sr={name:"grep",description:"Search text patterns",category:"text",params:["[-i] [-v] [-n] [-r] <pattern> [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let{flags:s,positionals:o}=le(r,{flags:["-i","-v","-n","-r","-c","-l","-L","-q","--quiet","--silent"]}),a=s.has("-i"),l=s.has("-v"),c=s.has("-n"),u=s.has("-r"),d=s.has("-c"),m=s.has("-l"),p=s.has("-q")||s.has("--quiet")||s.has("--silent"),g=o[0],S=o.slice(1);if(!g)return{stderr:"grep: no pattern specified",exitCode:1};let v;try{let P=a?"mi":"m";v=new RegExp(g,P)}catch{return{stderr:`grep: invalid regex: ${g}`,exitCode:1}}let E=(P,N="")=>{let I=P.split(`
`),x=[];for(let A=0;A<I.length;A++){let T=I[A]??"",k=v.test(T);if(l?!k:k){let D=c?`${A+1}:`:"";x.push(`${N}${D}${T}`)}}return x},F=P=>{if(!t.vfs.exists(P))return[];if(t.vfs.stat(P).type==="file")return[P];if(!u)return[];let I=[],x=A=>{for(let T of t.vfs.list(A)){let k=`${A}/${T}`;t.vfs.stat(k).type==="file"?I.push(k):x(k)}};return x(P),I},_=[];if(S.length===0){if(!i)return{stdout:"",exitCode:1};let P=E(i);if(d)return{stdout:`${P.length}
`,exitCode:P.length>0?0:1};if(p)return{exitCode:P.length>0?0:1};_.push(...P)}else{let P=S.flatMap(N=>{let I=M(n,N);return F(I).map(x=>({file:N,path:x}))});for(let{file:N,path:I}of P)try{V(e,I,"grep");let x=t.vfs.readFile(I),A=P.length>1?`${N}:`:"",T=E(x,A);d?_.push(P.length>1?`${N}:${T.length}`:String(T.length)):m?T.length>0&&_.push(N):_.push(...T)}catch{return{stderr:`grep: ${N}: No such file or directory`,exitCode:1}}}return{stdout:_.length>0?`${_.join(`
`)}
`:"",exitCode:_.length>0?0:1}}};var ir={name:"groups",description:"Print group memberships",category:"system",params:["[user]"],run:({authUser:e,shell:t,args:n})=>{let r=n[0]??e;return{stdout:t.users.isSudoer(r)?`${r} sudo root`:r,exitCode:0}}};var or={name:"gzip",description:"Compress files",category:"archive",params:["[-k] [-d] <file>"],run:({shell:e,cwd:t,args:n})=>{if(!e.packageManager.isInstalled("gzip"))return{stderr:`bash: gzip: command not found
...
`,exitCode:1};let o=e.vfs.readFile(s),a=s.slice(0,-3);return e.vfs.writeFile(a,o),r||e.vfs.remove(s),{exitCode:0}}};var lr={name:"head",description:"Output first lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=ae(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
...
`),exitCode:0}}};var cr=["navigation","files","text","archive","system","package","network","shell","users","misc"],dr={navigation:"Navigation",files:"Files & Filesystem",text:"Text Processing",archive:"Archive & Compression",system:"System",package:"Package Management",network:"Network",shell:"Shell & Scripting",users:"Users & Permissions",misc:"Miscellaneous"},mr="\x1B[1m",ve="\x1B[0m",Ni="\x1B[36m",Ai="\x1B[33m",rt="\x1B[2m",_i="\x1B[32m";function ur(e,t){return e.length>=t?e:e+" ".repeat(t-e.length)}function Oi(e){let t=e.aliases?.length?` ${rt}(${e.aliases.join(", ")})${ve}`:"";return`  ${Ni}${ur(e.name,16)}${ve}${t}${ur("",(e.aliases?.length,0))} ${e.description??""}`}function Fi(e){let t={};for(let s of e){let o=s.category??"misc";t[o]||(t[o]=[]),t[o].push(s)}let n=[`${mr}Available commands${ve}`,`${rt}Type 'help <command>' for detailed usage.${ve}`,""],r=[...cr.filter(s=>t[s]),...Object.keys(t).filter(s=>!cr.includes(s)).sort()];for(let s of r){let o=t[s];if(!o?.length)continue;n.push(`${Ai}${dr[s]??s}${ve}`);let a=[...o].sort((l,c)=>l.name.localeCompare(c.name));for(let l of a)n.push(Oi(l));n.push("")}let i=e.length;return n.push(`${rt}${i} commands available.${ve}`),n.join(`
`)}function Ti(e){let t=[];if(t.push(`${mr}${e.name}${ve} \u2014 ${e.description??"no description"}`),e.aliases?.length&&t.push(`${rt}Aliases: ${e.aliases.join(", ")}${ve}`),t.push(""),t.push(`${_i}Usage:${ve}`),e.params.length)for(let r of e.params)t.push(`  ${e.name} ${r}`);else t.push(`  ${e.name}`);let n=dr[e.category??"misc"]??e.category??"misc";return t.push(""),t.push(`${rt}Category: ${n}${ve}`),t.join(`
`)}function pr(e){return{name:"help",description:"List all commands, or show usage for a specific command",category:"shell",params:["[command]"],run:({args:t})=>{let n=Qt();if(t[0]){let r=t[0].toLowerCase(),i=n.find(s=>s.name===r||s.aliases?.includes(r));return i?{stdout:Ti(i),exitCode:0}:{stderr:`help: no help entry for '${t[0]}'`,exitCode:1}}return{stdout:Fi(n),exitCode:0}}}}var fr={name:"history",description:"Display command history",category:"shell",params:["[n]"],run:({args:e,shell:t,authUser:n})=>{let r=`/home/${n}/.bash_history`;if(!t.vfs.exists(r))return{stdout:"",exitCode:0};let s=t.vfs.readFile(r).split(`
...
`,exitCode:1}}};var wr={name:"seq",description:"Print a sequence of numbers",category:"text",params:["[FIRST [INCREMENT]] LAST"],run:({args:e})=>{let t=e.filter(d=>!d.startsWith("-")||/^-[\d.]/.test(d)).map(Number),n=(()=>{let d=e.indexOf("-s");return d!==-1?e[d+1]??`
`:`
...
`)}
`,exitCode:0}}};var Di="\x1B[0m",Ri="\x1B[1;34m",Li="\x1B[1;36m",Vi="\x1B[1;32m",Ui="",Bi="\x1B[30;42m",zi="\x1B[37;44m",Hi="\x1B[34;42m";function Ke(e,t){return t?`${t}${e}${Di}`:e}function en(e,t,n){if(n)return Li;if(t==="directory"){let r=!!(e&512),i=!!(e&2);return r&&i?Bi:r?zi:i?Hi:Ri}return e&73?Vi:Ui}function Cr(e,t,n){let r;n?r="l":t==="directory"?r="d":r="-";let i=c=>e&c?"r":"-",s=c=>e&c?"w":"-",o=(()=>{let c=!!(e&64);return e&2048?c?"s":"S":c?"x":"-"})(),a=(()=>{let c=!!(e&8);return e&1024?c?"s":"S":c?"x":"-"})(),l=(()=>{let c=!!(e&1);return t==="directory"&&e&512?c?"t":"T":c?"x":"-"})();return`${r}${i(256)}${s(128)}${o}${i(32)}${s(16)}${a}${i(4)}${s(2)}${l}`}var ji=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function Xt(e){let t=new Date,n=4320*3600*1e3,r=Math.abs(t.getTime()-e.getTime())<n,i=String(e.getDate()).padStart(2," "),s=ji[e.getMonth()]??"";if(r){let o=String(e.getHours()).padStart(2,"0"),a=String(e.getMinutes()).padStart(2,"0");return`${i} ${s.padEnd(3)} ${o}:${a}`}return`${i} ${s.padEnd(3)} ${e.getFullYear()}`}function kt(e,t){try{return e.readFile(t)}catch{return"?"}}function Wi(e,t,n){let r=t==="/"?"":t;return n.map(i=>{let s=`${r}/${i}`,o=e.isSymlink(s),a;try{a=e.stat(s)}catch{return i}let l=en(a.mode,a.type,o);return Ke(i,l)}).join("  ")}function Ki(e,t,n){let r=t==="/"?"":t,i=n.map(d=>{let m=`${r}/${d}`,p=e.isSymlink(m),g;try{g=e.stat(m)}catch{return{perms:"----------",nlink:"1",size:"0",date:Xt(new Date),label:d}}let S=p?41471:g.mode,v=Cr(S,g.type,p),E=g.type==="directory"?String((g.childrenCount??0)+2):"1",F=p?kt(e,m).length:g.type==="file"?g.size??0:(g.childrenCount??0)*4096,_=String(F),P=Xt(g.updatedAt),N=en(S,g.type,p),I=p?`${Ke(d,N)} -> ${kt(e,m)}`:Ke(d,N);return{perms:v,nlink:E,size:_,date:P,label:I}}),s=Math.max(...i.map(d=>d.nlink.length)),o=Math.max(...i.map(d=>d.size.length)),a="root",l="root",c=n.length*8,u=i.map(d=>`${d.perms} ${d.nlink.padStart(s)} ${a} ${l} ${d.size.padStart(o)} ${d.date} ${d.label}`);return`total ${c}
${u.join(`
`)}`}var Pr={name:"ls",description:"List directory contents",category:"navigation",params:["[-la] [path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-l","--long","-la","-al"]),s=C(r,["-a","--all","-la","-al"]),o=$e(r,0,{flags:["-l","--long","-a","--all","-la","-al"]}),a=M(n,o??n);if(V(e,a,"ls"),t.vfs.exists(a)){let u=t.vfs.stat(a),d=t.vfs.isSymlink(a);if(u.type==="file"||d){let m=a.split("/").pop()??a,p=en(d?41471:u.mode,u.type,d);if(i){let g=d?41471:u.mode,S=d?kt(t.vfs,a).length:u.size??0,v=Cr(g,u.type,d),E=d?`${Ke(m,p)} -> ${kt(t.vfs,a)}`:Ke(m,p);return{stdout:`${v} 1 root root ${S} ${Xt(u.updatedAt)} ${E}
`,exitCode:0}}return{stdout:`${Ke(m,p)}
`,exitCode:0}}}let l=t.vfs.list(a).filter(u=>s||!u.startsWith("."));return{stdout:`${i?Ki(t.vfs,a,l):Wi(t.vfs,a,l)}
`,exitCode:0}}};var Er={name:"lsb_release",description:"Print distribution-specific information",category:"system",params:["[-a] [-i] [-d] [-r] [-c]"],run:({args:e,shell:t})=>{let n=t.properties?.os??"Fortune GNU/Linux x64",r="nyx",i="1.0";try{let d=t.vfs.readFile("/etc/os-release");for(let m of d.split(`
...
`),exitCode:0}}};var $r={adduser:`ADDUSER(8)                User Commands                ADDUSER(8)
...
DESCRIPTION
       Read items from stdin and execute COMMAND with those items as arguments.`};var qi={gunzip:"gzip"},kr={name:"man",description:"Interface to the system reference manuals",category:"shell",params:["<command>"],run:async({args:e,shell:t})=>{let n=e[0];if(!n)return{stderr:"What manual page do you want?",exitCode:1};let r=`/usr/share/man/man1/${n}.1`;if(t.vfs.exists(r))return{stdout:t.vfs.readFile(r),exitCode:0};let i=n.toLowerCase(),s=qi[i]??i,o=$r[s]??null;return o?{stdout:o,exitCode:0}:{stderr:`No manual entry for ${n}`,exitCode:16}}};var Mr={name:"mkdir",description:"Make directories",category:"files",params:["<dir>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"mkdir: missing operand",exitCode:1};for(let i=0;i<r.length;i++){let s=$e(r,i);if(!s)return{stderr:"mkdir: missing operand",exitCode:1};let o=M(n,s);V(e,o,"mkdir"),t.vfs.mkdir(o)}return{exitCode:0}}};var Ir={name:"mv",description:"Move or rename files",category:"files",params:["<source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r.filter(c=>!c.startsWith("-")),[s,o]=i;if(!s||!o)return{stderr:"mv: missing operand",exitCode:1};let a=M(n,s),l=M(n,o);try{if(V(e,a,"mv"),V(e,l,"mv"),!t.vfs.exists(a))return{stderr:`mv: ${s}: No such file or directory`,exitCode:1};let c=t.vfs.exists(l)&&t.vfs.stat(l).type==="directory"?`${l}/${s.split("/").pop()}`:l;return t.vfs.move(a,c),{exitCode:0}}catch(c){return{stderr:`mv: ${c instanceof Error?c.message:String(c)}`,exitCode:1}}}};var Nr={name:"nano",description:"Text editor",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r[0];if(!i)return{stderr:"nano: missing file operand",exitCode:1};let s=M(n,i);V(e,s,"nano");let o=t.vfs.exists(s)?t.vfs.readFile(s):"",a=z.basename(s)||"buffer",l=`/tmp/sshmimic-nano-${Date.now()}-${a}.tmp`;return{openEditor:{targetPath:s,tempPath:l,initialContent:o},exitCode:0}}};var Gi="vfs-fs-shim",Ie="files",Mt=null;function tn(){return Mt?Promise.resolve(Mt):new Promise((e,t)=>{let n=indexedDB.open(Gi,1);n.onupgradeneeded=r=>r.target.result.createObjectStore(Ie),n.onsuccess=r=>{Mt=r.target.result,e(Mt)},n.onerror=r=>t(r.target.error)})}var ie=new Map;tn().then(e=>{let n=e.transaction(Ie,"readonly").objectStore(Ie).openCursor();n.onsuccess=r=>{let i=r.target.result;i&&(ie.set(i.key,i.value),i.continue())}});function qe(e,t){tn().then(n=>{let r=n.transaction(Ie,"readwrite");t===null?r.objectStore(Ie).delete(e):r.objectStore(Ie).put(t,e)})}function Yi(e,t="utf8"){if(e instanceof Uint8Array)return e;if(typeof e=="string"){if(t==="hex"){let n=new Uint8Array(e.length/2);for(let r=0;r<n.length;r++)n[r]=parseInt(e.slice(r*2,r*2+2),16);return n}return new TextEncoder().encode(e)}return new Uint8Array(e)}function Ji(e,t="utf8"){return!t||t==="utf8"?new TextDecoder().decode(e):t==="hex"?Array.from(e).map(n=>n.toString(16).padStart(2,"0")).join(""):t==="base64"?btoa(String.fromCharCode(...e)):new TextDecoder().decode(e)}function ee(e){return ie.has(e)}function pe(e,t){if(!ie.has(e))throw Object.assign(new Error(`ENOENT: no such file: ${e}`),{code:"ENOENT"});let n=ie.get(e);if(n==="__DIR__")throw Object.assign(new Error(`EISDIR: ${e}`),{code:"EISDIR"});let r=typeof t=="string"?t:t?.encoding;return r?Ji(n,r):globalThis.Buffer.from(n)}function Ge(e,t,n){let r=typeof n=="string"?n:n?.encoding,i=Yi(t,r);ie.set(e,i),qe(e,i)}function st(e){ie.delete(e),qe(e,null)}function Ar(e,t={}){if(t.recursive)for(let n of[...ie.keys()])(n===e||n.startsWith(e+"/"))&&(ie.delete(n),qe(n,null));else st(e)}function Ye(e,t={}){if(t.recursive){let n=e.split("/").filter(Boolean),r="";for(let i of n)r+="/"+i,ie.has(r)||(ie.set(r,"__DIR__"),qe(r,"__DIR__"))}else ie.set(e,"__DIR__"),qe(e,"__DIR__")}function it(e){let t=e.endsWith("/")?e:e+"/";return[...ie.keys()].filter(n=>n.startsWith(t)&&n.slice(t.length).split("/").length===1).map(n=>n.slice(t.length))}function Nt(e){if(!ie.has(e))throw Object.assign(new Error(`ENOENT: ${e}`),{code:"ENOENT"});let t=ie.get(e),n=t==="__DIR__";return{isDirectory:()=>n,isFile:()=>!n,size:n?0:t.length}}var It=new Map,Zi=10,ot={O_WRONLY:1,O_CREAT:64,O_APPEND:1024};function _r(e,t){let n=Zi++,r=(t&ot.O_APPEND)!==0,i=ie.has(e)?ie.get(e):new Uint8Array(0);return It.set(n,{path:e,data:r?i:new Uint8Array(0)}),n}function Or(e,t){let n=It.get(e);if(!n)return;let r=new Uint8Array(n.data.length+t.length);r.set(n.data),r.set(t,n.data.length),n.data=r}function Fr(e){let t=It.get(e);t&&(ie.set(t.path,t.data),qe(t.path,t.data),It.delete(e))}var Qi=tn().then(e=>new Promise(t=>{let r=e.transaction(Ie,"readonly").objectStore(Ie).openCursor();r.onsuccess=i=>{let s=i.target.result;if(!s)return t(!0);ie.set(s.key,s.value),s.continue()}}));globalThis.__fsReady__=Qi;function Xi(e){let t=Math.max(1,Math.floor(e/60)),n=Math.floor(t/1440),r=Math.floor(t%1440/60),i=t%60,s=[];return n>0&&s.push(`${n} day${n>1?"s":""}`),r>0&&s.push(`${r} hour${r>1?"s":""}`),(i>0||s.length===0)&&s.push(`${i} min${i>1?"s":""}`),s.join(", ")}function Dr(e){return`\x1B[${e}m   \x1B[0m`}function eo(){let e=[40,41,42,43,44,45,46,47].map(Dr).join(""),t=[100,101,102,103,104,105,106,107].map(Dr).join("");return[e,t]}function Rr(e,t,n){if(e.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s=n<=1?0:t/(n-1),o=Math.round(r.r+(i.r-r.r)*s),a=Math.round(r.g+(i.g-r.g)*s),l=Math.round(r.b+(i.b-r.b)*s);return`\x1B[38;2;${o};${a};${l}m${e}\x1B[0m`}function to(e){if(e.trim().length===0)return e;let t=e.indexOf(":");if(t===-1)return e.includes("@")?Lr(e):e;let n=e.substring(0,t+1),r=e.substring(t+1);return Lr(n)+r}function Lr(e){let t=new RegExp("\x1B\\[[\\d;]*m","g"),n=e.replace(t,"");if(n.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s="";for(let o=0;o<n.length;o+=1){let a=n.length<=1?0:o/(n.length-1),l=Math.round(r.r+(i.r-r.r)*a),c=Math.round(r.g+(i.g-r.g)*a),u=Math.round(r.b+(i.b-r.b)*a);s+=`\x1B[38;2;${l};${c};${u}m${n[o]}\x1B[0m`}return s}function Vr(e){return Math.max(0,Math.round(e/(1024*1024)))}function Ur(){try{let e=pe("/etc/os-release","utf8");for(let t of e.split(`
`)){if(!t.startsWith("PRETTY_NAME="))continue;return t.slice(12).trim().replace(/^"|"$/g,"")}}catch{return}}function Br(e){try{let t=pe(e,"utf8").split(`
`)[0]?.trim();return!t||t.length===0?void 0:t}catch{return}}function no(e){let t=Br("/sys/devices/virtual/dmi/id/sys_vendor"),n=Br("/sys/devices/virtual/dmi/id/product_name");return t&&n?`${t} ${n}`:n||e}function ro(){let e=["/var/lib/dpkg/status","/usr/local/var/lib/dpkg/status"];for(let t of e)if(ee(t))try{return pe(t,"utf8").match(/^Package:\s+/gm)?.length??0}catch{}}function so(){let e=["/snap","/var/lib/snapd/snaps"];for(let t of e)if(ee(t))try{return it(t,{withFileTypes:!0}).filter(i=>i.isDirectory()).length}catch{}}function io(){let e=ro(),t=so();return e!==void 0&&t!==void 0?`${e} (dpkg), ${t} (snap)`:e!==void 0?`${e} (dpkg)`:t!==void 0?`${t} (snap)`:"n/a"}function oo(){let e=Et();if(e.length===0)return"unknown";let t=e[0];if(!t)return"unknown";let n=(t.speed/1e3).toFixed(2);return`${t.model} (${e.length}) @ ${n}GHz`}function ao(e){return!e||e.trim().length===0?"unknown":z.basename(e.trim())}function lo(e){let t=we(),n=Re(),r=Math.max(0,t-n),i=e.shellProps,s=y.uptime();return e.uptimeSeconds===void 0&&(e.uptimeSeconds=Math.round(s)),{user:e.user,host:e.host,osName:i?.os??e.osName??`${Ur()??Yt()} ${$t()}`,kernel:i?.kernel??e.kernel??Jt(),uptimeSeconds:e.uptimeSeconds??nr(),packages:e.packages??io(),shell:ao(e.shell),shellProps:e.shellProps??{kernel:e.kernel??Jt(),os:e.osName??`${Ur()??Yt()} ${$t()}`,arch:$t()},resolution:e.resolution??"n/a (ssh)",terminal:e.terminal??"unknown",cpu:e.cpu??oo(),gpu:e.gpu??"n/a",memoryUsedMiB:e.memoryUsedMiB??Vr(r),memoryTotalMiB:e.memoryTotalMiB??Vr(t)}}function zr(e){let t=lo(e),n=Xi(t.uptimeSeconds),r=eo(),i=["                               .. .:.    "," .::..                       ..     ..   ",".    ....                  ...       ..  ",":       ....             .:.          .. ",":           .:.:........:.            .. ",":                                     .. ",".                                      : ",".                                      : ","..                                     : "," :.                                   .. "," ..                                   .. "," :-.                                  :: ","  .:.                                 :. ","   ..:                               ... ","   ..:                               :.. ","  :...                              :....","   ..                                ....","   .                                  .. ","  .:.                                 .: ","  :.                                  .. "," ::.                                 ..  ",".....                          ..:...    ","...:.                         ..         ",".:...:.       ::.           ..           ","  ... ..:::::..  ..:::::::..             "],s=[`${t.user}@${t.host}`,"-------------------------",`OS: ${t.osName}`,`Host: ${no(t.host)}`,`Kernel: ${t.kernel}`,`Uptime: ${n}`,`Packages: ${t.packages}`,`Shell: ${t.shell}`,`Resolution: ${t.resolution}`,`Terminal: ${t.terminal}`,`CPU: ${t.cpu}`,`GPU: ${t.gpu}`,`Memory: ${t.memoryUsedMiB}MiB / ${t.memoryTotalMiB}MiB`,"",r[0],r[1]],o=Math.max(i.length,s.length),a=[];for(let l=0;l<o;l+=1){let c=i[l]??"",u=s[l]??"";if(u.length>0){let d=Rr(c.padEnd(31," "),l,i.length),m=to(u);a.push(`${d}  ${m}`);continue}a.push(Rr(c,l,i.length))}return a.join(`
`)}var Hr={name:"neofetch",description:"System info display",category:"system",params:["[--off]"],run:({args:e,authUser:t,hostname:n,shell:r,env:i})=>r.packageManager.isInstalled("neofetch")?C(e,"--help")?{stdout:"Usage: neofetch [--off]",exitCode:0}:C(e,"--off")?{stdout:`${t}@${n}`,exitCode:0}:{stdout:zr({user:t,host:n,shell:i.vars.SHELL,shellProps:r.properties,terminal:i.vars.TERM,uptimeSeconds:Math.floor((Date.now()-r.startTime)/1e3),packages:`${r.packageManager?.installedCount()??0} (dpkg)`}),exitCode:0}:{stderr:`bash: neofetch: command not found
Hint: install it with: apt install neofetch
`,exitCode:127}};function At(e,t){let n=new Function("exports","require","module","__filename","__dirname",e),r={exports:{}};return n(r.exports,()=>{throw new Error("require not supported in vm shim")},r,"",""),r.exports}var _t="v18.19.0",jr={node:_t,npm:"9.2.0",v8:"10.2.154.26-node.22"};function co(e,t){let n={version:_t,versions:jr,platform:"linux",arch:"x64",env:{NODE_ENV:"production",HOME:"/root",PATH:"/usr/local/bin:/usr/bin:/bin"},argv:["node"],stdout:{write:s=>(e.push(s),!0)},stderr:{write:s=>(t.push(s),!0)},exit:(s=0)=>{throw new Ot(s)},cwd:()=>"/root",hrtime:()=>[0,0]},r={log:(...s)=>e.push(s.map(Ce).join(" ")),error:(...s)=>t.push(s.map(Ce).join(" ")),warn:(...s)=>t.push(s.map(Ce).join(" ")),info:(...s)=>e.push(s.map(Ce).join(" ")),dir:s=>e.push(Ce(s))},i=s=>{switch(s){case"path":return{join:(...o)=>o.join("/").replace(/\/+/g,"/"),resolve:(...o)=>`/${o.join("/").replace(/^\/+/,"")}`,dirname:o=>o.split("/").slice(0,-1).join("/")||"/",basename:o=>o.split("/").pop()??"",extname:o=>{let a=o.split("/").pop()??"",l=a.lastIndexOf(".");return l>0?a.slice(l):""},sep:"/",delimiter:":"};case"os":return{platform:()=>"linux",arch:()=>"x64",type:()=>"Linux",hostname:()=>"fortune-vm",homedir:()=>"/root",tmpdir:()=>"/tmp",EOL:`
`};case"util":return{format:(...o)=>o.map(Ce).join(" "),inspect:o=>Ce(o)};case"fs":case"fs/promises":throw new Error(`Cannot require '${s}': filesystem access not available in virtual runtime`);case"child_process":case"net":case"http":case"https":throw new Error(`Cannot require '${s}': not available in virtual runtime`);default:throw new Error(`Cannot find module '${s}'`)}};return i.resolve=s=>{throw new Error(`Cannot resolve '${s}'`)},i.cache={},i.extensions={},At.createContext({console:r,process:n,require:i,Math,JSON,Object,Array,String,Number,Boolean,Symbol,Date,RegExp,Error,TypeError,RangeError,SyntaxError,Promise,Map,Set,WeakMap,WeakSet,parseInt,parseFloat,isNaN,isFinite,encodeURIComponent,decodeURIComponent,encodeURI,decodeURI,setTimeout:()=>{},clearTimeout:()=>{},setInterval:()=>{},clearInterval:()=>{},queueMicrotask:()=>{},globalThis:void 0,undefined:void 0,Infinity:1/0,NaN:NaN})}var Ot=class{constructor(t){this.code=t}code};function Ce(e){if(e===null)return"null";if(e===void 0)return"undefined";if(typeof e=="string")return e;if(typeof e=="function")return`[Function: ${e.name||"(anonymous)"}]`;if(Array.isArray(e))return`[ ${e.map(Ce).join(", ")} ]`;if(e instanceof Error)return`${e.name}: ${e.message}`;if(typeof e=="object")try{return`{ ${Object.entries(e).map(([n,r])=>`${n}: ${Ce(r)}`).join(", ")} }`}catch{return"[Object]"}return String(e)}function Ft(e){let t=[],n=[],r=co(t,n),i=0;try{let s=At.runInContext(e,r,{timeout:5e3});s!==void 0&&t.length===0&&t.push(Ce(s))}catch(s){s instanceof Ot?i=s.code:s instanceof Error?(n.push(`${s.name}: ${s.message}`),i=1):(n.push(String(s)),i=1)}return{stdout:t.length?`${t.join(`
`)}
`:"",stderr:n.length?`${n.join(`
`)}
`:"",exitCode:i}}function uo(e){let t=e.trim();return!t.includes(`
`)&&!t.startsWith("const ")&&!t.startsWith("let ")&&!t.startsWith("var ")&&!t.startsWith("function ")&&!t.startsWith("class ")&&!t.startsWith("if ")&&!t.startsWith("for ")&&!t.startsWith("while ")&&!t.startsWith("import ")&&!t.startsWith("//")?Ft(t):Ft(`(async () => { ${e} })()`)}var Wr={name:"node",description:"JavaScript runtime (virtual)",category:"system",params:["[--version] [-e <expr>] [-p <expr>] [file]"],run:({args:e,shell:t,cwd:n})=>{if(!t.packageManager.isInstalled("nodejs"))return{stderr:`bash: node: command not found
Hint: install it with: apt install nodejs
`,exitCode:127};if(C(e,["--version","-v"]))return{stdout:`${_t}
`,exitCode:0};if(C(e,["--versions"]))return{stdout:`${JSON.stringify(jr,null,2)}
`,exitCode:0};let r=e.findIndex(o=>o==="-e"||o==="--eval");if(r!==-1){let o=e[r+1];if(!o)return{stderr:`node: -e requires an argument
`,exitCode:1};let{stdout:a,stderr:l,exitCode:c}=Ft(o);return{stdout:a||void 0,stderr:l||void 0,exitCode:c}}let i=e.findIndex(o=>o==="-p"||o==="--print");if(i!==-1){let o=e[i+1];if(!o)return{stderr:`node: -p requires an argument
`,exitCode:1};let{stdout:a,stderr:l,exitCode:c}=Ft(o);return{stdout:a||(c===0?`
...
`,exitCode:1};let a=t.vfs.readFile(o),{stdout:l,stderr:c,exitCode:u}=uo(a);return{stdout:l||void 0,stderr:c||void 0,exitCode:u}}return{stdout:[`Welcome to Node.js ${_t}.`,'Type ".exit" to exit the REPL.',"> "].join(`
`),exitCode:0}}};var Tt="9.2.0",mo="18.19.0",Kr={name:"npm",description:"Node.js package manager (virtual)",category:"system",params:["<command> [args]"],run:({args:e,shell:t})=>{if(!t.packageManager.isInstalled("npm"))return{stderr:`bash: npm: command not found
Hint: install it with: apt install npm
`,exitCode:127};if(C(e,["--version","-v"]))return{stdout:`${Tt}
`,exitCode:0};let n=e[0]?.toLowerCase();switch(n){case"version":case"-version":return{stdout:`{ npm: '${Tt}', node: '${mo}', v8: '10.2.154.26' }
...
\u2514\u2500\u2500 (empty)
`,exitCode:0};case"help":case void 0:return{stdout:`${[`npm ${Tt}`,"","Usage: npm <command>","","Commands:","  install       (not available in virtual runtime)","  run           (not available in virtual runtime)","  exec          (not available in virtual runtime)","  list          List installed packages","  version       Print versions","  --version     Print npm version"].join(`
`)}
`,exitCode:0};default:return{stderr:`npm error: unknown command: ${n}
`,exitCode:1}}}},qr={name:"npx",description:"Node.js package runner (virtual)",category:"system",params:["<package> [args]"],run:({args:e,shell:t})=>t.packageManager.isInstalled("npm")?C(e,["--version"])?{stdout:`${Tt}
...
`),exitCode:0}}};var Qr={name:"pwd",description:"Print working directory",category:"navigation",params:[],run:({cwd:e})=>({stdout:e,exitCode:0})};var fo="Python 3.11.2";var Dt="3.11.2 (default, Mar 13 2023, 12:18:29) [GCC 12.2.0]",b={__pytype__:"none"};function Z(e=[]){return{__pytype__:"dict",data:new Map(e)}}function nn(e,t,n=1){return{__pytype__:"range",start:e,stop:t,step:n}}function G(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="dict"}function Ze(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="range"}function Pe(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="func"}function rn(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="class"}function at(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="instance"}function Me(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="none"}function ne(e){return e===null||Me(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?`'${e.replace(/'/g,"\\'")}'`:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:G(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Ze(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:Pe(e)?`<function ${e.name} at 0x...>`:rn(e)?`<class '${e.name}'>`:at(e)?`<${e.cls.name} object at 0x...>`:String(e)}function R(e){return e===null||Me(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?e:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:G(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Ze(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:ne(e)}function ue(e){return e===null||Me(e)?!1:typeof e=="boolean"?e:typeof e=="number"?e!==0:typeof e=="string"||Array.isArray(e)?e.length>0:G(e)?e.data.size>0:Ze(e)?es(e)>0:!0}function es(e){if(e.step===0)return 0;let t=Math.ceil((e.stop-e.start)/e.step);return Math.max(0,t)}function ho(e){let t=[];for(let n=e.start;(e.step>0?n<e.stop:n>e.stop)&&(t.push(n),!(t.length>1e4));n+=e.step);return t}function te(e){if(Array.isArray(e))return e;if(typeof e=="string")return[...e];if(Ze(e))return ho(e);if(G(e))return[...e.data.keys()];throw new J("TypeError",`'${Le(e)}' object is not iterable`)}function Le(e){return e===null||Me(e)?"NoneType":typeof e=="boolean"?"bool":typeof e=="number"?Number.isInteger(e)?"int":"float":typeof e=="string"?"str":Array.isArray(e)?"list":G(e)?"dict":Ze(e)?"range":Pe(e)?"function":rn(e)?"type":at(e)?e.cls.name:"object"}var J=class{constructor(t,n){this.type=t;this.message=n}type;message;toString(){return`${this.type}: ${this.message}`}},Je=class{constructor(t){this.value=t}value},lt=class{},ct=class{},ut=class{constructor(t){this.code=t}code};function go(e){let t=new Map,n=Z([["sep","/"],["linesep",`
`],["curdir","."],["pardir",".."]]);return n.__methods__={getcwd:()=>e,getenv:r=>typeof r=="string"?y.env[r]??b:b,path:Z([["join",b],["exists",b],["dirname",b],["basename",b]]),listdir:()=>[]},t.set("__builtins__",b),t.set("__name__","__main__"),t.set("__cwd__",e),t}function yo(e){let t=Z([["sep","/"],["curdir","."]]),n=Z([["sep","/"],["linesep",`
`],["name","posix"]]);return n._cwd=e,t._cwd=e,n.path=t,n}function So(){return Z([["version",Dt],["version_info",Z([["major",3],["minor",11],["micro",2]].map(([e,t])=>[e,t]))],["platform","linux"],["executable","/usr/bin/python3"],["prefix","/usr"],["path",["/usr/lib/python3.11","/usr/lib/python3.11/lib-dynload"]],["argv",[""]],["maxsize",9007199254740991]])}function bo(){return Z([["pi",Math.PI],["e",Math.E],["tau",Math.PI*2],["inf",1/0],["nan",NaN],["sqrt",b],["floor",b],["ceil",b],["log",b],["pow",b],["sin",b],["cos",b],["tan",b],["fabs",b],["factorial",b]])}function xo(){return Z([["dumps",b],["loads",b]])}function wo(){return Z([["match",b],["search",b],["findall",b],["sub",b],["split",b],["compile",b]])}var Xr={os:yo,sys:()=>So(),math:()=>bo(),json:()=>xo(),re:()=>wo(),random:()=>Z([["random",b],["randint",b],["choice",b],["shuffle",b]]),time:()=>Z([["time",b],["sleep",b],["ctime",b]]),datetime:()=>Z([["datetime",b],["date",b],["timedelta",b]]),collections:()=>Z([["Counter",b],["defaultdict",b],["OrderedDict",b]]),itertools:()=>Z([["chain",b],["product",b],["combinations",b],["permutations",b]]),functools:()=>Z([["reduce",b],["partial",b],["lru_cache",b]]),string:()=>Z([["ascii_letters","abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"],["digits","0123456789"],["punctuation","!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"]])},Rt=class{constructor(t){this.cwd=t}cwd;output=[];stderr=[];modules=new Map;getOutput(){return this.output.join(`
`)+(this.output.length?`
...
`).replace(/\\t/g,"	"),a=new Rt(n),{stdout:l,stderr:c,exitCode:u}=a.run(o);return{stdout:l||void 0,stderr:c||void 0,exitCode:u}}let i=e.find(s=>!s.startsWith("-"));if(i){let s=M(n,i);if(!t.vfs.exists(s))return{stderr:`python3: can't open file '${i}': [Errno 2] No such file or directory
`,exitCode:2};let o=t.vfs.readFile(s),a=new Rt(n),{stdout:l,stderr:c,exitCode:u}=a.run(o);return{stdout:l||void 0,stderr:c||void 0,exitCode:u}}return{stdout:`${Dt}
Type "help", "copyright", "credits" or "license" for more information.
>>> `,exitCode:0}}};var ns={name:"read",description:"Read a line from stdin into variables",category:"shell",params:["[-r] [-p prompt] <var...>"],run:({args:e,stdin:t,env:n})=>{let r=e.indexOf("-p"),i=e.filter((a,l)=>a!=="-r"&&a!=="-p"&&e[l-1]!=="-p"),s=(t??"").split(`
`)[0]??"",o=C(e,["-r"])?s:s.replace(/\\(?:\r?\n|.)/g,a=>a[1]===`
`||a[1]==="\r"?"":a[1]);if(!n)return{exitCode:0};if(i.length===0)n.vars.REPLY=o;else if(i.length===1)n.vars[i[0]]=o;else{let a=o.split(/\s+/);for(let l=0;l<i.length;l++)n.vars[i[l]]=l<i.length-1?a[l]??"":a.slice(l).join(" ")}return{exitCode:0}}};var rs={name:"rm",description:"Remove files or directories",category:"files",params:["[-r|-rf] <path>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"rm: missing operand",exitCode:1};let i=C(r,["-r","-rf","-fr"]),s=[];for(let o=0;;o+=1){let a=$e(r,o,{flags:["-r","-rf","-fr"]});if(!a)break;s.push(a)}if(s.length===0)return{stderr:"rm: missing operand",exitCode:1};for(let o of s){let a=M(n,o);V(e,a,"rm"),t.vfs.remove(a,{recursive:i})}return{exitCode:0}}};var ss={name:"sed",description:"Stream editor for filtering and transforming text",category:"text",params:["-e <expr> [file]","s/pattern/replace/[g]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-i"]),o=ae(r,["-e"])??r.find(v=>!v.startsWith("-")),a=r.filter(v=>!v.startsWith("-")&&v!==o).pop();if(!o)return{stderr:"sed: no expression",exitCode:1};let l=i??"";if(a){let v=M(n,a);try{l=t.vfs.readFile(v)}catch{return{stderr:`sed: ${a}: No such file or directory`,exitCode:1}}}let c=o.match(/^s([^a-zA-Z0-9])(.+?)\1(.*?)\1([gi]*)$/);if(!c)return{stderr:`sed: unrecognized command: ${o}`,exitCode:1};let[,,u,d,m]=c,p=(m??"").includes("i")?"gi":(m??"").includes("g")?"g":"",g;try{g=new RegExp(u,p||"")}catch{return{stderr:`sed: invalid regex: ${u}`,exitCode:1}}let S=((m??"").includes("g")||p.includes("g"),l.replace(g,d??""));if(s&&a){let v=M(n,a);return t.writeFileAsUser(e,v,S),{exitCode:0}}return{stdout:S,exitCode:0}}};var is={name:"set",description:"Display or set shell variables",category:"shell",params:["[VAR=value]"],run:({args:e,env:t})=>{if(e.length===0)return{stdout:Object.entries(t.vars).map(([r,i])=>`${r}=${i}`).join(`
`),exitCode:0};for(let n of e)if(n.includes("=")){let r=n.indexOf("=");t.vars[n.slice(0,r)]=n.slice(r+1)}return{exitCode:0}}};async function on(e,t,n,r){return Pt(e,t,n,i=>Q(i,r.authUser,r.hostname,r.mode,r.cwd,r.shell,void 0,r.env).then(s=>s.stdout??""))}function Ne(e){let t=[],n=0;for(;n<e.length;){let r=e[n].trim();if(!r||r.startsWith("#")){n++;continue}let i=r.match(/^(?:function\s+)?(\w+)\s*\(\s*\)\s*\{(.+)\}\s*$/),s=i??(r.match(/^(?:function\s+)?(\w+)\s*\(\s*\)\s*\{?\s*$/)||r.match(/^function\s+(\w+)\s*\{?\s*$/));if(s){let a=s[1],l=[];if(i){l.push(...i[2].split(";").map(c=>c.trim()).filter(Boolean)),t.push({type:"func",name:a,body:l}),n++;continue}for(n++;n<e.length&&e[n]?.trim()!=="}"&&n<e.length+1;){let c=e[n].trim().replace(/^do\s+/,"");c&&c!=="{"&&l.push(c),n++}n++,t.push({type:"func",name:a,body:l});continue}let o=r.match(/^\(\(\s*(.+?)\s*\)\)$/);if(o){t.push({type:"arith",expr:o[1]}),n++;continue}if(r.startsWith("if ")||r==="if"){let a=r.replace(/^if\s+/,"").replace(/;\s*then\s*$/,"").trim(),l=[],c=[],u=[],d="then",m="";for(n++;n<e.length&&e[n]?.trim()!=="fi";){let p=e[n].trim();p.startsWith("elif ")?(d="elif",m=p.replace(/^elif\s+/,"").replace(/;\s*then\s*$/,"").trim(),c.push({cond:m,body:[]})):p==="else"?d="else":p!=="then"&&(d==="then"?l.push(p):d==="elif"&&c.length>0?c[c.length-1].body.push(p):u.push(p)),n++}t.push({type:"if",cond:a,then_:l,elif:c,else_:u})}else if(r.startsWith("for ")){let a=r.match(/^for\s+(\w+)\s+in\s+(.+?)(?:\s*;\s*do)?$/);if(a){let l=[];for(n++;n<e.length&&e[n]?.trim()!=="done";){let c=e[n].trim().replace(/^do\s+/,"");c&&c!=="do"&&l.push(c),n++}t.push({type:"for",var:a[1],list:a[2],body:l})}else t.push({type:"cmd",line:r})}else if(r.startsWith("while ")){let a=r.replace(/^while\s+/,"").replace(/;\s*do\s*$/,"").trim(),l=[];for(n++;n<e.length&&e[n]?.trim()!=="done";){let c=e[n].trim().replace(/^do\s+/,"");c&&c!=="do"&&l.push(c),n++}t.push({type:"while",cond:a,body:l})}else t.push({type:"cmd",line:r});n++}return t}async function sn(e,t){let n=await on(e,t.env.vars,t.env.lastExitCode,t),r=n.match(/^\[?\s*(.+?)\s*\]?$/);if(r){let s=r[1],o=s.match(/^-([fdeznr])\s+(.+)$/);if(o){let[,c,u]=o,d=M(t.cwd,u);if(c==="f")return t.shell.vfs.exists(d)&&t.shell.vfs.stat(d).type==="file";if(c==="d")return t.shell.vfs.exists(d)&&t.shell.vfs.stat(d).type==="directory";if(c==="e")return t.shell.vfs.exists(d);if(c==="z")return(u??"").length===0;if(c==="n")return(u??"").length>0}let a=s.match(/^"?([^"]*)"?\s*(==|!=|=|<|>)\s*"?([^"]*)"?$/);if(a){let[,c,u,d]=a;if(u==="=="||u==="=")return c===d;if(u==="!=")return c!==d}let l=s.match(/^(\S+)\s+(-eq|-ne|-lt|-le|-gt|-ge)\s+(\S+)$/);if(l){let[,c,u,d]=l,m=Number(c),p=Number(d);if(u==="-eq")return m===p;if(u==="-ne")return m!==p;if(u==="-lt")return m<p;if(u==="-le")return m<=p;if(u==="-gt")return m>p;if(u==="-ge")return m>=p}}return((await Q(n,t.authUser,t.hostname,t.mode,t.cwd,t.shell,void 0,t.env)).exitCode??0)===0}async function Ae(e,t){let n={exitCode:0},r="";for(let i of e)if(i.type==="cmd"){let s=await on(i.line,t.env.vars,t.env.lastExitCode,t),o=/^([A-Za-z_][A-Za-z0-9_]*)=(.*)/,a=s.trim().split(/\s+/);if(a.length>0&&o.test(a[0])&&a.every(u=>o.test(u))){for(let u of a){let d=u.match(o);t.env.vars[d[1]]=d[2]}t.env.lastExitCode=0;continue}let l=await(async()=>{let c=s.trim().split(/\s+/)[0]??"",u=t.env.vars[`__func_${c}`];if(u){let d=s.trim().split(/\s+/).slice(1),m={...t.env.vars};d.forEach((S,v)=>{t.env.vars[String(v+1)]=S}),t.env.vars[0]=c;let p=u.split(`
`),g=await Ae(Ne(p),t);for(let S=1;S<=d.length;S++)delete t.env.vars[String(S)];return Object.assign(t.env.vars,{...m,...t.env.vars}),g}return Q(s,t.authUser,t.hostname,t.mode,t.cwd,t.shell,void 0,t.env)})();if(t.env.lastExitCode=l.exitCode??0,l.stdout&&(r+=`${l.stdout}
`),l.stderr)return{...l,stdout:r.trim()};n=l}else if(i.type==="if"){let s=!1;if(await sn(i.cond,t)){let o=await Ae(Ne(i.then_),t);o.stdout&&(r+=`${o.stdout}
`),s=!0}else{for(let o of i.elif)if(await sn(o.cond,t)){let a=await Ae(Ne(o.body),t);a.stdout&&(r+=`${a.stdout}
`),s=!0;break}if(!s&&i.else_.length>0){let o=await Ae(Ne(i.else_),t);o.stdout&&(r+=`${o.stdout}
`)}}}else if(i.type==="func")t.env.vars[`__func_${i.name}`]=i.body.join(`
`);else if(i.type==="arith"){let s=i.expr.trim(),o=s.match(/^(\w+)\s*(\+\+|--)$/);if(o){let a=parseInt(t.env.vars[o[1]]??"0",10);t.env.vars[o[1]]=String(o[2]==="++"?a+1:a-1)}else{let a=s.match(/^(\w+)\s*([+\-*/])=\s*(.+)$/);if(a){let l=parseInt(t.env.vars[a[1]]??"0",10),c=parseInt(a[3],10),u={"+":l+c,"-":l-c,"*":l*c,"/":Math.floor(l/c)};t.env.vars[a[1]]=String(u[a[2]]??l)}else{let l=Gt(s,t.env.vars);Number.isNaN(l)||(t.env.lastExitCode=l===0?1:0)}}}else if(i.type==="for"){let o=(await on(i.list,t.env.vars,t.env.lastExitCode,t)).trim().split(/\s+/).flatMap(Ct);for(let a of o){t.env.vars[i.var]=a;let l=await Ae(Ne(i.body),t);if(l.stdout&&(r+=`${l.stdout}
`),l.closeSession)return l}}else if(i.type==="while"){let s=0;for(;s<1e3&&await sn(i.cond,t);){let o=await Ae(Ne(i.body),t);if(o.stdout&&(r+=`${o.stdout}
`),o.closeSession)return o;s++}}return{...n,stdout:r.trim()||n.stdout}}function os(e){let t=[],n="",r=0,i=!1,s=!1,o=0;for(;o<e.length;){let l=e[o];if(!i&&!s){if(l==="'"){i=!0,n+=l,o++;continue}if(l==='"'){s=!0,n+=l,o++;continue}if(l==="{"){r++,n+=l,o++;continue}if(l==="}"){if(r--,n+=l,o++,r===0){let c=n.trim();for(c&&t.push(c),n="";o<e.length&&(e[o]===";"||e[o]===" ");)o++}continue}if(r===0&&(l===";"||l===`
`)){let c=n.trim();c&&!c.startsWith("#")&&t.push(c),n="",o++;continue}}else i&&l==="'"?i=!1:s&&l==='"'&&(s=!1);n+=l,o++}let a=n.trim();return a&&!a.startsWith("#")&&t.push(a),t}var as={name:"sh",aliases:["bash"],description:"Execute shell script or command",category:"shell",params:["-c <script>","[<file>]"],run:async e=>{let{args:t,shell:n,cwd:r}=e;if(C(t,"-c")){let s=t[t.indexOf("-c")+1]??"";if(!s)return{stderr:"sh: -c requires a script",exitCode:1};let o=os(s),a=Ne(o);return Ae(a,e)}let i=t[0];if(i){let s=M(r,i);if(!n.vfs.exists(s))return{stderr:`sh: ${i}: No such file or directory`,exitCode:1};let o=n.vfs.readFile(s),a=os(o),l=Ne(a);return Ae(l,e)}return{stderr:"sh: invalid usage. Use: sh -c 'cmd' or sh <file>",exitCode:1}}};var ls={name:"shift",description:"Shift positional parameters",category:"shell",params:["[n]"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};let n=parseInt(e[0]??"1",10)||1,r=t.vars.__argv?.split("\0").filter(Boolean)??[];t.vars.__argv=r.slice(n).join("\0");let i=r.slice(n);for(let s=1;s<=9;s++)t.vars[String(s)]=i[s-1]??"";return{exitCode:0}}},cs={name:"trap",description:"Trap signals and events",category:"shell",params:["[action] [signal...]"],run:({args:e,env:t})=>{if(!t||e.length===0)return{exitCode:0};let n=e[0]??"",r=e.slice(1);for(let i of r)t.vars[`__trap_${i.toUpperCase()}`]=n;return{exitCode:0}}},us={name:"return",description:"Return from a shell function",category:"shell",params:["[n]"],run:({args:e,env:t})=>{let n=parseInt(e[0]??"0",10);return t&&(t.lastExitCode=n),{exitCode:n}}};var ds={name:"sleep",description:"Delay execution",category:"system",params:["<seconds>"],run:async({args:e})=>{let t=parseFloat(e[0]??"1");return Number.isNaN(t)||t<0?{stderr:"sleep: invalid time",exitCode:1}:(await new Promise(n=>setTimeout(n,t*1e3)),{exitCode:0})}};var ms={name:"sort",description:"Sort lines of text",category:"text",params:["[-r] [-n] [-u] [-k <col>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-r"]),o=C(r,["-n"]),a=C(r,["-u"]),l=r.filter(g=>!g.startsWith("-")),d=[...(l.length>0?l.map(g=>{try{return V(e,M(n,g),"sort"),t.vfs.readFile(M(n,g))}catch{return""}}).join(`
`):i??"").split(`
`).filter(Boolean)].sort((g,S)=>o?Number(g)-Number(S):g.localeCompare(S)),m=s?d.reverse():d;return{stdout:(a?[...new Set(m)]:m).join(`
`),exitCode:0}}};var ps={name:"source",aliases:["."],description:"Execute commands from a file in the current shell environment",category:"shell",params:["<file> [args...]"],run:async({args:e,authUser:t,hostname:n,cwd:r,shell:i,env:s})=>{let o=e[0];if(!o)return{stderr:"source: missing filename",exitCode:1};let a=M(r,o);if(!i.vfs.exists(a))return{stderr:`source: ${o}: No such file or directory`,exitCode:1};let l=i.vfs.readFile(a),c=0;for(let u of l.split(`
`)){let d=u.trim();if(!d||d.startsWith("#"))continue;let m=await Q(d,t,n,"shell",r,i,void 0,s);if(c=m.exitCode??0,m.closeSession||m.switchUser)return m}return{exitCode:c}}};var fs={name:"su",description:"Switch user",category:"users",params:["[-] [-c <cmd>] [username]"],run:async({authUser:e,shell:t,args:n,hostname:r,mode:i,cwd:s})=>{let o=n.includes("-")||n.includes("-l")||n.includes("--login"),a=n.indexOf("-c"),l=a!==-1?n[a+1]:void 0,u=n.filter((d,m)=>m!==a&&m!==a+1).filter(d=>d!=="-"&&d!=="-l"&&d!=="--login").find(d=>!d.startsWith("-"))??"root";return t.users.listUsers().includes(u)?e==="root"?l?Q(l,u,r,i,o?`/home/${u}`:s,t):{switchUser:u,nextCwd:o?`/home/${u}`:void 0,exitCode:0}:t.users.isSudoer(e)?{sudoChallenge:{username:u,targetUser:u,commandLine:l??null,loginShell:o,prompt:"Password: "},exitCode:0}:{stderr:`su: permission denied
`,exitCode:1}:{stderr:`su: user '${u}' does not exist
`,exitCode:1}}};function vo(e){let{flags:t,flagsWithValues:n,positionals:r}=le(e,{flags:["-i","-S"],flagsWithValue:["-u","--user"]}),i=t.has("-i"),s=n.get("-u")||n.get("--user")||"root",o=r.length>0?r.join(" "):null;return{targetUser:s,loginShell:i,commandLine:o}}var hs={name:"sudo",description:"Execute as superuser",category:"users",params:["<command...>"],run:async({authUser:e,hostname:t,mode:n,cwd:r,shell:i,args:s})=>{let{targetUser:o,loginShell:a,commandLine:l}=vo(s);if(e!=="root"&&!i.users.isSudoer(e))return{stderr:"sudo: permission denied",exitCode:1};let c=o||"root",u=`[sudo] password for ${e}: `;return e==="root"?!l&&a?{switchUser:c,nextCwd:`/home/${c}`,exitCode:0}:l?Q(l,c,t,n,a?`/home/${c}`:r,i):{stderr:"sudo: missing command",exitCode:1}:{sudoChallenge:{username:e,targetUser:c,commandLine:l,loginShell:a,prompt:u},exitCode:0}}};var gs={name:"tail",description:"Output last lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=ae(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
`),p=d.endsWith(`
...
`),exitCode:0}}};var ys={name:"tar",description:"Archive utility",category:"archive",params:["[-czf|-xzf|-tf] <archive> [files...]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=[],s=!1;for(let m of r)if(/^-[a-zA-Z]{2,}$/.test(m))for(let p of m.slice(1))i.push(`-${p}`);else if(!s&&/^[cxtdru]{1,}[a-zA-Z]*$/.test(m)&&!m.includes("/")&&!m.startsWith("-")){s=!0;for(let p of m)i.push(`-${p}`)}else i.push(m);let o=i.includes("-c"),a=i.includes("-x"),l=i.includes("-t"),c=i.indexOf("-f"),u=c!==-1?i[c+1]:i.find(m=>m.endsWith(".tar")||m.endsWith(".tar.gz")||m.endsWith(".tgz"));if(!o&&!a&&!l)return{stderr:`tar: must specify -c, -x, or -t
`,exitCode:1};if(!u)return{stderr:`tar: no archive specified
`,exitCode:1};let d=M(n,u);if(o){let m=new Set;c!==-1&&m.add(c+1);let p=i.filter((S,v)=>!S.startsWith("-")&&S!==u&&!m.has(v)),g={};for(let S of p){let v=M(n,S);try{if(t.vfs.stat(v).type==="file")g[S]=t.vfs.readFile(v);else{let F=(_,P)=>{for(let N of t.vfs.list(_)){let I=`${_}/${N}`,x=`${P}/${N}`;t.vfs.stat(I).type==="file"?g[x]=t.vfs.readFile(I):F(I,x)}};F(v,S)}}catch{return{stderr:`tar: ${S}: No such file or directory`,exitCode:1}}}return t.writeFileAsUser(e,d,JSON.stringify(g)),{exitCode:0}}if(l||a){let m;try{m=JSON.parse(t.vfs.readFile(d))}catch{return{stderr:`tar: ${u}: cannot open archive`,exitCode:1}}if(l)return{stdout:Object.keys(m).join(`
`),exitCode:0};for(let[p,g]of Object.entries(m))t.writeFileAsUser(e,M(n,p),g);return{exitCode:0}}return{stderr:"tar: must specify -c, -x, or -t",exitCode:1}}};var Ss={name:"tee",description:"Read stdin, write to stdout and files",category:"text",params:["[-a] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-a"]),o=r.filter(l=>!l.startsWith("-")),a=i??"";for(let l of o){let c=M(n,l);if(s){let u=(()=>{try{return t.vfs.readFile(c)}catch{return""}})();t.writeFileAsUser(e,c,u+a)}else t.writeFileAsUser(e,c,a)}return{stdout:a,exitCode:0}}};function Qe(e,t,n){if(e[e.length-1]==="]"&&(e=e.slice(0,-1)),e[0]==="["&&(e=e.slice(1)),e.length===0)return!1;if(e[0]==="!")return!Qe(e.slice(1),t,n);let r=e.indexOf("-a");if(r!==-1)return Qe(e.slice(0,r),t,n)&&Qe(e.slice(r+1),t,n);let i=e.indexOf("-o");if(i!==-1)return Qe(e.slice(0,i),t,n)||Qe(e.slice(i+1),t,n);if(e.length===2){let[s,o=""]=e,l=(c=>c.startsWith("/")?c:`${n}/${c}`.replace(/\/+/g,"/"))(o);switch(s){case"-e":return t.vfs.exists(l);case"-f":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file";case"-d":return t.vfs.exists(l)&&t.vfs.stat(l).type==="directory";case"-r":return t.vfs.exists(l);case"-w":return t.vfs.exists(l);case"-x":return t.vfs.exists(l)&&!!(t.vfs.stat(l).mode&73);case"-s":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file"&&t.vfs.stat(l).size>0;case"-z":return o.length===0;case"-n":return o.length>0;case"-L":return t.vfs.isSymlink(l)}}if(e.length===3){let[s="",o,a=""]=e,l=Number(s),c=Number(a);switch(o){case"=":case"==":return s===a;case"!=":return s!==a;case"<":return s<a;case">":return s>a;case"-eq":return l===c;case"-ne":return l!==c;case"-lt":return l<c;case"-le":return l<=c;case"-gt":return l>c;case"-ge":return l>=c}}return e.length===1?(e[0]??"").length>0:!1}var bs={name:"test",aliases:["["],description:"Evaluate conditional expression",category:"shell",params:["<expression>"],run:({args:e,shell:t,cwd:n})=>{try{return{exitCode:Qe([...e],t,n)?0:1}}catch{return{stderr:"test: malformed expression",exitCode:2}}}};var xs={name:"touch",description:"Create or update files",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"touch: missing file operand",exitCode:1};for(let i of r){let s=M(n,i);V(e,s,"touch"),t.vfs.exists(s)||t.writeFileAsUser(e,s,"")}return{exitCode:0}}};function Co(e){return e.replace(/\\n/g,`
`).replace(/\\t/g,"	").replace(/\\r/g,"\r").replace(/\\\\/g,"\\")}function ws(e){let t=[],n=Co(e),r=0;for(;r<n.length;){if(r+2<n.length&&n[r+1]==="-"){let i=n.charCodeAt(r),s=n.charCodeAt(r+2);if(i<=s){for(let o=i;o<=s;o++)t.push(String.fromCharCode(o));r+=3;continue}}t.push(n[r]),r++}return t}var vs={name:"tr",description:"Translate or delete characters",category:"text",params:["[-d] [-s] <set1> [set2]"],run:({args:e,stdin:t})=>{let n=C(e,["-d"]),r=C(e,["-s"]),i=e.filter(l=>!l.startsWith("-")),s=ws(i[0]??""),o=ws(i[1]??""),a=t??"";if(n){let l=new Set(s);a=[...a].filter(c=>!l.has(c)).join("")}else if(o.length>0){let l=new Map;for(let c=0;c<s.length;c++)l.set(s[c],o[c]??o[o.length-1]??"");a=[...a].map(c=>l.get(c)??c).join("")}if(r&&o.length>0){let l=new Set(o);a=a.replace(/(.)\1+/g,(c,u)=>l.has(u)?u:c)}return{stdout:a,exitCode:0}}};var Cs={name:"tree",description:"Display directory tree",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=M(n,$e(r,0)??n);return V(e,i,"tree"),{stdout:t.vfs.tree(i),exitCode:0}}};var Ps={name:"true",description:"Return success exit code",category:"shell",params:[],run:()=>({exitCode:0})},Es={name:"false",description:"Return failure exit code",category:"shell",params:[],run:()=>({exitCode:1})};var $s={name:"type",description:"Describe how a command would be interpreted",category:"shell",params:["<command...>"],run:({args:e,shell:t,env:n})=>{if(e.length===0)return{stderr:"type: missing argument",exitCode:1};let r=(n?.vars?.PATH??"/usr/local/bin:/usr/bin:/bin").split(":"),i=[],s=0;for(let o of e){if(Se(o)){i.push(`${o} is a shell builtin`);continue}let a=!1;for(let l of r){let c=`${l}/${o}`;if(t.vfs.exists(c)){i.push(`${o} is ${c}`),a=!0;break}}a||(i.push(`${o}: not found`),s=1)}return{stdout:i.join(`
`),exitCode:s}}};var ks={name:"uname",description:"Print system information",category:"system",params:["[-a] [-s] [-r] [-m]"],run:({shell:e,args:t})=>{let n=C(t,["-a"]),r="Linux",i=e.properties?.kernel??"5.15.0",s=e.properties?.arch??"x86_64",o=e.hostname;return n?{stdout:`${r} ${o} ${i} #1 SMP ${s} GNU/Linux`,exitCode:0}:C(t,["-r"])?{stdout:i,exitCode:0}:C(t,["-m"])?{stdout:s,exitCode:0}:{stdout:r,exitCode:0}}};var Ms={name:"uniq",description:"Report or filter out repeated lines",category:"text",params:["[-c] [-d] [-u] [file]"],run:({args:e,stdin:t})=>{let n=C(e,["-c"]),r=C(e,["-d"]),i=C(e,["-u"]),s=(t??"").split(`
...
`),exitCode:s?1:0}}};function Lt(e){let t=e.toLocaleString("en-US",{weekday:"short"}),n=e.toLocaleString("en-US",{month:"short"}),r=e.getDate().toString().padStart(2,"0"),i=e.getHours().toString().padStart(2,"0"),s=e.getMinutes().toString().padStart(2,"0"),o=e.getSeconds().toString().padStart(2,"0"),a=e.getFullYear();return`${t} ${n} ${r} ${i}:${s}:${o} ${a}`}var Fs={name:"who",description:"Show active sessions",category:"system",params:[],run:({shell:e})=>({stdout:e.users.listActiveSessions().map(n=>{let r=new Date(n.startedAt),i=Number.isNaN(r.getTime())?n.startedAt:Lt(r);return`${n.username} ${n.tty} ${i} (${n.remoteAddress||"unknown"})`}).join(`
`),exitCode:0})};var Ts={name:"whoami",description:"Print current user",category:"system",params:[],run:({authUser:e})=>({stdout:e,exitCode:0})};var Ds={name:"xargs",description:"Build and execute command lines from stdin",category:"text",params:["[command] [args...]"],run:async({authUser:e,hostname:t,mode:n,cwd:r,args:i,stdin:s,shell:o,env:a})=>{let l=i[0]??"echo",c=i.slice(1),u=(s??"").trim().split(/\s+/).filter(Boolean);if(u.length===0)return{exitCode:0};let d=[l,...c,...u].join(" ");return Q(d,e,t,n,r,o,void 0,a)}};var Po=[Qr,Rn,Pr,Cs,Dn,xs,rs,Mr,Un,Ir,br,xr,Ln,wr,vr,tr,sr,ss,Fn,ms,Ms,As,lr,gs,zn,vs,Ss,Ds,qn,ys,or,ar,Tn,Ts,Fs,hr,yr,ir,ks,Zr,Sr,Kn,Jn,Hn,ds,Yr,Zn,Qn,er,is,Is,as,Vn,Xn,Nr,gr,Bn,_s,En,Gr,Wn,hs,fs,Hr,_n,On,Gn,Yn,Os,$s,kr,kn,Mn,bs,ps,fr,Jr,ns,jn,ls,cs,us,Ps,Es,Kr,qr,Wr,ts,Ns,rr,Er],Rs=[],dt=new Map,Vt=null,Eo=pr(()=>ln().map(e=>e.name));function an(){dt.clear();for(let e of ln()){dt.set(e.name,e);for(let t of e.aliases??[])dt.set(t,e)}Vt=Array.from(dt.keys()).sort()}function ln(){return[...Po,...Rs,Eo]}function cn(e){let t={...e,name:e.name.trim().toLowerCase(),aliases:e.aliases?.map(r=>r.trim().toLowerCase())};if([t.name,...t.aliases??[]].some(r=>r.length===0||/\s/.test(r)))throw new Error("Command names must be non-empty and contain no spaces");Rs.push(t),an()}function un(e,t,n){return{name:e,params:t,run:n}}function dn(){return Vt||an(),Vt}function Qt(){return ln()}function Se(e){return Vt||an(),dt.get(e.toLowerCase())}async function Ls(e,t,n,r,i,s,o){let a={exitCode:0},l=[],c=i,u=0;for(;u<e.length;){let m=e[u];if(a=await $o(m.pipeline,t,n,r,c,s,o),o.lastExitCode=a.exitCode??0,a.nextCwd&&(a.exitCode??0)===0&&(c=a.nextCwd),a.stdout&&l.push(a.stdout),a.closeSession||a.switchUser)return{...a,stdout:l.join("")||a.stdout};let p=m.op;if(!(!p||p===";")){if(p==="&&"){if((a.exitCode??0)!==0)for(;u<e.length&&e[u]?.op==="&&";)u++}else if(p==="||"&&(a.exitCode??0)===0)for(;u<e.length&&e[u]?.op==="||";)u++}u++}let d=l.join("");return{...a,stdout:d||a.stdout,nextCwd:c!==i?c:void 0}}async function $o(e,t,n,r,i,s,o){if(!e.isValid)return{stderr:e.error||"Syntax error",exitCode:1};if(e.commands.length===0)return{exitCode:0};let a=o??{vars:{},lastExitCode:0};return e.commands.length===1?ko(e.commands[0],t,n,r,i,s,a):Mo(e.commands,t,n,r,i,s,a)}async function ko(e,t,n,r,i,s,o){let a;if(e.inputFile){let c=M(i,e.inputFile);try{a=s.vfs.readFile(c)}catch{return{stderr:`${e.inputFile}: No such file or directory`,exitCode:1}}}let l=await Xe(e.name,e.args,t,n,r,i,s,a,o);if(e.outputFile){let c=M(i,e.outputFile),u=l.stdout||"";try{if(e.appendOutput){let d=(()=>{try{return s.vfs.readFile(c)}catch{return""}})();s.writeFileAsUser(t,c,d+u)}else s.writeFileAsUser(t,c,u);return{...l,stdout:""}}catch{return{...l,stderr:`Failed to write to ${e.outputFile}`,exitCode:1}}}return l}async function Mo(e,t,n,r,i,s,o){let a="",l=0;for(let c=0;c<e.length;c++){let u=e[c];if(c===0&&u.inputFile){let m=M(i,u.inputFile);try{a=s.vfs.readFile(m)}catch{return{stderr:`${u.inputFile}: No such file or directory`,exitCode:1}}}let d=await Xe(u.name,u.args,t,n,r,i,s,a,o);if(l=d.exitCode??0,c===e.length-1&&u.outputFile){let m=M(i,u.outputFile),p=d.stdout||"";try{if(u.appendOutput){let g=(()=>{try{return s.vfs.readFile(m)}catch{return""}})();s.writeFileAsUser(t,m,g+p)}else s.writeFileAsUser(t,m,p);a=""}catch{return{stderr:`Failed to write to ${u.outputFile}`,exitCode:1}}}else a=d.stdout||"";if(d.stderr&&l!==0)return{stderr:d.stderr,exitCode:l};if(d.closeSession||d.switchUser)return d}return{stdout:a,exitCode:l}}function mt(e){let t=[],n="",r=!1,i="",s=0;for(;s<e.length;){let o=e[s],a=e[s+1];if((o==='"'||o==="'")&&!r){r=!0,i=o,s++;continue}if(r&&o===i){r=!1,i="",s++;continue}if(r){n+=o,s++;continue}if(o===" "){n&&(t.push(n),n=""),s++;continue}if(!r&&o==="2"&&a===">"){let l=e.slice(s+1);if(l.startsWith(">>&1")||l.startsWith(">> &1")){n&&(t.push(n),n=""),t.push("2>>&1"),s+=5;continue}if(l.startsWith(">&1")){n&&(t.push(n),n=""),t.push("2>&1"),s+=4;continue}if(l.startsWith(">>")){n&&(t.push(n),n=""),t.push("2>>"),s+=3;continue}if(l.startsWith(">")){n&&(t.push(n),n=""),t.push("2>"),s+=2;continue}}if((o===">"||o==="<")&&!r){n&&(t.push(n),n=""),o===">"&&a===">"?(t.push(">>"),s+=2):(t.push(o),s++);continue}n+=o,s++}return n&&t.push(n),t}function Vs(e){let t=e.trim();if(!t)return{statements:[],isValid:!0};try{return{statements:Io(t),isValid:!0}}catch(n){return{statements:[],isValid:!1,error:n.message}}}function Io(e){let t=No(e),n=[];for(let r of t){let s={pipeline:{commands:Ao(r.text.trim()),isValid:!0}};r.op&&(s.op=r.op),n.push(s)}return n}function No(e){let t=[],n="",r=0,i=!1,s="",o=0,a=l=>{n.trim()&&t.push({text:n,op:l}),n=""};for(;o<e.length;){let l=e[o],c=e.slice(o,o+2);if((l==='"'||l==="'")&&!i){i=!0,s=l,n+=l,o++;continue}if(i&&l===s){i=!1,n+=l,o++;continue}if(i){n+=l,o++;continue}if(l==="("){r++,n+=l,o++;continue}if(l===")"){r--,n+=l,o++;continue}if(r>0){n+=l,o++;continue}if(c==="&&"){a("&&"),o+=2;continue}if(c==="||"){a("||"),o+=2;continue}if(l===";"){a(";"),o++;continue}n+=l,o++}return a(),t}function Ao(e){return _o(e).map(Oo)}function _o(e){let t=[],n="",r=!1,i="";for(let o=0;o<e.length;o++){let a=e[o];if((a==='"'||a==="'")&&!r){r=!0,i=a,n+=a;continue}if(r&&a===i){r=!1,n+=a;continue}if(r){n+=a;continue}if(a==="|"&&e[o+1]!=="|"){if(!n.trim())throw new Error("Syntax error near unexpected token '|'");t.push(n.trim()),n=""}else n+=a}let s=n.trim();if(!s&&t.length>0)throw new Error("Syntax error near unexpected token '|'");return s&&t.push(s),t}function Oo(e){let t=mt(e);if(t.length===0)return{name:"",args:[]};let n=[],r,i,s=!1,o=0,a,l=!1,c=!1;for(;o<t.length;){let m=t[o];if(m==="<"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after <");r=t[o],o++}else if(m===">>"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after >>");i=t[o],s=!0,o++}else if(m===">"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after >");i=t[o],s=!1,o++}else if(m==="2>&1")c=!0,o++;else if(m==="2>>"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after 2>>");a=t[o],l=!0,o++}else if(m==="2>"){if(o++,o>=t.length)throw new Error("Syntax error: expected filename after 2>");a=t[o],l=!1,o++}else n.push(m),o++}let u=n[0]??"";return{name:/^([A-Za-z_][A-Za-z0-9_]*)=(.*)$/.test(u)?u:u.toLowerCase(),args:n.slice(1),inputFile:r,outputFile:i,appendOutput:s,stderrFile:a,stderrAppend:l,stderrToStdout:c}}function Y(e){return e==="root"?"/root":`/home/${e}`}function Ve(e,t){return{vars:{PATH:"/usr/local/bin:/usr/bin:/bin",HOME:Y(e),USER:e,LOGNAME:e,SHELL:"/bin/sh",TERM:"xterm-256color",HOSTNAME:t,PS1:"\\u@\\h:\\w\\$ "},lastExitCode:0}}function Us(e,t,n,r){if(e.startsWith("/")){if(!n.vfs.exists(e))return null;try{let s=n.vfs.stat(e);return s.type!=="file"||!(s.mode&73)||(e.startsWith("/sbin/")||e.startsWith("/usr/sbin/"))&&r!=="root"?null:e}catch{return null}}let i=(t.vars.PATH??"/usr/local/bin:/usr/bin:/bin").split(":");for(let s of i){if((s==="/sbin"||s==="/usr/sbin")&&r!=="root")continue;let o=`${s}/${e}`;if(n.vfs.exists(o))try{let a=n.vfs.stat(o);if(a.type!=="file"||!(a.mode&73))continue;return o}catch{}}return null}async function Xe(e,t,n,r,i,s,o,a,l){let c=/^([A-Za-z_][A-Za-z0-9_]*)=(.*)$/,u=[e,...t],d=0;for(;d<u.length&&c.test(u[d]);)d+=1;if(d>0){let g=u.slice(0,d).map(E=>E.match(c)),S=u.slice(d),v=[];for(let[,E,F]of g)v.push([E,l.vars[E]]),l.vars[E]=F;if(S.length===0)return{exitCode:0};try{return await Xe(S[0],S.slice(1),n,r,i,s,o,a,l)}finally{for(let[E,F]of v)F===void 0?delete l.vars[E]:l.vars[E]=F}}let m=l.vars[`__alias_${e}`];if(m)return Q(`${m} ${t.join(" ")}`,n,r,i,s,o,a,l);let p=Se(e);if(!p){let g=Us(e,l,o,n);if(g){let S=o.vfs.readFile(g),v=S.match(/exec\s+builtin\s+(\S+)/);if(v){let F=Se(v[1]);if(F)return await F.run({authUser:n,hostname:r,activeSessions:o.users.listActiveSessions(),rawInput:[e,...t].join(" "),mode:i,args:t,stdin:a,cwd:s,shell:o,env:l})}let E=Se("sh");if(E)return await E.run({authUser:n,hostname:r,activeSessions:o.users.listActiveSessions(),rawInput:`sh -c ${JSON.stringify(S)}`,mode:i,args:["-c",S,"--",...t],stdin:a,cwd:s,shell:o,env:l})}return{stderr:`${e}: command not found`,exitCode:127}}try{return await p.run({authUser:n,hostname:r,activeSessions:o.users.listActiveSessions(),rawInput:[e,...t].join(" "),mode:i,args:t,stdin:a,cwd:s,shell:o,env:l})}catch(g){return{stderr:g instanceof Error?g.message:"Command failed",exitCode:1}}}async function Q(e,t,n,r,i,s,o,a){let l=e.trim();if(l.length===0)return{exitCode:0};let c=a??Ve(t,n),d=mt(l)[0]?.toLowerCase()??"",m=c.vars[`__alias_${d}`],p=m?l.replace(d,m):l,g=/\bfor\s+\w+\s+in\b/.test(p)||/\bwhile\s+/.test(p)||/\bif\s+/.test(p)||/\w+\s*\(\s*\)\s*\{/.test(p)||/\bfunction\s+\w+/.test(p)||/\(\(\s*.+\s*\)\)/.test(p),S=/(?<![|&])[|](?![|])/.test(p)||p.includes(">")||p.includes("<")||p.includes("&&")||p.includes("||")||p.includes(";");if(g&&d!=="sh"&&d!=="bash"||S){if(g&&d!=="sh"&&d!=="bash"){let x=Se("sh");if(x)return await x.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:p,mode:r,args:["-c",p],stdin:void 0,cwd:i,shell:s,env:c})}let I=Vs(p);if(!I.isValid)return{stderr:I.error||"Syntax error",exitCode:1};try{return await Ls(I.statements,t,n,r,i,s,c)}catch(x){return{stderr:x instanceof Error?x.message:"Execution failed",exitCode:1}}}let v=await Pt(p,c.vars,c.lastExitCode,I=>Q(I,t,n,r,i,s,void 0,c).then(x=>x.stdout??"")),E=mt(v.trim());if(E.length===0)return{exitCode:0};if(/^([A-Za-z_][A-Za-z0-9_]*)=(.*)$/.test(E[0]))return Xe(E[0],E.slice(1),t,n,r,i,s,o,c);let _=E[0]?.toLowerCase()??"",P=E.slice(1).flatMap(Ct),N=Se(_);if(!N){let I=Us(_,c,s,t);if(I){let x=s.vfs.readFile(I),A=x.match(/exec\s+builtin\s+(\S+)/);if(A){let k=A[1],w=Se(k);if(w)return await w.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:[_,...P].join(" "),mode:r,args:P,stdin:o,cwd:i,shell:s,env:c})}let T=Se("sh");if(T)return await T.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:`sh -c ${JSON.stringify(x)}`,mode:r,args:["-c",x,"--",...P],stdin:o,cwd:i,shell:s,env:c})}return{stderr:`${_}: command not found`,exitCode:127}}try{return await N.run({authUser:t,hostname:n,activeSessions:s.users.listActiveSessions(),rawInput:v,mode:r,args:P,stdin:o,cwd:i,shell:s,env:c})}catch(I){return{stderr:I instanceof Error?I.message:"Command failed",exitCode:1}}}function Bs(e){return e==="1"||e==="true"}function zs(){return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}function Fo(){return Bs(y.env.DEV_MODE)||Bs(y.env.RENDER_PERF)}function fe(e){let t=Fo();if(!t)return{enabled:t,mark:()=>{},done:()=>{}};let n=zs(),r=s=>{let o=zs()-n;console.log(`[perf][${e}] ${s}: ${o.toFixed(1)}ms`)};return{enabled:t,mark:r,done:(s="done")=>{r(s)}}}var bg=fe("HoneyPot");var Eg=fe("SshClient");var he=class{constructor(){this._events=Object.create(null)}on(t,n){return(this._events[t]||=[]).push(n),this}addListener(t,n){return this.on(t,n)}emit(t,...n){let r=this._events[t]||[];for(let i of r)try{i(...n)}catch{}return r.length>0}removeListener(t,n){this._events[t]&&(this._events[t]=this._events[t].filter(r=>r!==n))}};function _e(e){return function(){throw new Error(`ssh2: ${e} not implemented in browser`)}}var _g={generateKeyPair:_e("utils.generateKeyPair"),generateKeyPairSync:_e("utils.generateKeyPairSync"),parseKey:_e("utils.parseKey"),parsePrivateKey:_e("utils.parsePrivateKey"),parsePublicKey:_e("utils.parsePublicKey"),decryptKey:_e("utils.decryptKey"),sftp:{OPEN_MODE:{},STATUS_CODE:{},flagsToString:_e("utils.sftp.flagsToString"),stringToFlags:_e("utils.sftp.stringToFlags")}};var hn=Buffer.from([86,70,83,33]),To=1,mn=1,Hs=2,pn=class{chunks=[];write(t){this.chunks.push(t)}writeUint8(t){let n=Buffer.allocUnsafe(1);n.writeUInt8(t,0),this.chunks.push(n)}writeUint16(t){let n=Buffer.allocUnsafe(2);n.writeUInt16LE(t,0),this.chunks.push(n)}writeUint32(t){let n=Buffer.allocUnsafe(4);n.writeUInt32LE(t,0),this.chunks.push(n)}writeFloat64(t){let n=Buffer.allocUnsafe(8);n.writeDoubleBE(t,0),this.chunks.push(n)}writeString(t){let n=Buffer.from(t,"utf8");this.writeUint16(n.length),this.chunks.push(n)}writeBytes(t){this.writeUint32(t.length),this.chunks.push(t)}toBuffer(){return Buffer.concat(this.chunks)}};function js(e,t){if(t.type==="file"){let n=t;e.writeUint8(mn),e.writeString(n.name),e.writeUint32(n.mode),e.writeFloat64(n.createdAt),e.writeFloat64(n.updatedAt),e.writeUint8(n.compressed?1:0),e.writeBytes(n.content)}else if(t.type==="stub"){let n=t;e.writeUint8(mn),e.writeString(n.name),e.writeUint32(n.mode),e.writeFloat64(n.createdAt),e.writeFloat64(n.updatedAt),e.writeUint8(0),e.writeBytes(Buffer.from(n.stubContent,"utf8"))}else{let n=t;e.writeUint8(Hs),e.writeString(n.name),e.writeUint32(n.mode),e.writeFloat64(n.createdAt),e.writeFloat64(n.updatedAt);let r=Object.values(n.children);e.writeUint32(r.length);for(let i of r)js(e,i)}}function gn(e){let t=new pn;return t.write(hn),t.writeUint8(To),js(t,e),t.toBuffer()}var fn=class{constructor(t){this.buf=t}buf;pos=0;readUint8(){return this.buf.readUInt8(this.pos++)}readUint16(){let t=this.buf.readUInt16LE(this.pos);return this.pos+=2,t}readUint32(){let t=this.buf.readUInt32LE(this.pos);return this.pos+=4,t}readFloat64(){let t=this.buf.readDoubleBE(this.pos);return this.pos+=8,t}readString(){let t=this.readUint16(),n=this.buf.toString("utf8",this.pos,this.pos+t);return this.pos+=t,n}readBytes(){let t=this.readUint32(),n=this.buf.slice(this.pos,this.pos+t);return this.pos+=t,n}remaining(){return this.buf.length-this.pos}};function Ws(e){let t=e.readUint8(),n=e.readString(),r=e.readUint32(),i=e.readFloat64(),s=e.readFloat64();if(t===mn){let o=e.readUint8()===1,a=e.readBytes();return{type:"file",name:n,mode:r,createdAt:i,updatedAt:s,compressed:o,content:a}}if(t===Hs){let o=e.readUint32(),a=Object.create(null);for(let l=0;l<o;l++){let c=Ws(e);a[c.name]=c}return{type:"directory",name:n,mode:r,createdAt:i,updatedAt:s,children:a,_childCount:o}}throw new Error(`[VFS binary] Unknown node type: 0x${t.toString(16)}`)}function Oe(e){if(e.length<5)throw new Error("[VFS binary] Buffer too short");if(!e.slice(0,4).equals(hn))throw new Error("[VFS binary] Invalid magic \u2014 not a VFS binary snapshot");let n=new fn(e);for(let i=0;i<5;i++)n.readUint8();let r=Ws(n);if(r.type!=="directory")throw new Error("[VFS binary] Root node must be a directory");return r}function Ks(e){return e.length>=4&&e.slice(0,4).equals(hn)}var j={WRITE:1,MKDIR:2,REMOVE:3,CHMOD:4,MOVE:5,SYMLINK:6},pt="utf8";function Do(e,t,n){let r=Buffer.from(n,pt);return e.writeUInt16LE(r.length,t),r.copy(e,t+2),2+r.length}function Ro(e){let t=Buffer.from(e.path,pt),n=0;e.op===j.WRITE?n=4+(e.content?.length??0)+4:e.op===j.MKDIR?n=4:e.op===j.REMOVE?n=0:e.op===j.CHMOD?n=4:(e.op===j.MOVE||e.op===j.SYMLINK)&&(n=2+Buffer.byteLength(e.dest??"",pt));let r=3+t.length+n,i=Buffer.allocUnsafe(r),s=0;if(i.writeUInt8(e.op,s++),i.writeUInt16LE(t.length,s),s+=2,t.copy(i,s),s+=t.length,e.op===j.WRITE){let o=e.content??Buffer.alloc(0);i.writeUInt32LE(o.length,s),s+=4,o.copy(i,s),s+=o.length,i.writeUInt32LE(e.mode??420,s),s+=4}else e.op===j.MKDIR?(i.writeUInt32LE(e.mode??493,s),s+=4):e.op===j.CHMOD?(i.writeUInt32LE(e.mode??420,s),s+=4):(e.op===j.MOVE||e.op===j.SYMLINK)&&(s+=Do(i,s,e.dest??""));return i}function Lo(e){let t=[],n=0;try{for(;n<e.length&&!(n+3>e.length);){let r=e.readUInt8(n++),i=e.readUInt16LE(n);if(n+=2,n+i>e.length)break;let s=e.subarray(n,n+i).toString(pt);if(n+=i,r===j.WRITE){if(n+4>e.length)break;let o=e.readUInt32LE(n);if(n+=4,n+o+4>e.length)break;let a=Buffer.from(e.subarray(n,n+o));n+=o;let l=e.readUInt32LE(n);n+=4,t.push({op:r,path:s,content:a,mode:l})}else if(r===j.MKDIR){if(n+4>e.length)break;let o=e.readUInt32LE(n);n+=4,t.push({op:r,path:s,mode:o})}else if(r===j.REMOVE)t.push({op:r,path:s});else if(r===j.CHMOD){if(n+4>e.length)break;let o=e.readUInt32LE(n);n+=4,t.push({op:r,path:s,mode:o})}else if(r===j.MOVE||r===j.SYMLINK){if(n+2>e.length)break;let o=e.readUInt16LE(n);if(n+=2,n+o>e.length)break;let a=e.subarray(n,n+o).toString(pt);n+=o,t.push({op:r,path:s,dest:a})}else break}}catch{}return t}function qs(e,t){let n=Ro(t);if(ee(e)){let r=_r(e,ot.O_WRONLY|ot.O_CREAT|ot.O_APPEND);try{Or(r,n)}finally{Fr(r)}}else ee(".vfs")||Ye(".vfs"),Ge(e,n)}function yn(e){if(!ee(e))return[];let t=pe(e);return t.length===0?[]:Lo(t)}function Gs(e){ee(e)&&st(e)}function W(e){if(!e||e.trim()==="")return"/";let t=z.normalize(e.startsWith("/")?e:`/${e}`);return t===""?"/":t}function Vo(e){return e.split("/").filter(Boolean)}function oe(e,t){let n=W(t);if(n==="/")return e;let r=Vo(n),i=e;for(let s of r){if(i.type!=="directory")throw new Error(`Path '${n}' does not exist.`);let o=i.children[s];if(!o)throw new Error(`Path '${n}' does not exist.`);i=o}return i}function Ue(e,t,n,r){let i=W(t);if(i==="/")throw new Error("Root path has no parent directory.");let s=z.dirname(i),o=z.basename(i);if(!o)throw new Error(`Invalid path '${t}'.`);n&&r(s);let a=oe(e,s);if(a.type!=="directory")throw new Error(`Parent path '${s}' is not a directory.`);return{parent:a,name:o}}var Sn=class e extends he{root;mode;snapshotFile;journalFile;evictionThreshold;flushAfterNWrites;_writesSinceFlush=0;_flushTimer=null;_dirty=!1;mounts=new Map;static isBrowser=typeof y>"u"||typeof y.versions?.node>"u";constructor(t={}){if(super(),this.mode=t.mode??"memory",this.mode==="fs"){if(!t.snapshotPath)throw new Error('VirtualFileSystem: "snapshotPath" is required when mode is "fs".');this.snapshotFile=nt(t.snapshotPath,"vfs-snapshot.vfsb"),this.journalFile=nt(t.snapshotPath,"vfs-journal.bin"),this.evictionThreshold=t.evictionThresholdBytes??64*1024,this.flushAfterNWrites=t.flushAfterNWrites??500;let n=t.flushIntervalMs??1e3;n>0&&(this._flushTimer=setInterval(()=>{this._dirty&&this._autoFlush()},n),typeof this._flushTimer=="object"&&this._flushTimer!==null&&"unref"in this._flushTimer&&this._flushTimer.unref())}else this.snapshotFile=null,this.journalFile=null,this.evictionThreshold=0,this.flushAfterNWrites=0;this.root=this.makeDir("",493)}makeDir(t,n){let r=Date.now();return{type:"directory",name:t,mode:n,createdAt:r,updatedAt:r,children:Object.create(null),_childCount:0}}makeFile(t,n,r,i){let s=Date.now();return{type:"file",name:t,content:n,mode:r,compressed:i,createdAt:s,updatedAt:s}}makeStub(t,n,r){let i=Date.now();return{type:"stub",name:t,stubContent:n,mode:r,createdAt:i,updatedAt:i}}writeStub(t,n,r=420){let i=W(t),{parent:s,name:o}=Ue(this.root,i,!0,l=>this.mkdirRecursive(l,493)),a=s.children[o];if(a?.type==="directory")throw new Error(`Cannot write stub '${i}': path is a directory.`);a?.type!=="file"&&(a||s._childCount++,s.children[o]=this.makeStub(o,n,r))}mkdirRecursive(t,n){let r=W(t);if(r==="/")return;let i=r.split("/").filter(Boolean),s=this.root,o="";for(let a of i){o+=`/${a}`;let l=s.children[a];if(!l)l=this.makeDir(a,n),s.children[a]=l,s._childCount++,this.emit("dir:create",{path:o,mode:n}),this._journal({op:j.MKDIR,path:o,mode:n});else if(l.type!=="directory")throw new Error(`Cannot create directory '${o}': path is a file.`);s=l}}async restoreMirror(){if(!(this.mode!=="fs"||!this.snapshotFile)){if(!ee(this.snapshotFile)){if(this.journalFile){let t=yn(this.journalFile);t.length>0&&this._replayJournal(t)}return}try{let t=pe(this.snapshotFile);if(Ks(t))this.root=Oe(t);else{let n=JSON.parse(t.toString("utf8"));this.root=this.deserializeDir(n.root,""),console.info("[VirtualFileSystem] Migrating legacy JSON snapshot to binary format.")}if(this.emit("snapshot:restore",{path:this.snapshotFile}),this.journalFile){let n=yn(this.journalFile);n.length>0&&this._replayJournal(n)}}catch(t){console.warn(`[VirtualFileSystem] Could not restore snapshot from ${this.snapshotFile}:`,t instanceof Error?t.message:String(t))}}}async flushMirror(){if(this.mode!=="fs"||!this.snapshotFile){this.emit("mirror:flush");return}let t=wt(this.snapshotFile);Ye(t,{recursive:!0});let n=this.root,r=gn(n);Ge(this.snapshotFile,r),this.journalFile&&Gs(this.journalFile),this._dirty=!1,this._writesSinceFlush=0,this.emit("mirror:flush",{path:this.snapshotFile}),this.evictLargeFiles()}getMode(){return this.mode}getSnapshotPath(){return this.snapshotFile}async _autoFlush(){this._dirty&&await this.flushMirror()}_markDirty(){this._dirty=!0,this.flushAfterNWrites>0&&(this._writesSinceFlush++,this._writesSinceFlush>=this.flushAfterNWrites&&(this._writesSinceFlush=0,this._autoFlush()))}async stopAutoFlush(){this._flushTimer!==null&&(clearInterval(this._flushTimer),this._flushTimer=null),this._dirty&&await this.flushMirror()}importRootTree(t){let n=this._replayMode;this._replayMode=!0;try{this.root=t}finally{this._replayMode=n}}mergeRootTree(t){let n=this._replayMode;this._replayMode=!0;try{this._mergeDir(this.root,t)}finally{this._replayMode=n}}_mergeDir(t,n){for(let[r,i]of Object.entries(n.children)){let s=t.children[r];i.type==="directory"?s?s.type==="directory"&&this._mergeDir(s,i):(t.children[r]=i,t._childCount++):s||(t.children[r]=i,t._childCount++)}}encodeBinary(){return gn(this.root)}releaseTree(){this.root=this.makeDir("",493)}_replayMode=!1;_journal(t){this.journalFile&&!this._replayMode&&(qs(this.journalFile,t),this._markDirty())}_replayJournal(t){this._replayMode=!0;try{for(let n of t)try{n.op===j.WRITE?this.writeFile(n.path,n.content??Buffer.alloc(0),{mode:n.mode}):n.op===j.MKDIR?this.mkdir(n.path,n.mode):n.op===j.REMOVE?this.exists(n.path)&&this.remove(n.path,{recursive:!0}):n.op===j.CHMOD?this.exists(n.path)&&this.chmod(n.path,n.mode??420):n.op===j.MOVE?this.exists(n.path)&&n.dest&&this.move(n.path,n.dest):n.op===j.SYMLINK&&n.dest&&this.symlink(n.dest,n.path)}catch{}}finally{this._replayMode=!1}}evictLargeFiles(){!this.snapshotFile||this.evictionThreshold===0||ee(this.snapshotFile)&&this._evictDir(this.root)}_evictDir(t){for(let n of Object.values(t.children))if(n.type==="directory")this._evictDir(n);else if(n.type==="file"&&!n.evicted){let r=n.compressed?n.size??n.content.length*2:n.content.length;r>this.evictionThreshold&&(n.size=r,n.content=Buffer.alloc(0),n.evicted=!0)}}_reloadEvicted(t,n){if(!(!t.evicted||!this.snapshotFile)&&ee(this.snapshotFile))try{let r=pe(this.snapshotFile),i=Oe(r),s=n.split("/").filter(Boolean),o=i;for(let a of s){if(o.type!=="directory")return;let l=o.children[a];if(!l)return;o=l}o.type==="file"&&(t.content=o.content,t.compressed=o.compressed,t.evicted=void 0)}catch{}}mount(t,n,{readOnly:r=!0}={}){if(e.isBrowser)return;let i=W(t),s=nt(n);if(!ee(s))throw new Error(`VirtualFileSystem.mount: host path does not exist: "${s}"`);if(!Nt(s).isDirectory())throw new Error(`VirtualFileSystem.mount: host path is not a directory: "${s}"`);this.mkdir(i),this.mounts.set(i,{hostPath:s,readOnly:r}),this.emit("mount",{vPath:i,hostPath:s,readOnly:r})}unmount(t){let n=W(t);this.mounts.delete(n)&&this.emit("unmount",{vPath:n})}getMounts(){return[...this.mounts.entries()].map(([t,n])=>({vPath:t,...n}))}resolveMount(t){let n=W(t),r=[...this.mounts.entries()].sort(([i],[s])=>s.length-i.length);for(let[i,s]of r)if(n===i||n.startsWith(`${i}/`)){let o=n.slice(i.length).replace(/^\//,""),a=o?In(s.hostPath,o):s.hostPath;return{hostPath:s.hostPath,readOnly:s.readOnly,relPath:o,fullHostPath:a}}return null}mkdir(t,n=493){let r=W(t),i=(()=>{try{return oe(this.root,r)}catch{return null}})();if(i&&i.type!=="directory")throw new Error(`Cannot create directory '${r}': path is a file.`);this.mkdirRecursive(r,n)}writeFile(t,n,r={}){let i=this.resolveMount(t);if(i){if(i.readOnly)throw new Error(`EROFS: read-only file system, open '${i.fullHostPath}'`);let p=wt(i.fullHostPath);ee(p)||Ye(p,{recursive:!0}),Ge(i.fullHostPath,Buffer.isBuffer(n)?n:Buffer.from(n,"utf8"));return}let s=W(t),{parent:o,name:a}=Ue(this.root,s,!0,p=>this.mkdirRecursive(p,493)),l=o.children[a];if(l?.type==="directory")throw new Error(`Cannot write file '${s}': path is a directory.`);let c=Buffer.isBuffer(n)?n:Buffer.from(n,"utf8"),u=r.compress??!1,d=u?c:c,m=r.mode??420;if(l&&l.type==="file"){let p=l;p.content=d,p.compressed=u,p.mode=m,p.updatedAt=Date.now()}else l||o._childCount++,o.children[a]=this.makeFile(a,d,m,u);this.emit("file:write",{path:s,size:d.length}),this._journal({op:j.WRITE,path:s,content:c,mode:m})}readFile(t){let n=this.resolveMount(t);if(n){if(!ee(n.fullHostPath))throw new Error(`ENOENT: no such file or directory, open '${n.fullHostPath}'`);return pe(n.fullHostPath,"utf8")}let r=W(t),i=oe(this.root,r);if(i.type==="stub")return this.emit("file:read",{path:r,size:i.stubContent.length}),i.stubContent;if(i.type!=="file")throw new Error(`Cannot read '${t}': not a file.`);let s=i;s.evicted&&this._reloadEvicted(s,r);let o=s.compressed?s.content:s.content;return this.emit("file:read",{path:r,size:o.length}),o.toString("utf8")}readFileRaw(t){let n=this.resolveMount(t);if(n){if(!ee(n.fullHostPath))throw new Error(`ENOENT: no such file or directory, open '${n.fullHostPath}'`);return pe(n.fullHostPath)}let r=W(t),i=oe(this.root,r);if(i.type==="stub"){let a=Buffer.from(i.stubContent,"utf8");return this.emit("file:read",{path:r,size:a.length}),a}if(i.type!=="file")throw new Error(`Cannot read '${t}': not a file.`);let s=i;s.evicted&&this._reloadEvicted(s,r);let o=s.compressed?s.content:s.content;return this.emit("file:read",{path:r,size:o.length}),o}exists(t){let n=this.resolveMount(t);if(n)return ee(n.fullHostPath);try{return oe(this.root,W(t)),!0}catch{return!1}}chmod(t,n){let r=W(t);oe(this.root,r).mode=n,this._journal({op:j.CHMOD,path:r,mode:n})}stat(t){let n=this.resolveMount(t);if(n){if(!ee(n.fullHostPath))throw new Error(`ENOENT: stat '${n.fullHostPath}'`);let a=Nt(n.fullHostPath),l=n.relPath.split("/").pop()??n.fullHostPath.split("/").pop()??"",c=a.mtime;return a.isDirectory()?{type:"directory",name:l,path:W(t),mode:493,createdAt:a.birthtime,updatedAt:c,childrenCount:it(n.fullHostPath).length}:{type:"file",name:l,path:W(t),mode:n.readOnly?292:420,createdAt:a.birthtime,updatedAt:c,compressed:!1,size:a.size}}let r=W(t),i=oe(this.root,r),s=r==="/"?"":z.basename(r);if(i.type==="stub"){let a=i;return{type:"file",name:s,path:r,mode:a.mode,createdAt:new Date(a.createdAt),updatedAt:new Date(a.updatedAt),compressed:!1,size:a.stubContent.length}}if(i.type==="file"){let a=i;return{type:"file",name:s,path:r,mode:a.mode,createdAt:new Date(a.createdAt),updatedAt:new Date(a.updatedAt),compressed:a.compressed,size:a.evicted?a.size??0:a.content.length}}let o=i;return{type:"directory",name:s,path:r,mode:o.mode,createdAt:new Date(o.createdAt),updatedAt:new Date(o.updatedAt),childrenCount:o._childCount}}list(t="/"){let n=this.resolveMount(t);if(n){if(!ee(n.fullHostPath))return[];try{return it(n.fullHostPath).sort()}catch{return[]}}let r=W(t),i=oe(this.root,r);if(i.type!=="directory")throw new Error(`Cannot list '${t}': not a directory.`);return Object.keys(i.children).sort()}tree(t="/"){let n=W(t),r=oe(this.root,n);if(r.type!=="directory")throw new Error(`Cannot render tree for '${t}': not a directory.`);let i=t==="/"?"/":z.basename(n);return this.renderTreeLines(r,i)}renderTreeLines(t,n){let r=[n],i=Object.keys(t.children).sort();for(let s=0;s<i.length;s++){let o=i[s],a=t.children[o],l=s===i.length-1,c=l?"\u2514\u2500\u2500 ":"\u251C\u2500\u2500 ",u=l?"    ":"\u2502   ";if(r.push(`${c}${o}`),a.type==="directory"){let d=this.renderTreeLines(a,"").split(`
`).slice(1).map(m=>`${u}${m}`);r.push(...d)}}return r.join(`
`)}getUsageBytes(t="/"){return this.computeUsage(oe(this.root,W(t)))}computeUsage(t){if(t.type==="file")return t.content.length;if(t.type==="stub")return t.stubContent.length;let n=0;for(let r of Object.values(t.children))n+=this.computeUsage(r);return n}compressFile(t){let n=oe(this.root,W(t));if(n.type!=="file")throw new Error(`Cannot compress '${t}': not a file.`);let r=n;r.compressed||(r.content=r.content,r.compressed=!0,r.updatedAt=Date.now())}decompressFile(t){let n=oe(this.root,W(t));if(n.type!=="file")throw new Error(`Cannot decompress '${t}': not a file.`);let r=n;r.compressed&&(r.content=r.content,r.compressed=!1,r.updatedAt=Date.now())}symlink(t,n){let r=W(n),i=t.startsWith("/")?W(t):t,{parent:s,name:o}=Ue(this.root,r,!0,l=>this.mkdirRecursive(l,493)),a={type:"file",name:o,content:Buffer.from(i,"utf8"),mode:41471,compressed:!1,createdAt:Date.now(),updatedAt:Date.now()};s.children[o]=a,s._childCount++,this._journal({op:j.SYMLINK,path:r,dest:i}),this.emit("symlink:create",{link:r,target:i})}isSymlink(t){try{let n=oe(this.root,W(t));return n.type==="file"&&n.mode===41471}catch{return!1}}resolveSymlink(t,n=8){let r=W(t);for(let i=0;i<n;i++){try{let s=oe(this.root,r);if(s.type==="file"&&s.mode===41471){let o=s.content.toString("utf8");r=o.startsWith("/")?o:W(z.join(z.dirname(r),o));continue}}catch{break}return r}throw new Error(`Too many levels of symbolic links: ${t}`)}remove(t,n={}){let r=this.resolveMount(t);if(r){if(r.readOnly)throw new Error(`EROFS: read-only file system, unlink '${r.fullHostPath}'`);if(!ee(r.fullHostPath))throw new Error(`ENOENT: no such file or directory, unlink '${r.fullHostPath}'`);Nt(r.fullHostPath).isDirectory()?Ar(r.fullHostPath,{recursive:n.recursive??!1}):st(r.fullHostPath);return}let i=W(t);if(i==="/")throw new Error("Cannot remove root directory.");let s=oe(this.root,i);if(s.type==="directory"){let l=s;if(!n.recursive&&l._childCount>0)throw new Error(`Directory '${i}' is not empty. Use recursive option.`)}let{parent:o,name:a}=Ue(this.root,i,!1,()=>{});delete o.children[a],o._childCount--,this.emit("node:remove",{path:i}),this._journal({op:j.REMOVE,path:i})}move(t,n){let r=W(t),i=W(n);if(r==="/"||i==="/")throw new Error("Cannot move root directory.");let s=oe(this.root,r);if(this.exists(i))throw new Error(`Destination '${i}' already exists.`);this.mkdirRecursive(z.dirname(i),493);let{parent:o,name:a}=Ue(this.root,i,!1,()=>{}),{parent:l,name:c}=Ue(this.root,r,!1,()=>{});delete l.children[c],l._childCount--,s.name=a,o.children[a]=s,o._childCount++,this._journal({op:j.MOVE,path:r,dest:i})}toSnapshot(){return{root:this.serializeDir(this.root)}}serializeDir(t){let n=[];for(let r of Object.values(t.children))r.type==="stub"?n.push({type:"file",name:r.name,mode:r.mode,createdAt:new Date(r.createdAt).toISOString(),updatedAt:new Date(r.updatedAt).toISOString(),compressed:!1,contentBase64:Buffer.from(r.stubContent,"utf8").toString("base64")}):r.type==="file"?n.push(this.serializeFile(r)):n.push(this.serializeDir(r));return{type:"directory",name:t.name,mode:t.mode,createdAt:new Date(t.createdAt).toISOString(),updatedAt:new Date(t.updatedAt).toISOString(),children:n}}serializeFile(t){return{type:"file",name:t.name,mode:t.mode,createdAt:new Date(t.createdAt).toISOString(),updatedAt:new Date(t.updatedAt).toISOString(),compressed:t.compressed,contentBase64:t.content.toString("base64")}}static fromSnapshot(t){let n=new e;return n.root=n.deserializeDir(t.root,""),n}importSnapshot(t){this.root=this.deserializeDir(t.root,""),this.emit("snapshot:import")}deserializeDir(t,n){let r={type:"directory",name:n,mode:t.mode,createdAt:Date.parse(t.createdAt),updatedAt:Date.parse(t.updatedAt),children:Object.create(null),_childCount:0};for(let i of t.children){if(i.type==="file"){let s=i;r.children[s.name]={type:"file",name:s.name,mode:s.mode,createdAt:Date.parse(s.createdAt),updatedAt:Date.parse(s.updatedAt),compressed:s.compressed,content:Buffer.from(s.contentBase64,"base64")}}else{let s=this.deserializeDir(i,i.name);r.children[i.name]=s}r._childCount++}return r}},ft=Sn;function h(e,t,n=493){e.exists(t)||e.mkdir(t,n)}function f(e,t,n,r=420){e.writeStub(t,n,r)}function $(e,t,n){e.writeFile(t,n)}function Uo(e){let t=2166136261;for(let n=0;n<e.length;n++)t^=e.charCodeAt(n),t=Math.imul(t,16777619);return t>>>0}function Bo(e,t,n){h(e,"/etc"),f(e,"/etc/os-release",`${['NAME="Fortune GNU/Linux"',`PRETTY_NAME="${n.os}"`,"ID=fortune","ID_LIKE=debian",'HOME_URL="https://github.com/itsrealfortune/typescript-virtual-container"',"VERSION_CODENAME=nyx",'VERSION_ID="24.04"',"FORTUNE_CODENAME=nyx"].join(`
`)}
...
`),$(e,"/proc/hostname",`${n}
`);let E=(Math.random()*.3).toFixed(2),F=1+i.length;$(e,"/proc/loadavg",`${E} ${E} ${E} ${F}/${F} 1
...
`)}
`),Js(e,1,"root","pts/0","/sbin/init",new Date(r).toISOString(),{});for(let I of i){let x=Ys(I.tty);Js(e,x,I.username,I.tty,"bash",I.startedAt,{USER:I.username,HOME:`/home/${I.username}`,TERM:"xterm-256color",SHELL:"/bin/bash",LANG:"en_US.UTF-8",PATH:"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",LOGNAME:I.username})}let P=i.length>0?Ys(i[i.length-1].tty):1;try{e.remove("/proc/self")}catch{}let N=`/proc/${P}`;if(h(e,"/proc/self"),h(e,"/proc/self/fd"),h(e,"/proc/self/fdinfo"),h(e,"/proc/self/net"),e.exists(N))for(let I of e.list(N)){let x=`${N}/${I}`,A=`/proc/self/${I}`;try{e.stat(x).type==="file"&&$(e,A,e.readFile(x))}catch{}}else $(e,"/proc/self/cmdline","bash\0"),$(e,"/proc/self/comm","bash"),$(e,"/proc/self/status",`Name:	bash
State:	S (sleeping)
Pid:	1
PPid:	0
`),$(e,"/proc/self/environ",""),$(e,"/proc/self/cwd","/root\0"),$(e,"/proc/self/exe","/bin/bash\0")}function Ho(e,t,n){h(e,"/sys"),h(e,"/sys/devices"),h(e,"/sys/devices/virtual"),h(e,"/sys/devices/system"),h(e,"/sys/devices/system/cpu"),h(e,"/sys/devices/system/cpu/cpu0"),f(e,"/sys/devices/system/cpu/cpu0/online",`1
...
`);let r=Uo(t),i=r.toString(16).padStart(8,"0");f(e,"/sys/class/net/eth0/address",`52:54:00:${i.slice(0,2)}:${i.slice(2,4)}:${i.slice(4,6)}
...
`),f(e,`/sys/fs/cgroup/${a}/release_agent`,""));f(e,"/sys/fs/cgroup/memory/memory.limit_in_bytes",`${we()}
`),f(e,"/sys/fs/cgroup/memory/memory.usage_in_bytes",`${we()-Re()}
`),f(e,"/sys/fs/cgroup/memory/memory.memsw.limit_in_bytes",`${we()}
...
`),f(e,"/sys/kernel/osrelease",`${n.kernel}
`),f(e,"/sys/kernel/ostype",`Linux
`)}function jo(e){h(e,"/dev"),f(e,"/dev/null","",438),f(e,"/dev/zero","",438),f(e,"/dev/full","",438),f(e,"/dev/random","",292),f(e,"/dev/urandom","",292),f(e,"/dev/mem","",416),f(e,"/dev/port","",416),f(e,"/dev/kmsg","",432),f(e,"/dev/hwrng","",432),f(e,"/dev/fuse","",432),f(e,"/dev/autofs","",432),f(e,"/dev/userfaultfd","",432),f(e,"/dev/cpu_dma_latency","",432),f(e,"/dev/ptp0","",432),f(e,"/dev/snapshot","",432),f(e,"/dev/console","",384),f(e,"/dev/tty","",438),f(e,"/dev/ttyS0","",432),f(e,"/dev/ptmx","",438);for(let t=0;t<=63;t++)f(e,`/dev/tty${t}`,"",400);f(e,"/dev/vcs","",400),f(e,"/dev/vcs1","",400),f(e,"/dev/vcsa","",400),f(e,"/dev/vcsa1","",400),f(e,"/dev/vcsu","",400),f(e,"/dev/vcsu1","",400);for(let t=0;t<8;t++)f(e,`/dev/loop${t}`,"",432);h(e,"/dev/loop-control"),f(e,"/dev/vda","",432),f(e,"/dev/vdb","",432),f(e,"/dev/vdc","",432),f(e,"/dev/vdd","",432),h(e,"/dev/net"),f(e,"/dev/net/tun","",432),h(e,"/dev/pts"),h(e,"/dev/shm"),h(e,"/dev/cpu"),f(e,"/dev/stdin","",438),f(e,"/dev/stdout","",438),f(e,"/dev/stderr","",438),h(e,"/dev/fd"),f(e,"/dev/vga_arbiter","",432),f(e,"/dev/vsock","",432)}function Wo(e){h(e,"/usr"),h(e,"/usr/bin"),h(e,"/usr/sbin"),h(e,"/usr/local"),h(e,"/usr/local/bin"),h(e,"/usr/local/lib"),h(e,"/usr/local/share"),h(e,"/usr/local/include"),h(e,"/usr/local/sbin"),h(e,"/usr/share"),h(e,"/usr/share/doc"),h(e,"/usr/share/man"),h(e,"/usr/share/man/man1"),h(e,"/usr/share/man/man5"),h(e,"/usr/share/man/man8"),h(e,"/usr/share/common-licenses"),h(e,"/usr/share/ca-certificates"),h(e,"/usr/share/zoneinfo"),h(e,"/usr/lib"),h(e,"/usr/lib/x86_64-linux-gnu"),h(e,"/usr/lib/python3"),h(e,"/usr/lib/python3/dist-packages"),h(e,"/usr/lib/python3.12"),h(e,"/usr/lib/jvm"),h(e,"/usr/lib/jvm/java-21-openjdk-amd64"),h(e,"/usr/lib/jvm/java-21-openjdk-amd64/bin"),h(e,"/usr/lib/node_modules"),h(e,"/usr/lib/node_modules/npm"),h(e,"/usr/include"),h(e,"/usr/src");let t=["sh","bash","ls","cat","echo","grep","find","sort","head","tail","cut","tr","sed","awk","wc","tee","tar","gzip","gunzip","touch","mkdir","rm","mv","cp","chmod","ln","pwd","env","date","sleep","id","whoami","hostname","uname","ps","kill","df","du","curl","wget","nano","diff","uniq","xargs","base64"];for(let r of t)f(e,`/usr/bin/${r}`,`#!/bin/sh
exec builtin ${r} "$@"
...
CONFIG_VIRTIO_NET=y
CONFIG_KVM_GUEST=y
`,420),e.exists("/vmlinuz")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz"),e.exists("/vmlinuz.old")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz.old"),e.exists("/initrd.img")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img"),e.exists("/initrd.img.old")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img.old"),h(e,"/lost+found",448),h(e,"/home")}var Zs=new Map;function Qo(e,t){return`${e}|${t.kernel}|${t.os}|${t.arch}`}function Xo(e,t){let n=Qo(e,t),r=Zs.get(n);if(r)return r;let i=new ft({mode:"memory"});Bo(i,e,t),Ho(i,e,t),jo(i),Wo(i),qo(i),Go(i),Yo(i),Zo(i,t),zo(i,t);let s=i.encodeBinary();return Zs.set(n,s),s}function Qs(e,t,n,r,i,s=[]){let o=Xo(n,r);e.getMode()==="fs"&&e.exists("/home")?e.mergeRootTree(Oe(o)):e.importRootTree(Oe(o)),Jo(e),Ut(e,r,n,i,s),bn(e,t)}var xn=[{name:"vim",version:"2:9.0.1378-2",section:"editors",description:"Vi IMproved - enhanced vi editor",shortDesc:"Vi IMproved",installedSizeKb:3812,files:[{path:"/usr/bin/vim",content:`#!/bin/sh
...
`),exitCode:0}}search(t){let n=t.toLowerCase();return xn.filter(r=>r.name.includes(n)||r.description.toLowerCase().includes(n)||(r.shortDesc??"").toLowerCase().includes(n)).sort((r,i)=>r.name.localeCompare(i.name))}show(t){let n=this.findInRegistry(t);if(!n)return null;let r=this.installed.get(t);return[`Package: ${n.name}`,`Version: ${n.version}`,`Architecture: ${n.architecture??"amd64"}`,`Maintainer: ${n.maintainer??"Fortune Maintainers <pkg@fortune.local>"}`,`Installed-Size: ${n.installedSizeKb??0}`,`Depends: ${(n.depends??[]).join(", ")||"(none)"}`,`Section: ${n.section??"misc"}`,"Priority: optional",`Description: ${n.description}`,`Status: ${r?"install ok installed":"install ok not-installed"}`].join(`
`)}};var ea=new Uint32Array([1116352408,1899447441,3049323471,3921009573,961987163,1508970993,2453635748,2870763221,3624381080,310598401,607225278,1426881987,1925078388,2162078206,2614888103,3248222580,3835390401,4022224774,264347078,604807628,770255983,1249150122,1555081692,1996064986,2554220882,2821834349,2952996808,3210313671,3336571891,3584528711,113926993,338241895,666307205,773529912,1294757372,1396182291,1695183700,1986661051,2177026350,2456956037,2730485921,2820302411,3259730800,3345764771,3516065817,3600352804,4094571909,275423344,430227734,8111177861,659060556,883997877,958139571,1322822218,1537002063,1747873779,1955562222,2024104815,2227730452,2361852424,2428436474,2756734187,3204031479,3329325298]);function Bt(e){let t=e instanceof Uint8Array?e:new TextEncoder().encode(e),n=t.length*8,r=Math.ceil((t.length+9)/64)*64,i=new Uint8Array(r);i.set(t),i[t.length]=128,new DataView(i.buffer).setUint32(r-4,n>>>0,!1);let o=1779033703,a=3144134277,l=1013904242,c=2773480762,u=1359893119,d=2600822924,m=528734635,p=1541459225,g=new Uint32Array(64),S=new DataView(i.buffer);for(let F=0;F<r;F+=64){for(let w=0;w<16;w++)g[w]=S.getUint32(F+w*4,!1);for(let w=16;w<64;w++){let D=(g[w-15]>>>7|g[w-15]<<25)^(g[w-15]>>>18|g[w-15]<<14)^g[w-15]>>>3,q=(g[w-2]>>>17|g[w-2]<<15)^(g[w-2]>>>19|g[w-2]<<13)^g[w-2]>>>10;g[w]=g[w-16]+D+g[w-7]+q|0}let _=o,P=a,N=l,I=c,x=u,A=d,T=m,k=p;for(let w=0;w<64;w++){let D=(x>>>6|x<<26)^(x>>>11|x<<21)^(x>>>25|x<<7),q=x&A^~x&T,K=k+D+q+ea[w]+g[w]|0,X=(_>>>2|_<<30)^(_>>>13|_<<19)^(_>>>22|_<<10),ye=_&P^_&N^P&N,de=X+ye|0;k=T,T=A,A=x,x=I+K|0,I=N,N=P,P=_,_=K+de|0}o=o+_|0,a=a+P|0,l=l+N|0,c=c+I|0,u=u+x|0,d=d+A|0,m=m+T|0,p=p+k|0}let v=new Uint8Array(32),E=new DataView(v.buffer);return[o,a,l,c,u,d,m,p].forEach((F,_)=>E.setUint32(_*4,F,!1)),v}function Xs(e,t){let r=e instanceof Uint8Array?e:new TextEncoder().encode(e);r.length>64&&(r=Bt(r));let i=new Uint8Array(64);i.set(r);let s=i.map(c=>c^54),o=i.map(c=>c^92),a=new Uint8Array(64+t.length);a.set(s),a.set(t,64);let l=new Uint8Array(96);return l.set(o),l.set(Bt(a),64),Bt(l)}function ta(e,t,n,r){let i=e instanceof Uint8Array?e:new TextEncoder().encode(e),s=t instanceof Uint8Array?t:new TextEncoder().encode(t),o=32,a=Math.ceil(r/o),l=new Uint8Array(r);for(let c=1;c<=a;c++){let u=new Uint8Array(4);new DataView(u.buffer).setUint32(0,c,!1);let d=new Uint8Array(s.length+4);d.set(s),d.set(u,s.length);let m=Xs(i,d),p=new Uint8Array(m);for(let S=1;S<n;S++){m=Xs(i,m);for(let v=0;v<o;v++)p[v]^=m[v]}let g=(c-1)*o;l.set(p.slice(0,r-g),g)}return l}function ei(e){let t=new Uint8Array(e);return crypto.getRandomValues(t),t}function ti(){return crypto.randomUUID?crypto.randomUUID():("10000000-1000-4000-8000"+-1e11).replace(/[018]/g,e=>(e^crypto.getRandomValues(new Uint8Array(1))[0]&15>>e/4).toString(16))}function wn(e){let t=[];return{update(n){return t.push(n instanceof Uint8Array?n:new TextEncoder().encode(String(n))),this},digest(n="hex"){let r=t.reduce((a,l)=>a+l.length,0),i=new Uint8Array(r),s=0;for(let a of t)i.set(a,s),s+=a.length;let o=Bt(i);return n==="hex"?Array.from(o).map(a=>a.toString(16).padStart(2,"0")).join(""):n==="base64"?btoa(String.fromCharCode(...o)):o}}}function ni(e,t,n,r={}){let i=r.N??16384,s=Math.max(1e3,Math.round(Math.log2(i)*1e3)),o=typeof e=="string"?new TextEncoder().encode(e):e,a=typeof t=="string"?new TextEncoder().encode(t):t;return ta(o,a,s,n)}function ri(e,t){if(e.length!==t.length)return!1;let n=0;for(let r=0;r<e.length;r++)n|=e[r]^t[r];return n===0}function na(){let e=y.env.SSH_MIMIC_FAST_PASSWORD_HASH;return!!e&&!["0","false","no","off"].includes(e.toLowerCase())}var re=fe("VirtualUserManager"),gt=class e extends he{constructor(n,r=!0){super();this.vfs=n;this.autoSudoForNewUsers=r;re.mark("constructor")}vfs;autoSudoForNewUsers;static recordCache=new Map;static fastPasswordHash=na();usersPath="/etc/htpasswd";sudoersPath="/etc/sudoers";quotasPath="/etc/quotas";authDirPath="/.virtual-env-js/.auth";users=new Map;sudoers=new Set;quotas=new Map;activeSessions=new Map;nextTty=0;async initialize(){re.mark("initialize"),this.loadFromVfs(),this.loadSudoersFromVfs(),this.loadQuotasFromVfs();let n=!1;this.users.has("root")||(this.users.set("root",this.createRecord("root","")),n=!0),this.sudoers.add("root");let r="/root";this.vfs.exists(r)||(this.vfs.mkdir(r,493),this.vfs.writeFile(`${r}/README.txt`,"Welcome to the virtual environment, root")),n&&await this.persist(),this.emit("initialized")}async setQuotaBytes(n,r){if(re.mark("setQuotaBytes"),this.validateUsername(n),!this.users.has(n))throw new Error(`quota: user '${n}' does not exist`);if(!Number.isFinite(r)||r<0)throw new Error("quota: maxBytes must be a non-negative number");this.quotas.set(n,Math.floor(r)),await this.persist()}async clearQuota(n){re.mark("clearQuota"),this.validateUsername(n),this.quotas.delete(n),await this.persist()}getQuotaBytes(n){return re.mark("getQuotaBytes"),this.quotas.get(n)??null}getUsageBytes(n){re.mark("getUsageBytes");let r=n==="root"?"/root":`/home/${n}`;return this.vfs.exists(r)?this.vfs.getUsageBytes(r):0}assertWriteWithinQuota(n,r,i){re.mark("assertWriteWithinQuota");let s=this.quotas.get(n);if(s===void 0)return;let o=si(r),a=si(n==="root"?"/root":`/home/${n}`);if(!(o===a||o.startsWith(`${a}/`)))return;let c=this.getUsageBytes(n),u=0;if(this.vfs.exists(o)){let p=this.vfs.stat(o);p.type==="file"&&(u=p.size)}let d=Buffer.isBuffer(i)?i.length:Buffer.byteLength(i,"utf8"),m=c-u+d;if(m>s)throw new Error(`quota exceeded for '${n}': ${m}/${s} bytes`)}verifyPassword(n,r){re.mark("verifyPassword");let i=this.users.get(n);if(!i)return this.hashPassword(r,""),!1;let s=this.hashPassword(r,i.salt),o=i.passwordHash;try{let a=Buffer.from(s,"hex"),l=Buffer.from(o,"hex");return a.length!==l.length?!1:ri(a,l)}catch{return s===o}}async addUser(n,r){if(re.mark("addUser"),this.validateUsername(n),this.validatePassword(r),this.users.has(n))return;this.users.set(n,this.createRecord(n,r)),this.autoSudoForNewUsers&&this.sudoers.add(n);let i=n==="root"?"/root":`/home/${n}`;this.vfs.exists(i)||(this.vfs.mkdir(i,493),this.vfs.writeFile(`${i}/README.txt`,`Welcome to the virtual environment, ${n}`)),await this.persist(),this.emit("user:add",{username:n})}getPasswordHash(n){re.mark("getPasswordHash");let r=this.users.get(n);return r?r.passwordHash:null}async setPassword(n,r){if(re.mark("setPassword"),this.validateUsername(n),this.validatePassword(r),!this.users.has(n))throw new Error(`passwd: user '${n}' does not exist`);this.users.set(n,this.createRecord(n,r)),await this.persist()}async deleteUser(n){if(re.mark("deleteUser"),this.validateUsername(n),n==="root")throw new Error("deluser: cannot delete root");if(!this.users.delete(n))throw new Error(`deluser: user '${n}' does not exist`);this.sudoers.delete(n),this.emit("user:delete",{username:n}),await this.persist()}isSudoer(n){return re.mark("isSudoer"),this.sudoers.has(n)}async addSudoer(n){if(re.mark("addSudoer"),this.validateUsername(n),!this.users.has(n))throw new Error(`sudoers: user '${n}' does not exist`);this.sudoers.add(n),await this.persist()}async removeSudoer(n){if(re.mark("removeSudoer"),this.validateUsername(n),n==="root")throw new Error("sudoers: cannot remove root");this.sudoers.delete(n),await this.persist()}registerSession(n,r){re.mark("registerSession");let i={id:ti(),username:n,tty:`pts/${this.nextTty++}`,remoteAddress:r,startedAt:new Date().toISOString()};return this.activeSessions.set(i.id,i),this.emit("session:register",{sessionId:i.id,username:n,remoteAddress:r}),i}unregisterSession(n){if(re.mark("unregisterSession"),!n)return;let r=this.activeSessions.get(n);this.activeSessions.delete(n),r&&this.emit("session:unregister",{sessionId:n,username:r.username}),this.activeSessions.delete(n)}updateSession(n,r,i){if(re.mark("updateSession"),!n)return;let s=this.activeSessions.get(n);s&&this.activeSessions.set(n,{...s,username:r,remoteAddress:i})}listActiveSessions(){return re.mark("listActiveSessions"),Array.from(this.activeSessions.values()).sort((n,r)=>n.startedAt.localeCompare(r.startedAt))}listUsers(){return Array.from(this.users.keys()).sort()}loadFromVfs(){if(this.users.clear(),!this.vfs.exists(this.usersPath))return;let n=this.vfs.readFile(this.usersPath);for(let r of n.split(`
...
`:"",384)||s,s&&await this.vfs.flushMirror()}writeIfChanged(n,r,i){return this.vfs.exists(n)&&this.vfs.readFile(n)===r?(this.vfs.chmod(n,i),!1):(this.vfs.writeFile(n,r,{mode:i}),!0)}createRecord(n,r){let i=wn("sha256").update(n).update(":").update(r).digest("hex"),s=e.recordCache.get(i);if(s)return s;let o=ei(16).toString("hex"),a={username:n,salt:o,passwordHash:this.hashPassword(r,o)};return e.recordCache.set(i,a),a}hasPassword(n){re.mark("hasPassword");let r=this.users.get(n);if(!r)return!1;let i=this.hashPassword("",r.salt);return r.passwordHash===i?!1:!!r.passwordHash}hashPassword(n,r=""){return e.fastPasswordHash?wn("sha256").update(r).update(n).digest("hex"):ni(n,r||"",32).toString("hex")}validateUsername(n){if(!n||n.trim()==="")throw new Error("invalid username");if(!/^[a-z_][a-z0-9_-]{0,31}$/i.test(n))throw new Error("invalid username")}validatePassword(n){if(!n||n.trim()==="")throw new Error("invalid password")}authorizedKeys=new Map;addAuthorizedKey(n,r,i){re.mark("addAuthorizedKey");let s=this.authorizedKeys.get(n)??[];s.push({algo:r,data:i}),this.authorizedKeys.set(n,s),this.emit("key:add",{username:n,algo:r})}removeAuthorizedKeys(n){this.authorizedKeys.delete(n),this.emit("key:remove",{username:n})}getAuthorizedKeys(n){return this.authorizedKeys.get(n)??[]}};function si(e){let t=z.normalize(e);return t.startsWith("/")?t:`/${t}`}var yt=class extends he{vfs;idleThresholdMs;checkIntervalMs;_state="active";_lastActivity=Date.now();_frozenBuffer=null;_checkTimer=null;constructor(t,n={}){super(),this.vfs=t,this.idleThresholdMs=n.idleThresholdMs??6e4,this.checkIntervalMs=n.checkIntervalMs??15e3}start(){this._checkTimer||(this._lastActivity=Date.now(),this._checkTimer=setInterval(()=>this._check(),this.checkIntervalMs),typeof this._checkTimer=="object"&&this._checkTimer!==null&&"unref"in this._checkTimer&&this._checkTimer.unref())}async stop(){this._checkTimer&&(clearInterval(this._checkTimer),this._checkTimer=null),this._state==="frozen"&&this._thaw()}ping(){this._lastActivity=Date.now(),this._state==="frozen"&&this._thaw()}get state(){return this._state}get idleMs(){return Date.now()-this._lastActivity}_check(){this._state!=="frozen"&&Date.now()-this._lastActivity>=this.idleThresholdMs&&this._freeze()}async _freeze(){this._state!=="frozen"&&(await this.vfs.stopAutoFlush(),this._frozenBuffer=this.vfs.encodeBinary(),this.vfs.releaseTree(),this._state="frozen",this.emit("freeze"))}_thaw(){if(this._state!=="frozen"||!this._frozenBuffer)return;let t=Oe(this._frozenBuffer);this.vfs.importRootTree(t),this._frozenBuffer=null,this._state="active",this.emit("thaw")}};async function zt(){throw new Error("node:fs/promises.readFile is not supported in browser")}async function ii(){throw new Error("node:fs/promises.writeFile is not supported in browser")}async function oi(){throw new Error("node:fs/promises.unlink is not supported in browser")}function vn(e){return`'${e.replace(/'/g,"'\\''")}'`}function St(e){return e.replace(/\r\n/g,`
`).replace(/\r/g,`
`).replace(/\n/g,`\r
`)}function ai(e,t){let n=Number.isFinite(t.cols)&&t.cols>0?Math.floor(t.cols):80,r=Number.isFinite(t.rows)&&t.rows>0?Math.floor(t.rows):24;return`stty cols ${n} rows ${r} 2>/dev/null; ${e}`}function li(e,t){return!t||t.trim()===""||t==="."?e:t.startsWith("/")?z.normalize(t):z.normalize(z.join(e,t))}async function ci(e){try{let n=(await zt(`/proc/${e}/task/${e}/children`,"utf8")).trim().split(/\s+/).filter(Boolean).map(i=>Number.parseInt(i,10)).filter(i=>Number.isInteger(i)&&i>0),r=await Promise.all(n.map(i=>ci(i)));return[...n,...r.flat()]}catch{return[]}}async function ui(e=y.pid){let t=await ci(e),n=Array.from(new Set(t)).sort((r,i)=>r-i);return n.length===0?null:n.join(",")}function di(e,t,n){let r=ai(e,t),i=qt("script",["-qfec",r,"/dev/null"],{stdio:["pipe","pipe","pipe"],env:{...y.env,TERM:y.env.TERM??"xterm-256color"}});return i.stdout.on("data",s=>{n.write(s.toString("utf8"))}),i.stderr.on("data",s=>{n.write(s.toString("utf8"))}),i}function mi(e,t,n){return di(`nano -- ${vn(e)}`,t,n)}function pi(e,t,n){return di(`htop -p ${vn(e)}`,t,n)}function Ht(e,t,n){let r=[`Linux ${e} ${t.kernel} ${t.arch}`,"","The programs included with the Fortune GNU/Linux system are free software;","the exact distribution terms for each program are described in the","individual files in /usr/share/doc/*/copyright.","","Fortune GNU/Linux comes with ABSOLUTELY NO WARRANTY, to the extent","permitted by applicable law."];if(n){let i=new Date(n.at),s=Number.isNaN(i.getTime())?n.at:Lt(i);r.push(`Last login: ${s} from ${n.from||"unknown"}`)}return r.push(""),`${r.map(i=>`${i}\r
`).join("")}`}function jt(e,t,n){let r=e==="root",i=r?"\x1B[31;1m":"\x1B[35;1m",s="\x1B[37;1m",o="\x1B[34;1m",a="\x1B[0m";return`${s}[${i}${e}${s}@${o}${t}${a} ${n}${s}]${a}${r?"#":"$"} `}function fi(e,t,n,r,i,s="unknown",o={cols:80,rows:24},a){let l="",c=0,u=ra(a.vfs,n),d=null,m="",p=Y(n),g=Ve(n,r),S=null,v=null,E=()=>{let O=Y(n),U=p===O?"~":z.basename(p)||"/";return jt(n,r,U)},F=Array.from(new Set(dn())).sort();console.log(`[${i}] Shell started for user '${n}' at ${s}`),(async()=>{let O=`${Y(n)}/.bashrc`;if(a.vfs.exists(O))try{let U=a.vfs.readFile(O);for(let H of U.split(`
...
`),L.openEditor){await A(L.openEditor.targetPath,L.openEditor.initialContent,L.openEditor.tempPath);return}if(L.openHtop){await T();return}L.clearScreen&&t.write("\x1B[2J\x1B[H"),L.stdout&&t.write(`${St(L.stdout)}\r
`),L.stderr&&t.write(`${St(L.stderr)}\r
`),L.switchUser?(n=L.switchUser,p=L.nextCwd??Y(n),a.users.updateSession(i,n,s)):L.nextCwd&&(p=L.nextCwd),_()}async function x(){if(!S)return;let O=S;if(O.kind==="nano"){try{let U=await zt(O.tempPath,"utf8");a.writeFileAsUser(n,O.targetPath,U)}catch{}await oi(O.tempPath).catch(()=>{})}S=null,l="",c=0,t.write(`\r
`),_()}async function A(O,U,H){a.vfs.exists(O)&&await ii(H,U,"utf8");let L=mi(H,o,t);L.on("error",se=>{t.write(`nano: ${se.message}\r
`),x()}),L.on("close",()=>{x()}),S={kind:"nano",targetPath:O,tempPath:H,process:L}}async function T(){let O=await ui();if(!O){t.write(`htop: no child_process processes to display\r
`);return}let U=pi(O,o,t);U.on("error",H=>{t.write(`htop: ${H.message}\r
`),x()}),U.on("close",()=>{x()}),S={kind:"htop",targetPath:"",tempPath:"",process:U}}function k(O){l=O,c=l.length,_()}function w(O){l=`${l.slice(0,c)}${O}${l.slice(c)}`,c+=O.length,_()}function D(O,U){let H=U;for(;H>0&&!/\s/.test(O[H-1]);)H-=1;let L=U;for(;L<O.length&&!/\s/.test(O[L]);)L+=1;return{start:H,end:L}}function q(O){let U=O.lastIndexOf("/"),H=U>=0?O.slice(0,U+1):"",L=U>=0?O.slice(U+1):O,se=li(p,H||".");try{return a.vfs.list(se).filter(B=>!B.startsWith(".")).filter(B=>B.startsWith(L)).map(B=>{let xe=z.join(se,B),De=a.vfs.stat(xe).type==="directory"?"/":"";return`${H}${B}${De}`}).sort()}catch{return[]}}function K(){let{start:O,end:U}=D(l,c),H=l.slice(O,c);if(H.length===0)return;let se=l.slice(0,O).trim().length===0?F.filter(me=>me.startsWith(H)):[],B=q(H),xe=Array.from(new Set([...se,...B])).sort();if(xe.length!==0){if(xe.length===1){let me=xe[0],De=me.endsWith("/")?"":" ";l=`${l.slice(0,O)}${me}${De}${l.slice(U)}`,c=O+me.length+De.length,_();return}t.write(`\r
`),t.write(`${xe.join("  ")}\r
`),_()}}function X(O){if(O.length===0)return;u.push(O),u.length>500&&(u=u.slice(u.length-500));let U=u.length>0?`${u.join(`
`)}
`:"";a.vfs.writeFile(`${Y(n)}/.bash_history`,U)}function ye(){let O=`${Y(n)}/.lastlog.json`;if(!a.vfs.exists(O))return null;try{return JSON.parse(a.vfs.readFile(O))}catch{return null}}function de(O){let U=`${Y(n)}/.lastlog`;a.vfs.writeFile(U,JSON.stringify({at:O,from:s}))}function Ee(){let O=ye(),U=new Date().toISOString();t.write(Ht(r,e,O)),de(U)}Ee(),_(),t.on("data",async O=>{if(S){S.process.stdin.write(O);return}if(v){let H=O.toString("utf8");for(let L=0;L<H.length;L+=1){let se=H[L];if(se===""){v=null,t.write(`^C\r
`),_();return}if(se==="\x7F"||se==="\b"){v.buffer=v.buffer.slice(0,-1);continue}if(se==="\r"||se===`
`){let B=v.buffer;if(v.buffer="",v.onPassword){let{result:me,nextPrompt:De}=await v.onPassword(B,a);t.write(`\r
`),me!==null?(v=null,me.stdout&&t.write(me.stdout.replace(/\n/g,`\r
`)),me.stderr&&t.write(me.stderr.replace(/\n/g,`\r
`)),_()):(De&&(v.prompt=De),t.write(v.prompt));return}let xe=a.users.verifyPassword(v.username,B);await I(xe);return}se>=" "&&(v.buffer+=se)}return}let U=O.toString("utf8");for(let H=0;H<U.length;H+=1){let L=U[H];if(L===""){l="",c=0,d=null,m="",t.write(`bye\r
`),X("bye"),t.write(`logout\r
`),t.exit(0),t.end();return}if(L==="	"){K();continue}if(L==="\x1B"){let se=U[H+1],B=U[H+2],xe=U[H+3];if(se==="["&&B){if(B==="A"){H+=2,u.length>0&&(d===null?(m=l,d=u.length-1):d>0&&(d-=1),k(u[d]??""));continue}if(B==="B"){H+=2,d!==null&&(d<u.length-1?(d+=1,k(u[d]??"")):(d=null,k(m)));continue}if(B==="C"){H+=2,c<l.length&&(c+=1,t.write("\x1B[C"));continue}if(B==="D"){H+=2,c>0&&(c-=1,t.write("\x1B[D"));continue}if(B==="3"&&xe==="~"){H+=3,c<l.length&&(l=`${l.slice(0,c)}${l.slice(c+1)}`,_());continue}}}if(L===""){l="",c=0,d=null,m="",t.write(`^C\r
`),_();continue}if(L==="\r"||L===`
`){let se=l.trim();if(l="",c=0,d=null,m="",t.write(`\r
`),se.length>0){let B=await Promise.resolve(Q(se,n,r,"shell",p,a,void 0,g));if(X(se),B.openEditor){await A(B.openEditor.targetPath,B.openEditor.initialContent,B.openEditor.tempPath);return}if(B.openHtop){await T();return}if(B.sudoChallenge){N(B.sudoChallenge);return}if(B.clearScreen&&t.write("\x1B[2J\x1B[H"),B.stdout&&t.write(`${St(B.stdout)}\r
`),B.stderr&&t.write(`${St(B.stderr)}\r
`),B.closeSession){t.write(`logout\r
`),t.exit(B.exitCode??0),t.end();return}B.nextCwd&&(p=B.nextCwd),B.switchUser&&(n=B.switchUser,p=B.nextCwd??Y(n),a.users.updateSession(i,n,s),l="",c=0)}_();continue}if(L==="\x7F"||L==="\b"){c>0&&(l=`${l.slice(0,c-1)}${l.slice(c)}`,c-=1,_());continue}w(L)}}),t.on("close",()=>{S&&(S.process.kill("SIGTERM"),S=null)})}function ra(e,t){let n=`${Y(t)}/.bash_history`;return e.exists(n)?e.readFile(n).split(`
`).map(i=>i.trim()).filter(i=>i.length>0):(e.writeFile(n,""),[])}function sa(e){return typeof e=="object"&&e!==null&&"vfsInstance"in e&&hi(e.vfsInstance)}function hi(e){if(typeof e!="object"||e===null)return!1;let t=e;return typeof t.restoreMirror=="function"&&typeof t.flushMirror=="function"&&typeof t.writeFile=="function"&&typeof t.readFile=="function"&&typeof t.mkdir=="function"&&typeof t.exists=="function"&&typeof t.stat=="function"&&typeof t.list=="function"&&typeof t.remove=="function"&&typeof t.copy=="function"&&typeof t.move=="function"&&typeof t.touch=="function"}var ia={kernel:"1.0.0+itsrealfortune+1-amd64",os:"Fortune GNU/Linux x64",arch:"x86_64"},bt=fe("VirtualShell");function oa(){let e=y.env.SSH_MIMIC_AUTO_SUDO_NEW_USERS;return e?!["0","false","no","off"].includes(e.toLowerCase()):!0}var Be=class extends he{vfs;users;packageManager;hostname;properties;startTime;_idle=null;initialized;constructor(t,n,r){super(),bt.mark("constructor"),this.hostname=t,this.properties=n||ia,this.startTime=Date.now(),hi(r)?this.vfs=r:sa(r)?this.vfs=r.vfsInstance:this.vfs=new ft(r??{}),this.users=new gt(this.vfs,oa()),this.packageManager=new ht(this.vfs,this.users);let i=this.vfs,s=this.users,o=this.packageManager,a=this.properties,l=this.hostname,c=this.startTime;this.initialized=(async()=>{await i.restoreMirror(),await s.initialize(),Qs(i,s,l,a,c),o.load(),this.emit("initialized")})()}async ensureInitialized(){bt.mark("ensureInitialized"),await this.initialized}addCommand(t,n,r){let i=t.trim().toLowerCase();if(i.length===0||/\s/.test(i))throw new Error("Command name must be non-empty and contain no spaces");cn(un(i,n,r))}executeCommand(t,n,r){bt.mark("executeCommand"),this._idle?.ping();let i=Q(t,n,this.hostname,"shell",r,this);return this.emit("command",{command:t,user:n,cwd:r}),i}startInteractiveSession(t,n,r,i,s){bt.mark("startInteractiveSession"),this._idle?.ping(),this.emit("session:start",{user:n,sessionId:r,remoteAddress:i}),fi(this.properties,t,n,this.hostname,r,i,s,this),this.refreshProcSessions()}refreshProcFs(){Ut(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}mount(t,n,r={}){this.vfs.mount(t,n,r)}unmount(t){this.vfs.unmount(t)}getMounts(){return this.vfs.getMounts()}refreshProcSessions(){Ut(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}syncPasswd(){bn(this.vfs,this.users)}getVfs(){return this?.vfs??null}getUsers(){return this?.users??null}getHostname(){return this?.hostname}writeFileAsUser(t,n,r){bt.mark("writeFileAsUser"),this.users.assertWriteWithinQuota(t,n,r),this.vfs.writeFile(n,r)}enableIdleManagement(t){this._idle||(this._idle=new yt(this.vfs,t),this._idle.on("freeze",()=>this.emit("shell:freeze")),this._idle.on("thaw",()=>this.emit("shell:thaw")),this._idle.start())}async disableIdleManagement(){this._idle&&(await this._idle.stop(),this._idle=null)}get idleState(){return this._idle?.state??"active"}get idleMs(){return this._idle?.idleMs??0}pingIdle(){this._idle?.ping()}};var Cn=!!y.env.DEV_MODE,Ny=Cn?console.log.bind(console):()=>{},Ay=Cn?console.warn.bind(console):()=>{},_y=Cn?console.error.bind(console):()=>{};var Oy=fe("SftpMimic");var Wy=fe("SshMimic"),ua=!!y.env.DEV_MODE,Ky=ua?console.log.bind(console):()=>{};var tt="my-vm",gi=500,je=document.getElementById("output"),ze=document.getElementById("cmd");ze.focus();var yi={30:"#000",31:"#c00",32:"#0c0",33:"#cc0",34:"#00c",35:"#c0c",36:"#0cc",37:"#ccc",90:"#555",91:"#f55",92:"#5f5",93:"#ff5",94:"#55f",95:"#f5f",96:"#5ff",97:"#fff"},Si={40:"#000",41:"#c00",42:"#0c0",43:"#cc0",44:"#00c",45:"#c0c",46:"#0cc",47:"#ccc"};function bi(e){let t="",n=!1,r="",i="";for(let s of e.split(/(\x1b\[[0-9;]*m)/)){let o=s.match(/^\x1b\[([0-9;]*)m$/);if(o)for(let a of o[1].split(";").map(Number))a===0?(n=!1,r="",i=""):a===1?n=!0:yi[a]?r=yi[a]:Si[a]&&(i=Si[a]);else if(s){let a=[r?`color:${r}`:"",i?`background:${i}`:"",n?"font-weight:bold":""].filter(Boolean).join(";"),l=s.replace(/&/g,"&amp;").replace(/</g,"&lt;");t+=a?`<span style="${a}">${l}</span>`:l}}return t}function et(e){je.innerHTML+=bi(e),je.scrollTop=je.scrollHeight}function xi(){let e=Te===Y(be)?"~":Te.split("/").at(-1)||"/";je.innerHTML+=bi(jt(be,tt,e)),je.scrollTop=je.scrollHeight}await globalThis.__fsReady__;var Wt=new Be(tt,void 0,{mode:"fs",snapshotPath:"/vfs-data",flushIntervalMs:1e4}),ce=Wt.vfs;await ce.restoreMirror();var da=!ce.exists("/bin");da&&(await Wt.ensureInitialized(),ce.exists("/root")||ce.mkdir("/root",448),ce.writeFile("/root/README.txt",`Welcome to ${tt}
`),await ce.flushMirror());window.addEventListener("beforeunload",()=>{ce.flushMirror()});var be="root",Te=Y(be),He=Ve(be,tt);He.vars.PWD=Te;function ma(e){e.switchUser?(be=e.switchUser,Te=e.nextCwd??Y(be),He.vars.USER=be,He.vars.LOGNAME=be,He.vars.HOME=Y(be),He.vars.PWD=Te):e.nextCwd&&(Te=e.nextCwd,He.vars.PWD=Te)}function Pn(){return`${Y(be)}/.bash_history`}function pa(){try{return ce.exists(Pn())?ce.readFile(Pn()).split(`
`).map(e=>e.trim()).filter(e=>e.length>0):[]}catch{return[]}}function fa(){ce.writeFile(Pn(),ge.length>0?`${ge.join(`
`)}
`:"")}var ge=pa(),Fe=-1;function ha(){try{return ce.exists("/root/.lastlog")?JSON.parse(ce.readFile("/root/.lastlog")):null}catch{return null}}function ga(){ce.writeFile("/root/.lastlog",JSON.stringify({at:new Date().toISOString(),from:"browser"}))}var ya=Ht(tt,Wt.properties,ha());et(ya);ga();await ce.flushMirror();xi();ze.addEventListener("keydown",async e=>{if(e.key==="ArrowUp"){e.preventDefault(),Fe<ge.length-1&&(Fe++,ze.value=ge[ge.length-1-Fe]);return}if(e.key==="ArrowDown"){e.preventDefault(),Fe>0?(Fe--,ze.value=ge[ge.length-1-Fe]):(Fe=-1,ze.value="");return}if(e.key!=="Enter")return;let t=ze.value.trim();ze.value="",Fe=-1,et(`${t}
`),t&&(ge.push(t),ge.length>gi&&(ge=ge.slice(ge.length-gi)),fa());try{let n=await Q(t,be,tt,"shell",Te,Wt,void 0,He);if(n.clearScreen&&(je.innerHTML=""),n.stdout&&et(n.stdout),n.stderr&&et(n.stderr),ma(n),await ce.flushMirror(),n.closeSession){et(`
Session closed.
