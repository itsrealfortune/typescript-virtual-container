...
| **Web shell** | `builds/fortune-nyx-v1.7.3-web.min.js` (ESM) | Embedded terminals, interactive tutorials, browser demos — run `startxfce4` for a full XFCE desktop |
| **Standalone CLI** | `builds/fortune-nyx-v1.7.3-directbash-k6.1.0.mjs` (single file) | Local shell, one-liner demos, no install required |
<!-- /BUILD:mode-table -->
...
```bash
curl -s https://raw.githubusercontent.com/itsrealfortune/typescript-virtual-container/refs/heads/main/builds/fortune-nyx-v1.7.3-directbash-k6.1.0.mjs -o fortune-nyx-v1.7.3-directbash-k6.1.0.mjs && node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs
```

#### SSH server with built-in SFTP subsystem (scp / sftp on port 2222)
```bash
curl -s https://raw.githubusercontent.com/itsrealfortune/typescript-virtual-container/refs/heads/main/builds/fortune-nyx-v1.7.3-ssh.cjs -o fortune-nyx-v1.7.3-ssh.cjs && node fortune-nyx-v1.7.3-ssh.cjs
```

#### Custom SSH port
```bash
node fortune-nyx-v1.7.3-ssh.cjs --ssh-port=2022
```

#### SSH disabled (handler only, no server started)
```bash
node fortune-nyx-v1.7.3-ssh.cjs --no-ssh
```
<!-- /BUILD:curl-start -->
...
**`fortune-nyx-v1.7.3-directbash-k6.1.0.mjs` options:**

```bash
node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs                          # boot as root
node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs --user alice             # boot as alice (prompts for password if set)
node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs --user=alice             # same, inline form
node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs --hostname=my-box        # custom hostname
node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs --snapshot=/data/.vfs    # custom VFS snapshot path
node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs --help                   # show all options
node fortune-nyx-v1.7.3-directbash-k6.1.0.mjs --version                # print version
```
<!-- /BUILD:selfStandalone-options -->
...
|--------|--------|-------------|----------|
| `builds/fortune-nyx-v1.7.3-web.min.js` | ESM | `createWebShell()` | Embedded terminals, modern bundlers |
<!-- /BUILD:web-table -->
...
<!-- BUILD:web-options -->
**`fortune-nyx-v1.7.3-web.min.js`** — lightweight shell with IndexedDB VFS:

```html
<script type="module">
  import { createWebShell } from "./builds/fortune-nyx-v1.7.3-web.min.js";
...
**`fortune-nyx-v1.7.3-web.min.js`** — mirrors the `VirtualShell` programmatic API:

```html
<script type="module">
  import { createVirtualShellShim } from "./builds/fortune-nyx-v1.7.3-web.min.js";
