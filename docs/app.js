...
`),exitCode:0}}};var Dt="9.2.0",ho="18.19.0",Gr={name:"npm",description:"Node.js package manager (virtual)",category:"system",params:["<command> [args]"],run:({args:e,shell:t})=>{if(!t.packageManager.isInstalled("npm"))return{stderr:`bash: npm: command not found
Hint: install it with: apt install npm
`,exitCode:127};if(C(e,["--version","-v"]))return{stdout:`${Dt}
`,exitCode:0};let n=e[0]?.toLowerCase();switch(n){case"version":case"-version":return{stdout:`{ npm: '${Dt}', node: '${ho}', v8: '10.2.154.26' }
...
`,exitCode:1}}}},Yr={name:"npx",description:"Node.js package runner (virtual)",category:"system",params:["<package> [args]"],run:({args:e,shell:t})=>t.packageManager.isInstalled("npm")?C(e,["--version"])?{stdout:`${Dt}
...
`)||void 0,exitCode:0}):{stdout:S,exitCode:0}}};var Fs={name:"which",description:"Locate a command in PATH",category:"shell",params:["<command...>"],run:({args:e,shell:t,env:n})=>{if(e.length===0)return{stderr:"which: missing argument",exitCode:1};let r=(n?.vars?.PATH??"/usr/local/bin:/usr/bin:/bin").split(":"),i=[],s=!1;for(let o of e){let a=!1;for(let l of r){let c=`${l}/${o}`;if(t.vfs.exists(c)&&t.vfs.stat(c).type==="file"){i.push(c),a=!0;break}}a||(s=!0)}return i.length===0?{exitCode:1}:{stdout:i.join(`
...
CONFIG_VIRTIO_NET=y
CONFIG_KVM_GUEST=y
`,420),e.exists("/vmlinuz")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz"),e.exists("/vmlinuz.old")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz.old"),e.exists("/initrd.img")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img"),e.exists("/initrd.img.old")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img.old"),h(e,"/lost+found",448),h(e,"/home")}var Xs=new Map;function ta(e,t){return`${e}|${t.kernel}|${t.os}|${t.arch}`}function na(e,t){let n=ta(e,t),r=Xs.get(n);if(r)return r;let i=new pt({mode:"memory"});jo(i,e,t),qo(i,e,t),Ko(i),Go(i),Jo(i),Zo(i),Qo(i),ea(i,t),Wo(i,t);let s=i.encodeBinary();return Xs.set(n,s),s}function ei(e,t,n,r,i,s=[]){let o=na(n,r);e.getMode()==="fs"&&e.exists("/home")?e.mergeRootTree(Fe(o)):e.importRootTree(Fe(o)),Xo(e),Bt(e,r,n,i,s),xn(e,t)}var wn=[{name:"vim",version:"2:9.0.1378-2",section:"editors",description:"Vi IMproved - enhanced vi editor",shortDesc:"Vi IMproved",installedSizeKb:3812,files:[{path:"/usr/bin/vim",content:`#!/bin/sh
...
`),L.openEditor){await A(L.openEditor.targetPath,L.openEditor.initialContent,L.openEditor.tempPath);return}if(L.openHtop){await F();return}L.clearScreen&&t.write("\x1B[2J\x1B[H"),L.stdout&&t.write(`${yt(L.stdout)}\r
...
`),x()}),L.on("close",()=>{x()}),S={kind:"nano",targetPath:O,tempPath:H,process:L}}async function F(){let O=await mi();if(!O){t.write(`htop: no child_process processes to display\r
`);return}let U=hi(O,o,t);U.on("error",H=>{t.write(`htop: ${H.message}\r
`),x()}),U.on("close",()=>{x()}),S={kind:"htop",targetPath:"",tempPath:"",process:U}}function k(O){l=O,c=l.length,_()}function w(O){l=`${l.slice(0,c)}${O}${l.slice(c)}`,c+=O.length,_()}function D(O,U){let H=U;for(;H>0&&!/\s/.test(O[H-1]);)H-=1;let L=U;for(;L<O.length&&!/\s/.test(O[L]);)L+=1;return{start:H,end:L}}function K(O){let U=O.lastIndexOf("/"),H=U>=0?O.slice(0,U+1):"",L=U>=0?O.slice(U+1):O,se=ui(p,H||".");try{return a.vfs.list(se).filter(B=>!B.startsWith(".")).filter(B=>B.startsWith(L)).map(B=>{let ve=z.join(se,B),Le=a.vfs.stat(ve).type==="directory"?"/":"";return`${H}${B}${Le}`}).sort()}catch{return[]}}function q(){let{start:O,end:U}=D(l,c),H=l.slice(O,c);if(H.length===0)return;let se=l.slice(0,O).trim().length===0?T.filter(pe=>pe.startsWith(H)):[],B=K(H),ve=Array.from(new Set([...se,...B])).sort();if(ve.length!==0){if(ve.length===1){let pe=ve[0],Le=pe.endsWith("/")?"":" ";l=`${l.slice(0,O)}${pe}${Le}${l.slice(U)}`,c=O+pe.length+Le.length,_();return}t.write(`\r
`),t.write(`${ve.join("  ")}\r
...
`),se.length>0){let B=await Promise.resolve(Q(se,n,r,"shell",p,a,void 0,g));if(X(se),B.openEditor){await A(B.openEditor.targetPath,B.openEditor.initialContent,B.openEditor.tempPath);return}if(B.openHtop){await F();return}if(B.sudoChallenge){N(B.sudoChallenge);return}if(B.clearScreen&&t.write("\x1B[2J\x1B[H"),B.stdout&&t.write(`${yt(B.stdout)}\r
