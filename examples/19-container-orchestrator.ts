/**
 * Example 19: Container orchestration simulator
 *
...
async function runOrchestrator() {
	console.log("☸️  Container Orchestration Simulator\n");

	// Create cluster network
	const cluster = new Baie("k8s-cluster", "172.16.0.0/16");

	// ── Deploy pods ───────────────────────────────────────────────
	console.log("📦 Deploying pods...\n");
	...
	for (const spec of podSpecs) {
		...
		vm.vfs.setRamCap(256 * 1024 * 1024); // 256 MB per pod
	...
	// Network stats (aggregate per-MAC byte counters)
	let totalSent = 0;
	let totalReceived = 0;
	...
	for (const pod of pods) {
		...
		_totalRam += procs.length * 10; // rough estimate
	}
