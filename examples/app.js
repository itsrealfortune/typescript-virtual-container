...
`)||void 0,exitCode:0}}},kn={name:"unalias",description:"Remove alias definitions",category:"shell",params:["<name...> | -a"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};if(C(e,["-a"])){for(let n of Object.keys(t.vars))n.startsWith("__alias_")&&delete t.vars[n];return{exitCode:0}}for(let n of e)delete t.vars[`__alias_${n}`];return{exitCode:0}}};function Wt(){throw new Error("child_process.spawn not supported in browser")}var z={basename(e){let t=e.split("/").filter(Boolean);return t.length?t[t.length-1]:""},dirname(e){if(!e)return".";let t=e.split("/").filter(Boolean);return t.pop(),t.length?"/"+t.join("/"):"/"},join(...e){return e.join("/").replace(/\/+/g,"/")},resolve(...e){let t=e.join("/");return t.startsWith("/")?t:"/"+t},normalize(e){let t=e.split("/"),n=[];for(let r of t)r===".."?n.pop():r&&r!=="."&&n.push(r);return(e.startsWith("/")?"/":"")+n.join("/")||"."}};function wt(e){return z.dirname(e)}function et(...e){return z.resolve(...e)}function Mn(...e){return e.join("/").replace(/\/+/g,"/")}var vi=["/.virtual-env-js/.auth","/etc/htpasswd"];function M(e,t,n){if(!t||t.trim()==="")return e;if(t.startsWith("~")){let r=n??"/root";return z.normalize(`${r}${t.slice(1)}`)}return t.startsWith("/")?z.normalize(t):z.normalize(z.join(e,t))}function Ci(e){let t=e.startsWith("/")?z.normalize(e):z.normalize(`/${e}`);return vi.some(n=>t===n||t.startsWith(`${n}/`))}function L(e,t,n){if(e!=="root"&&Ci(t))throw new Error(`${n}: permission denied: ${t}`)}function In(e){let n=(e.split("?")[0]?.split("#")[0]??e).split("/").filter(Boolean).pop();return n&&n.length>0?n:"index.html"}function Pi(e,t){let n=Array.from({length:e.length+1},()=>Array(t.length+1).fill(0));for(let r=0;r<=e.length;r+=1)n[r][0]=r;for(let r=0;r<=t.length;r+=1)n[0][r]=r;for(let r=1;r<=e.length;r+=1)for(let i=1;i<=t.length;i+=1){let s=e[r-1]===t[i-1]?0:1;n[r][i]=Math.min(n[r-1][i]+1,n[r][i-1]+1,n[r-1][i-1]+s)}return n[e.length][t.length]}function Nn(e,t,n){let r=M(t,n);if(e.exists(r))return r;let i=z.dirname(r),s=z.basename(r),o=e.list(i),a=o.filter(c=>c.toLowerCase()===s.toLowerCase());if(a.length===1)return z.join(i,a[0]);let l=o.filter(c=>Pi(c.toLowerCase(),s.toLowerCase())<=1);return l.length===1?z.join(i,l[0]):r}function We(e){return e.packageManager}var An={name:"apt",aliases:["apt-get"],description:"Package manager",category:"package",params:["<install|remove|update|upgrade|search|show|list> [pkg...]"],run:({args:e,shell:t,authUser:n})=>{let r=We(t);if(!r)return{stderr:"apt: package manager not initialised",exitCode:1};let i=e[0]?.toLowerCase(),s=e.slice(1),o=C(s,["-q","--quiet","-qq"]),a=C(s,["--purge"]),l=s.filter(u=>!u.startsWith("-"));if(["install","remove","purge","upgrade","update"].includes(i??"")&&n!=="root")return{stderr:`E: Could not open lock file /var/lib/dpkg/lock-frontend - open (13: Permission denied)
...
`),exitCode:0}}default:return{stderr:`apt-cache: unknown command '${r??""}'`,exitCode:1}}}};var On={name:"awk",description:"Pattern scanning and processing language",category:"text",params:["[-F <sep>] '<program>' [file]"],run:({authUser:e,args:t,stdin:n,cwd:r,shell:i})=>{let s=ae(t,["-F"])??" ",o=t.filter(P=>!P.startsWith("-")&&P!==s),a=o[0],l=o[1];if(!a)return{stderr:"awk: no program",exitCode:1};let c=n??"";if(l){let P=M(r,l);try{L(e,P,"awk"),c=i.vfs.readFile(P)}catch{return{stderr:`awk: ${l}: No such file or directory`,exitCode:1}}}let u=c.split(`
...
`)+(p.length>0?`
`:""),exitCode:0}}};var Tn={name:"base64",description:"Encode/decode base64",category:"text",params:["[-d] [file]"],run:({args:e,stdin:t})=>{let n=C(e,["-d","--decode"]),r=t??"";if(n)try{return{stdout:Buffer.from(r.trim(),"base64").toString("utf8"),exitCode:0}}catch{return{stderr:"base64: invalid input",exitCode:1}}return{stdout:Buffer.from(r).toString("base64"),exitCode:0}}};var Dn={name:"cat",description:"Concatenate and print files",category:"files",params:["[-n] [-b] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-n","--number"]),o=C(r,["-b","--number-nonblank"]),a=r.filter(m=>!m.startsWith("-"));if(a.length===0&&i!==void 0)return{stdout:i,exitCode:0};if(a.length===0)return{stderr:"cat: missing file operand",exitCode:1};let l=[];for(let m of a){let p=Nn(t.vfs,n,m);L(e,p,"cat"),l.push(t.vfs.readFile(p))}let c=l.join("");if(!s&&!o)return{stdout:c,exitCode:0};let u=1;return{stdout:c.split(`
`).map(m=>o&&m.trim()===""?m:`${String(u++).padStart(6)}	${m}`).join(`
`),exitCode:0}}};var Fn={name:"cd",description:"Change directory",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=M(n,r[0]??"~",Y(e));return L(e,i,"cd"),t.vfs.stat(i).type!=="directory"?{stderr:`cd: not a directory: ${i}`,exitCode:1}:{nextCwd:i,exitCode:0}}};function Ei(e,t){let n=/^([ugoa]*)([+\-=])([rwx]*)$/,r=t.split(","),i=e;for(let s of r){let o=s.trim().match(n);if(!o)return null;let[,a="a",l,c=""]=o,u=a===""||a==="a"?["u","g","o"]:a.split(""),d={u:{r:256,w:128,x:64},g:{r:32,w:16,x:8},o:{r:4,w:2,x:1}};for(let m of u)for(let p of c.split("")){let g=d[m]?.[p];if(g!==void 0){if(l==="+")i|=g;else if(l==="-")i&=~g;else if(l==="="){let S=Object.values(d[m]??{}).reduce((v,E)=>v|E,0);i=i&~S|g}}}}return i}var Rn={name:"chmod",description:"Change file permissions",category:"files",params:["<mode> <file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let[i,s]=r;if(!i||!s)return{stderr:"chmod: missing operand",exitCode:1};let o=M(n,s);try{if(L(e,o,"chmod"),!t.vfs.exists(o))return{stderr:`chmod: ${s}: No such file or directory`,exitCode:1};let a,l=parseInt(i,8);if(!Number.isNaN(l)&&/^[0-7]+$/.test(i))a=l;else{let c=t.vfs.stat(o).mode,u=Ei(c,i);if(u===null)return{stderr:`chmod: invalid mode: ${i}`,exitCode:1};a=u}return t.vfs.chmod(o,a),{exitCode:0}}catch(a){return{stderr:`chmod: ${a instanceof Error?a.message:String(a)}`,exitCode:1}}}};var Vn={name:"clear",description:"Clear the terminal screen",category:"shell",params:[],run:()=>({clearScreen:!0,stdout:"",exitCode:0})};var Ln={name:"cp",description:"Copy files or directories",category:"files",params:["[-r] <source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-r","-R","--recursive"]),s=r.filter(u=>!u.startsWith("-")),[o,a]=s;if(!o||!a)return{stderr:"cp: missing operand",exitCode:1};let l=M(n,o),c=M(n,a);try{if(L(e,l,"cp"),L(e,c,"cp"),!t.vfs.exists(l))return{stderr:`cp: ${o}: No such file or directory`,exitCode:1};if(t.vfs.stat(l).type==="directory"){if(!i)return{stderr:`cp: ${o}: is a directory (use -r)`,exitCode:1};let d=(p,g)=>{t.vfs.mkdir(g,493);for(let S of t.vfs.list(p)){let v=`${p}/${S}`,E=`${g}/${S}`;if(t.vfs.stat(v).type==="directory")d(v,E);else{let _=t.vfs.readFileRaw(v);t.writeFileAsUser(e,E,_)}}},m=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c;d(l,m)}else{let d=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c,m=t.vfs.readFileRaw(l);t.writeFileAsUser(e,d,m)}return{exitCode:0}}catch(u){return{stderr:`cp: ${u instanceof Error?u.message:String(u)}`,exitCode:1}}}};var Un={name:"curl",description:"Transfer data from or to a server (pure fetch)",category:"network",params:["[options] <url>"],run:async({authUser:e,cwd:t,args:n,shell:r})=>{let{flagsWithValues:i,positionals:s}=le(n,{flagsWithValue:["-o","--output","-X","--request","-d","--data","-H","--header","-u","--user"]});if(C(n,["--help","-h"]))return{stdout:["Usage: curl [options] <url>","  -o, --output <file>    Write to file","  -X, --request <method> HTTP method","  -d, --data <data>      POST data","  -H, --header <hdr>     Extra header","  -s, --silent           Silent mode","  -I, --head             Fetch headers only","  -L, --location         Follow redirects","  -v, --verbose          Verbose"].join(`
...
Type the username to confirm: `,mode:"confirm",onPassword:s},exitCode:0}}};var Wn={name:"df",description:"Report filesystem disk space usage",category:"system",params:["[-h]"],run:({shell:e})=>{let n=(e.vfs.getUsageBytes()/1024).toFixed(0),r="1048576",i=String(Number(r)-Number(n)),s=Math.round(Number(n)/Number(r)*100),o="Filesystem     1K-blocks    Used Available Use% Mounted on",a=`virtual-fs     ${r.padStart(9)} ${n.padStart(7)} ${i.padStart(9)} ${s}% /`;return{stdout:`${o}
${a}`,exitCode:0}}};var Kn={name:"diff",description:"Compare files line by line",category:"text",params:["<file1> <file2>"],run:({shell:e,cwd:t,args:n})=>{let[r,i]=n;if(!r||!i)return{stderr:"diff: missing operand",exitCode:1};let s=M(t,r),o=M(t,i),a,l;try{a=e.vfs.readFile(s).split(`
...
`,exitCode:P.length>0?0:1};if(p)return{exitCode:P.length>0?0:1};_.push(...P)}else{let P=S.flatMap(N=>{let I=M(n,N);return T(I).map(x=>({file:N,path:x}))});for(let{file:N,path:I}of P)try{L(e,I,"grep");let x=t.vfs.readFile(I),A=P.length>1?`${N}:`:"",D=E(x,A);d?_.push(P.length>1?`${N}:${D.length}`:String(D.length)):m?D.length>0&&_.push(N):_.push(...D)}catch{return{stderr:`grep: ${N}: No such file or directory`,exitCode:1}}}return{stdout:_.length>0?`${_.join(`
`)}
`:"",exitCode:_.length>0?0:1}}};var sr={name:"groups",description:"Print group memberships",category:"system",params:["[user]"],run:({authUser:e,shell:t,args:n})=>{let r=n[0]??e;return{stdout:t.users.isSudoer(r)?`${r} sudo root`:r,exitCode:0}}};var ir={name:"gzip",description:"Compress files",category:"archive",params:["[-k] [-d] <file>"],run:({shell:e,cwd:t,args:n})=>{if(!e.packageManager.isInstalled("gzip"))return{stderr:`bash: gzip: command not found
...
`,exitCode:1};let o=e.vfs.readFile(s),a=s.slice(0,-3);return e.vfs.writeFile(a,o),r||e.vfs.remove(s),{exitCode:0}}};var ar={name:"head",description:"Output first lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=ae(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
`),p=m.slice(0,a);return p.join(`
`)+(d.endsWith(`
`)&&p.length===m.slice(0,a).length?`
`:"")};if(l.length===0)return{stdout:c(i??""),exitCode:0};let u=[];for(let d of l){let m=M(n,d);try{L(e,m,"head"),u.push(c(t.vfs.readFile(m)))}catch{return{stderr:`head: ${d}: No such file or directory`,exitCode:1}}}return{stdout:u.join(`
`),exitCode:0}}};var lr=["navigation","files","text","archive","system","package","network","shell","users","misc"],ur={navigation:"Navigation",files:"Files & Filesystem",text:"Text Processing",archive:"Archive & Compression",system:"System",package:"Package Management",network:"Network",shell:"Shell & Scripting",users:"Users & Permissions",misc:"Miscellaneous"},dr="\x1B[1m",ve="\x1B[0m",Ni="\x1B[36m",Ai="\x1B[33m",tt="\x1B[2m",_i="\x1B[32m";function cr(e,t){return e.length>=t?e:e+" ".repeat(t-e.length)}function Oi(e){let t=e.aliases?.length?` ${tt}(${e.aliases.join(", ")})${ve}`:"";return`  ${Ni}${cr(e.name,16)}${ve}${t}${cr("",(e.aliases?.length,0))} ${e.description??""}`}function Ti(e){let t={};for(let s of e){let o=s.category??"misc";t[o]||(t[o]=[]),t[o].push(s)}let n=[`${dr}Available commands${ve}`,`${tt}Type 'help <command>' for detailed usage.${ve}`,""],r=[...lr.filter(s=>t[s]),...Object.keys(t).filter(s=>!lr.includes(s)).sort()];for(let s of r){let o=t[s];if(!o?.length)continue;n.push(`${Ai}${ur[s]??s}${ve}`);let a=[...o].sort((l,c)=>l.name.localeCompare(c.name));for(let l of a)n.push(Oi(l));n.push("")}let i=e.length;return n.push(`${tt}${i} commands available.${ve}`),n.join(`
`)}function Di(e){let t=[];if(t.push(`${dr}${e.name}${ve} \u2014 ${e.description??"no description"}`),e.aliases?.length&&t.push(`${tt}Aliases: ${e.aliases.join(", ")}${ve}`),t.push(""),t.push(`${_i}Usage:${ve}`),e.params.length)for(let r of e.params)t.push(`  ${e.name} ${r}`);else t.push(`  ${e.name}`);let n=ur[e.category??"misc"]??e.category??"misc";return t.push(""),t.push(`${tt}Category: ${n}${ve}`),t.join(`
`)}function mr(e){return{name:"help",description:"List all commands, or show usage for a specific command",category:"shell",params:["[command]"],run:({args:t})=>{let n=Jt();if(t[0]){let r=t[0].toLowerCase(),i=n.find(s=>s.name===r||s.aliases?.includes(r));return i?{stdout:Di(i),exitCode:0}:{stderr:`help: no help entry for '${t[0]}'`,exitCode:1}}return{stdout:Ti(n),exitCode:0}}}}var pr={name:"history",description:"Display command history",category:"shell",params:["[n]"],run:({args:e,shell:t,authUser:n})=>{let r=`/home/${n}/.bash_history`;if(!t.vfs.exists(r))return{stdout:"",exitCode:0};let s=t.vfs.readFile(r).split(`
...
`),exitCode:0}}};var fr={name:"hostname",description:"Print hostname",category:"system",params:[],run:({hostname:e})=>({stdout:e,exitCode:0})};var hr={name:"htop",description:"System monitor",category:"system",params:[],run:({mode:e})=>e==="exec"?{stderr:"htop: interactive terminal required",exitCode:1}:{openHtop:!0,exitCode:0}};var gr={name:"id",description:"Print user identity",category:"system",params:["[user]"],run:({authUser:e,shell:t,args:n})=>{let r=n[0]??e,i=r==="root"?0:1e3,s=i,a=t.users.isSudoer(r)?`${s}(${r}),0(root)`:`${s}(${r})`;return{stdout:`uid=${i}(${r}) gid=${s}(${r}) groups=${a}`,exitCode:0}}};var yr={name:"kill",description:"Send signal to process",category:"system",params:["[-9] <pid>"],run:({args:e})=>e.find(n=>!n.startsWith("-"))?{stdout:"",exitCode:0}:{stderr:"kill: no pid specified",exitCode:1}};var Sr={name:"ln",description:"Create links",category:"files",params:["[-s] <target> <link_name>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-s","--symbolic"]),s=r.filter(u=>!u.startsWith("-")),[o,a]=s;if(!o||!a)return{stderr:"ln: missing operand",exitCode:1};let l=M(n,a),c=i?o:M(n,o);try{if(L(e,l,"ln"),i)t.vfs.symlink(c,l);else{let u=M(n,o);if(L(e,u,"ln"),!t.vfs.exists(u))return{stderr:`ln: ${o}: No such file or directory`,exitCode:1};let d=t.vfs.readFile(u);t.writeFileAsUser(e,l,d)}return{exitCode:0}}catch(u){return{stderr:`ln: ${u instanceof Error?u.message:String(u)}`,exitCode:1}}}},br={name:"readlink",description:"Print resolved path of symbolic link",category:"files",params:["[-f] <path>"],run:({shell:e,cwd:t,args:n})=>{let r=n.includes("-f")||n.includes("-e"),i=n.find(a=>!a.startsWith("-"));if(!i)return{stderr:`readlink: missing operand
...
`,exitCode:1}}};var xr={name:"seq",description:"Print a sequence of numbers",category:"text",params:["[FIRST [INCREMENT]] LAST"],run:({args:e})=>{let t=e.filter(d=>!d.startsWith("-")||/^-[\d.]/.test(d)).map(Number),n=(()=>{let d=e.indexOf("-s");return d!==-1?e[d+1]??`
`:`
...
`,exitCode:0}}};var wr={name:"stat",description:"Display file status",category:"files",params:["[-c <format>] <file>"],run:({shell:e,cwd:t,args:n})=>{let r=n.findIndex(E=>E==="-c"||E==="--format"),i=r!==-1?n[r+1]:void 0,s=n.find(E=>!E.startsWith("-")&&E!==i);if(!s)return{stderr:`stat: missing operand
...
`)}
`,exitCode:0}}};var Fi="\x1B[0m",Ri="\x1B[1;34m",Vi="\x1B[1;36m",Li="\x1B[1;32m",Ui="",Bi="\x1B[30;42m",zi="\x1B[37;44m",Hi="\x1B[34;42m";function Ke(e,t){return t?`${t}${e}${Fi}`:e}function Qt(e,t,n){if(n)return Vi;if(t==="directory"){let r=!!(e&512),i=!!(e&2);return r&&i?Bi:r?zi:i?Hi:Ri}return e&73?Li:Ui}function vr(e,t,n){let r;n?r="l":t==="directory"?r="d":r="-";let i=c=>e&c?"r":"-",s=c=>e&c?"w":"-",o=(()=>{let c=!!(e&64);return e&2048?c?"s":"S":c?"x":"-"})(),a=(()=>{let c=!!(e&8);return e&1024?c?"s":"S":c?"x":"-"})(),l=(()=>{let c=!!(e&1);return t==="directory"&&e&512?c?"t":"T":c?"x":"-"})();return`${r}${i(256)}${s(128)}${o}${i(32)}${s(16)}${a}${i(4)}${s(2)}${l}`}var ji=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function Zt(e){let t=new Date,n=4320*3600*1e3,r=Math.abs(t.getTime()-e.getTime())<n,i=String(e.getDate()).padStart(2," "),s=ji[e.getMonth()]??"";if(r){let o=String(e.getHours()).padStart(2,"0"),a=String(e.getMinutes()).padStart(2,"0");return`${i} ${s.padEnd(3)} ${o}:${a}`}return`${i} ${s.padEnd(3)} ${e.getFullYear()}`}function kt(e,t){try{return e.readFile(t)}catch{return"?"}}function Wi(e,t,n){let r=t==="/"?"":t;return n.map(i=>{let s=`${r}/${i}`,o=e.isSymlink(s),a;try{a=e.stat(s)}catch{return i}let l=Qt(a.mode,a.type,o);return Ke(i,l)}).join("  ")}function Ki(e,t,n){let r=t==="/"?"":t,i=n.map(d=>{let m=`${r}/${d}`,p=e.isSymlink(m),g;try{g=e.stat(m)}catch{return{perms:"----------",nlink:"1",size:"0",date:Zt(new Date),label:d}}let S=p?41471:g.mode,v=vr(S,g.type,p),E=g.type==="directory"?String((g.childrenCount??0)+2):"1",T=p?kt(e,m).length:g.type==="file"?g.size??0:(g.childrenCount??0)*4096,_=String(T),P=Zt(g.updatedAt),N=Qt(S,g.type,p),I=p?`${Ke(d,N)} -> ${kt(e,m)}`:Ke(d,N);return{perms:v,nlink:E,size:_,date:P,label:I}}),s=Math.max(...i.map(d=>d.nlink.length)),o=Math.max(...i.map(d=>d.size.length)),a="root",l="root",c=n.length*8,u=i.map(d=>`${d.perms} ${d.nlink.padStart(s)} ${a} ${l} ${d.size.padStart(o)} ${d.date} ${d.label}`);return`total ${c}
${u.join(`
`)}`}var Cr={name:"ls",description:"List directory contents",category:"navigation",params:["[-la] [path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-l","--long","-la","-al"]),s=C(r,["-a","--all","-la","-al"]),o=$e(r,0,{flags:["-l","--long","-a","--all","-la","-al"]}),a=M(n,o??n);if(L(e,a,"ls"),t.vfs.exists(a)){let u=t.vfs.stat(a),d=t.vfs.isSymlink(a);if(u.type==="file"||d){let m=a.split("/").pop()??a,p=Qt(d?41471:u.mode,u.type,d);if(i){let g=d?41471:u.mode,S=d?kt(t.vfs,a).length:u.size??0,v=vr(g,u.type,d),E=d?`${Ke(m,p)} -> ${kt(t.vfs,a)}`:Ke(m,p);return{stdout:`${v} 1 root root ${S} ${Zt(u.updatedAt)} ${E}
...
`,exitCode:0}}};var Pr={name:"lsb_release",description:"Print distribution-specific information",category:"system",params:["[-a] [-i] [-d] [-r] [-c]"],run:({args:e,shell:t})=>{let n=t.properties?.os??"Fortune GNU/Linux x64",r="nyx",i="1.0";try{let d=t.vfs.readFile("/etc/os-release");for(let m of d.split(`
...
DESCRIPTION
       Read items from stdin and execute COMMAND with those items as arguments.`};var qi={gunzip:"gzip"},$r={name:"man",description:"Interface to the system reference manuals",category:"shell",params:["<command>"],run:async({args:e,shell:t})=>{let n=e[0];if(!n)return{stderr:"What manual page do you want?",exitCode:1};let r=`/usr/share/man/man1/${n}.1`;if(t.vfs.exists(r))return{stdout:t.vfs.readFile(r),exitCode:0};let i=n.toLowerCase(),s=qi[i]??i,o=Er[s]??null;return o?{stdout:o,exitCode:0}:{stderr:`No manual entry for ${n}`,exitCode:16}}};var kr={name:"mkdir",description:"Make directories",category:"files",params:["<dir>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"mkdir: missing operand",exitCode:1};for(let i=0;i<r.length;i++){let s=$e(r,i);if(!s)return{stderr:"mkdir: missing operand",exitCode:1};let o=M(n,s);L(e,o,"mkdir"),t.vfs.mkdir(o)}return{exitCode:0}}};var Mr={name:"mv",description:"Move or rename files",category:"files",params:["<source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r.filter(c=>!c.startsWith("-")),[s,o]=i;if(!s||!o)return{stderr:"mv: missing operand",exitCode:1};let a=M(n,s),l=M(n,o);try{if(L(e,a,"mv"),L(e,l,"mv"),!t.vfs.exists(a))return{stderr:`mv: ${s}: No such file or directory`,exitCode:1};let c=t.vfs.exists(l)&&t.vfs.stat(l).type==="directory"?`${l}/${s.split("/").pop()}`:l;return t.vfs.move(a,c),{exitCode:0}}catch(c){return{stderr:`mv: ${c instanceof Error?c.message:String(c)}`,exitCode:1}}}};var Ir={name:"nano",description:"Text editor",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r[0];if(!i)return{stderr:"nano: missing file operand",exitCode:1};let s=M(n,i);L(e,s,"nano");let o=t.vfs.exists(s)?t.vfs.readFile(s):"",a=z.basename(s)||"buffer",l=`/tmp/sshmimic-nano-${Date.now()}-${a}.tmp`;return{openEditor:{targetPath:s,tempPath:l,initialContent:o},exitCode:0}}};var Gi="vfs-fs-shim",Ie="files",Mt=null;function Xt(){return Mt?Promise.resolve(Mt):new Promise((e,t)=>{let n=indexedDB.open(Gi,1);n.onupgradeneeded=r=>r.target.result.createObjectStore(Ie),n.onsuccess=r=>{Mt=r.target.result,e(Mt)},n.onerror=r=>t(r.target.error)})}var ie=new Map;Xt().then(e=>{let n=e.transaction(Ie,"readonly").objectStore(Ie).openCursor();n.onsuccess=r=>{let i=r.target.result;i&&(ie.set(i.key,i.value),i.continue())}});function qe(e,t){Xt().then(n=>{let r=n.transaction(Ie,"readwrite");t===null?r.objectStore(Ie).delete(e):r.objectStore(Ie).put(t,e)})}function Yi(e,t="utf8"){if(e instanceof Uint8Array)return e;if(typeof e=="string"){if(t==="hex"){let n=new Uint8Array(e.length/2);for(let r=0;r<n.length;r++)n[r]=parseInt(e.slice(r*2,r*2+2),16);return n}return new TextEncoder().encode(e)}return new Uint8Array(e)}function Ji(e,t="utf8"){return!t||t==="utf8"?new TextDecoder().decode(e):t==="hex"?Array.from(e).map(n=>n.toString(16).padStart(2,"0")).join(""):t==="base64"?btoa(String.fromCharCode(...e)):new TextDecoder().decode(e)}function ee(e){return ie.has(e)}function me(e,t){if(!ie.has(e))throw Object.assign(new Error(`ENOENT: no such file: ${e}`),{code:"ENOENT"});let n=ie.get(e);if(n==="__DIR__")throw Object.assign(new Error(`EISDIR: ${e}`),{code:"EISDIR"});let r=typeof t=="string"?t:t?.encoding;return r?Ji(n,r):globalThis.Buffer.from(n)}function Ge(e,t,n){let r=typeof n=="string"?n:n?.encoding,i=Yi(t,r);ie.set(e,i),qe(e,i)}function nt(e){ie.delete(e),qe(e,null)}function Nr(e,t={}){if(t.recursive)for(let n of[...ie.keys()])(n===e||n.startsWith(e+"/"))&&(ie.delete(n),qe(n,null));else nt(e)}function Ye(e,t={}){if(t.recursive){let n=e.split("/").filter(Boolean),r="";for(let i of n)r+="/"+i,ie.has(r)||(ie.set(r,"__DIR__"),qe(r,"__DIR__"))}else ie.set(e,"__DIR__"),qe(e,"__DIR__")}function rt(e){let t=e.endsWith("/")?e:e+"/";return[...ie.keys()].filter(n=>n.startsWith(t)&&n.slice(t.length).split("/").length===1).map(n=>n.slice(t.length))}function Nt(e){if(!ie.has(e))throw Object.assign(new Error(`ENOENT: ${e}`),{code:"ENOENT"});let t=ie.get(e),n=t==="__DIR__";return{isDirectory:()=>n,isFile:()=>!n,size:n?0:t.length}}var It=new Map,Zi=10,st={O_WRONLY:1,O_CREAT:64,O_APPEND:1024};function Ar(e,t){let n=Zi++,r=(t&st.O_APPEND)!==0,i=ie.has(e)?ie.get(e):new Uint8Array(0);return It.set(n,{path:e,data:r?i:new Uint8Array(0)}),n}function _r(e,t){let n=It.get(e);if(!n)return;let r=new Uint8Array(n.data.length+t.length);r.set(n.data),r.set(t,n.data.length),n.data=r}function Or(e){let t=It.get(e);t&&(ie.set(t.path,t.data),qe(t.path,t.data),It.delete(e))}var Qi=Xt().then(e=>new Promise(t=>{let r=e.transaction(Ie,"readonly").objectStore(Ie).openCursor();r.onsuccess=i=>{let s=i.target.result;if(!s)return t(!0);ie.set(s.key,s.value),s.continue()}}));globalThis.__fsReady__=Qi;function Xi(e){let t=Math.max(1,Math.floor(e/60)),n=Math.floor(t/1440),r=Math.floor(t%1440/60),i=t%60,s=[];return n>0&&s.push(`${n} day${n>1?"s":""}`),r>0&&s.push(`${r} hour${r>1?"s":""}`),(i>0||s.length===0)&&s.push(`${i} min${i>1?"s":""}`),s.join(", ")}function Dr(e){return`\x1B[${e}m   \x1B[0m`}function eo(){let e=[40,41,42,43,44,45,46,47].map(Dr).join(""),t=[100,101,102,103,104,105,106,107].map(Dr).join("");return[e,t]}function Fr(e,t,n){if(e.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s=n<=1?0:t/(n-1),o=Math.round(r.r+(i.r-r.r)*s),a=Math.round(r.g+(i.g-r.g)*s),l=Math.round(r.b+(i.b-r.b)*s);return`\x1B[38;2;${o};${a};${l}m${e}\x1B[0m`}function to(e){if(e.trim().length===0)return e;let t=e.indexOf(":");if(t===-1)return e.includes("@")?Rr(e):e;let n=e.substring(0,t+1),r=e.substring(t+1);return Rr(n)+r}function Rr(e){let t=new RegExp("\x1B\\[[\\d;]*m","g"),n=e.replace(t,"");if(n.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s="";for(let o=0;o<n.length;o+=1){let a=n.length<=1?0:o/(n.length-1),l=Math.round(r.r+(i.r-r.r)*a),c=Math.round(r.g+(i.g-r.g)*a),u=Math.round(r.b+(i.b-r.b)*a);s+=`\x1B[38;2;${l};${c};${u}m${n[o]}\x1B[0m`}return s}function Vr(e){return Math.max(0,Math.round(e/(1024*1024)))}function Lr(){try{let e=me("/etc/os-release","utf8");for(let t of e.split(`
`)){if(!t.startsWith("PRETTY_NAME="))continue;return t.slice(12).trim().replace(/^"|"$/g,"")}}catch{return}}function Ur(e){try{let t=me(e,"utf8").split(`
`)[0]?.trim();return!t||t.length===0?void 0:t}catch{return}}function no(e){let t=Ur("/sys/devices/virtual/dmi/id/sys_vendor"),n=Ur("/sys/devices/virtual/dmi/id/product_name");return t&&n?`${t} ${n}`:n||e}function ro(){let e=["/var/lib/dpkg/status","/usr/local/var/lib/dpkg/status"];for(let t of e)if(ee(t))try{return me(t,"utf8").match(/^Package:\s+/gm)?.length??0}catch{}}function so(){let e=["/snap","/var/lib/snapd/snaps"];for(let t of e)if(ee(t))try{return rt(t,{withFileTypes:!0}).filter(i=>i.isDirectory()).length}catch{}}function io(){let e=ro(),t=so();return e!==void 0&&t!==void 0?`${e} (dpkg), ${t} (snap)`:e!==void 0?`${e} (dpkg)`:t!==void 0?`${t} (snap)`:"n/a"}function oo(){let e=Et();if(e.length===0)return"unknown";let t=e[0];if(!t)return"unknown";let n=(t.speed/1e3).toFixed(2);return`${t.model} (${e.length}) @ ${n}GHz`}function ao(e){return!e||e.trim().length===0?"unknown":z.basename(e.trim())}function lo(e){let t=we(),n=Re(),r=Math.max(0,t-n),i=e.shellProps,s=y.uptime();return e.uptimeSeconds===void 0&&(e.uptimeSeconds=Math.round(s)),{user:e.user,host:e.host,osName:i?.os??e.osName??`${Lr()??qt()} ${$t()}`,kernel:i?.kernel??e.kernel??Gt(),uptimeSeconds:e.uptimeSeconds??tr(),packages:e.packages??io(),shell:ao(e.shell),shellProps:e.shellProps??{kernel:e.kernel??Gt(),os:e.osName??`${Lr()??qt()} ${$t()}`,arch:$t()},resolution:e.resolution??"n/a (ssh)",terminal:e.terminal??"unknown",cpu:e.cpu??oo(),gpu:e.gpu??"n/a",memoryUsedMiB:e.memoryUsedMiB??Vr(r),memoryTotalMiB:e.memoryTotalMiB??Vr(t)}}function Br(e){let t=lo(e),n=Xi(t.uptimeSeconds),r=eo(),i=["                               .. .:.    "," .::..                       ..     ..   ",".    ....                  ...       ..  ",":       ....             .:.          .. ",":           .:.:........:.            .. ",":                                     .. ",".                                      : ",".                                      : ","..                                     : "," :.                                   .. "," ..                                   .. "," :-.                                  :: ","  .:.                                 :. ","   ..:                               ... ","   ..:                               :.. ","  :...                              :....","   ..                                ....","   .                                  .. ","  .:.                                 .: ","  :.                                  .. "," ::.                                 ..  ",".....                          ..:...    ","...:.                         ..         ",".:...:.       ::.           ..           ","  ... ..:::::..  ..:::::::..             "],s=[`${t.user}@${t.host}`,"-------------------------",`OS: ${t.osName}`,`Host: ${no(t.host)}`,`Kernel: ${t.kernel}`,`Uptime: ${n}`,`Packages: ${t.packages}`,`Shell: ${t.shell}`,`Resolution: ${t.resolution}`,`Terminal: ${t.terminal}`,`CPU: ${t.cpu}`,`GPU: ${t.gpu}`,`Memory: ${t.memoryUsedMiB}MiB / ${t.memoryTotalMiB}MiB`,"",r[0],r[1]],o=Math.max(i.length,s.length),a=[];for(let l=0;l<o;l+=1){let c=i[l]??"",u=s[l]??"";if(u.length>0){let d=Fr(c.padEnd(31," "),l,i.length),m=to(u);a.push(`${d}  ${m}`);continue}a.push(Fr(c,l,i.length))}return a.join(`
`)}var zr={name:"neofetch",description:"System info display",category:"system",params:["[--off]"],run:({args:e,authUser:t,hostname:n,shell:r,env:i})=>r.packageManager.isInstalled("neofetch")?C(e,"--help")?{stdout:"Usage: neofetch [--off]",exitCode:0}:C(e,"--off")?{stdout:`${t}@${n}`,exitCode:0}:{stdout:Br({user:t,host:n,shell:i.vars.SHELL,shellProps:r.properties,terminal:i.vars.TERM,uptimeSeconds:Math.floor((Date.now()-r.startTime)/1e3),packages:`${r.packageManager?.installedCount()??0} (dpkg)`}),exitCode:0}:{stderr:`bash: neofetch: command not found
Hint: install it with: apt install neofetch
`,exitCode:127}};function At(e,t){let n=new Function("exports","require","module","__filename","__dirname",e),r={exports:{}};return n(r.exports,()=>{throw new Error("require not supported in vm shim")},r,"",""),r.exports}var _t="v18.19.0",Hr={node:_t,npm:"9.2.0",v8:"10.2.154.26-node.22"};function co(e,t){let n={version:_t,versions:Hr,platform:"linux",arch:"x64",env:{NODE_ENV:"production",HOME:"/root",PATH:"/usr/local/bin:/usr/bin:/bin"},argv:["node"],stdout:{write:s=>(e.push(s),!0)},stderr:{write:s=>(t.push(s),!0)},exit:(s=0)=>{throw new Ot(s)},cwd:()=>"/root",hrtime:()=>[0,0]},r={log:(...s)=>e.push(s.map(Ce).join(" ")),error:(...s)=>t.push(s.map(Ce).join(" ")),warn:(...s)=>t.push(s.map(Ce).join(" ")),info:(...s)=>e.push(s.map(Ce).join(" ")),dir:s=>e.push(Ce(s))},i=s=>{switch(s){case"path":return{join:(...o)=>o.join("/").replace(/\/+/g,"/"),resolve:(...o)=>`/${o.join("/").replace(/^\/+/,"")}`,dirname:o=>o.split("/").slice(0,-1).join("/")||"/",basename:o=>o.split("/").pop()??"",extname:o=>{let a=o.split("/").pop()??"",l=a.lastIndexOf(".");return l>0?a.slice(l):""},sep:"/",delimiter:":"};case"os":return{platform:()=>"linux",arch:()=>"x64",type:()=>"Linux",hostname:()=>"fortune-vm",homedir:()=>"/root",tmpdir:()=>"/tmp",EOL:`
`};case"util":return{format:(...o)=>o.map(Ce).join(" "),inspect:o=>Ce(o)};case"fs":case"fs/promises":throw new Error(`Cannot require '${s}': filesystem access not available in virtual runtime`);case"child_process":case"net":case"http":case"https":throw new Error(`Cannot require '${s}': not available in virtual runtime`);default:throw new Error(`Cannot find module '${s}'`)}};return i.resolve=s=>{throw new Error(`Cannot resolve '${s}'`)},i.cache={},i.extensions={},At.createContext({console:r,process:n,require:i,Math,JSON,Object,Array,String,Number,Boolean,Symbol,Date,RegExp,Error,TypeError,RangeError,SyntaxError,Promise,Map,Set,WeakMap,WeakSet,parseInt,parseFloat,isNaN,isFinite,encodeURIComponent,decodeURIComponent,encodeURI,decodeURI,setTimeout:()=>{},clearTimeout:()=>{},setInterval:()=>{},clearInterval:()=>{},queueMicrotask:()=>{},globalThis:void 0,undefined:void 0,Infinity:1/0,NaN:NaN})}var Ot=class{constructor(t){this.code=t}code};function Ce(e){if(e===null)return"null";if(e===void 0)return"undefined";if(typeof e=="string")return e;if(typeof e=="function")return`[Function: ${e.name||"(anonymous)"}]`;if(Array.isArray(e))return`[ ${e.map(Ce).join(", ")} ]`;if(e instanceof Error)return`${e.name}: ${e.message}`;if(typeof e=="object")try{return`{ ${Object.entries(e).map(([n,r])=>`${n}: ${Ce(r)}`).join(", ")} }`}catch{return"[Object]"}return String(e)}function Tt(e){let t=[],n=[],r=co(t,n),i=0;try{let s=At.runInContext(e,r,{timeout:5e3});s!==void 0&&t.length===0&&t.push(Ce(s))}catch(s){s instanceof Ot?i=s.code:s instanceof Error?(n.push(`${s.name}: ${s.message}`),i=1):(n.push(String(s)),i=1)}return{stdout:t.length?`${t.join(`
`)}
`:"",stderr:n.length?`${n.join(`
`)}
`:"",exitCode:i}}function uo(e){let t=e.trim();return!t.includes(`
`)&&!t.startsWith("const ")&&!t.startsWith("let ")&&!t.startsWith("var ")&&!t.startsWith("function ")&&!t.startsWith("class ")&&!t.startsWith("if ")&&!t.startsWith("for ")&&!t.startsWith("while ")&&!t.startsWith("import ")&&!t.startsWith("//")?Tt(t):Tt(`(async () => { ${e} })()`)}var jr={name:"node",description:"JavaScript runtime (virtual)",category:"system",params:["[--version] [-e <expr>] [-p <expr>] [file]"],run:({args:e,shell:t,cwd:n})=>{if(!t.packageManager.isInstalled("nodejs"))return{stderr:`bash: node: command not found
Hint: install it with: apt install nodejs
`,exitCode:127};if(C(e,["--version","-v"]))return{stdout:`${_t}
`,exitCode:0};if(C(e,["--versions"]))return{stdout:`${JSON.stringify(Hr,null,2)}
`,exitCode:0};let r=e.findIndex(o=>o==="-e"||o==="--eval");if(r!==-1){let o=e[r+1];if(!o)return{stderr:`node: -e requires an argument
`,exitCode:1};let{stdout:a,stderr:l,exitCode:c}=Tt(o);return{stdout:a||void 0,stderr:l||void 0,exitCode:c}}let i=e.findIndex(o=>o==="-p"||o==="--print");if(i!==-1){let o=e[i+1];if(!o)return{stderr:`node: -p requires an argument
`,exitCode:1};let{stdout:a,stderr:l,exitCode:c}=Tt(o);return{stdout:a||(c===0?`
...
`),exitCode:0}}};var Dt="9.2.0",mo="18.19.0",Wr={name:"npm",description:"Node.js package manager (virtual)",category:"system",params:["<command> [args]"],run:({args:e,shell:t})=>{if(!t.packageManager.isInstalled("npm"))return{stderr:`bash: npm: command not found
Hint: install it with: apt install npm
`,exitCode:127};if(C(e,["--version","-v"]))return{stdout:`${Dt}
`,exitCode:0};let n=e[0]?.toLowerCase();switch(n){case"version":case"-version":return{stdout:`{ npm: '${Dt}', node: '${mo}', v8: '10.2.154.26' }
...
\u2514\u2500\u2500 (empty)
`,exitCode:0};case"help":case void 0:return{stdout:`${[`npm ${Dt}`,"","Usage: npm <command>","","Commands:","  install       (not available in virtual runtime)","  run           (not available in virtual runtime)","  exec          (not available in virtual runtime)","  list          List installed packages","  version       Print versions","  --version     Print npm version"].join(`
`)}
`,exitCode:0};default:return{stderr:`npm error: unknown command: ${n}
`,exitCode:1}}}},Kr={name:"npx",description:"Node.js package runner (virtual)",category:"system",params:["<package> [args]"],run:({args:e,shell:t})=>t.packageManager.isInstalled("npm")?C(e,["--version"])?{stdout:`${Dt}
...
`),exitCode:0}}};var Zr={name:"pwd",description:"Print working directory",category:"navigation",params:[],run:({cwd:e})=>({stdout:e,exitCode:0})};var fo="Python 3.11.2";var Ft="3.11.2 (default, Mar 13 2023, 12:18:29) [GCC 12.2.0]",b={__pytype__:"none"};function Z(e=[]){return{__pytype__:"dict",data:new Map(e)}}function en(e,t,n=1){return{__pytype__:"range",start:e,stop:t,step:n}}function G(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="dict"}function Ze(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="range"}function Pe(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="func"}function tn(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="class"}function it(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="instance"}function Me(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="none"}function ne(e){return e===null||Me(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?`'${e.replace(/'/g,"\\'")}'`:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:G(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Ze(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:Pe(e)?`<function ${e.name} at 0x...>`:tn(e)?`<class '${e.name}'>`:it(e)?`<${e.cls.name} object at 0x...>`:String(e)}function R(e){return e===null||Me(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?e:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:G(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Ze(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:ne(e)}function ce(e){return e===null||Me(e)?!1:typeof e=="boolean"?e:typeof e=="number"?e!==0:typeof e=="string"||Array.isArray(e)?e.length>0:G(e)?e.data.size>0:Ze(e)?Xr(e)>0:!0}function Xr(e){if(e.step===0)return 0;let t=Math.ceil((e.stop-e.start)/e.step);return Math.max(0,t)}function ho(e){let t=[];for(let n=e.start;(e.step>0?n<e.stop:n>e.stop)&&(t.push(n),!(t.length>1e4));n+=e.step);return t}function te(e){if(Array.isArray(e))return e;if(typeof e=="string")return[...e];if(Ze(e))return ho(e);if(G(e))return[...e.data.keys()];throw new J("TypeError",`'${Ve(e)}' object is not iterable`)}function Ve(e){return e===null||Me(e)?"NoneType":typeof e=="boolean"?"bool":typeof e=="number"?Number.isInteger(e)?"int":"float":typeof e=="string"?"str":Array.isArray(e)?"list":G(e)?"dict":Ze(e)?"range":Pe(e)?"function":tn(e)?"type":it(e)?e.cls.name:"object"}var J=class{constructor(t,n){this.type=t;this.message=n}type;message;toString(){return`${this.type}: ${this.message}`}},Je=class{constructor(t){this.value=t}value},ot=class{},at=class{},lt=class{constructor(t){this.code=t}code};function go(e){let t=new Map,n=Z([["sep","/"],["linesep",`
...
`),l.stderr)return{...l,stdout:r.trim()};n=l}else if(i.type==="if"){let s=!1;if(await nn(i.cond,t)){let o=await Ae(Ne(i.then_),t);o.stdout&&(r+=`${o.stdout}
`),s=!0}else{for(let o of i.elif)if(await nn(o.cond,t)){let a=await Ae(Ne(o.body),t);a.stdout&&(r+=`${a.stdout}
...
`);else if(i.type==="arith"){let s=i.expr.trim(),o=s.match(/^(\w+)\s*(\+\+|--)$/);if(o){let a=parseInt(t.env.vars[o[1]]??"0",10);t.env.vars[o[1]]=String(o[2]==="++"?a+1:a-1)}else{let a=s.match(/^(\w+)\s*([+\-*/])=\s*(.+)$/);if(a){let l=parseInt(t.env.vars[a[1]]??"0",10),c=parseInt(a[3],10),u={"+":l+c,"-":l-c,"*":l*c,"/":Math.floor(l/c)};t.env.vars[a[1]]=String(u[a[2]]??l)}else{let l=Kt(s,t.env.vars);Number.isNaN(l)||(t.env.lastExitCode=l===0?1:0)}}}else if(i.type==="for"){let o=(await rn(i.list,t.env.vars,t.env.lastExitCode,t)).trim().split(/\s+/).flatMap(Ct);for(let a of o){t.env.vars[i.var]=a;let l=await Ae(Ne(i.body),t);if(l.stdout&&(r+=`${l.stdout}
`),l.closeSession)return l}}else if(i.type==="while"){let s=0;for(;s<1e3&&await nn(i.cond,t);){let o=await Ae(Ne(i.body),t);if(o.stdout&&(r+=`${o.stdout}
`),o.closeSession)return o;s++}}return{...n,stdout:r.trim()||n.stdout}}function is(e){let t=[],n="",r=0,i=!1,s=!1,o=0;for(;o<e.length;){let l=e[o];if(!i&&!s){if(l==="'"){i=!0,n+=l,o++;continue}if(l==='"'){s=!0,n+=l,o++;continue}if(l==="{"){r++,n+=l,o++;continue}if(l==="}"){if(r--,n+=l,o++,r===0){let c=n.trim();for(c&&t.push(c),n="";o<e.length&&(e[o]===";"||e[o]===" ");)o++}continue}if(r===0&&(l===";"||l===`
`)){let c=n.trim();c&&!c.startsWith("#")&&t.push(c),n="",o++;continue}}else i&&l==="'"?i=!1:s&&l==='"'&&(s=!1);n+=l,o++}let a=n.trim();return a&&!a.startsWith("#")&&t.push(a),t}var os={name:"sh",aliases:["bash"],description:"Execute shell script or command",category:"shell",params:["-c <script>","[<file>]"],run:async e=>{let{args:t,shell:n,cwd:r}=e;if(C(t,"-c")){let s=t[t.indexOf("-c")+1]??"";if(!s)return{stderr:"sh: -c requires a script",exitCode:1};let o=is(s),a=Ne(o);return Ae(a,e)}let i=t[0];if(i){let s=M(r,i);if(!n.vfs.exists(s))return{stderr:`sh: ${i}: No such file or directory`,exitCode:1};let o=n.vfs.readFile(s),a=is(o),l=Ne(a);return Ae(l,e)}return{stderr:"sh: invalid usage. Use: sh -c 'cmd' or sh <file>",exitCode:1}}};var as={name:"shift",description:"Shift positional parameters",category:"shell",params:["[n]"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};let n=parseInt(e[0]??"1",10)||1,r=t.vars.__argv?.split("\0").filter(Boolean)??[];t.vars.__argv=r.slice(n).join("\0");let i=r.slice(n);for(let s=1;s<=9;s++)t.vars[String(s)]=i[s-1]??"";return{exitCode:0}}},ls={name:"trap",description:"Trap signals and events",category:"shell",params:["[action] [signal...]"],run:({args:e,env:t})=>{if(!t||e.length===0)return{exitCode:0};let n=e[0]??"",r=e.slice(1);for(let i of r)t.vars[`__trap_${i.toUpperCase()}`]=n;return{exitCode:0}}},cs={name:"return",description:"Return from a shell function",category:"shell",params:["[n]"],run:({args:e,env:t})=>{let n=parseInt(e[0]??"0",10);return t&&(t.lastExitCode=n),{exitCode:n}}};var us={name:"sleep",description:"Delay execution",category:"system",params:["<seconds>"],run:async({args:e})=>{let t=parseFloat(e[0]??"1");return Number.isNaN(t)||t<0?{stderr:"sleep: invalid time",exitCode:1}:(await new Promise(n=>setTimeout(n,t*1e3)),{exitCode:0})}};var ds={name:"sort",description:"Sort lines of text",category:"text",params:["[-r] [-n] [-u] [-k <col>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-r"]),o=C(r,["-n"]),a=C(r,["-u"]),l=r.filter(g=>!g.startsWith("-")),d=[...(l.length>0?l.map(g=>{try{return L(e,M(n,g),"sort"),t.vfs.readFile(M(n,g))}catch{return""}}).join(`
`):i??"").split(`
`).filter(Boolean)].sort((g,S)=>o?Number(g)-Number(S):g.localeCompare(S)),m=s?d.reverse():d;return{stdout:(a?[...new Set(m)]:m).join(`
`),exitCode:0}}};var ms={name:"source",aliases:["."],description:"Execute commands from a file in the current shell environment",category:"shell",params:["<file> [args...]"],run:async({args:e,authUser:t,hostname:n,cwd:r,shell:i,env:s})=>{let o=e[0];if(!o)return{stderr:"source: missing filename",exitCode:1};let a=M(r,o);if(!i.vfs.exists(a))return{stderr:`source: ${o}: No such file or directory`,exitCode:1};let l=i.vfs.readFile(a),c=0;for(let u of l.split(`
`)){let d=u.trim();if(!d||d.startsWith("#"))continue;let m=await Q(d,t,n,"shell",r,i,void 0,s);if(c=m.exitCode??0,m.closeSession||m.switchUser)return m}return{exitCode:c}}};var ps={name:"su",description:"Switch user",category:"users",params:["[-] [-c <cmd>] [username]"],run:async({authUser:e,shell:t,args:n,hostname:r,mode:i,cwd:s})=>{let o=n.includes("-")||n.includes("-l")||n.includes("--login"),a=n.indexOf("-c"),l=a!==-1?n[a+1]:void 0,u=n.filter((d,m)=>m!==a&&m!==a+1).filter(d=>d!=="-"&&d!=="-l"&&d!=="--login").find(d=>!d.startsWith("-"))??"root";return t.users.listUsers().includes(u)?e==="root"?l?Q(l,u,r,i,o?`/home/${u}`:s,t):{switchUser:u,nextCwd:o?`/home/${u}`:void 0,exitCode:0}:t.users.isSudoer(e)?{sudoChallenge:{username:u,targetUser:u,commandLine:l??null,loginShell:o,prompt:"Password: "},exitCode:0}:{stderr:`su: permission denied
`,exitCode:1}:{stderr:`su: user '${u}' does not exist
`,exitCode:1}}};function vo(e){let{flags:t,flagsWithValues:n,positionals:r}=le(e,{flags:["-i","-S"],flagsWithValue:["-u","--user"]}),i=t.has("-i"),s=n.get("-u")||n.get("--user")||"root",o=r.length>0?r.join(" "):null;return{targetUser:s,loginShell:i,commandLine:o}}var fs={name:"sudo",description:"Execute as superuser",category:"users",params:["<command...>"],run:async({authUser:e,hostname:t,mode:n,cwd:r,shell:i,args:s})=>{let{targetUser:o,loginShell:a,commandLine:l}=vo(s);if(e!=="root"&&!i.users.isSudoer(e))return{stderr:"sudo: permission denied",exitCode:1};let c=o||"root",u=`[sudo] password for ${e}: `;return e==="root"?!l&&a?{switchUser:c,nextCwd:`/home/${c}`,exitCode:0}:l?Q(l,c,t,n,a?`/home/${c}`:r,i):{stderr:"sudo: missing command",exitCode:1}:{sudoChallenge:{username:e,targetUser:c,commandLine:l,loginShell:a,prompt:u},exitCode:0}}};var hs={name:"tail",description:"Output last lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=ae(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
`),p=d.endsWith(`
`),g=p?m.slice(0,-1):m;return g.slice(Math.max(0,g.length-a)).join(`
`)+(p?`
`:"")};if(l.length===0)return{stdout:c(i??""),exitCode:0};let u=[];for(let d of l){let m=M(n,d);try{L(e,m,"tail"),u.push(c(t.vfs.readFile(m)))}catch{return{stderr:`tail: ${d}: No such file or directory`,exitCode:1}}}return{stdout:u.join(`
...
`),exitCode:0};for(let[p,g]of Object.entries(m))t.writeFileAsUser(e,M(n,p),g);return{exitCode:0}}return{stderr:"tar: must specify -c, -x, or -t",exitCode:1}}};var ys={name:"tee",description:"Read stdin, write to stdout and files",category:"text",params:["[-a] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-a"]),o=r.filter(l=>!l.startsWith("-")),a=i??"";for(let l of o){let c=M(n,l);if(s){let u=(()=>{try{return t.vfs.readFile(c)}catch{return""}})();t.writeFileAsUser(e,c,u+a)}else t.writeFileAsUser(e,c,a)}return{stdout:a,exitCode:0}}};function Qe(e,t,n){if(e[e.length-1]==="]"&&(e=e.slice(0,-1)),e[0]==="["&&(e=e.slice(1)),e.length===0)return!1;if(e[0]==="!")return!Qe(e.slice(1),t,n);let r=e.indexOf("-a");if(r!==-1)return Qe(e.slice(0,r),t,n)&&Qe(e.slice(r+1),t,n);let i=e.indexOf("-o");if(i!==-1)return Qe(e.slice(0,i),t,n)||Qe(e.slice(i+1),t,n);if(e.length===2){let[s,o=""]=e,l=(c=>c.startsWith("/")?c:`${n}/${c}`.replace(/\/+/g,"/"))(o);switch(s){case"-e":return t.vfs.exists(l);case"-f":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file";case"-d":return t.vfs.exists(l)&&t.vfs.stat(l).type==="directory";case"-r":return t.vfs.exists(l);case"-w":return t.vfs.exists(l);case"-x":return t.vfs.exists(l)&&!!(t.vfs.stat(l).mode&73);case"-s":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file"&&t.vfs.stat(l).size>0;case"-z":return o.length===0;case"-n":return o.length>0;case"-L":return t.vfs.isSymlink(l)}}if(e.length===3){let[s="",o,a=""]=e,l=Number(s),c=Number(a);switch(o){case"=":case"==":return s===a;case"!=":return s!==a;case"<":return s<a;case">":return s>a;case"-eq":return l===c;case"-ne":return l!==c;case"-lt":return l<c;case"-le":return l<=c;case"-gt":return l>c;case"-ge":return l>=c}}return e.length===1?(e[0]??"").length>0:!1}var Ss={name:"test",aliases:["["],description:"Evaluate conditional expression",category:"shell",params:["<expression>"],run:({args:e,shell:t,cwd:n})=>{try{return{exitCode:Qe([...e],t,n)?0:1}}catch{return{stderr:"test: malformed expression",exitCode:2}}}};var bs={name:"touch",description:"Create or update files",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"touch: missing file operand",exitCode:1};for(let i of r){let s=M(n,i);L(e,s,"touch"),t.vfs.exists(s)||t.writeFileAsUser(e,s,"")}return{exitCode:0}}};function Co(e){return e.replace(/\\n/g,`
`).replace(/\\t/g,"	").replace(/\\r/g,"\r").replace(/\\\\/g,"\\")}function xs(e){let t=[],n=Co(e),r=0;for(;r<n.length;){if(r+2<n.length&&n[r+1]==="-"){let i=n.charCodeAt(r),s=n.charCodeAt(r+2);if(i<=s){for(let o=i;o<=s;o++)t.push(String.fromCharCode(o));r+=3;continue}}t.push(n[r]),r++}return t}var ws={name:"tr",description:"Translate or delete characters",category:"text",params:["[-d] [-s] <set1> [set2]"],run:({args:e,stdin:t})=>{let n=C(e,["-d"]),r=C(e,["-s"]),i=e.filter(l=>!l.startsWith("-")),s=xs(i[0]??""),o=xs(i[1]??""),a=t??"";if(n){let l=new Set(s);a=[...a].filter(c=>!l.has(c)).join("")}else if(o.length>0){let l=new Map;for(let c=0;c<s.length;c++)l.set(s[c],o[c]??o[o.length-1]??"");a=[...a].map(c=>l.get(c)??c).join("")}if(r&&o.length>0){let l=new Set(o);a=a.replace(/(.)\1+/g,(c,u)=>l.has(u)?u:c)}return{stdout:a,exitCode:0}}};var vs={name:"tree",description:"Display directory tree",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=M(n,$e(r,0)??n);return L(e,i,"tree"),{stdout:t.vfs.tree(i),exitCode:0}}};var Cs={name:"true",description:"Return success exit code",category:"shell",params:[],run:()=>({exitCode:0})},Ps={name:"false",description:"Return failure exit code",category:"shell",params:[],run:()=>({exitCode:1})};var Es={name:"type",description:"Describe how a command would be interpreted",category:"shell",params:["<command...>"],run:({args:e,shell:t,env:n})=>{if(e.length===0)return{stderr:"type: missing argument",exitCode:1};let r=(n?.vars?.PATH??"/usr/local/bin:/usr/bin:/bin").split(":"),i=[],s=0;for(let o of e){if(ye(o)){i.push(`${o} is a shell builtin`);continue}let a=!1;for(let l of r){let c=`${l}/${o}`;if(t.vfs.exists(c)){i.push(`${o} is ${c}`),a=!0;break}}a||(i.push(`${o}: not found`),s=1)}return{stdout:i.join(`
`),exitCode:s}}};var $s={name:"uname",description:"Print system information",category:"system",params:["[-a] [-s] [-r] [-m]"],run:({shell:e,args:t})=>{let n=C(t,["-a"]),r="Linux",i=e.properties?.kernel??"5.15.0",s=e.properties?.arch??"x86_64",o=e.hostname;return n?{stdout:`${r} ${o} ${i} #1 SMP ${s} GNU/Linux`,exitCode:0}:C(t,["-r"])?{stdout:i,exitCode:0}:C(t,["-m"])?{stdout:s,exitCode:0}:{stdout:r,exitCode:0}}};var ks={name:"uniq",description:"Report or filter out repeated lines",category:"text",params:["[-c] [-d] [-u] [file]"],run:({args:e,stdin:t})=>{let n=C(e,["-c"]),r=C(e,["-d"]),i=C(e,["-u"]),s=(t??"").split(`
...
`)}
`),Ys(e,1,"root","pts/0","/sbin/init",new Date(r).toISOString(),{});for(let I of i){let x=Gs(I.tty);Ys(e,x,I.username,I.tty,"bash",I.startedAt,{USER:I.username,HOME:`/home/${I.username}`,TERM:"xterm-256color",SHELL:"/bin/bash",LANG:"en_US.UTF-8",PATH:"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",LOGNAME:I.username})}let P=i.length>0?Gs(i[i.length-1].tty):1;try{e.remove("/proc/self")}catch{}let N=`/proc/${P}`;if(h(e,"/proc/self"),h(e,"/proc/self/fd"),h(e,"/proc/self/fdinfo"),h(e,"/proc/self/net"),e.exists(N))for(let I of e.list(N)){let x=`${N}/${I}`,A=`/proc/self/${I}`;try{e.stat(x).type==="file"&&$(e,A,e.readFile(x))}catch{}}else $(e,"/proc/self/cmdline","bash\0"),$(e,"/proc/self/comm","bash"),$(e,"/proc/self/status",`Name:	bash
State:	S (sleeping)
Pid:	1
PPid:	0
...
CONFIG_VIRTIO_NET=y
CONFIG_KVM_GUEST=y
`,420),e.exists("/vmlinuz")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz"),e.exists("/vmlinuz.old")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz.old"),e.exists("/initrd.img")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img"),e.exists("/initrd.img.old")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img.old"),h(e,"/lost+found",448),h(e,"/home")}var Js=new Map;function Qo(e,t){return`${e}|${t.kernel}|${t.os}|${t.arch}`}function Xo(e,t){let n=Qo(e,t),r=Js.get(n);if(r)return r;let i=new mt({mode:"memory"});Bo(i,e,t),Ho(i,e,t),jo(i),Wo(i),qo(i),Go(i),Yo(i),Zo(i,t),zo(i,t);let s=i.encodeBinary();return Js.set(n,s),s}function Zs(e,t,n,r,i,s=[]){let o=Xo(n,r);e.getMode()==="fs"&&e.exists("/home")?e.mergeRootTree(Oe(o)):e.importRootTree(Oe(o)),Jo(e),Ut(e,r,n,i,s),yn(e,t)}var Sn=[{name:"vim",version:"2:9.0.1378-2",section:"editors",description:"Vi IMproved - enhanced vi editor",shortDesc:"Vi IMproved",installedSizeKb:3812,files:[{path:"/usr/bin/vim",content:`#!/bin/sh
...
`:"",384)||s,s&&await this.vfs.flushMirror()}writeIfChanged(n,r,i){return this.vfs.exists(n)&&this.vfs.readFile(n)===r?(this.vfs.chmod(n,i),!1):(this.vfs.writeFile(n,r,{mode:i}),!0)}createRecord(n,r){let i=bn("sha256").update(n).update(":").update(r).digest("hex"),s=e.recordCache.get(i);if(s)return s;let o=Xs(16).toString("hex"),a={username:n,salt:o,passwordHash:this.hashPassword(r,o)};return e.recordCache.set(i,a),a}hasPassword(n){re.mark("hasPassword");let r=this.users.get(n);if(!r)return!1;let i=this.hashPassword("",r.salt);return r.passwordHash===i?!1:!!r.passwordHash}hashPassword(n,r=""){return e.fastPasswordHash?bn("sha256").update(r).update(n).digest("hex"):ti(n,r||"",32).toString("hex")}validateUsername(n){if(!n||n.trim()==="")throw new Error("invalid username");if(!/^[a-z_][a-z0-9_-]{0,31}$/i.test(n))throw new Error("invalid username")}validatePassword(n){if(!n||n.trim()==="")throw new Error("invalid password")}authorizedKeys=new Map;addAuthorizedKey(n,r,i){re.mark("addAuthorizedKey");let s=this.authorizedKeys.get(n)??[];s.push({algo:r,data:i}),this.authorizedKeys.set(n,s),this.emit("key:add",{username:n,algo:r})}removeAuthorizedKeys(n){this.authorizedKeys.delete(n),this.emit("key:remove",{username:n})}getAuthorizedKeys(n){return this.authorizedKeys.get(n)??[]}};function ri(e){let t=z.normalize(e);return t.startsWith("/")?t:`/${t}`}var ht=class extends fe{vfs;idleThresholdMs;checkIntervalMs;_state="active";_lastActivity=Date.now();_frozenBuffer=null;_checkTimer=null;constructor(t,n={}){super(),this.vfs=t,this.idleThresholdMs=n.idleThresholdMs??6e4,this.checkIntervalMs=n.checkIntervalMs??15e3}start(){this._checkTimer||(this._lastActivity=Date.now(),this._checkTimer=setInterval(()=>this._check(),this.checkIntervalMs),typeof this._checkTimer=="object"&&this._checkTimer!==null&&"unref"in this._checkTimer&&this._checkTimer.unref())}async stop(){this._checkTimer&&(clearInterval(this._checkTimer),this._checkTimer=null),this._state==="frozen"&&this._thaw()}ping(){this._lastActivity=Date.now(),this._state==="frozen"&&this._thaw()}get state(){return this._state}get idleMs(){return Date.now()-this._lastActivity}_check(){this._state!=="frozen"&&Date.now()-this._lastActivity>=this.idleThresholdMs&&this._freeze()}async _freeze(){this._state!=="frozen"&&(await this.vfs.stopAutoFlush(),this._frozenBuffer=this.vfs.encodeBinary(),this.vfs.releaseTree(),this._state="frozen",this.emit("freeze"))}_thaw(){if(this._state!=="frozen"||!this._frozenBuffer)return;let t=Oe(this._frozenBuffer);this.vfs.importRootTree(t),this._frozenBuffer=null,this._state="active",this.emit("thaw")}};async function zt(){throw new Error("node:fs/promises.readFile is not supported in browser")}async function si(){throw new Error("node:fs/promises.writeFile is not supported in browser")}async function ii(){throw new Error("node:fs/promises.unlink is not supported in browser")}function xn(e){return`'${e.replace(/'/g,"'\\''")}'`}function gt(e){return e.replace(/\r\n/g,`
`).replace(/\r/g,`
`).replace(/\n/g,`\r
`)}function oi(e,t){let n=Number.isFinite(t.cols)&&t.cols>0?Math.floor(t.cols):80,r=Number.isFinite(t.rows)&&t.rows>0?Math.floor(t.rows):24;return`stty cols ${n} rows ${r} 2>/dev/null; ${e}`}function ai(e,t){return!t||t.trim()===""||t==="."?e:t.startsWith("/")?z.normalize(t):z.normalize(z.join(e,t))}async function li(e){try{let n=(await zt(`/proc/${e}/task/${e}/children`,"utf8")).trim().split(/\s+/).filter(Boolean).map(i=>Number.parseInt(i,10)).filter(i=>Number.isInteger(i)&&i>0),r=await Promise.all(n.map(i=>li(i)));return[...n,...r.flat()]}catch{return[]}}async function ci(e=y.pid){let t=await li(e),n=Array.from(new Set(t)).sort((r,i)=>r-i);return n.length===0?null:n.join(",")}function ui(e,t,n){let r=oi(e,t),i=Wt("script",["-qfec",r,"/dev/null"],{stdio:["pipe","pipe","pipe"],env:{...y.env,TERM:y.env.TERM??"xterm-256color"}});return i.stdout.on("data",s=>{n.write(s.toString("utf8"))}),i.stderr.on("data",s=>{n.write(s.toString("utf8"))}),i}function di(e,t,n){return ui(`nano -- ${xn(e)}`,t,n)}function mi(e,t,n){return ui(`htop -p ${xn(e)}`,t,n)}function pi(e,t,n){let r=[`Linux ${e} ${t.kernel} ${t.arch}`,"","The programs included with the Fortune GNU/Linux system are free software;","the exact distribution terms for each program are described in the","individual files in /usr/share/doc/*/copyright.","","Fortune GNU/Linux comes with ABSOLUTELY NO WARRANTY, to the extent","permitted by applicable law."];if(n){let i=new Date(n.at),s=Number.isNaN(i.getTime())?n.at:Vt(i);r.push(`Last login: ${s} from ${n.from||"unknown"}`)}return r.push(""),`${r.map(i=>`${i}\r
`).join("")}`}function Ht(e,t,n){let r=e==="root",i=r?"\x1B[31;1m":"\x1B[35;1m",s="\x1B[37;1m",o="\x1B[34;1m",a="\x1B[0m";return`${s}[${i}${e}${s}@${o}${t}${a} ${n}${s}]${a}${r?"#":"$"} `}function fi(e,t,n,r,i,s="unknown",o={cols:80,rows:24},a){let l="",c=0,u=ra(a.vfs,n),d=null,m="",p=Y(n),g=Le(n,r),S=null,v=null,E=()=>{let O=Y(n),U=p===O?"~":z.basename(p)||"/";return Ht(n,r,U)},T=Array.from(new Set(cn())).sort();console.log(`[${i}] Shell started for user '${n}' at ${s}`),(async()=>{let O=`${Y(n)}/.bashrc`;if(a.vfs.exists(O))try{let U=a.vfs.readFile(O);for(let H of U.split(`
`)){let V=H.trim();!V||V.startsWith("#")||await Q(V,n,r,"shell",p,a,void 0,g)}}catch{}})();function _(){let O=E();t.write(`\r${O}${l}\x1B[K`);let U=l.length-c;U>0&&t.write(`\x1B[${U}D`)}function P(){t.write("\r\x1B[K")}function N(O){v={...O,buffer:""},P(),t.write(O.prompt)}async function I(O){if(!v)return;let U=v;if(v=null,!O){t.write(`\r
Sorry, try again.\r
`),_();return}if(!U.commandLine){n=U.targetUser,U.loginShell&&(p=Y(n)),a.users.updateSession(i,n,s),t.write(`\r
`),_();return}let H=U.loginShell?Y(U.targetUser):p,V=await Promise.resolve(Q(U.commandLine,U.targetUser,r,"shell",H,a));if(t.write(`\r
`),V.openEditor){await A(V.openEditor.targetPath,V.openEditor.initialContent,V.openEditor.tempPath);return}if(V.openHtop){await D();return}V.clearScreen&&t.write("\x1B[2J\x1B[H"),V.stdout&&t.write(`${gt(V.stdout)}\r
`),V.stderr&&t.write(`${gt(V.stderr)}\r
`),V.switchUser?(n=V.switchUser,p=V.nextCwd??Y(n),a.users.updateSession(i,n,s)):V.nextCwd&&(p=V.nextCwd),_()}async function x(){if(!S)return;let O=S;if(O.kind==="nano"){try{let U=await zt(O.tempPath,"utf8");a.writeFileAsUser(n,O.targetPath,U)}catch{}await ii(O.tempPath).catch(()=>{})}S=null,l="",c=0,t.write(`\r
`),_()}async function A(O,U,H){a.vfs.exists(O)&&await si(H,U,"utf8");let V=di(H,o,t);V.on("error",se=>{t.write(`nano: ${se.message}\r
`),x()}),V.on("close",()=>{x()}),S={kind:"nano",targetPath:O,tempPath:H,process:V}}async function D(){let O=await ci();if(!O){t.write(`htop: no child_process processes to display\r
`);return}let U=mi(O,o,t);U.on("error",H=>{t.write(`htop: ${H.message}\r
`),x()}),U.on("close",()=>{x()}),S={kind:"htop",targetPath:"",tempPath:"",process:U}}function k(O){l=O,c=l.length,_()}function w(O){l=`${l.slice(0,c)}${O}${l.slice(c)}`,c+=O.length,_()}function F(O,U){let H=U;for(;H>0&&!/\s/.test(O[H-1]);)H-=1;let V=U;for(;V<O.length&&!/\s/.test(O[V]);)V+=1;return{start:H,end:V}}function q(O){let U=O.lastIndexOf("/"),H=U>=0?O.slice(0,U+1):"",V=U>=0?O.slice(U+1):O,se=ai(p,H||".");try{return a.vfs.list(se).filter(B=>!B.startsWith(".")).filter(B=>B.startsWith(V)).map(B=>{let xe=z.join(se,B),Fe=a.vfs.stat(xe).type==="directory"?"/":"";return`${H}${B}${Fe}`}).sort()}catch{return[]}}function K(){let{start:O,end:U}=F(l,c),H=l.slice(O,c);if(H.length===0)return;let se=l.slice(0,O).trim().length===0?T.filter(de=>de.startsWith(H)):[],B=q(H),xe=Array.from(new Set([...se,...B])).sort();if(xe.length!==0){if(xe.length===1){let de=xe[0],Fe=de.endsWith("/")?"":" ";l=`${l.slice(0,O)}${de}${Fe}${l.slice(U)}`,c=O+de.length+Fe.length,_();return}t.write(`\r
`),t.write(`${xe.join("  ")}\r
`),_()}}function X(O){if(O.length===0)return;u.push(O),u.length>500&&(u=u.slice(u.length-500));let U=u.length>0?`${u.join(`
`)}
`:"";a.vfs.writeFile(`${Y(n)}/.bash_history`,U)}function ge(){let O=`${Y(n)}/.lastlog.json`;if(!a.vfs.exists(O))return null;try{return JSON.parse(a.vfs.readFile(O))}catch{return null}}function ue(O){let U=`${Y(n)}/.lastlog`;a.vfs.writeFile(U,JSON.stringify({at:O,from:s}))}function Ee(){let O=ge(),U=new Date().toISOString();t.write(pi(r,e,O)),ue(U)}Ee(),_(),t.on("data",async O=>{if(S){S.process.stdin.write(O);return}if(v){let H=O.toString("utf8");for(let V=0;V<H.length;V+=1){let se=H[V];if(se===""){v=null,t.write(`^C\r
`),_();return}if(se==="\x7F"||se==="\b"){v.buffer=v.buffer.slice(0,-1);continue}if(se==="\r"||se===`
`){let B=v.buffer;if(v.buffer="",v.onPassword){let{result:de,nextPrompt:Fe}=await v.onPassword(B,a);t.write(`\r
`),de!==null?(v=null,de.stdout&&t.write(de.stdout.replace(/\n/g,`\r
`)),de.stderr&&t.write(de.stderr.replace(/\n/g,`\r
`)),_()):(Fe&&(v.prompt=Fe),t.write(v.prompt));return}let xe=a.users.verifyPassword(v.username,B);await I(xe);return}se>=" "&&(v.buffer+=se)}return}let U=O.toString("utf8");for(let H=0;H<U.length;H+=1){let V=U[H];if(V===""){l="",c=0,d=null,m="",t.write(`bye\r
`),X("bye"),t.write(`logout\r
`),t.exit(0),t.end();return}if(V==="	"){K();continue}if(V==="\x1B"){let se=U[H+1],B=U[H+2],xe=U[H+3];if(se==="["&&B){if(B==="A"){H+=2,u.length>0&&(d===null?(m=l,d=u.length-1):d>0&&(d-=1),k(u[d]??""));continue}if(B==="B"){H+=2,d!==null&&(d<u.length-1?(d+=1,k(u[d]??"")):(d=null,k(m)));continue}if(B==="C"){H+=2,c<l.length&&(c+=1,t.write("\x1B[C"));continue}if(B==="D"){H+=2,c>0&&(c-=1,t.write("\x1B[D"));continue}if(B==="3"&&xe==="~"){H+=3,c<l.length&&(l=`${l.slice(0,c)}${l.slice(c+1)}`,_());continue}}}if(V===""){l="",c=0,d=null,m="",t.write(`^C\r
`),_();continue}if(V==="\r"||V===`
`){let se=l.trim();if(l="",c=0,d=null,m="",t.write(`\r
`),se.length>0){let B=await Promise.resolve(Q(se,n,r,"shell",p,a,void 0,g));if(X(se),B.openEditor){await A(B.openEditor.targetPath,B.openEditor.initialContent,B.openEditor.tempPath);return}if(B.openHtop){await D();return}if(B.sudoChallenge){N(B.sudoChallenge);return}if(B.clearScreen&&t.write("\x1B[2J\x1B[H"),B.stdout&&t.write(`${gt(B.stdout)}\r
`),B.stderr&&t.write(`${gt(B.stderr)}\r
`),B.closeSession){t.write(`logout\r
`),t.exit(B.exitCode??0),t.end();return}B.nextCwd&&(p=B.nextCwd),B.switchUser&&(n=B.switchUser,p=B.nextCwd??Y(n),a.users.updateSession(i,n,s),l="",c=0)}_();continue}if(V==="\x7F"||V==="\b"){c>0&&(l=`${l.slice(0,c-1)}${l.slice(c)}`,c-=1,_());continue}w(V)}}),t.on("close",()=>{S&&(S.process.kill("SIGTERM"),S=null)})}function ra(e,t){let n=`${Y(t)}/.bash_history`;return e.exists(n)?e.readFile(n).split(`
`).map(i=>i.trim()).filter(i=>i.length>0):(e.writeFile(n,""),[])}function sa(e){return typeof e=="object"&&e!==null&&"vfsInstance"in e&&hi(e.vfsInstance)}function hi(e){if(typeof e!="object"||e===null)return!1;let t=e;return typeof t.restoreMirror=="function"&&typeof t.flushMirror=="function"&&typeof t.writeFile=="function"&&typeof t.readFile=="function"&&typeof t.mkdir=="function"&&typeof t.exists=="function"&&typeof t.stat=="function"&&typeof t.list=="function"&&typeof t.remove=="function"&&typeof t.copy=="function"&&typeof t.move=="function"&&typeof t.touch=="function"}var ia={kernel:"1.0.0+itsrealfortune+1-amd64",os:"Fortune GNU/Linux x64",arch:"x86_64"},yt=pe("VirtualShell");function oa(){let e=y.env.SSH_MIMIC_AUTO_SUDO_NEW_USERS;return e?!["0","false","no","off"].includes(e.toLowerCase()):!0}var Be=class extends fe{vfs;users;packageManager;hostname;properties;startTime;_idle=null;initialized;constructor(t,n,r){super(),yt.mark("constructor"),this.hostname=t,this.properties=n||ia,this.startTime=Date.now(),hi(r)?this.vfs=r:sa(r)?this.vfs=r.vfsInstance:this.vfs=new mt(r??{}),this.users=new ft(this.vfs,oa()),this.packageManager=new pt(this.vfs,this.users);let i=this.vfs,s=this.users,o=this.packageManager,a=this.properties,l=this.hostname,c=this.startTime;this.initialized=(async()=>{await i.restoreMirror(),await s.initialize(),Zs(i,s,l,a,c),o.load(),this.emit("initialized")})()}async ensureInitialized(){yt.mark("ensureInitialized"),await this.initialized}addCommand(t,n,r){let i=t.trim().toLowerCase();if(i.length===0||/\s/.test(i))throw new Error("Command name must be non-empty and contain no spaces");an(ln(i,n,r))}executeCommand(t,n,r){yt.mark("executeCommand"),this._idle?.ping();let i=Q(t,n,this.hostname,"shell",r,this);return this.emit("command",{command:t,user:n,cwd:r}),i}startInteractiveSession(t,n,r,i,s){yt.mark("startInteractiveSession"),this._idle?.ping(),this.emit("session:start",{user:n,sessionId:r,remoteAddress:i}),fi(this.properties,t,n,this.hostname,r,i,s,this),this.refreshProcSessions()}refreshProcFs(){Ut(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}mount(t,n,r={}){this.vfs.mount(t,n,r)}unmount(t){this.vfs.unmount(t)}getMounts(){return this.vfs.getMounts()}refreshProcSessions(){Ut(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}syncPasswd(){yn(this.vfs,this.users)}getVfs(){return this?.vfs??null}getUsers(){return this?.users??null}getHostname(){return this?.hostname}writeFileAsUser(t,n,r){yt.mark("writeFileAsUser"),this.users.assertWriteWithinQuota(t,n,r),this.vfs.writeFile(n,r)}enableIdleManagement(t){this._idle||(this._idle=new ht(this.vfs,t),this._idle.on("freeze",()=>this.emit("shell:freeze")),this._idle.on("thaw",()=>this.emit("shell:thaw")),this._idle.start())}async disableIdleManagement(){this._idle&&(await this._idle.stop(),this._idle=null)}get idleState(){return this._idle?.state??"active"}get idleMs(){return this._idle?.idleMs??0}pingIdle(){this._idle?.ping()}};var wn=!!y.env.DEV_MODE,ky=wn?console.log.bind(console):()=>{},My=wn?console.warn.bind(console):()=>{},Iy=wn?console.error.bind(console):()=>{};var Ny=pe("SftpMimic");var zy=pe("SshMimic"),ua=!!y.env.DEV_MODE,Hy=ua?console.log.bind(console):()=>{};var bt="my-vm",gi=500,je=document.getElementById("output"),ze=document.getElementById("cmd");ze.focus();var yi={30:"#000",31:"#c00",32:"#0c0",33:"#cc0",34:"#00c",35:"#c0c",36:"#0cc",37:"#ccc",90:"#555",91:"#f55",92:"#5f5",93:"#ff5",94:"#55f",95:"#f5f",96:"#5ff",97:"#fff"},Si={40:"#000",41:"#c00",42:"#0c0",43:"#cc0",44:"#00c",45:"#c0c",46:"#0cc",47:"#ccc"};function bi(e){let t="",n=!1,r="",i="";for(let s of e.split(/(\x1b\[[0-9;]*m)/)){let o=s.match(/^\x1b\[([0-9;]*)m$/);if(o)for(let a of o[1].split(";").map(Number))a===0?(n=!1,r="",i=""):a===1?n=!0:yi[a]?r=yi[a]:Si[a]&&(i=Si[a]);else if(s){let a=[r?`color:${r}`:"",i?`background:${i}`:"",n?"font-weight:bold":""].filter(Boolean).join(";"),l=s.replace(/&/g,"&amp;").replace(/</g,"&lt;");t+=a?`<span style="${a}">${l}</span>`:l}}return t}function St(e){je.innerHTML+=bi(e),je.scrollTop=je.scrollHeight}function xi(){let e=De===Y(Se)?"~":De.split("/").at(-1)||"/";je.innerHTML+=bi(Ht(Se,bt,e)),je.scrollTop=je.scrollHeight}var Cn=new Be(bt,void 0,{mode:"fs",snapshotPath:"/vfs-data",flushIntervalMs:1e4}),be=Cn.vfs;await be.restoreMirror();var da=!be.exists("/bin");da&&(await Cn.ensureInitialized(),be.exists("/root")||be.mkdir("/root",448),be.writeFile("/root/README.txt",`Welcome to ${bt}
`),await be.flushMirror());window.addEventListener("beforeunload",()=>{be.flushMirror()});var Se="root",De=Y(Se),He=Le(Se,bt);He.vars.PWD=De;function ma(e){e.switchUser?(Se=e.switchUser,De=e.nextCwd??Y(Se),He.vars.USER=Se,He.vars.LOGNAME=Se,He.vars.HOME=Y(Se),He.vars.PWD=De):e.nextCwd&&(De=e.nextCwd,He.vars.PWD=De)}function vn(){return`${Y(Se)}/.bash_history`}function pa(){try{return be.exists(vn())?be.readFile(vn()).split(`
`).map(e=>e.trim()).filter(e=>e.length>0):[]}catch{return[]}}function fa(){be.writeFile(vn(),he.length>0?`${he.join(`
`)}
`:"")}var he=pa(),Te=-1;xi();ze.addEventListener("keydown",async e=>{if(e.key==="ArrowUp"){e.preventDefault(),Te<he.length-1&&(Te++,ze.value=he[he.length-1-Te]);return}if(e.key==="ArrowDown"){e.preventDefault(),Te>0?(Te--,ze.value=he[he.length-1-Te]):(Te=-1,ze.value="");return}if(e.key!=="Enter")return;let t=ze.value.trim();ze.value="",Te=-1,St(`${t}
`),t&&(he.push(t),he.length>gi&&(he=he.slice(he.length-gi)),fa());try{let n=await Q(t,Se,bt,"shell",De,Cn,void 0,He);if(n.clearScreen&&(je.innerHTML=""),n.stdout&&St(n.stdout),n.stderr&&St(n.stderr),ma(n),await be.flushMirror(),n.closeSession){St(`
Session closed.
`);return}}catch(n){St(`${String(n)}
`)}xi()});
