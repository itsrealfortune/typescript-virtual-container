...
`)+(p.length>0?`
`:""),exitCode:0}}};var Dn={name:"base64",description:"Encode/decode base64",category:"text",params:["[-d] [file]"],run:({args:e,stdin:t})=>{let n=C(e,["-d","--decode"]),r=t??"";if(n)try{return{stdout:Buffer.from(r.trim(),"base64").toString("utf8"),exitCode:0}}catch{return{stderr:"base64: invalid input",exitCode:1}}return{stdout:Buffer.from(r).toString("base64"),exitCode:0}}};var Rn={name:"cat",description:"Concatenate and print files",category:"files",params:["[-n] [-b] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-n","--number"]),o=C(r,["-b","--number-nonblank"]),a=r.filter(m=>!m.startsWith("-"));if(a.length===0&&i!==void 0)return{stdout:i,exitCode:0};if(a.length===0)return{stderr:"cat: missing file operand",exitCode:1};let l=[];for(let m of a){let p=_n(t.vfs,n,m);V(e,p,"cat"),l.push(t.vfs.readFile(p))}let c=l.join("");if(!s&&!o)return{stdout:c,exitCode:0};let u=1;return{stdout:c.split(`
`).map(m=>o&&m.trim()===""?m:`${String(u++).padStart(6)}	${m}`).join(`
`),exitCode:0}}};var Ln={name:"cd",description:"Change directory",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=M(n,r[0]??"~",Y(e));return V(e,i,"cd"),t.vfs.stat(i).type!=="directory"?{stderr:`cd: not a directory: ${i}`,exitCode:1}:{nextCwd:i,exitCode:0}}};function $i(e,t){let n=/^([ugoa]*)([+\-=])([rwx]*)$/,r=t.split(","),i=e;for(let s of r){let o=s.trim().match(n);if(!o)return null;let[,a="a",l,c=""]=o,u=a===""||a==="a"?["u","g","o"]:a.split(""),d={u:{r:256,w:128,x:64},g:{r:32,w:16,x:8},o:{r:4,w:2,x:1}};for(let m of u)for(let p of c.split("")){let g=d[m]?.[p];if(g!==void 0){if(l==="+")i|=g;else if(l==="-")i&=~g;else if(l==="="){let S=Object.values(d[m]??{}).reduce((v,E)=>v|E,0);i=i&~S|g}}}}return i}var Vn={name:"chmod",description:"Change file permissions",category:"files",params:["<mode> <file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let[i,s]=r;if(!i||!s)return{stderr:"chmod: missing operand",exitCode:1};let o=M(n,s);try{if(V(e,o,"chmod"),!t.vfs.exists(o))return{stderr:`chmod: ${s}: No such file or directory`,exitCode:1};let a,l=parseInt(i,8);if(!Number.isNaN(l)&&/^[0-7]+$/.test(i))a=l;else{let c=t.vfs.stat(o).mode,u=$i(c,i);if(u===null)return{stderr:`chmod: invalid mode: ${i}`,exitCode:1};a=u}return t.vfs.chmod(o,a),{exitCode:0}}catch(a){return{stderr:`chmod: ${a instanceof Error?a.message:String(a)}`,exitCode:1}}}};var Un={name:"clear",description:"Clear the terminal screen",category:"shell",params:[],run:()=>({clearScreen:!0,stdout:"",exitCode:0})};var Bn={name:"cp",description:"Copy files or directories",category:"files",params:["[-r] <source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-r","-R","--recursive"]),s=r.filter(u=>!u.startsWith("-")),[o,a]=s;if(!o||!a)return{stderr:"cp: missing operand",exitCode:1};let l=M(n,o),c=M(n,a);try{if(V(e,l,"cp"),V(e,c,"cp"),!t.vfs.exists(l))return{stderr:`cp: ${o}: No such file or directory`,exitCode:1};if(t.vfs.stat(l).type==="directory"){if(!i)return{stderr:`cp: ${o}: is a directory (use -r)`,exitCode:1};let d=(p,g)=>{t.vfs.mkdir(g,493);for(let S of t.vfs.list(p)){let v=`${p}/${S}`,E=`${g}/${S}`;if(t.vfs.stat(v).type==="directory")d(v,E);else{let _=t.vfs.readFileRaw(v);t.writeFileAsUser(e,E,_)}}},m=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c;d(l,m)}else{let d=t.vfs.exists(c)&&t.vfs.stat(c).type==="directory"?`${c}/${o.split("/").pop()}`:c,m=t.vfs.readFileRaw(l);t.writeFileAsUser(e,d,m)}return{exitCode:0}}catch(u){return{stderr:`cp: ${u instanceof Error?u.message:String(u)}`,exitCode:1}}}};var zn={name:"curl",description:"Transfer data from or to a server (pure fetch)",category:"network",params:["[options] <url>"],run:async({authUser:e,cwd:t,args:n,shell:r})=>{let{flagsWithValues:i,positionals:s}=ce(n,{flagsWithValue:["-o","--output","-X","--request","-d","--data","-H","--header","-u","--user"]});if(C(n,["--help","-h"]))return{stdout:["Usage: curl [options] <url>","  -o, --output <file>    Write to file","  -X, --request <method> HTTP method","  -d, --data <data>      POST data","  -H, --header <hdr>     Extra header","  -s, --silent           Silent mode","  -I, --head             Fetch headers only","  -L, --location         Follow redirects","  -v, --verbose          Verbose"].join(`
...
${a}`,exitCode:0}}};var Gn={name:"diff",description:"Compare files line by line",category:"text",params:["<file1> <file2>"],run:({shell:e,cwd:t,args:n})=>{let[r,i]=n;if(!r||!i)return{stderr:"diff: missing operand",exitCode:1};let s=M(t,r),o=M(t,i),a,l;try{a=e.vfs.readFile(s).split(`
...
`)}
`:"",exitCode:_.length>0?0:1}}};var or={name:"groups",description:"Print group memberships",category:"system",params:["[user]"],run:({authUser:e,shell:t,args:n})=>{let r=n[0]??e;return{stdout:t.users.isSudoer(r)?`${r} sudo root`:r,exitCode:0}}};var ar={name:"gzip",description:"Compress files",category:"archive",params:["[-k] [-d] <file>"],run:({shell:e,cwd:t,args:n})=>{if(!e.packageManager.isInstalled("gzip"))return{stderr:`bash: gzip: command not found
...
`,exitCode:1};let o=e.vfs.readFile(s),a=s.slice(0,-3);return e.vfs.writeFile(a,o),r||e.vfs.remove(s),{exitCode:0}}};var cr={name:"head",description:"Output first lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=le(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
...
`),exitCode:0}}};var ur=["navigation","files","text","archive","system","package","network","shell","users","misc"],mr={navigation:"Navigation",files:"Files & Filesystem",text:"Text Processing",archive:"Archive & Compression",system:"System",package:"Package Management",network:"Network",shell:"Shell & Scripting",users:"Users & Permissions",misc:"Miscellaneous"},pr="\x1B[1m",Ee="\x1B[0m",Ai="\x1B[36m",_i="\x1B[33m",rt="\x1B[2m",Oi="\x1B[32m";function dr(e,t){return e.length>=t?e:e+" ".repeat(t-e.length)}function Ti(e){let t=e.aliases?.length?` ${rt}(${e.aliases.join(", ")})${Ee}`:"";return`  ${Ai}${dr(e.name,16)}${Ee}${t}${dr("",(e.aliases?.length,0))} ${e.description??""}`}function Fi(e){let t={};for(let s of e){let o=s.category??"misc";t[o]||(t[o]=[]),t[o].push(s)}let n=[`${pr}Available commands${Ee}`,`${rt}Type 'help <command>' for detailed usage.${Ee}`,""],r=[...ur.filter(s=>t[s]),...Object.keys(t).filter(s=>!ur.includes(s)).sort()];for(let s of r){let o=t[s];if(!o?.length)continue;n.push(`${_i}${mr[s]??s}${Ee}`);let a=[...o].sort((l,c)=>l.name.localeCompare(c.name));for(let l of a)n.push(Ti(l));n.push("")}let i=e.length;return n.push(`${rt}${i} commands available.${Ee}`),n.join(`
`)}function Di(e){let t=[];if(t.push(`${pr}${e.name}${Ee} \u2014 ${e.description??"no description"}`),e.aliases?.length&&t.push(`${rt}Aliases: ${e.aliases.join(", ")}${Ee}`),t.push(""),t.push(`${Oi}Usage:${Ee}`),e.params.length)for(let r of e.params)t.push(`  ${e.name} ${r}`);else t.push(`  ${e.name}`);let n=mr[e.category??"misc"]??e.category??"misc";return t.push(""),t.push(`${rt}Category: ${n}${Ee}`),t.join(`
`)}function fr(e){return{name:"help",description:"List all commands, or show usage for a specific command",category:"shell",params:["[command]"],run:({args:t})=>{let n=Xt();if(t[0]){let r=t[0].toLowerCase(),i=n.find(s=>s.name===r||s.aliases?.includes(r));return i?{stdout:Di(i),exitCode:0}:{stderr:`help: no help entry for '${t[0]}'`,exitCode:1}}return{stdout:Fi(n),exitCode:0}}}}var hr={name:"history",description:"Display command history",category:"shell",params:["[n]"],run:({args:e,shell:t,authUser:n})=>{let r=`/home/${n}/.bash_history`;if(!t.vfs.exists(r))return{stdout:"",exitCode:0};let s=t.vfs.readFile(r).split(`
...
`),exitCode:0}}};var gr={name:"hostname",description:"Print hostname",category:"system",params:[],run:({hostname:e})=>({stdout:e,exitCode:0})};var yr={name:"htop",description:"System monitor",category:"system",params:[],run:({mode:e})=>e==="exec"?{stderr:"htop: interactive terminal required",exitCode:1}:{openHtop:!0,exitCode:0}};var Sr={name:"id",description:"Print user identity",category:"system",params:["[user]"],run:({authUser:e,shell:t,args:n})=>{let r=n[0]??e,i=r==="root"?0:1e3,s=i,a=t.users.isSudoer(r)?`${s}(${r}),0(root)`:`${s}(${r})`;return{stdout:`uid=${i}(${r}) gid=${s}(${r}) groups=${a}`,exitCode:0}}};var br={name:"kill",description:"Send signal to process",category:"system",params:["[-9] <pid>"],run:({args:e})=>e.find(n=>!n.startsWith("-"))?{stdout:"",exitCode:0}:{stderr:"kill: no pid specified",exitCode:1}};var xr={name:"ln",description:"Create links",category:"files",params:["[-s] <target> <link_name>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-s","--symbolic"]),s=r.filter(u=>!u.startsWith("-")),[o,a]=s;if(!o||!a)return{stderr:"ln: missing operand",exitCode:1};let l=M(n,a),c=i?o:M(n,o);try{if(V(e,l,"ln"),i)t.vfs.symlink(c,l);else{let u=M(n,o);if(V(e,u,"ln"),!t.vfs.exists(u))return{stderr:`ln: ${o}: No such file or directory`,exitCode:1};let d=t.vfs.readFile(u);t.writeFileAsUser(e,l,d)}return{exitCode:0}}catch(u){return{stderr:`ln: ${u instanceof Error?u.message:String(u)}`,exitCode:1}}}},wr={name:"readlink",description:"Print resolved path of symbolic link",category:"files",params:["[-f] <path>"],run:({shell:e,cwd:t,args:n})=>{let r=n.includes("-f")||n.includes("-e"),i=n.find(a=>!a.startsWith("-"));if(!i)return{stderr:`readlink: missing operand
...
`,exitCode:1}}};var vr={name:"seq",description:"Print a sequence of numbers",category:"text",params:["[FIRST [INCREMENT]] LAST"],run:({args:e})=>{let t=e.filter(d=>!d.startsWith("-")||/^-[\d.]/.test(d)).map(Number),n=(()=>{let d=e.indexOf("-s");return d!==-1?e[d+1]??`
`:`
...
`,exitCode:0}}};var Cr={name:"stat",description:"Display file status",category:"files",params:["[-c <format>] <file>"],run:({shell:e,cwd:t,args:n})=>{let r=n.findIndex(E=>E==="-c"||E==="--format"),i=r!==-1?n[r+1]:void 0,s=n.find(E=>!E.startsWith("-")&&E!==i);if(!s)return{stderr:`stat: missing operand
...
`)}
`,exitCode:0}}};var Ri="\x1B[0m",Li="\x1B[1;34m",Vi="\x1B[1;36m",Ui="\x1B[1;32m",Bi="",zi="\x1B[30;42m",Hi="\x1B[37;44m",ji="\x1B[34;42m";function Ke(e,t){return t?`${t}${e}${Ri}`:e}function tn(e,t,n){if(n)return Vi;if(t==="directory"){let r=!!(e&512),i=!!(e&2);return r&&i?zi:r?Hi:i?ji:Li}return e&73?Ui:Bi}function Pr(e,t,n){let r;n?r="l":t==="directory"?r="d":r="-";let i=c=>e&c?"r":"-",s=c=>e&c?"w":"-",o=(()=>{let c=!!(e&64);return e&2048?c?"s":"S":c?"x":"-"})(),a=(()=>{let c=!!(e&8);return e&1024?c?"s":"S":c?"x":"-"})(),l=(()=>{let c=!!(e&1);return t==="directory"&&e&512?c?"t":"T":c?"x":"-"})();return`${r}${i(256)}${s(128)}${o}${i(32)}${s(16)}${a}${i(4)}${s(2)}${l}`}var Wi=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function en(e){let t=new Date,n=4320*3600*1e3,r=Math.abs(t.getTime()-e.getTime())<n,i=String(e.getDate()).padStart(2," "),s=Wi[e.getMonth()]??"";if(r){let o=String(e.getHours()).padStart(2,"0"),a=String(e.getMinutes()).padStart(2,"0");return`${i} ${s.padEnd(3)} ${o}:${a}`}return`${i} ${s.padEnd(3)} ${e.getFullYear()}`}function Mt(e,t){try{return e.readFile(t)}catch{return"?"}}function qi(e,t,n){let r=t==="/"?"":t;return n.map(i=>{let s=`${r}/${i}`,o=e.isSymlink(s),a;try{a=e.stat(s)}catch{return i}let l=tn(a.mode,a.type,o);return Ke(i,l)}).join("  ")}function Ki(e,t,n){let r=t==="/"?"":t,i=n.map(d=>{let m=`${r}/${d}`,p=e.isSymlink(m),g;try{g=e.stat(m)}catch{return{perms:"----------",nlink:"1",size:"0",date:en(new Date),label:d}}let S=p?41471:g.mode,v=Pr(S,g.type,p),E=g.type==="directory"?String((g.childrenCount??0)+2):"1",T=p?Mt(e,m).length:g.type==="file"?g.size??0:(g.childrenCount??0)*4096,_=String(T),P=en(g.updatedAt),N=tn(S,g.type,p),I=p?`${Ke(d,N)} -> ${Mt(e,m)}`:Ke(d,N);return{perms:v,nlink:E,size:_,date:P,label:I}}),s=Math.max(...i.map(d=>d.nlink.length)),o=Math.max(...i.map(d=>d.size.length)),a="root",l="root",c=n.length*8,u=i.map(d=>`${d.perms} ${d.nlink.padStart(s)} ${a} ${l} ${d.size.padStart(o)} ${d.date} ${d.label}`);return`total ${c}
${u.join(`
`)}`}var Er={name:"ls",description:"List directory contents",category:"navigation",params:["[-la] [path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=C(r,["-l","--long","-la","-al"]),s=C(r,["-a","--all","-la","-al"]),o=Ie(r,0,{flags:["-l","--long","-a","--all","-la","-al"]}),a=M(n,o??n);if(V(e,a,"ls"),t.vfs.exists(a)){let u=t.vfs.stat(a),d=t.vfs.isSymlink(a);if(u.type==="file"||d){let m=a.split("/").pop()??a,p=tn(d?41471:u.mode,u.type,d);if(i){let g=d?41471:u.mode,S=d?Mt(t.vfs,a).length:u.size??0,v=Pr(g,u.type,d),E=d?`${Ke(m,p)} -> ${Mt(t.vfs,a)}`:Ke(m,p);return{stdout:`${v} 1 root root ${S} ${en(u.updatedAt)} ${E}
`,exitCode:0}}return{stdout:`${Ke(m,p)}
`,exitCode:0}}}let l=t.vfs.list(a).filter(u=>s||!u.startsWith("."));return{stdout:`${i?Ki(t.vfs,a,l):qi(t.vfs,a,l)}
`,exitCode:0}}};var $r={name:"lsb_release",description:"Print distribution-specific information",category:"system",params:["[-a] [-i] [-d] [-r] [-c]"],run:({args:e,shell:t})=>{let n=t.properties?.os??"Fortune GNU/Linux x64",r="nyx",i="1.0";try{let d=t.vfs.readFile("/etc/os-release");for(let m of d.split(`
...
`),exitCode:0}}};var kr={adduser:`ADDUSER(8)                User Commands                ADDUSER(8)
...
DESCRIPTION
       Read items from stdin and execute COMMAND with those items as arguments.`};var Gi={gunzip:"gzip"},Mr={name:"man",description:"Interface to the system reference manuals",category:"shell",params:["<command>"],run:async({args:e,shell:t})=>{let n=e[0];if(!n)return{stderr:"What manual page do you want?",exitCode:1};let r=`/usr/share/man/man1/${n}.1`;if(t.vfs.exists(r))return{stdout:t.vfs.readFile(r),exitCode:0};let i=n.toLowerCase(),s=Gi[i]??i,o=kr[s]??null;return o?{stdout:o,exitCode:0}:{stderr:`No manual entry for ${n}`,exitCode:16}}};var Ir={name:"mkdir",description:"Make directories",category:"files",params:["<dir>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"mkdir: missing operand",exitCode:1};for(let i=0;i<r.length;i++){let s=Ie(r,i);if(!s)return{stderr:"mkdir: missing operand",exitCode:1};let o=M(n,s);V(e,o,"mkdir"),t.vfs.mkdir(o)}return{exitCode:0}}};var Nr={name:"mv",description:"Move or rename files",category:"files",params:["<source> <dest>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r.filter(c=>!c.startsWith("-")),[s,o]=i;if(!s||!o)return{stderr:"mv: missing operand",exitCode:1};let a=M(n,s),l=M(n,o);try{if(V(e,a,"mv"),V(e,l,"mv"),!t.vfs.exists(a))return{stderr:`mv: ${s}: No such file or directory`,exitCode:1};let c=t.vfs.exists(l)&&t.vfs.stat(l).type==="directory"?`${l}/${s.split("/").pop()}`:l;return t.vfs.move(a,c),{exitCode:0}}catch(c){return{stderr:`mv: ${c instanceof Error?c.message:String(c)}`,exitCode:1}}}};var Ar={name:"nano",description:"Text editor",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=r[0];if(!i)return{stderr:"nano: missing file operand",exitCode:1};let s=M(n,i);V(e,s,"nano");let o=t.vfs.exists(s)?t.vfs.readFile(s):"",a=z.basename(s)||"buffer",l=`/tmp/sshmimic-nano-${Date.now()}-${a}.tmp`;return{openEditor:{targetPath:s,tempPath:l,initialContent:o},exitCode:0}}};var Yi="vfs-fs-shim",_e="files",It=null;function nn(){return It?Promise.resolve(It):new Promise((e,t)=>{let n=indexedDB.open(Yi,1);n.onupgradeneeded=r=>r.target.result.createObjectStore(_e),n.onsuccess=r=>{It=r.target.result,e(It)},n.onerror=r=>t(r.target.error)})}var oe=new Map;nn().then(e=>{let n=e.transaction(_e,"readonly").objectStore(_e).openCursor();n.onsuccess=r=>{let i=r.target.result;i&&(oe.set(i.key,i.value),i.continue())}});function Ge(e,t){nn().then(n=>{let r=n.transaction(_e,"readwrite");t===null?r.objectStore(_e).delete(e):r.objectStore(_e).put(t,e)})}function Ji(e,t="utf8"){if(e instanceof Uint8Array)return e;if(typeof e=="string"){if(t==="hex"){let n=new Uint8Array(e.length/2);for(let r=0;r<n.length;r++)n[r]=parseInt(e.slice(r*2,r*2+2),16);return n}return new TextEncoder().encode(e)}return new Uint8Array(e)}function Zi(e,t="utf8"){return!t||t==="utf8"?new TextDecoder().decode(e):t==="hex"?Array.from(e).map(n=>n.toString(16).padStart(2,"0")).join(""):t==="base64"?btoa(String.fromCharCode(...e)):new TextDecoder().decode(e)}function ee(e){return oe.has(e)}function fe(e,t){if(!oe.has(e))throw Object.assign(new Error(`ENOENT: no such file: ${e}`),{code:"ENOENT"});let n=oe.get(e);if(n==="__DIR__")throw Object.assign(new Error(`EISDIR: ${e}`),{code:"EISDIR"});let r=typeof t=="string"?t:t?.encoding;return r?Zi(n,r):globalThis.Buffer.from(n)}function Ye(e,t,n){let r=typeof n=="string"?n:n?.encoding,i=Ji(t,r);oe.set(e,i),Ge(e,i)}function st(e){oe.delete(e),Ge(e,null)}function _r(e,t={}){if(t.recursive)for(let n of[...oe.keys()])(n===e||n.startsWith(e+"/"))&&(oe.delete(n),Ge(n,null));else st(e)}function Je(e,t={}){if(t.recursive){let n=e.split("/").filter(Boolean),r="";for(let i of n)r+="/"+i,oe.has(r)||(oe.set(r,"__DIR__"),Ge(r,"__DIR__"))}else oe.set(e,"__DIR__"),Ge(e,"__DIR__")}function it(e){let t=e.endsWith("/")?e:e+"/";return[...oe.keys()].filter(n=>n.startsWith(t)&&n.slice(t.length).split("/").length===1).map(n=>n.slice(t.length))}function At(e){if(!oe.has(e))throw Object.assign(new Error(`ENOENT: ${e}`),{code:"ENOENT"});let t=oe.get(e),n=t==="__DIR__";return{isDirectory:()=>n,isFile:()=>!n,size:n?0:t.length}}var Nt=new Map,Qi=10,ot={O_WRONLY:1,O_CREAT:64,O_APPEND:1024};function Or(e,t){let n=Qi++,r=(t&ot.O_APPEND)!==0,i=oe.has(e)?oe.get(e):new Uint8Array(0);return Nt.set(n,{path:e,data:r?i:new Uint8Array(0)}),n}function Tr(e,t){let n=Nt.get(e);if(!n)return;let r=new Uint8Array(n.data.length+t.length);r.set(n.data),r.set(t,n.data.length),n.data=r}function Fr(e){let t=Nt.get(e);t&&(oe.set(t.path,t.data),Ge(t.path,t.data),Nt.delete(e))}var Xi=nn().then(e=>new Promise(t=>{let r=e.transaction(_e,"readonly").objectStore(_e).openCursor();r.onsuccess=i=>{let s=i.target.result;if(!s)return t(!0);oe.set(s.key,s.value),s.continue()}}));globalThis.__fsReady__=Xi;function eo(e){let t=Math.max(1,Math.floor(e/60)),n=Math.floor(t/1440),r=Math.floor(t%1440/60),i=t%60,s=[];return n>0&&s.push(`${n} day${n>1?"s":""}`),r>0&&s.push(`${r} hour${r>1?"s":""}`),(i>0||s.length===0)&&s.push(`${i} min${i>1?"s":""}`),s.join(", ")}function Rr(e){return`\x1B[${e}m   \x1B[0m`}function to(){let e=[40,41,42,43,44,45,46,47].map(Rr).join(""),t=[100,101,102,103,104,105,106,107].map(Rr).join("");return[e,t]}function Lr(e,t,n){if(e.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s=n<=1?0:t/(n-1),o=Math.round(r.r+(i.r-r.r)*s),a=Math.round(r.g+(i.g-r.g)*s),l=Math.round(r.b+(i.b-r.b)*s);return`\x1B[38;2;${o};${a};${l}m${e}\x1B[0m`}function no(e){if(e.trim().length===0)return e;let t=e.indexOf(":");if(t===-1)return e.includes("@")?Vr(e):e;let n=e.substring(0,t+1),r=e.substring(t+1);return Vr(n)+r}function Vr(e){let t=new RegExp("\x1B\\[[\\d;]*m","g"),n=e.replace(t,"");if(n.trim().length===0)return e;let r={r:255,g:255,b:255},i={r:168,g:85,b:247},s="";for(let o=0;o<n.length;o+=1){let a=n.length<=1?0:o/(n.length-1),l=Math.round(r.r+(i.r-r.r)*a),c=Math.round(r.g+(i.g-r.g)*a),u=Math.round(r.b+(i.b-r.b)*a);s+=`\x1B[38;2;${l};${c};${u}m${n[o]}\x1B[0m`}return s}function Ur(e){return Math.max(0,Math.round(e/(1024*1024)))}function Br(){try{let e=fe("/etc/os-release","utf8");for(let t of e.split(`
`)){if(!t.startsWith("PRETTY_NAME="))continue;return t.slice(12).trim().replace(/^"|"$/g,"")}}catch{return}}function zr(e){try{let t=fe(e,"utf8").split(`
`)[0]?.trim();return!t||t.length===0?void 0:t}catch{return}}function ro(e){let t=zr("/sys/devices/virtual/dmi/id/sys_vendor"),n=zr("/sys/devices/virtual/dmi/id/product_name");return t&&n?`${t} ${n}`:n||e}function so(){let e=["/var/lib/dpkg/status","/usr/local/var/lib/dpkg/status"];for(let t of e)if(ee(t))try{return fe(t,"utf8").match(/^Package:\s+/gm)?.length??0}catch{}}function io(){let e=["/snap","/var/lib/snapd/snaps"];for(let t of e)if(ee(t))try{return it(t,{withFileTypes:!0}).filter(i=>i.isDirectory()).length}catch{}}function oo(){let e=so(),t=io();return e!==void 0&&t!==void 0?`${e} (dpkg), ${t} (snap)`:e!==void 0?`${e} (dpkg)`:t!==void 0?`${t} (snap)`:"n/a"}function ao(){let e=$t();if(e.length===0)return"unknown";let t=e[0];if(!t)return"unknown";let n=(t.speed/1e3).toFixed(2);return`${t.model} (${e.length}) @ ${n}GHz`}function lo(e){return!e||e.trim().length===0?"unknown":z.basename(e.trim())}function co(e){let t=Pe(),n=Ue(),r=Math.max(0,t-n),i=e.shellProps,s=y.uptime();return e.uptimeSeconds===void 0&&(e.uptimeSeconds=Math.round(s)),{user:e.user,host:e.host,osName:i?.os??e.osName??`${Br()??Jt()} ${kt()}`,kernel:i?.kernel??e.kernel??Zt(),uptimeSeconds:e.uptimeSeconds??rr(),packages:e.packages??oo(),shell:lo(e.shell),shellProps:e.shellProps??{kernel:e.kernel??Zt(),os:e.osName??`${Br()??Jt()} ${kt()}`,arch:kt()},resolution:e.resolution??"n/a (ssh)",terminal:e.terminal??"unknown",cpu:e.cpu??ao(),gpu:e.gpu??"n/a",memoryUsedMiB:e.memoryUsedMiB??Ur(r),memoryTotalMiB:e.memoryTotalMiB??Ur(t)}}function Hr(e){let t=co(e),n=eo(t.uptimeSeconds),r=to(),i=["                               .. .:.    "," .::..                       ..     ..   ",".    ....                  ...       ..  ",":       ....             .:.          .. ",":           .:.:........:.            .. ",":                                     .. ",".                                      : ",".                                      : ","..                                     : "," :.                                   .. "," ..                                   .. "," :-.                                  :: ","  .:.                                 :. ","   ..:                               ... ","   ..:                               :.. ","  :...                              :....","   ..                                ....","   .                                  .. ","  .:.                                 .: ","  :.                                  .. "," ::.                                 ..  ",".....                          ..:...    ","...:.                         ..         ",".:...:.       ::.           ..           ","  ... ..:::::..  ..:::::::..             "],s=[`${t.user}@${t.host}`,"-------------------------",`OS: ${t.osName}`,`Host: ${ro(t.host)}`,`Kernel: ${t.kernel}`,`Uptime: ${n}`,`Packages: ${t.packages}`,`Shell: ${t.shell}`,`Resolution: ${t.resolution}`,`Terminal: ${t.terminal}`,`CPU: ${t.cpu}`,`GPU: ${t.gpu}`,`Memory: ${t.memoryUsedMiB}MiB / ${t.memoryTotalMiB}MiB`,"",r[0],r[1]],o=Math.max(i.length,s.length),a=[];for(let l=0;l<o;l+=1){let c=i[l]??"",u=s[l]??"";if(u.length>0){let d=Lr(c.padEnd(31," "),l,i.length),m=no(u);a.push(`${d}  ${m}`);continue}a.push(Lr(c,l,i.length))}return a.join(`
`)}var jr={name:"neofetch",description:"System info display",category:"system",params:["[--off]"],run:({args:e,authUser:t,hostname:n,shell:r,env:i})=>r.packageManager.isInstalled("neofetch")?C(e,"--help")?{stdout:"Usage: neofetch [--off]",exitCode:0}:C(e,"--off")?{stdout:`${t}@${n}`,exitCode:0}:{stdout:Hr({user:t,host:n,shell:i.vars.SHELL,shellProps:r.properties,terminal:i.vars.TERM,uptimeSeconds:Math.floor((Date.now()-r.startTime)/1e3),packages:`${r.packageManager?.installedCount()??0} (dpkg)`}),exitCode:0}:{stderr:`bash: neofetch: command not found
Hint: install it with: apt install neofetch
`,exitCode:127}};function _t(e,t){let n=new Function("exports","require","module","__filename","__dirname",e),r={exports:{}};return n(r.exports,()=>{throw new Error("require not supported in vm shim")},r,"",""),r.exports}var Ot="v18.19.0",Wr={node:Ot,npm:"9.2.0",v8:"10.2.154.26-node.22"};function uo(e,t){let n={version:Ot,versions:Wr,platform:"linux",arch:"x64",env:{NODE_ENV:"production",HOME:"/root",PATH:"/usr/local/bin:/usr/bin:/bin"},argv:["node"],stdout:{write:s=>(e.push(s),!0)},stderr:{write:s=>(t.push(s),!0)},exit:(s=0)=>{throw new Tt(s)},cwd:()=>"/root",hrtime:()=>[0,0]},r={log:(...s)=>e.push(s.map($e).join(" ")),error:(...s)=>t.push(s.map($e).join(" ")),warn:(...s)=>t.push(s.map($e).join(" ")),info:(...s)=>e.push(s.map($e).join(" ")),dir:s=>e.push($e(s))},i=s=>{switch(s){case"path":return{join:(...o)=>o.join("/").replace(/\/+/g,"/"),resolve:(...o)=>`/${o.join("/").replace(/^\/+/,"")}`,dirname:o=>o.split("/").slice(0,-1).join("/")||"/",basename:o=>o.split("/").pop()??"",extname:o=>{let a=o.split("/").pop()??"",l=a.lastIndexOf(".");return l>0?a.slice(l):""},sep:"/",delimiter:":"};case"os":return{platform:()=>"linux",arch:()=>"x64",type:()=>"Linux",hostname:()=>"fortune-vm",homedir:()=>"/root",tmpdir:()=>"/tmp",EOL:`
`};case"util":return{format:(...o)=>o.map($e).join(" "),inspect:o=>$e(o)};case"fs":case"fs/promises":throw new Error(`Cannot require '${s}': filesystem access not available in virtual runtime`);case"child_process":case"net":case"http":case"https":throw new Error(`Cannot require '${s}': not available in virtual runtime`);default:throw new Error(`Cannot find module '${s}'`)}};return i.resolve=s=>{throw new Error(`Cannot resolve '${s}'`)},i.cache={},i.extensions={},_t.createContext({console:r,process:n,require:i,Math,JSON,Object,Array,String,Number,Boolean,Symbol,Date,RegExp,Error,TypeError,RangeError,SyntaxError,Promise,Map,Set,WeakMap,WeakSet,parseInt,parseFloat,isNaN,isFinite,encodeURIComponent,decodeURIComponent,encodeURI,decodeURI,setTimeout:()=>{},clearTimeout:()=>{},setInterval:()=>{},clearInterval:()=>{},queueMicrotask:()=>{},globalThis:void 0,undefined:void 0,Infinity:1/0,NaN:NaN})}var Tt=class{constructor(t){this.code=t}code};function $e(e){if(e===null)return"null";if(e===void 0)return"undefined";if(typeof e=="string")return e;if(typeof e=="function")return`[Function: ${e.name||"(anonymous)"}]`;if(Array.isArray(e))return`[ ${e.map($e).join(", ")} ]`;if(e instanceof Error)return`${e.name}: ${e.message}`;if(typeof e=="object")try{return`{ ${Object.entries(e).map(([n,r])=>`${n}: ${$e(r)}`).join(", ")} }`}catch{return"[Object]"}return String(e)}function Ft(e){let t=[],n=[],r=uo(t,n),i=0;try{let s=_t.runInContext(e,r,{timeout:5e3});s!==void 0&&t.length===0&&t.push($e(s))}catch(s){s instanceof Tt?i=s.code:s instanceof Error?(n.push(`${s.name}: ${s.message}`),i=1):(n.push(String(s)),i=1)}return{stdout:t.length?`${t.join(`
`)}
`:"",stderr:n.length?`${n.join(`
`)}
`:"",exitCode:i}}function mo(e){let t=e.trim();return!t.includes(`
`)&&!t.startsWith("const ")&&!t.startsWith("let ")&&!t.startsWith("var ")&&!t.startsWith("function ")&&!t.startsWith("class ")&&!t.startsWith("if ")&&!t.startsWith("for ")&&!t.startsWith("while ")&&!t.startsWith("import ")&&!t.startsWith("//")?Ft(t):Ft(`(async () => { ${e} })()`)}var qr={name:"node",description:"JavaScript runtime (virtual)",category:"system",params:["[--version] [-e <expr>] [-p <expr>] [file]"],run:({args:e,shell:t,cwd:n})=>{if(!t.packageManager.isInstalled("nodejs"))return{stderr:`bash: node: command not found
Hint: install it with: apt install nodejs
`,exitCode:127};if(C(e,["--version","-v"]))return{stdout:`${Ot}
`,exitCode:0};if(C(e,["--versions"]))return{stdout:`${JSON.stringify(Wr,null,2)}
...
`,exitCode:1};let a=t.vfs.readFile(o),{stdout:l,stderr:c,exitCode:u}=mo(a);return{stdout:l||void 0,stderr:c||void 0,exitCode:u}}return{stdout:[`Welcome to Node.js ${Ot}.`,'Type ".exit" to exit the REPL.',"> "].join(`
`),exitCode:0}}};var Dt="9.2.0",po="18.19.0",Kr={name:"npm",description:"Node.js package manager (virtual)",category:"system",params:["<command> [args]"],run:({args:e,shell:t})=>{if(!t.packageManager.isInstalled("npm"))return{stderr:`bash: npm: command not found
Hint: install it with: apt install npm
`,exitCode:127};if(C(e,["--version","-v"]))return{stdout:`${Dt}
`,exitCode:0};let n=e[0]?.toLowerCase();switch(n){case"version":case"-version":return{stdout:`{ npm: '${Dt}', node: '${po}', v8: '10.2.154.26' }
...
`,exitCode:1}}}},Gr={name:"npx",description:"Node.js package runner (virtual)",category:"system",params:["<package> [args]"],run:({args:e,shell:t})=>t.packageManager.isInstalled("npm")?C(e,["--version"])?{stdout:`${Dt}
...
`),exitCode:0}}};var Xr={name:"pwd",description:"Print working directory",category:"navigation",params:[],run:({cwd:e})=>({stdout:e,exitCode:0})};var ho="Python 3.11.2";var Rt="3.11.2 (default, Mar 13 2023, 12:18:29) [GCC 12.2.0]",b={__pytype__:"none"};function Z(e=[]){return{__pytype__:"dict",data:new Map(e)}}function rn(e,t,n=1){return{__pytype__:"range",start:e,stop:t,step:n}}function G(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="dict"}function Qe(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="range"}function ke(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="func"}function sn(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="class"}function at(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="instance"}function Ae(e){return!!e&&typeof e=="object"&&!Array.isArray(e)&&e.__pytype__==="none"}function ne(e){return e===null||Ae(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?`'${e.replace(/'/g,"\\'")}'`:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:G(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Qe(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:ke(e)?`<function ${e.name} at 0x...>`:sn(e)?`<class '${e.name}'>`:at(e)?`<${e.cls.name} object at 0x...>`:String(e)}function R(e){return e===null||Ae(e)?"None":e===!0?"True":e===!1?"False":typeof e=="number"?Number.isInteger(e)?String(e):e.toPrecision(12).replace(/\.?0+$/,""):typeof e=="string"?e:Array.isArray(e)?`[${e.map(ne).join(", ")}]`:G(e)?`{${[...e.data.entries()].map(([t,n])=>`'${t}': ${ne(n)}`).join(", ")}}`:Qe(e)?`range(${e.start}, ${e.stop}${e.step!==1?`, ${e.step}`:""})`:ne(e)}function de(e){return e===null||Ae(e)?!1:typeof e=="boolean"?e:typeof e=="number"?e!==0:typeof e=="string"||Array.isArray(e)?e.length>0:G(e)?e.data.size>0:Qe(e)?ts(e)>0:!0}function ts(e){if(e.step===0)return 0;let t=Math.ceil((e.stop-e.start)/e.step);return Math.max(0,t)}function go(e){let t=[];for(let n=e.start;(e.step>0?n<e.stop:n>e.stop)&&(t.push(n),!(t.length>1e4));n+=e.step);return t}function te(e){if(Array.isArray(e))return e;if(typeof e=="string")return[...e];if(Qe(e))return go(e);if(G(e))return[...e.data.keys()];throw new J("TypeError",`'${Be(e)}' object is not iterable`)}function Be(e){return e===null||Ae(e)?"NoneType":typeof e=="boolean"?"bool":typeof e=="number"?Number.isInteger(e)?"int":"float":typeof e=="string"?"str":Array.isArray(e)?"list":G(e)?"dict":Qe(e)?"range":ke(e)?"function":sn(e)?"type":at(e)?e.cls.name:"object"}var J=class{constructor(t,n){this.type=t;this.message=n}type;message;toString(){return`${this.type}: ${this.message}`}},Ze=class{constructor(t){this.value=t}value},lt=class{},ct=class{},ut=class{constructor(t){this.code=t}code};function yo(e){let t=new Map,n=Z([["sep","/"],["linesep",`
`],["curdir","."],["pardir",".."]]);return n.__methods__={getcwd:()=>e,getenv:r=>typeof r=="string"?y.env[r]??b:b,path:Z([["join",b],["exists",b],["dirname",b],["basename",b]]),listdir:()=>[]},t.set("__builtins__",b),t.set("__name__","__main__"),t.set("__cwd__",e),t}function So(e){let t=Z([["sep","/"],["curdir","."]]),n=Z([["sep","/"],["linesep",`
`],["name","posix"]]);return n._cwd=e,t._cwd=e,n.path=t,n}function bo(){return Z([["version",Rt],["version_info",Z([["major",3],["minor",11],["micro",2]].map(([e,t])=>[e,t]))],["platform","linux"],["executable","/usr/bin/python3"],["prefix","/usr"],["path",["/usr/lib/python3.11","/usr/lib/python3.11/lib-dynload"]],["argv",[""]],["maxsize",9007199254740991]])}function xo(){return Z([["pi",Math.PI],["e",Math.E],["tau",Math.PI*2],["inf",1/0],["nan",NaN],["sqrt",b],["floor",b],["ceil",b],["log",b],["pow",b],["sin",b],["cos",b],["tan",b],["fabs",b],["factorial",b]])}function wo(){return Z([["dumps",b],["loads",b]])}function vo(){return Z([["match",b],["search",b],["findall",b],["sub",b],["split",b],["compile",b]])}var es={os:So,sys:()=>bo(),math:()=>xo(),json:()=>wo(),re:()=>vo(),random:()=>Z([["random",b],["randint",b],["choice",b],["shuffle",b]]),time:()=>Z([["time",b],["sleep",b],["ctime",b]]),datetime:()=>Z([["datetime",b],["date",b],["timedelta",b]]),collections:()=>Z([["Counter",b],["defaultdict",b],["OrderedDict",b]]),itertools:()=>Z([["chain",b],["product",b],["combinations",b],["permutations",b]]),functools:()=>Z([["reduce",b],["partial",b],["lru_cache",b]]),string:()=>Z([["ascii_letters","abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"],["digits","0123456789"],["punctuation","!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"]])},Lt=class{constructor(t){this.cwd=t}cwd;output=[];stderr=[];modules=new Map;getOutput(){return this.output.join(`
`)+(this.output.length?`
...
`);else if(i.type==="arith"){let s=i.expr.trim(),o=s.match(/^(\w+)\s*(\+\+|--)$/);if(o){let a=parseInt(t.env.vars[o[1]]??"0",10);t.env.vars[o[1]]=String(o[2]==="++"?a+1:a-1)}else{let a=s.match(/^(\w+)\s*([+\-*/])=\s*(.+)$/);if(a){let l=parseInt(t.env.vars[a[1]]??"0",10),c=parseInt(a[3],10),u={"+":l+c,"-":l-c,"*":l*c,"/":Math.floor(l/c)};t.env.vars[a[1]]=String(u[a[2]]??l)}else{let l=Yt(s,t.env.vars);Number.isNaN(l)||(t.env.lastExitCode=l===0?1:0)}}}else if(i.type==="for"){let o=(await an(i.list,t.env.vars,t.env.lastExitCode,t)).trim().split(/\s+/).flatMap(Pt);for(let a of o){t.env.vars[i.var]=a;let l=await Te(Oe(i.body),t);if(l.stdout&&(r+=`${l.stdout}
`),l.closeSession)return l}}else if(i.type==="while"){let s=0;for(;s<1e3&&await on(i.cond,t);){let o=await Te(Oe(i.body),t);if(o.stdout&&(r+=`${o.stdout}
`),o.closeSession)return o;s++}}return{...n,stdout:r.trim()||n.stdout}}function as(e){let t=[],n="",r=0,i=!1,s=!1,o=0;for(;o<e.length;){let l=e[o];if(!i&&!s){if(l==="'"){i=!0,n+=l,o++;continue}if(l==='"'){s=!0,n+=l,o++;continue}if(l==="{"){r++,n+=l,o++;continue}if(l==="}"){if(r--,n+=l,o++,r===0){let c=n.trim();for(c&&t.push(c),n="";o<e.length&&(e[o]===";"||e[o]===" ");)o++}continue}if(r===0&&(l===";"||l===`
`)){let c=n.trim();c&&!c.startsWith("#")&&t.push(c),n="",o++;continue}}else i&&l==="'"?i=!1:s&&l==='"'&&(s=!1);n+=l,o++}let a=n.trim();return a&&!a.startsWith("#")&&t.push(a),t}var ls={name:"sh",aliases:["bash"],description:"Execute shell script or command",category:"shell",params:["-c <script>","[<file>]"],run:async e=>{let{args:t,shell:n,cwd:r}=e;if(C(t,"-c")){let s=t[t.indexOf("-c")+1]??"";if(!s)return{stderr:"sh: -c requires a script",exitCode:1};let o=as(s),a=Oe(o);return Te(a,e)}let i=t[0];if(i){let s=M(r,i);if(!n.vfs.exists(s))return{stderr:`sh: ${i}: No such file or directory`,exitCode:1};let o=n.vfs.readFile(s),a=as(o),l=Oe(a);return Te(l,e)}return{stderr:"sh: invalid usage. Use: sh -c 'cmd' or sh <file>",exitCode:1}}};var cs={name:"shift",description:"Shift positional parameters",category:"shell",params:["[n]"],run:({args:e,env:t})=>{if(!t)return{exitCode:0};let n=parseInt(e[0]??"1",10)||1,r=t.vars.__argv?.split("\0").filter(Boolean)??[];t.vars.__argv=r.slice(n).join("\0");let i=r.slice(n);for(let s=1;s<=9;s++)t.vars[String(s)]=i[s-1]??"";return{exitCode:0}}},us={name:"trap",description:"Trap signals and events",category:"shell",params:["[action] [signal...]"],run:({args:e,env:t})=>{if(!t||e.length===0)return{exitCode:0};let n=e[0]??"",r=e.slice(1);for(let i of r)t.vars[`__trap_${i.toUpperCase()}`]=n;return{exitCode:0}}},ds={name:"return",description:"Return from a shell function",category:"shell",params:["[n]"],run:({args:e,env:t})=>{let n=parseInt(e[0]??"0",10);return t&&(t.lastExitCode=n),{exitCode:n}}};var ms={name:"sleep",description:"Delay execution",category:"system",params:["<seconds>"],run:async({args:e})=>{let t=parseFloat(e[0]??"1");return Number.isNaN(t)||t<0?{stderr:"sleep: invalid time",exitCode:1}:(await new Promise(n=>setTimeout(n,t*1e3)),{exitCode:0})}};var ps={name:"sort",description:"Sort lines of text",category:"text",params:["[-r] [-n] [-u] [-k <col>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-r"]),o=C(r,["-n"]),a=C(r,["-u"]),l=r.filter(g=>!g.startsWith("-")),d=[...(l.length>0?l.map(g=>{try{return V(e,M(n,g),"sort"),t.vfs.readFile(M(n,g))}catch{return""}}).join(`
`):i??"").split(`
`).filter(Boolean)].sort((g,S)=>o?Number(g)-Number(S):g.localeCompare(S)),m=s?d.reverse():d;return{stdout:(a?[...new Set(m)]:m).join(`
`),exitCode:0}}};var fs={name:"source",aliases:["."],description:"Execute commands from a file in the current shell environment",category:"shell",params:["<file> [args...]"],run:async({args:e,authUser:t,hostname:n,cwd:r,shell:i,env:s})=>{let o=e[0];if(!o)return{stderr:"source: missing filename",exitCode:1};let a=M(r,o);if(!i.vfs.exists(a))return{stderr:`source: ${o}: No such file or directory`,exitCode:1};let l=i.vfs.readFile(a),c=0;for(let u of l.split(`
`)){let d=u.trim();if(!d||d.startsWith("#"))continue;let m=await Q(d,t,n,"shell",r,i,void 0,s);if(c=m.exitCode??0,m.closeSession||m.switchUser)return m}return{exitCode:c}}};var hs={name:"su",description:"Switch user",category:"users",params:["[-] [-c <cmd>] [username]"],run:async({authUser:e,shell:t,args:n,hostname:r,mode:i,cwd:s})=>{let o=n.includes("-")||n.includes("-l")||n.includes("--login"),a=n.indexOf("-c"),l=a!==-1?n[a+1]:void 0,u=n.filter((d,m)=>m!==a&&m!==a+1).filter(d=>d!=="-"&&d!=="-l"&&d!=="--login").find(d=>!d.startsWith("-"))??"root";return t.users.listUsers().includes(u)?e==="root"?l?Q(l,u,r,i,o?`/home/${u}`:s,t):{switchUser:u,nextCwd:o?`/home/${u}`:void 0,exitCode:0}:t.users.isSudoer(e)?{sudoChallenge:{username:u,targetUser:u,commandLine:l??null,loginShell:o,prompt:"Password: "},exitCode:0}:{stderr:`su: permission denied
`,exitCode:1}:{stderr:`su: user '${u}' does not exist
`,exitCode:1}}};function Co(e){let{flags:t,flagsWithValues:n,positionals:r}=ce(e,{flags:["-i","-S"],flagsWithValue:["-u","--user"]}),i=t.has("-i"),s=n.get("-u")||n.get("--user")||"root",o=r.length>0?r.join(" "):null;return{targetUser:s,loginShell:i,commandLine:o}}var gs={name:"sudo",description:"Execute as superuser",category:"users",params:["<command...>"],run:async({authUser:e,hostname:t,mode:n,cwd:r,shell:i,args:s})=>{let{targetUser:o,loginShell:a,commandLine:l}=Co(s);if(e!=="root"&&!i.users.isSudoer(e))return{stderr:"sudo: permission denied",exitCode:1};let c=o||"root",u=`[sudo] password for ${e}: `;return e==="root"?!l&&a?{switchUser:c,nextCwd:`/home/${c}`,exitCode:0}:l?Q(l,c,t,n,a?`/home/${c}`:r,i):{stderr:"sudo: missing command",exitCode:1}:{sudoChallenge:{username:e,targetUser:c,commandLine:l,loginShell:a,prompt:u},exitCode:0}}};var ys={name:"tail",description:"Output last lines",category:"text",params:["[-n <lines>] [file...]"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=le(r,["-n"]),o=r.find(d=>/^-\d+$/.test(d)),a=typeof s=="string"?parseInt(s,10):o?parseInt(o.slice(1),10):10,l=r.filter(d=>!d.startsWith("-")&&d!==s&&d!==String(a)),c=d=>{let m=d.split(`
`),p=d.endsWith(`
...
`),exitCode:0}}};var Ss={name:"tar",description:"Archive utility",category:"archive",params:["[-czf|-xzf|-tf] <archive> [files...]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=[],s=!1;for(let m of r)if(/^-[a-zA-Z]{2,}$/.test(m))for(let p of m.slice(1))i.push(`-${p}`);else if(!s&&/^[cxtdru]{1,}[a-zA-Z]*$/.test(m)&&!m.includes("/")&&!m.startsWith("-")){s=!0;for(let p of m)i.push(`-${p}`)}else i.push(m);let o=i.includes("-c"),a=i.includes("-x"),l=i.includes("-t"),c=i.indexOf("-f"),u=c!==-1?i[c+1]:i.find(m=>m.endsWith(".tar")||m.endsWith(".tar.gz")||m.endsWith(".tgz"));if(!o&&!a&&!l)return{stderr:`tar: must specify -c, -x, or -t
...
`),exitCode:0};for(let[p,g]of Object.entries(m))t.writeFileAsUser(e,M(n,p),g);return{exitCode:0}}return{stderr:"tar: must specify -c, -x, or -t",exitCode:1}}};var bs={name:"tee",description:"Read stdin, write to stdout and files",category:"text",params:["[-a] <file...>"],run:({authUser:e,shell:t,cwd:n,args:r,stdin:i})=>{let s=C(r,["-a"]),o=r.filter(l=>!l.startsWith("-")),a=i??"";for(let l of o){let c=M(n,l);if(s){let u=(()=>{try{return t.vfs.readFile(c)}catch{return""}})();t.writeFileAsUser(e,c,u+a)}else t.writeFileAsUser(e,c,a)}return{stdout:a,exitCode:0}}};function Xe(e,t,n){if(e[e.length-1]==="]"&&(e=e.slice(0,-1)),e[0]==="["&&(e=e.slice(1)),e.length===0)return!1;if(e[0]==="!")return!Xe(e.slice(1),t,n);let r=e.indexOf("-a");if(r!==-1)return Xe(e.slice(0,r),t,n)&&Xe(e.slice(r+1),t,n);let i=e.indexOf("-o");if(i!==-1)return Xe(e.slice(0,i),t,n)||Xe(e.slice(i+1),t,n);if(e.length===2){let[s,o=""]=e,l=(c=>c.startsWith("/")?c:`${n}/${c}`.replace(/\/+/g,"/"))(o);switch(s){case"-e":return t.vfs.exists(l);case"-f":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file";case"-d":return t.vfs.exists(l)&&t.vfs.stat(l).type==="directory";case"-r":return t.vfs.exists(l);case"-w":return t.vfs.exists(l);case"-x":return t.vfs.exists(l)&&!!(t.vfs.stat(l).mode&73);case"-s":return t.vfs.exists(l)&&t.vfs.stat(l).type==="file"&&t.vfs.stat(l).size>0;case"-z":return o.length===0;case"-n":return o.length>0;case"-L":return t.vfs.isSymlink(l)}}if(e.length===3){let[s="",o,a=""]=e,l=Number(s),c=Number(a);switch(o){case"=":case"==":return s===a;case"!=":return s!==a;case"<":return s<a;case">":return s>a;case"-eq":return l===c;case"-ne":return l!==c;case"-lt":return l<c;case"-le":return l<=c;case"-gt":return l>c;case"-ge":return l>=c}}return e.length===1?(e[0]??"").length>0:!1}var xs={name:"test",aliases:["["],description:"Evaluate conditional expression",category:"shell",params:["<expression>"],run:({args:e,shell:t,cwd:n})=>{try{return{exitCode:Xe([...e],t,n)?0:1}}catch{return{stderr:"test: malformed expression",exitCode:2}}}};var ws={name:"touch",description:"Create or update files",category:"files",params:["<file>"],run:({authUser:e,shell:t,cwd:n,args:r})=>{if(r.length===0)return{stderr:"touch: missing file operand",exitCode:1};for(let i of r){let s=M(n,i);V(e,s,"touch"),t.vfs.exists(s)||t.writeFileAsUser(e,s,"")}return{exitCode:0}}};function Po(e){return e.replace(/\\n/g,`
`).replace(/\\t/g,"	").replace(/\\r/g,"\r").replace(/\\\\/g,"\\")}function vs(e){let t=[],n=Po(e),r=0;for(;r<n.length;){if(r+2<n.length&&n[r+1]==="-"){let i=n.charCodeAt(r),s=n.charCodeAt(r+2);if(i<=s){for(let o=i;o<=s;o++)t.push(String.fromCharCode(o));r+=3;continue}}t.push(n[r]),r++}return t}var Cs={name:"tr",description:"Translate or delete characters",category:"text",params:["[-d] [-s] <set1> [set2]"],run:({args:e,stdin:t})=>{let n=C(e,["-d"]),r=C(e,["-s"]),i=e.filter(l=>!l.startsWith("-")),s=vs(i[0]??""),o=vs(i[1]??""),a=t??"";if(n){let l=new Set(s);a=[...a].filter(c=>!l.has(c)).join("")}else if(o.length>0){let l=new Map;for(let c=0;c<s.length;c++)l.set(s[c],o[c]??o[o.length-1]??"");a=[...a].map(c=>l.get(c)??c).join("")}if(r&&o.length>0){let l=new Set(o);a=a.replace(/(.)\1+/g,(c,u)=>l.has(u)?u:c)}return{stdout:a,exitCode:0}}};var Ps={name:"tree",description:"Display directory tree",category:"navigation",params:["[path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=M(n,Ie(r,0)??n);return V(e,i,"tree"),{stdout:t.vfs.tree(i),exitCode:0}}};var Es={name:"true",description:"Return success exit code",category:"shell",params:[],run:()=>({exitCode:0})},$s={name:"false",description:"Return failure exit code",category:"shell",params:[],run:()=>({exitCode:1})};var ks={name:"type",description:"Describe how a command would be interpreted",category:"shell",params:["<command...>"],run:({args:e,shell:t,env:n})=>{if(e.length===0)return{stderr:"type: missing argument",exitCode:1};let r=(n?.vars?.PATH??"/usr/local/bin:/usr/bin:/bin").split(":"),i=[],s=0;for(let o of e){if(xe(o)){i.push(`${o} is a shell builtin`);continue}let a=!1;for(let l of r){let c=`${l}/${o}`;if(t.vfs.exists(c)){i.push(`${o} is ${c}`),a=!0;break}}a||(i.push(`${o}: not found`),s=1)}return{stdout:i.join(`
`),exitCode:s}}};var Ms={name:"uname",description:"Print system information",category:"system",params:["[-a] [-s] [-r] [-m]"],run:({shell:e,args:t})=>{let n=C(t,["-a"]),r="Linux",i=e.properties?.kernel??"5.15.0",s=e.properties?.arch??"x86_64",o=e.hostname;return n?{stdout:`${r} ${o} ${i} #1 SMP ${s} GNU/Linux`,exitCode:0}:C(t,["-r"])?{stdout:i,exitCode:0}:C(t,["-m"])?{stdout:s,exitCode:0}:{stdout:r,exitCode:0}}};var Is={name:"uniq",description:"Report or filter out repeated lines",category:"text",params:["[-c] [-d] [-u] [file]"],run:({args:e,stdin:t})=>{let n=C(e,["-c"]),r=C(e,["-d"]),i=C(e,["-u"]),s=(t??"").split(`
...
`)||void 0,exitCode:0}):{stdout:S,exitCode:0}}};var Ts={name:"which",description:"Locate a command in PATH",category:"shell",params:["<command...>"],run:({args:e,shell:t,env:n})=>{if(e.length===0)return{stderr:"which: missing argument",exitCode:1};let r=(n?.vars?.PATH??"/usr/local/bin:/usr/bin:/bin").split(":"),i=[],s=!1;for(let o of e){let a=!1;for(let l of r){let c=`${l}/${o}`;if(t.vfs.exists(c)&&t.vfs.stat(c).type==="file"){i.push(c),a=!0;break}}a||(s=!0)}return i.length===0?{exitCode:1}:{stdout:i.join(`
...
`)}
`),Zs(e,1,"root","pts/0","/sbin/init",new Date(r).toISOString(),{});for(let I of i){let x=Js(I.tty);Zs(e,x,I.username,I.tty,"bash",I.startedAt,{USER:I.username,HOME:`/home/${I.username}`,TERM:"xterm-256color",SHELL:"/bin/bash",LANG:"en_US.UTF-8",PATH:"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",LOGNAME:I.username})}let P=i.length>0?Js(i[i.length-1].tty):1;try{e.remove("/proc/self")}catch{}let N=`/proc/${P}`;if(h(e,"/proc/self"),h(e,"/proc/self/fd"),h(e,"/proc/self/fdinfo"),h(e,"/proc/self/net"),e.exists(N))for(let I of e.list(N)){let x=`${N}/${I}`,A=`/proc/self/${I}`;try{e.stat(x).type==="file"&&$(e,A,e.readFile(x))}catch{}}else $(e,"/proc/self/cmdline","bash\0"),$(e,"/proc/self/comm","bash"),$(e,"/proc/self/status",`Name:	bash
State:	S (sleeping)
Pid:	1
PPid:	0
`),$(e,"/proc/self/environ",""),$(e,"/proc/self/cwd","/root\0"),$(e,"/proc/self/exe","/bin/bash\0")}function jo(e,t,n){h(e,"/sys"),h(e,"/sys/devices"),h(e,"/sys/devices/virtual"),h(e,"/sys/devices/system"),h(e,"/sys/devices/system/cpu"),h(e,"/sys/devices/system/cpu/cpu0"),f(e,"/sys/devices/system/cpu/cpu0/online",`1
...
`);let r=Bo(t),i=r.toString(16).padStart(8,"0");f(e,"/sys/class/net/eth0/address",`52:54:00:${i.slice(0,2)}:${i.slice(2,4)}:${i.slice(4,6)}
...
`),f(e,"/sys/fs/cgroup/memory/memory.usage_in_bytes",`${Pe()-Ue()}
...
`),f(e,"/sys/kernel/osrelease",`${n.kernel}
`),f(e,"/sys/kernel/ostype",`Linux
`)}function Wo(e){h(e,"/dev"),f(e,"/dev/null","",438),f(e,"/dev/zero","",438),f(e,"/dev/full","",438),f(e,"/dev/random","",292),f(e,"/dev/urandom","",292),f(e,"/dev/mem","",416),f(e,"/dev/port","",416),f(e,"/dev/kmsg","",432),f(e,"/dev/hwrng","",432),f(e,"/dev/fuse","",432),f(e,"/dev/autofs","",432),f(e,"/dev/userfaultfd","",432),f(e,"/dev/cpu_dma_latency","",432),f(e,"/dev/ptp0","",432),f(e,"/dev/snapshot","",432),f(e,"/dev/console","",384),f(e,"/dev/tty","",438),f(e,"/dev/ttyS0","",432),f(e,"/dev/ptmx","",438);for(let t=0;t<=63;t++)f(e,`/dev/tty${t}`,"",400);f(e,"/dev/vcs","",400),f(e,"/dev/vcs1","",400),f(e,"/dev/vcsa","",400),f(e,"/dev/vcsa1","",400),f(e,"/dev/vcsu","",400),f(e,"/dev/vcsu1","",400);for(let t=0;t<8;t++)f(e,`/dev/loop${t}`,"",432);h(e,"/dev/loop-control"),f(e,"/dev/vda","",432),f(e,"/dev/vdb","",432),f(e,"/dev/vdc","",432),f(e,"/dev/vdd","",432),h(e,"/dev/net"),f(e,"/dev/net/tun","",432),h(e,"/dev/pts"),h(e,"/dev/shm"),h(e,"/dev/cpu"),f(e,"/dev/stdin","",438),f(e,"/dev/stdout","",438),f(e,"/dev/stderr","",438),h(e,"/dev/fd"),f(e,"/dev/vga_arbiter","",432),f(e,"/dev/vsock","",432)}function qo(e){h(e,"/usr"),h(e,"/usr/bin"),h(e,"/usr/sbin"),h(e,"/usr/local"),h(e,"/usr/local/bin"),h(e,"/usr/local/lib"),h(e,"/usr/local/share"),h(e,"/usr/local/include"),h(e,"/usr/local/sbin"),h(e,"/usr/share"),h(e,"/usr/share/doc"),h(e,"/usr/share/man"),h(e,"/usr/share/man/man1"),h(e,"/usr/share/man/man5"),h(e,"/usr/share/man/man8"),h(e,"/usr/share/common-licenses"),h(e,"/usr/share/ca-certificates"),h(e,"/usr/share/zoneinfo"),h(e,"/usr/lib"),h(e,"/usr/lib/x86_64-linux-gnu"),h(e,"/usr/lib/python3"),h(e,"/usr/lib/python3/dist-packages"),h(e,"/usr/lib/python3.12"),h(e,"/usr/lib/jvm"),h(e,"/usr/lib/jvm/java-21-openjdk-amd64"),h(e,"/usr/lib/jvm/java-21-openjdk-amd64/bin"),h(e,"/usr/lib/node_modules"),h(e,"/usr/lib/node_modules/npm"),h(e,"/usr/include"),h(e,"/usr/src");let t=["sh","bash","ls","cat","echo","grep","find","sort","head","tail","cut","tr","sed","awk","wc","tee","tar","gzip","gunzip","touch","mkdir","rm","mv","cp","chmod","ln","pwd","env","date","sleep","id","whoami","hostname","uname","ps","kill","df","du","curl","wget","nano","diff","uniq","xargs","base64"];for(let r of t)f(e,`/usr/bin/${r}`,`#!/bin/sh
exec builtin ${r} "$@"
...
`;function Go(e){h(e,"/var"),h(e,"/var/log"),h(e,"/var/log/apt"),h(e,"/var/log/journal"),h(e,"/var/log/private"),h(e,"/var/tmp"),h(e,"/var/cache"),h(e,"/var/cache/apt"),h(e,"/var/cache/apt/archives"),h(e,"/var/cache/apt/archives/partial"),h(e,"/var/cache/debconf"),h(e,"/var/cache/ldconfig"),h(e,"/var/cache/fontconfig"),h(e,"/var/cache/PackageKit"),h(e,"/var/lib"),h(e,"/var/lib/apt"),h(e,"/var/lib/apt/lists"),h(e,"/var/lib/apt/lists/partial"),h(e,"/var/lib/dpkg"),h(e,"/var/lib/dpkg/info"),h(e,"/var/lib/dpkg/updates"),h(e,"/var/lib/dpkg/alternatives"),h(e,"/var/lib/misc"),h(e,"/var/lib/systemd"),h(e,"/var/lib/systemd/coredump"),h(e,"/var/lib/pam"),h(e,"/var/lib/git"),h(e,"/var/lib/PackageKit"),h(e,"/var/lib/python"),h(e,"/var/spool"),h(e,"/var/spool/cron"),h(e,"/var/spool/mail"),h(e,"/var/mail"),h(e,"/var/backups"),h(e,"/var/www"),f(e,"/var/lib/dpkg/status",Ko),f(e,"/var/lib/dpkg/available",""),f(e,"/var/lib/dpkg/lock",""),f(e,"/var/lib/dpkg/lock-frontend",""),f(e,"/var/lib/apt/lists/lock",""),f(e,"/var/cache/apt/pkgcache.bin",""),f(e,"/var/cache/apt/srcpkgcache.bin",""),f(e,"/var/log/syslog",`${new Date().toUTCString()}  kernel: Virtual container started
`),f(e,"/var/log/auth.log",""),f(e,"/var/log/kern.log",""),f(e,"/var/log/dpkg.log",""),f(e,"/var/log/apt/history.log",""),f(e,"/var/log/apt/term.log",""),f(e,"/var/log/faillog",""),f(e,"/var/log/lastlog",""),f(e,"/var/log/wtmp",""),f(e,"/var/log/btmp",""),f(e,"/var/log/alternatives.log",""),h(e,"/run"),h(e,"/run/lock"),h(e,"/run/lock/subsys"),h(e,"/run/systemd"),h(e,"/run/systemd/ask-password"),h(e,"/run/systemd/sessions"),h(e,"/run/systemd/users"),h(e,"/run/user"),h(e,"/run/dbus"),h(e,"/run/adduser"),f(e,"/run/utmp",""),f(e,"/run/dbus/system_bus_socket","")}function Yo(e){e.exists("/bin")||e.symlink("/usr/bin","/bin"),e.exists("/sbin")||e.symlink("/usr/sbin","/sbin"),e.exists("/var/run")||e.symlink("/run","/var/run"),h(e,"/lib"),h(e,"/lib64"),h(e,"/lib/x86_64-linux-gnu"),h(e,"/lib/modules"),e.exists("/lib64/ld-linux-x86-64.so.2")||f(e,"/lib64/ld-linux-x86-64.so.2","",493)}function Jo(e){h(e,"/tmp",1023),h(e,"/tmp/node-compile-cache",1023)}function Zo(e){h(e,"/root",448),h(e,"/root/.ssh",448),h(e,"/root/.config",493),h(e,"/root/.config/pip",493),h(e,"/root/.local",493),h(e,"/root/.local/share",493),f(e,"/root/.bashrc",`${["# root .bashrc","export PS1='\\[\\033[0;31m\\]\\u@\\h\\[\\033[0m\\]:\\[\\033[0;34m\\]\\w\\[\\033[0m\\]# '","export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin","export LANG=en_US.UTF-8","alias ll='ls -la'","alias la='ls -A'","alias l='ls -CF'"].join(`
`)}
`),f(e,"/root/.profile",`[ -f ~/.bashrc ] && . ~/.bashrc
`),f(e,"/root/.bash_logout",`# ~/.bash_logout
`),f(e,"/root/.config/pip/pip.conf",`[global]
break-system-packages = true
`)}function Qo(e,t){h(e,"/opt"),h(e,"/opt/rclone"),h(e,"/srv"),h(e,"/mnt"),h(e,"/media"),h(e,"/boot"),h(e,"/boot/grub"),f(e,"/boot/grub/grub.cfg",`${["# GRUB configuration (virtual)","set default=0","set timeout=0","",'menuentry "Fortune GNU/Linux" {',`  linux   /vmlinuz-${t.kernel} root=/dev/vda rw console=ttyS0`,`  initrd  /initrd.img-${t.kernel}`,"}"].join(`
`)}
...
CONFIG_VIRTIO_NET=y
CONFIG_KVM_GUEST=y
`,420),e.exists("/vmlinuz")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz"),e.exists("/vmlinuz.old")||e.symlink(`/boot/vmlinuz-${n}`,"/vmlinuz.old"),e.exists("/initrd.img")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img"),e.exists("/initrd.img.old")||e.symlink(`/boot/initrd.img-${n}`,"/initrd.img.old"),h(e,"/lost+found",448),h(e,"/home")}var Qs=new Map;function Xo(e,t){return`${e}|${t.kernel}|${t.os}|${t.arch}`}function ea(e,t){let n=Xo(e,t),r=Qs.get(n);if(r)return r;let i=new ft({mode:"memory"});zo(i,e,t),jo(i,e,t),Wo(i),qo(i),Go(i),Yo(i),Jo(i),Qo(i,t),Ho(i,t);let s=i.encodeBinary();return Qs.set(n,s),s}function Xs(e,t,n,r,i,s=[]){let o=ea(n,r);e.getMode()==="fs"&&e.exists("/home")?e.mergeRootTree(De(o)):e.importRootTree(De(o)),Zo(e),Bt(e,r,n,i,s),xn(e,t)}var wn=[{name:"vim",version:"2:9.0.1378-2",section:"editors",description:"Vi IMproved - enhanced vi editor",shortDesc:"Vi IMproved",installedSizeKb:3812,files:[{path:"/usr/bin/vim",content:`#!/bin/sh
...
`:"",384)||s,s&&await this.vfs.flushMirror()}writeIfChanged(n,r,i){return this.vfs.exists(n)&&this.vfs.readFile(n)===r?(this.vfs.chmod(n,i),!1):(this.vfs.writeFile(n,r,{mode:i}),!0)}createRecord(n,r){let i=vn("sha256").update(n).update(":").update(r).digest("hex"),s=e.recordCache.get(i);if(s)return s;let o=ti(16).toString("hex"),a={username:n,salt:o,passwordHash:this.hashPassword(r,o)};return e.recordCache.set(i,a),a}hasPassword(n){re.mark("hasPassword");let r=this.users.get(n);if(!r)return!1;let i=this.hashPassword("",r.salt);return r.passwordHash===i?!1:!!r.passwordHash}hashPassword(n,r=""){return e.fastPasswordHash?vn("sha256").update(r).update(n).digest("hex"):ri(n,r||"",32).toString("hex")}validateUsername(n){if(!n||n.trim()==="")throw new Error("invalid username");if(!/^[a-z_][a-z0-9_-]{0,31}$/i.test(n))throw new Error("invalid username")}validatePassword(n){if(!n||n.trim()==="")throw new Error("invalid password")}authorizedKeys=new Map;addAuthorizedKey(n,r,i){re.mark("addAuthorizedKey");let s=this.authorizedKeys.get(n)??[];s.push({algo:r,data:i}),this.authorizedKeys.set(n,s),this.emit("key:add",{username:n,algo:r})}removeAuthorizedKeys(n){this.authorizedKeys.delete(n),this.emit("key:remove",{username:n})}getAuthorizedKeys(n){return this.authorizedKeys.get(n)??[]}};function ii(e){let t=z.normalize(e);return t.startsWith("/")?t:`/${t}`}var yt=class extends ge{vfs;idleThresholdMs;checkIntervalMs;_state="active";_lastActivity=Date.now();_frozenBuffer=null;_checkTimer=null;constructor(t,n={}){super(),this.vfs=t,this.idleThresholdMs=n.idleThresholdMs??6e4,this.checkIntervalMs=n.checkIntervalMs??15e3}start(){this._checkTimer||(this._lastActivity=Date.now(),this._checkTimer=setInterval(()=>this._check(),this.checkIntervalMs),typeof this._checkTimer=="object"&&this._checkTimer!==null&&"unref"in this._checkTimer&&this._checkTimer.unref())}async stop(){this._checkTimer&&(clearInterval(this._checkTimer),this._checkTimer=null),this._state==="frozen"&&this._thaw()}ping(){this._lastActivity=Date.now(),this._state==="frozen"&&this._thaw()}get state(){return this._state}get idleMs(){return Date.now()-this._lastActivity}_check(){this._state!=="frozen"&&Date.now()-this._lastActivity>=this.idleThresholdMs&&this._freeze()}async _freeze(){this._state!=="frozen"&&(await this.vfs.stopAutoFlush(),this._frozenBuffer=this.vfs.encodeBinary(),this.vfs.releaseTree(),this._state="frozen",this.emit("freeze"))}_thaw(){if(this._state!=="frozen"||!this._frozenBuffer)return;let t=De(this._frozenBuffer);this.vfs.importRootTree(t),this._frozenBuffer=null,this._state="active",this.emit("thaw")}};async function Ht(){throw new Error("node:fs/promises.readFile is not supported in browser")}async function oi(){throw new Error("node:fs/promises.writeFile is not supported in browser")}async function ai(){throw new Error("node:fs/promises.unlink is not supported in browser")}function Cn(e){return`'${e.replace(/'/g,"'\\''")}'`}function St(e){return e.replace(/\r\n/g,`
`).replace(/\r/g,`
`).replace(/\n/g,`\r
`)}function li(e,t){let n=Number.isFinite(t.cols)&&t.cols>0?Math.floor(t.cols):80,r=Number.isFinite(t.rows)&&t.rows>0?Math.floor(t.rows):24;return`stty cols ${n} rows ${r} 2>/dev/null; ${e}`}function ci(e,t){return!t||t.trim()===""||t==="."?e:t.startsWith("/")?z.normalize(t):z.normalize(z.join(e,t))}async function ui(e){try{let n=(await Ht(`/proc/${e}/task/${e}/children`,"utf8")).trim().split(/\s+/).filter(Boolean).map(i=>Number.parseInt(i,10)).filter(i=>Number.isInteger(i)&&i>0),r=await Promise.all(n.map(i=>ui(i)));return[...n,...r.flat()]}catch{return[]}}async function di(e=y.pid){let t=await ui(e),n=Array.from(new Set(t)).sort((r,i)=>r-i);return n.length===0?null:n.join(",")}function mi(e,t,n){let r=li(e,t),i=Gt("script",["-qfec",r,"/dev/null"],{stdio:["pipe","pipe","pipe"],env:{...y.env,TERM:y.env.TERM??"xterm-256color"}});return i.stdout.on("data",s=>{n.write(s.toString("utf8"))}),i.stderr.on("data",s=>{n.write(s.toString("utf8"))}),i}function pi(e,t,n){return mi(`nano -- ${Cn(e)}`,t,n)}function fi(e,t,n){return mi(`htop -p ${Cn(e)}`,t,n)}function jt(e,t,n){let r=[`Linux ${e} ${t.kernel} ${t.arch}`,"","The programs included with the Fortune GNU/Linux system are free software;","the exact distribution terms for each program are described in the","individual files in /usr/share/doc/*/copyright.","","Fortune GNU/Linux comes with ABSOLUTELY NO WARRANTY, to the extent","permitted by applicable law."];if(n){let i=new Date(n.at),s=Number.isNaN(i.getTime())?n.at:Vt(i);r.push(`Last login: ${s} from ${n.from||"unknown"}`)}return r.push(""),`${r.map(i=>`${i}\r
`).join("")}`}function Wt(e,t,n){let r=e==="root",i=r?"\x1B[31;1m":"\x1B[35;1m",s="\x1B[37;1m",o="\x1B[34;1m",a="\x1B[0m";return`${s}[${i}${e}${s}@${o}${t}${a} ${n}${s}]${a}${r?"#":"$"} `}function hi(e,t,n,r,i,s="unknown",o={cols:80,rows:24},a){let l="",c=0,u=sa(a.vfs,n),d=null,m="",p=Y(n),g=ze(n,r),S=null,v=null,E=()=>{let O=Y(n),U=p===O?"~":z.basename(p)||"/";return Wt(n,r,U)},T=Array.from(new Set(mn())).sort();console.log(`[${i}] Shell started for user '${n}' at ${s}`),(async()=>{let O=`${Y(n)}/.bashrc`;if(a.vfs.exists(O))try{let U=a.vfs.readFile(O);for(let H of U.split(`
...
`),L.openEditor){await A(L.openEditor.targetPath,L.openEditor.initialContent,L.openEditor.tempPath);return}if(L.openHtop){await F();return}L.clearScreen&&t.write("\x1B[2J\x1B[H"),L.stdout&&t.write(`${St(L.stdout)}\r
`),L.stderr&&t.write(`${St(L.stderr)}\r
`),L.switchUser?(n=L.switchUser,p=L.nextCwd??Y(n),a.users.updateSession(i,n,s)):L.nextCwd&&(p=L.nextCwd),_()}async function x(){if(!S)return;let O=S;if(O.kind==="nano"){try{let U=await Ht(O.tempPath,"utf8");a.writeFileAsUser(n,O.targetPath,U)}catch{}await ai(O.tempPath).catch(()=>{})}S=null,l="",c=0,t.write(`\r
`),_()}async function A(O,U,H){a.vfs.exists(O)&&await oi(H,U,"utf8");let L=pi(H,o,t);L.on("error",se=>{t.write(`nano: ${se.message}\r
`),x()}),L.on("close",()=>{x()}),S={kind:"nano",targetPath:O,tempPath:H,process:L}}async function F(){let O=await di();if(!O){t.write(`htop: no child_process processes to display\r
`);return}let U=fi(O,o,t);U.on("error",H=>{t.write(`htop: ${H.message}\r
`),x()}),U.on("close",()=>{x()}),S={kind:"htop",targetPath:"",tempPath:"",process:U}}function k(O){l=O,c=l.length,_()}function w(O){l=`${l.slice(0,c)}${O}${l.slice(c)}`,c+=O.length,_()}function D(O,U){let H=U;for(;H>0&&!/\s/.test(O[H-1]);)H-=1;let L=U;for(;L<O.length&&!/\s/.test(O[L]);)L+=1;return{start:H,end:L}}function K(O){let U=O.lastIndexOf("/"),H=U>=0?O.slice(0,U+1):"",L=U>=0?O.slice(U+1):O,se=ci(p,H||".");try{return a.vfs.list(se).filter(B=>!B.startsWith(".")).filter(B=>B.startsWith(L)).map(B=>{let Ce=z.join(se,B),Ve=a.vfs.stat(Ce).type==="directory"?"/":"";return`${H}${B}${Ve}`}).sort()}catch{return[]}}function q(){let{start:O,end:U}=D(l,c),H=l.slice(O,c);if(H.length===0)return;let se=l.slice(0,O).trim().length===0?T.filter(pe=>pe.startsWith(H)):[],B=K(H),Ce=Array.from(new Set([...se,...B])).sort();if(Ce.length!==0){if(Ce.length===1){let pe=Ce[0],Ve=pe.endsWith("/")?"":" ";l=`${l.slice(0,O)}${pe}${Ve}${l.slice(U)}`,c=O+pe.length+Ve.length,_();return}t.write(`\r
`),t.write(`${Ce.join("  ")}\r
`),_()}}function X(O){if(O.length===0)return;u.push(O),u.length>500&&(u=u.slice(u.length-500));let U=u.length>0?`${u.join(`
`)}
`:"";a.vfs.writeFile(`${Y(n)}/.bash_history`,U)}function be(){let O=`${Y(n)}/.lastlog.json`;if(!a.vfs.exists(O))return null;try{return JSON.parse(a.vfs.readFile(O))}catch{return null}}function me(O){let U=`${Y(n)}/.lastlog`;a.vfs.writeFile(U,JSON.stringify({at:O,from:s}))}function Me(){let O=be(),U=new Date().toISOString();t.write(jt(r,e,O)),me(U)}Me(),_(),t.on("data",async O=>{if(S){S.process.stdin.write(O);return}if(v){let H=O.toString("utf8");for(let L=0;L<H.length;L+=1){let se=H[L];if(se===""){v=null,t.write(`^C\r
`),_();return}if(se==="\x7F"||se==="\b"){v.buffer=v.buffer.slice(0,-1);continue}if(se==="\r"||se===`
`){let B=v.buffer;if(v.buffer="",v.onPassword){let{result:pe,nextPrompt:Ve}=await v.onPassword(B,a);t.write(`\r
`),pe!==null?(v=null,pe.stdout&&t.write(pe.stdout.replace(/\n/g,`\r
`)),pe.stderr&&t.write(pe.stderr.replace(/\n/g,`\r
`)),_()):(Ve&&(v.prompt=Ve),t.write(v.prompt));return}let Ce=a.users.verifyPassword(v.username,B);await I(Ce);return}se>=" "&&(v.buffer+=se)}return}let U=O.toString("utf8");for(let H=0;H<U.length;H+=1){let L=U[H];if(L===""){l="",c=0,d=null,m="",t.write(`bye\r
`),X("bye"),t.write(`logout\r
`),t.exit(0),t.end();return}if(L==="	"){q();continue}if(L==="\x1B"){let se=U[H+1],B=U[H+2],Ce=U[H+3];if(se==="["&&B){if(B==="A"){H+=2,u.length>0&&(d===null?(m=l,d=u.length-1):d>0&&(d-=1),k(u[d]??""));continue}if(B==="B"){H+=2,d!==null&&(d<u.length-1?(d+=1,k(u[d]??"")):(d=null,k(m)));continue}if(B==="C"){H+=2,c<l.length&&(c+=1,t.write("\x1B[C"));continue}if(B==="D"){H+=2,c>0&&(c-=1,t.write("\x1B[D"));continue}if(B==="3"&&Ce==="~"){H+=3,c<l.length&&(l=`${l.slice(0,c)}${l.slice(c+1)}`,_());continue}}}if(L===""){l="",c=0,d=null,m="",t.write(`^C\r
`),_();continue}if(L==="\r"||L===`
`){let se=l.trim();if(l="",c=0,d=null,m="",t.write(`\r
`),se.length>0){let B=await Promise.resolve(Q(se,n,r,"shell",p,a,void 0,g));if(X(se),B.openEditor){await A(B.openEditor.targetPath,B.openEditor.initialContent,B.openEditor.tempPath);return}if(B.openHtop){await F();return}if(B.sudoChallenge){N(B.sudoChallenge);return}if(B.clearScreen&&t.write("\x1B[2J\x1B[H"),B.stdout&&t.write(`${St(B.stdout)}\r
`),B.stderr&&t.write(`${St(B.stderr)}\r
`),B.closeSession){t.write(`logout\r
`),t.exit(B.exitCode??0),t.end();return}B.nextCwd&&(p=B.nextCwd),B.switchUser&&(n=B.switchUser,p=B.nextCwd??Y(n),a.users.updateSession(i,n,s),l="",c=0)}_();continue}if(L==="\x7F"||L==="\b"){c>0&&(l=`${l.slice(0,c-1)}${l.slice(c)}`,c-=1,_());continue}w(L)}}),t.on("close",()=>{S&&(S.process.kill("SIGTERM"),S=null)})}function sa(e,t){let n=`${Y(t)}/.bash_history`;return e.exists(n)?e.readFile(n).split(`
`).map(i=>i.trim()).filter(i=>i.length>0):(e.writeFile(n,""),[])}function ia(e){return typeof e=="object"&&e!==null&&"vfsInstance"in e&&gi(e.vfsInstance)}function gi(e){if(typeof e!="object"||e===null)return!1;let t=e;return typeof t.restoreMirror=="function"&&typeof t.flushMirror=="function"&&typeof t.writeFile=="function"&&typeof t.readFile=="function"&&typeof t.mkdir=="function"&&typeof t.exists=="function"&&typeof t.stat=="function"&&typeof t.list=="function"&&typeof t.remove=="function"&&typeof t.copy=="function"&&typeof t.move=="function"&&typeof t.touch=="function"}var oa={kernel:"1.0.0+itsrealfortune+1-amd64",os:"Fortune GNU/Linux x64",arch:"x86_64"},bt=he("VirtualShell");function aa(){let e=y.env.SSH_MIMIC_AUTO_SUDO_NEW_USERS;return e?!["0","false","no","off"].includes(e.toLowerCase()):!0}var je=class extends ge{vfs;users;packageManager;hostname;properties;startTime;_idle=null;initialized;constructor(t,n,r){super(),bt.mark("constructor"),this.hostname=t,this.properties=n||oa,this.startTime=Date.now(),gi(r)?this.vfs=r:ia(r)?this.vfs=r.vfsInstance:this.vfs=new ft(r??{}),this.users=new gt(this.vfs,aa()),this.packageManager=new ht(this.vfs,this.users);let i=this.vfs,s=this.users,o=this.packageManager,a=this.properties,l=this.hostname,c=this.startTime;this.initialized=(async()=>{await i.restoreMirror(),await s.initialize(),Xs(i,s,l,a,c),o.load(),this.emit("initialized")})()}async ensureInitialized(){bt.mark("ensureInitialized"),await this.initialized}addCommand(t,n,r){let i=t.trim().toLowerCase();if(i.length===0||/\s/.test(i))throw new Error("Command name must be non-empty and contain no spaces");un(dn(i,n,r))}executeCommand(t,n,r){bt.mark("executeCommand"),this._idle?.ping();let i=Q(t,n,this.hostname,"shell",r,this);return this.emit("command",{command:t,user:n,cwd:r}),i}startInteractiveSession(t,n,r,i,s){bt.mark("startInteractiveSession"),this._idle?.ping(),this.emit("session:start",{user:n,sessionId:r,remoteAddress:i}),hi(this.properties,t,n,this.hostname,r,i,s,this),this.refreshProcSessions()}refreshProcFs(){Bt(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}mount(t,n,r={}){this.vfs.mount(t,n,r)}unmount(t){this.vfs.unmount(t)}getMounts(){return this.vfs.getMounts()}refreshProcSessions(){Bt(this.vfs,this.properties,this.hostname,this.startTime,this.users.listActiveSessions())}syncPasswd(){xn(this.vfs,this.users)}getVfs(){return this?.vfs??null}getUsers(){return this?.users??null}getHostname(){return this?.hostname}writeFileAsUser(t,n,r){bt.mark("writeFileAsUser"),this.users.assertWriteWithinQuota(t,n,r),this.vfs.writeFile(n,r)}enableIdleManagement(t){this._idle||(this._idle=new yt(this.vfs,t),this._idle.on("freeze",()=>this.emit("shell:freeze")),this._idle.on("thaw",()=>this.emit("shell:thaw")),this._idle.start())}async disableIdleManagement(){this._idle&&(await this._idle.stop(),this._idle=null)}get idleState(){return this._idle?.state??"active"}get idleMs(){return this._idle?.idleMs??0}pingIdle(){this._idle?.ping()}};var Pn=!!y.env.DEV_MODE,Ny=Pn?console.log.bind(console):()=>{},Ay=Pn?console.warn.bind(console):()=>{},_y=Pn?console.error.bind(console):()=>{};var Oy=he("SftpMimic");var Wy=he("SshMimic"),da=!!y.env.DEV_MODE,qy=da?console.log.bind(console):()=>{};var tt="my-vm",yi=500,ve=document.getElementById("output"),Se=document.getElementById("cmd");Se.focus();document.addEventListener("click",()=>{window.getSelection()?.toString()||Se.focus()});var Si={30:"#000",31:"#c00",32:"#0c0",33:"#cc0",34:"#00c",35:"#c0c",36:"#0cc",37:"#ccc",90:"#555",91:"#f55",92:"#5f5",93:"#ff5",94:"#55f",95:"#f5f",96:"#5ff",97:"#fff"},bi={40:"#000",41:"#c00",42:"#0c0",43:"#cc0",44:"#00c",45:"#c0c",46:"#0cc",47:"#ccc"};function xi(e){let t="",n=!1,r="",i="";for(let s of e.split(/(\x1b\[[0-9;]*m)/)){let o=s.match(/^\x1b\[([0-9;]*)m$/);if(o)for(let a of o[1].split(";").map(Number))a===0?(n=!1,r="",i=""):a===1?n=!0:Si[a]?r=Si[a]:bi[a]&&(i=bi[a]);else if(s){let a=[r?`color:${r}`:"",i?`background:${i}`:"",n?"font-weight:bold":""].filter(Boolean).join(";"),l=s.replace(/&/g,"&amp;").replace(/</g,"&lt;");t+=a?`<span style="${a}">${l}</span>`:l}}return t}function xt(e){let t=document.createElement("span");t.innerHTML=xi(e),ie?ve.insertBefore(t,ie):ve.appendChild(t),ve.scrollTop=ve.scrollHeight}var ie=null;function wi(){ie&&(ie.remove(),ie=null),Se.value="",ie=document.createElement("span"),ie.className="input-line";let e=document.createElement("span"),t=Le===Y(we)?"~":Le.split("/").at(-1)||"/";e.innerHTML=xi(Wt(we,tt,t));let n=document.createElement("span");n.className="typed";let r=document.createElement("span");r.className="cursor",r.textContent="\xA0",ie.appendChild(e),ie.appendChild(n),ie.appendChild(r),ve.appendChild(ie),ve.scrollTop=ve.scrollHeight}Se.addEventListener("input",()=>{ie&&(ie.querySelector(".typed").textContent=Se.value,ve.scrollTop=ve.scrollHeight)});await globalThis.__fsReady__;var qt=new je(tt,void 0,{mode:"fs",snapshotPath:"/vfs-data",flushIntervalMs:1e4}),ue=qt.vfs;await ue.restoreMirror();var ma=!ue.exists("/bin");ma&&(await qt.ensureInitialized(),ue.exists("/root")||ue.mkdir("/root",448),ue.writeFile("/root/README.txt",`Welcome to ${tt}
`),await ue.flushMirror());window.addEventListener("beforeunload",()=>{ue.flushMirror()});var we="root",Le=Y(we),We=ze(we,tt);We.vars.PWD=Le;function pa(e){e.switchUser?(we=e.switchUser,Le=e.nextCwd??Y(we),We.vars.USER=we,We.vars.LOGNAME=we,We.vars.HOME=Y(we),We.vars.PWD=Le):e.nextCwd&&(Le=e.nextCwd,We.vars.PWD=Le)}function En(){return`${Y(we)}/.bash_history`}function fa(){try{return ue.exists(En())?ue.readFile(En()).split(`
`).map(e=>e.trim()).filter(e=>e.length>0):[]}catch{return[]}}function ha(){ue.writeFile(En(),ye.length>0?`${ye.join(`
`)}
`:"")}var ye=fa(),Re=-1;function ga(){try{return ue.exists("/root/.lastlog")?JSON.parse(ue.readFile("/root/.lastlog")):null}catch{return null}}function ya(){ue.writeFile("/root/.lastlog",JSON.stringify({at:new Date().toISOString(),from:"browser"}))}xt(jt(tt,qt.properties,ga()));ya();await ue.flushMirror();wi();Se.addEventListener("keydown",async e=>{if(e.key==="ArrowUp"){e.preventDefault(),Re<ye.length-1&&(Re++,Se.value=ye[ye.length-1-Re],ie&&(ie.querySelector(".typed").textContent=Se.value));return}if(e.key==="ArrowDown"){e.preventDefault(),Re>0?(Re--,Se.value=ye[ye.length-1-Re]):(Re=-1,Se.value=""),ie&&(ie.querySelector(".typed").textContent=Se.value);return}if(e.key!=="Enter")return;let t=Se.value.trim();Re=-1,ie&&(ie.querySelector(".cursor")?.remove(),ie=null),ve.appendChild(document.createTextNode(`
`)),t&&(ye.push(t),ye.length>yi&&(ye=ye.slice(ye.length-yi)),ha());try{let n=await Q(t,we,tt,"shell",Le,qt,void 0,We);if(n.clearScreen&&(ve.innerHTML=""),n.stdout&&xt(n.stdout),n.stderr&&xt(n.stderr),pa(n),await ue.flushMirror(),n.closeSession){xt(`
Session closed.
