...
`),L.openEditor){await A(L.openEditor.targetPath,L.openEditor.initialContent,L.openEditor.tempPath);return}if(L.openHtop){await D();return}L.clearScreen&&t.write("\x1B[2J\x1B[H"),L.stdout&&t.write(`${xt(L.stdout)}\r
...
`),x()}),L.on("close",()=>{x()}),S={kind:"nano",targetPath:O,tempPath:H,process:L}}async function D(){let O=await pi();if(!O){t.write(`htop: no child_process processes to display\r
`);return}let U=gi(O,o,t);U.on("error",H=>{t.write(`htop: ${H.message}\r
`),x()}),U.on("close",()=>{x()}),S={kind:"htop",targetPath:"",tempPath:"",process:U}}function k(O){l=O,c=l.length,_()}function w(O){l=`${l.slice(0,c)}${O}${l.slice(c)}`,c+=O.length,_()}function F(O,U){let H=U;for(;H>0&&!/\s/.test(O[H-1]);)H-=1;let L=U;for(;L<O.length&&!/\s/.test(O[L]);)L+=1;return{start:H,end:L}}function K(O){let U=O.lastIndexOf("/"),H=U>=0?O.slice(0,U+1):"",L=U>=0?O.slice(U+1):O,ie=Wt(p,H||".");try{return a.vfs.list(ie).filter(B=>!B.startsWith(".")).filter(B=>B.startsWith(L)).map(B=>{let ve=z.join(ie,B),Le=a.vfs.stat(ve).type==="directory"?"/":"";return`${H}${B}${Le}`}).sort()}catch{return[]}}function q(){let{start:O,end:U}=F(l,c),H=l.slice(O,c);if(H.length===0)return;let ie=l.slice(0,O).trim().length===0?T.filter(fe=>fe.startsWith(H)):[],B=K(H),ve=Array.from(new Set([...ie,...B])).sort();if(ve.length!==0){if(ve.length===1){let fe=ve[0],Le=fe.endsWith("/")?"":" ";l=`${l.slice(0,O)}${fe}${Le}${l.slice(U)}`,c=O+fe.length+Le.length,_();return}t.write(`\r
`),t.write(`${ve.join("  ")}\r
...
`),ie.length>0){let B=await Promise.resolve(Z(ie,n,r,"shell",p,a,void 0,g));if(ee(ie),B.openEditor){await A(B.openEditor.targetPath,B.openEditor.initialContent,B.openEditor.tempPath);return}if(B.openHtop){await D();return}if(B.sudoChallenge){N(B.sudoChallenge);return}if(B.clearScreen&&t.write("\x1B[2J\x1B[H"),B.stdout&&t.write(`${xt(B.stdout)}\r
