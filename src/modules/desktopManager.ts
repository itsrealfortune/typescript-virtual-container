...
export class DesktopManager {
	...
	start(): Promise<void> {
		...
		this._addDocListener(window, "beforeunload", () =>
			saveSession(this._windows)
		);
	...
	private _restoreSession(): void {
		...
		for (const sw of saved) {
			...
			switch (sw.contentType) {
				case "terminal":
					id = this.createTerminalWindow();
					break;
				case "thunar":
					id = this.createThunarWindow(sw.contentPath);
					break;
				case "editor":
					id = this.createEditorWindow(sw.contentPath);
					break;
				case "about":
		...
		this._zCounter =
			Math.max(this._zCounter, ...saved.map((s) => s.zIndex)) + 1;
		this._renderAll();
	}
	...
	createTerminalWindow(): string {
		...
		const tid = setTimeout(() => {
			...
			this._shell.startInteractiveSession(stream, "root", null, "desktop", {
				cols,
				rows,
			});
		}, 0);
		...
		if (w && w.content.type === "taskmanager") {
			w.content.refreshInterval = setInterval(() => {
				const el = this._container.querySelector(
					`.desktop-window[data-win-id="${id}"]`
				) as HTMLElement | null;
				if (el) {
					this._renderTaskManagerContent(el, id);
				}
			}, 3000);
		}
		return id;
	...
	closeWindow(id: string): void {
		...
		if (w.content.type === "terminal") {
			...
			if (w.content.stream && typeof w.content.stream.end === "function") {
				w.content.stream.end();
			}
		...
		if (this._windows.length > 0) {
			this.focusWindow(
				(this._windows[this._windows.length - 1] as DesktopWindow).id
			);
		}
		this._renderAll();
	}
	...
	private _ensureWindowElement(win: DesktopWindow): HTMLElement {
		let el = this._container.querySelector(
			`.desktop-window[data-win-id="${win.id}"]`
		) as HTMLElement;
		if (!el) {
	...
	private _setupEventDelegation(): void {
		...
		this._container.addEventListener("click", (e) => {
			...
			if (icon) {
				...
				if (action === "terminal") {
					...
					this.createThunarWindow(this._trashPath);
				}
				e.stopPropagation();
				return;
			}
			...
			// Panel menu button
			if (
				target.classList.contains("xfce-menu-button") ||
				target.closest(".xfce-menu-button")
			) {
			...
			if (target.classList.contains("taskmgr-kill")) {
				...
				if (pid) {
					...
					if (sessionIdx >= 0 && sessionIdx < sessions.length) {
						this._shell.users.unregisterSession(
							(sessions[sessionIdx] as (typeof sessions)[number]).id
						);
					} else {
					...
					const winId = target
						...
						?.getAttribute("data-win-id");
					if (winId) {
						this._renderTaskManagerContent(
							this._container.querySelector(
								`.desktop-window[data-win-id="${winId}"]`
							) as HTMLElement,
							winId
						);
					}
			...
			if (
				target.classList.contains("taskmgr-refresh") ||
				target.closest(".taskmgr-refresh")
			) {
		...
		const mouseMoveFn = (e: Event) => {
			...
			if (this._resizeState) {
				...
				this._resizeState.win.width = Math.max(
					240,
					this._resizeState.origW + dx
				);
				this._resizeState.win.height = Math.max(
					120,
					this._resizeState.origH + dy
				);
		...
		this._addDocListener(document, "keydown", (e) => {
			...
			if ((e.target as HTMLElement)?.classList?.contains("editor-textarea")) {
				return;
			}
		...
		this._container.addEventListener("keydown", (e) => {
			...
			if (!textarea.classList.contains("editor-textarea")) {
				return;
			}
			e.stopPropagation();
		...
		this._container.addEventListener("input", (e) => {
			...
			const dot = textarea
				.closest(".win-content")
				?.querySelector(".editor-dirty") as HTMLElement | null;
			if (dot) {
				dot.style.display = "";
			}
	...
	private _renderWindows(): void {
		...
		for (const w of this._windows) {
			if (w.minimized) {
				const el = this._container.querySelector(
					`.desktop-window[data-win-id="${w.id}"]`
				);
				if (el) {
					el.remove();
				}
	...
	private _renderEditorContent(
		...
		let fileText = "";
		try {
			fileText = this._shell.vfs.readFile(content.path);
		} catch {
			/* new file */
		}
	...
	private _saveEditor(winId: string): void {
		...
		if (w.content.path.endsWith("untitled.txt")) {
			...
			if (pathEl) {
				pathEl.textContent = w.content.path;
			}
		}
	...
	private _renderTaskManagerContent(el: HTMLElement, winId: string): void {
		...
		const desktopWindows = this._windows.filter(
			(w) => w.id !== winId && w.content.type !== "taskmanager"
		);
		...
		for (const w of desktopWindows) {
			const icon =
				w.content.type === "terminal"
					? "fa-terminal"
					: w.content.type === "thunar"
						? "fa-folder-open"
						: w.content.type === "editor"
							? "fa-file-pen"
							: w.content.type === "about"
								? "fa-circle-info"
								: "fa-window-restore";
			rows += `<tr>
        <td>—</td>
