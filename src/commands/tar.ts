...
export const tarCommand: ShellModule = {
	...
	run: ({ shell, cwd, args, uid, gid }) => {
		...
		const autoGzip =
			useGzip || archiveName.endsWith(".gz") || archiveName.endsWith(".tgz");

		if (create) {
			const skipSet = new Set<string>();
			if (fIdx !== -1 && expanded[fIdx + 1]) {
				skipSet.add(expanded[fIdx + 1] as string);
			}
			const fileArgs = expanded.filter(
			...
			return {
				stdout: verbose ? verboseLines.join("\n") : undefined,
				exitCode: 0,
			};
		}
		...
		if (list || extract) {
			...
			if (autoGzip) {
				try {
					...
					return {
						stderr: `tar: ${archiveName}: not a gzip file`,
						exitCode: 1,
					};
				}
			} else {
