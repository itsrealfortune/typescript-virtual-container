...
- [ ] Think about adding `delete` keyword on some modules and commands variables and functions to free up memory when they are no longer needed, especially for large data structures like the VFS, process table, and network connections (after reading a file, writing it, if its VFS snapshot file, we can delete it from memory, and only keep it on disk; after a process terminates, we can delete its entry from the process table; after a network connection is closed, we can delete its entry from the connection table, etc...)
