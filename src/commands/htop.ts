...
export const htopCommand: ShellModule = {
	...
	run: ({shell, authUser}) => {
		...
		const cpuCapCount =
			shell.resourceCaps?.cpuCapCores === undefined
				? hostCpus.length
				: Math.min(shell.resourceCaps.cpuCapCores, hostCpus.length);
		...
		// ── Summary line ───────────────────────────────────────────────────────
		const avgLoad =
			cpuLoads.slice(0, cpuCount).reduce((a, b) => a + b, 0) / cpuCount;
		...
		for (const p of procs) {
			...
			const userColor =
				p.user === "root" ? C.red : p.user === authUser ? C.green : C.cyan;
			const cmdColor =
				p.cmd === "htop" ? C.green : p.cmd === "bash" ? C.cyan : C.reset;
