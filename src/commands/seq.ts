...
export const seqCommand: ShellModule = {
	...
	run: ({args}) => {
		...
		let last: number;
		if (nums.length === 1) {
			last = nums[0] as number;
		} else if (nums.length === 2) {
			first = nums[0] as number;
			last = nums[1] as number;
		} else {
			first = nums[0] as number;
			inc = nums[1] as number;
			last = nums[2] as number;
		}
		...
		for (
			...
			if (fmt) {
				s = fmt
					.replace("%g", String(n))
					.replace("%f", n.toFixed(6))
					.replace("%d", String(Math.trunc(n)));
			} else {
				s = Number.isInteger(n)
					? String(n)
					: n.toPrecision(12).replace(/\.?0+$/, "");
			}
			if (width) {
