import type { VfsSnapshot, VfsSnapshotDirectoryNode, VfsSnapshotNode } from '../types/vfs';
...
function serializeDirectory(node: InternalDirectoryNode): VfsSnapshotDirectoryNode {
  return {
    type: 'directory',
    name: node.name,
    mode: node.mode,
...
function deserializeDirectory(node: VfsSnapshotDirectoryNode): InternalDirectoryNode {
  return {
    type: 'directory',
    name: node.name,
    mode: node.mode,
    createdAt: new Date(node.createdAt),
    updatedAt: new Date(node.updatedAt),
    children: new Map<string, InternalNode>(node.children.map((child) => [child.name, deserializeNode(child)]))
  };
}
...
export function applySnapshot(rootTarget: InternalDirectoryNode, snapshot: VfsSnapshot): void {
