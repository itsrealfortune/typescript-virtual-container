...
function parseSudoArgs(args: string[]): { targetUser: string; loginShell: boolean; commandLine: string | null } {
  let targetUser = 'root';
  ...
  run: async ({ authUser, hostname, users, mode, cwd, vfs, args }) => {
    ...
    if (authUser === 'root') {
      if (!commandLine && loginShell) {
        return { switchUser: effectiveUser, nextCwd: `/home/${effectiveUser}`, exitCode: 0 };
      }
      ...
      return runCommand(commandLine, effectiveUser, hostname, users, mode, loginShell ? `/home/${effectiveUser}` : cwd, vfs);
    }
