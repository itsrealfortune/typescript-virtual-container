...
`)}
`,exitCode:0}}};var oi="\x1B[0m",ai="\x1B[1;34m",li="\x1B[1;36m",ci="\x1B[1;32m",ui="",di="\x1B[30;42m",mi="\x1B[37;44m",pi="\x1B[34;42m";function Re(e,t){return t?`${t}${e}${oi}`:e}function At(e,t,n){if(n)return li;if(t==="directory"){let r=!!(e&512),i=!!(e&2);return r&&i?di:r?mi:i?pi:ai}return e&73?ci:ui}function Qn(e,t,n){let r;n?r="l":t==="directory"?r="d":r="-";let i=c=>e&c?"r":"-",s=c=>e&c?"w":"-",o=(()=>{let c=!!(e&64);return e&2048?c?"s":"S":c?"x":"-"})(),a=(()=>{let c=!!(e&8);return e&1024?c?"s":"S":c?"x":"-"})(),l=(()=>{let c=!!(e&1);return t==="directory"&&e&512?c?"t":"T":c?"x":"-"})();return`${r}${i(256)}${s(128)}${o}${i(32)}${s(16)}${a}${i(4)}${s(2)}${l}`}var fi=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function It(e){let t=new Date,n=4320*3600*1e3,r=Math.abs(t.getTime()-e.getTime())<n,i=String(e.getDate()).padStart(2," "),s=fi[e.getMonth()]??"";if(r){let o=String(e.getHours()).padStart(2,"0"),a=String(e.getMinutes()).padStart(2,"0");return`${i} ${s.padEnd(3)} ${o}:${a}`}return`${i} ${s.padEnd(3)} ${e.getFullYear()}`}function st(e,t){try{return e.readFile(t)}catch{return"?"}}function hi(e,t,n){let r=t==="/"?"":t;return n.map(i=>{let s=`${r}/${i}`,o=e.isSymlink(s),a;try{a=e.stat(s)}catch{return i}let l=At(a.mode,a.type,o);return Re(i,l)}).join("  ")}function gi(e,t,n){let r=t==="/"?"":t,i=n.map(d=>{let m=`${r}/${d}`,p=e.isSymlink(m),g;try{g=e.stat(m)}catch{return{perms:"----------",nlink:"1",size:"0",date:It(new Date),label:d}}let S=p?41471:g.mode,C=Qn(S,g.type,p),P=g.type==="directory"?String((g.childrenCount??0)+2):"1",x=p?st(e,m).length:g.type==="file"?g.size??0:(g.childrenCount??0)*4096,_=String(x),w=It(g.updatedAt),A=At(S,g.type,p),N=p?`${Re(d,A)} -> ${st(e,m)}`:Re(d,A);return{perms:C,nlink:P,size:_,date:w,label:N}}),s=Math.max(...i.map(d=>d.nlink.length)),o=Math.max(...i.map(d=>d.size.length)),a="root",l="root",c=n.length*8,u=i.map(d=>`${d.perms} ${d.nlink.padStart(s)} ${a} ${l} ${d.size.padStart(o)} ${d.date} ${d.label}`);return`total ${c}
${u.join(`
`)}`}var Xn={name:"ls",description:"List directory contents",category:"navigation",params:["[-la] [path]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=v(r,["-l","--long","-la","-al"]),s=v(r,["-a","--all","-la","-al"]),o=$e(r,0,{flags:["-l","--long","-a","--all","-la","-al"]}),a=I(n,o??n);if(L(e,a,"ls"),t.vfs.exists(a)){let u=t.vfs.stat(a),d=t.vfs.isSymlink(a);if(u.type==="file"||d){let m=a.split("/").pop()??a,p=At(d?41471:u.mode,u.type,d);if(i){let g=d?41471:u.mode,S=d?st(t.vfs,a).length:u.size??0,C=Qn(g,u.type,d),P=d?`${Re(m,p)} -> ${st(t.vfs,a)}`:Re(m,p);return{stdout:`${C} 1 root root ${S} ${It(u.updatedAt)} ${P}
`,exitCode:0}}return{stdout:`${Re(m,p)}
...
`,i+=2;continue;case"t":r+="	",i+=2;continue;case"r":r+="\r",i+=2;continue;case"\\":r+="\\",i+=2;continue;case"a":r+="\x07",i+=2;continue;case"b":r+="\b",i+=2;continue;case"f":r+="\f",i+=2;continue;case"v":r+="\v",i+=2;continue;default:r+=e[i],i++;continue}if(e[i]==="%"&&i+1<e.length){let s=i+1,o=!1;e[s]==="-"&&(o=!0,s++);let a=!1;e[s]==="0"&&(a=!0,s++);let l=0;for(;s<e.length&&/\d/.test(e[s]);)l=l*10+parseInt(e[s],10),s++;let c=-1;if(e[s]===".")for(s++,c=0;s<e.length&&/\d/.test(e[s]);)c=c*10+parseInt(e[s],10),s++;let u=e[s],d=t[n++]??"",m=(p,g=" ")=>{if(l<=0||p.length>=l)return p;let S=g.repeat(l-p.length);return o?p+S:S+p};switch(u){case"s":{let p=String(d);c>=0&&(p=p.slice(0,c)),r+=m(p);break}case"d":case"i":r+=m(String(parseInt(d,10)||0),a?"0":" ");break;case"f":{let p=c>=0?c:6;r+=m((parseFloat(d)||0).toFixed(p));break}case"o":r+=m((parseInt(d,10)||0).toString(8),a?"0":" ");break;case"x":r+=m((parseInt(d,10)||0).toString(16),a?"0":" ");break;case"X":r+=m((parseInt(d,10)||0).toString(16).toUpperCase(),a?"0":" ");break;case"%":r+="%",n--;break;default:r+=e[i],i++;continue}i=s+1;continue}r+=e[i],i++}return r}var Pr={name:"printf",description:"Format and print data",category:"shell",params:["<format> [args...]"],run:({args:e})=>{let t=e[0];return t?{stdout:_i(t,e.slice(1)),exitCode:0}:{stderr:"printf: missing format string",exitCode:1}}};var $r={name:"ps",description:"Report process status",category:"system",params:["[-a] [-u] [-x] [aux]"],run:({authUser:e,shell:t,args:n})=>{let r=t.users.listActiveSessions(),i=v(n,["-u"])||n.includes("u")||n.includes("aux")||n.includes("au"),s=v(n,["-a","-x"])||n.includes("a")||n.includes("aux");if(i){let u=["USER       PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND"],d=1e3;for(let m of r){let p=m.username.padEnd(10).slice(0,10),g=(Math.random()*.5).toFixed(1),S=Math.floor(Math.random()*2e4+5e3),C=Math.floor(Math.random()*5e3+1e3);u.push(`${p} ${String(d).padStart(6)}  0.0  ${g.padStart(4)} ${String(S).padStart(6)} ${String(C).padStart(5)} ${m.tty.padEnd(8)} Ss   00:00   0:00 bash`),d++}return u.push(`root       ${String(d).padStart(6)}  0.0   0.0      0      0 ?        S    00:00   0:00 ps`),{stdout:u.join(`
...
`),g=await Me(ke(p),t);for(let S=1;S<=d.length;S++)delete t.env.vars[String(S)];return Object.assign(t.env.vars,{...m,...t.env.vars}),g}return Z(s,t.authUser,t.hostname,t.mode,t.cwd,t.shell,void 0,t.env)})();if(t.env.lastExitCode=l.exitCode??0,l.stdout&&(r+=`${l.stdout}
...
`)){let d=u.trim();if(!d||d.startsWith("#"))continue;let m=await Z(d,t,n,"shell",r,i,void 0,s);if(c=m.exitCode??0,m.closeSession||m.switchUser)return m}return{exitCode:c}}};var Br={name:"su",description:"Switch user",category:"users",params:["[-] [-c <cmd>] [username]"],run:async({authUser:e,shell:t,args:n,hostname:r,mode:i,cwd:s})=>{let o=n.includes("-")||n.includes("-l")||n.includes("--login"),a=n.indexOf("-c"),l=a!==-1?n[a+1]:void 0,u=n.filter((d,m)=>m!==a&&m!==a+1).filter(d=>d!=="-"&&d!=="-l"&&d!=="--login").find(d=>!d.startsWith("-"))??"root";return t.users.listUsers().includes(u)?e==="root"?l?Z(l,u,r,i,o?`/home/${u}`:s,t):{switchUser:u,nextCwd:o?`/home/${u}`:void 0,exitCode:0}:t.users.isSudoer(e)?{sudoChallenge:{username:u,targetUser:u,commandLine:l??null,loginShell:o,prompt:"Password: "},exitCode:0}:{stderr:`su: permission denied
...
`),exitCode:0}}};var Hr={name:"tar",description:"Archive utility",category:"archive",params:["[-czf|-xzf|-tf] <archive> [files...]"],run:({authUser:e,shell:t,cwd:n,args:r})=>{let i=[],s=!1;for(let m of r)if(/^-[a-zA-Z]{2,}$/.test(m))for(let p of m.slice(1))i.push(`-${p}`);else if(!s&&/^[cxtdru]{1,}[a-zA-Z]*$/.test(m)&&!m.includes("/")&&!m.startsWith("-")){s=!0;for(let p of m)i.push(`-${p}`)}else i.push(m);let o=i.includes("-c"),a=i.includes("-x"),l=i.includes("-t"),c=i.indexOf("-f"),u=c!==-1?i[c+1]:i.find(m=>m.endsWith(".tar")||m.endsWith(".tar.gz")||m.endsWith(".tgz"));if(!o&&!a&&!l)return{stderr:`tar: must specify -c, -x, or -t
...
`),D.switchUser?(n=D.switchUser,p=D.nextCwd??q(n),a.users.updateSession(i,n,s)):D.nextCwd&&(p=D.nextCwd),_()}async function b(){if(!S)return;let O=S;if(O.kind==="nano"){try{let U=await vo(O.tempPath,"utf8");a.writeFileAsUser(n,O.targetPath,U)}catch{}await Co(O.tempPath).catch(()=>{})}S=null,l="",c=0,t.write(`\r
...
`),b()}),U.on("close",()=>{b()}),S={kind:"htop",targetPath:"",tempPath:"",process:U}}function k(O){l=O,c=l.length,_()}function M(O){l=`${l.slice(0,c)}${O}${l.slice(c)}`,c+=O.length,_()}function F(O,U){let z=U;for(;z>0&&!/\s/.test(O[z-1]);)z-=1;let D=U;for(;D<O.length&&!/\s/.test(O[D]);)D+=1;return{start:z,end:D}}function Q(O){let U=O.lastIndexOf("/"),z=U>=0?O.slice(0,U+1):"",D=U>=0?O.slice(U+1):O,se=ft(p,z||".");try{return a.vfs.list(se).filter(V=>!V.startsWith(".")).filter(V=>V.startsWith(D)).map(V=>{let ye=Jt.posix.join(se,V),Ee=a.vfs.stat(ye).type==="directory"?"/":"";return`${z}${V}${Ee}`}).sort()}catch{return[]}}function G(){let{start:O,end:U}=F(l,c),z=l.slice(O,c);if(z.length===0)return;let se=l.slice(0,O).trim().length===0?x.filter(pe=>pe.startsWith(z)):[],V=Q(z),ye=Array.from(new Set([...se,...V])).sort();if(ye.length!==0){if(ye.length===1){let pe=ye[0],Ee=pe.endsWith("/")?"":" ";l=`${l.slice(0,O)}${pe}${Ee}${l.slice(U)}`,c=O+pe.length+Ee.length,_();return}t.write(`\r
`),t.write(`${ye.join("  ")}\r
...
`){let V=C.buffer;if(C.buffer="",C.onPassword){let{result:pe,nextPrompt:Ee}=await C.onPassword(V,a);t.write(`\r
`),pe!==null?(C=null,pe.stdout&&t.write(pe.stdout.replace(/\n/g,`\r
`)),pe.stderr&&t.write(pe.stderr.replace(/\n/g,`\r
`)),_()):(Ee&&(C.prompt=Ee),t.write(C.prompt));return}let ye=a.users.verifyPassword(C.username,V);await N(ye);return}se>=" "&&(C.buffer+=se)}return}let U=O.toString("utf8");for(let z=0;z<U.length;z+=1){let D=U[z];if(D===""){l="",c=0,d=null,m="",t.write(`bye\r
...
`),E()})})}async function m(x){if(x.onPassword){let N=x.prompt;for(;;){let b=await Qe(c,N),E=await x.onPassword(b,j);if(E.result===null){N=E.nextPrompt??N;continue}await g(E.result);return}}let _=await Qe(c,x.prompt);if(!j.users.verifyPassword(x.username,_)){process.stderr.write(`Sorry, try again.
`);return}if(!x.commandLine){i=x.targetUser,s=q(i),r.vars.USER=i,r.vars.LOGNAME=i,r.vars.HOME=q(i),r.vars.PWD=s;return}let w=x.loginShell?q(x.targetUser):s,A=await Z(x.commandLine,x.targetUser,_e,"shell",w,j,void 0,r);await g(A)}async function p(x){let _=await Qe(c,x.prompt);if(x.confirmPrompt&&await Qe(c,x.confirmPrompt)!==_){process.stderr.write(`passwords do not match
