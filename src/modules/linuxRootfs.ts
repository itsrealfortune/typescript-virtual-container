...
function bootstrapEtc(
	...
	ensureFile(
		...
		`${[
			...
			'if [ "$(id -u)" -eq 0 ]; then',
			"  export PS1='\\[\\e[37m\\][\\[\\e[31;1m\\]\\u\\[\\e[37m\\]@\\[\\e[34;1m\\]\\h\\[\\e[0m\\] \\w]\\[\\e[31;1m\\]\\$\\[\\e[0m\\] '",
			"else",
...
export function refreshProc(
	...
	const ramCapKb =
		resourceCaps?.ramCapBytes === undefined
			? null
			: Math.floor(resourceCaps.ramCapBytes / 1024);
	const totalMemKb =
		ramCapKb === null ? hostTotalMemKb : Math.min(hostTotalMemKb, ramCapKb);
	const freeMemKb =
		ramCapKb === null
			? hostFreeMemKb
			: Math.floor(totalMemKb * (hostFreeMemKb / hostTotalMemKb));
	...
	const cpuCapCount =
		resourceCaps?.cpuCapCores === undefined
			? hostCpus.length
			: Math.min(resourceCaps.cpuCapCores, hostCpus.length);
	...
	if (network) {
		...
		const routeLines = routes.map((r) =>
			[
				...
				ipToHex(r.gateway),
				r.flags === "UG" ? "0003" : r.flags === "U" ? "0001" : "0000",
				"0",
				"0",
				"100",
				ipToHex(r.netmask),
...
function bootstrapRoot(vfs: VirtualFileSystem): void {
	...
	ensureFile(
		...
		`${[
			"# root .bashrc",
			"export PS1='\\[\\e[37m\\][\\[\\e[31;1m\\]\\u\\[\\e[37m\\]@\\[\\e[34;1m\\]\\h\\[\\e[0m\\] \\W\\[\\e[37m\\]]\\[\\e[31;1m\\]\\$\\[\\e[0m\\] '",
			"export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
