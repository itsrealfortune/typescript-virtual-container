...
export const topCommand: ShellModule = {
	...
	run: ({shell}) => {
		...
		const totalMem =
			ramCap === undefined ? hostTotalMem : Math.min(hostTotalMem, ramCap);
		const freeMem =
			ramCap === null
				? hostFreeMem
		...
		lines.push(
			`top - ${new Date().toLocaleTimeString()} up ${uptimeStr},  ${sessions.length} user(s), load average: ${loadAverages.map((l) => l.toFixed(2)).join(", ")}`
		);
		lines.push(
			`Tasks: ${sessions.length + procs.length} total,   ${procs.filter((p) => p.status === "running").length || 1} running`
		);
		...
		lines.push(
			`MiB Mem : ${memTotalMb.padStart(8)} total, ${memFreeMb.padStart(8)} free, ${memUsedMb.padStart(8)} used`
		);
		...
		lines.push(
			`MiB Swap: ${String(swapTotal).padStart(8)} total, ${String(swapFree).padStart(8)} free, ${String(swapUsed).padStart(8)} used`
		);
		...
		sessions.forEach((s, i) => {
			...
			lines.push(
				`${String(pid).padStart(5)} ${s.username.padEnd(8).slice(0, 8)}  20   0 ${String(virt).padStart(7)} ${String(res).padStart(6)} ${String(shr).padStart(6)} S  ${cpu.padStart(4)} ${mem.padStart(5)}   0:00.00 bash`
			);
		});
		...
		procs.forEach((p) => {
			...
			lines.push(
				`${String(p.pid).padStart(5)} ${p.username.padEnd(8).slice(0, 8)}  20   0 ${String(virt).padStart(7)} ${String(res).padStart(6)} ${String(shr).padStart(6)} ${status} ${cpu.padStart(4)} ${mem.padStart(5)}   0:00.00 ${p.command}`
			);
		});
