...
[typescript-virtual-container](../../modules.md) / [index](../README.md) / VirtualSshServer
...
Configuration object for the SSH server.

###### hostname?

`string` = `"typescript-vm"`

Virtual hostname used for the SSH ident and default shell label.

###### lockoutDurationMs?

`number` = `60_000`

Lockout window in ms after exceeding attempts (default: 60 000).

###### maxAuthAttempts?

`number` = `5`

Max failed attempts per IP before lockout (default: 5).
...
[`VirtualSftpServer`](VirtualSftpServer.md) \| `null` = `null`

Optional SftpMimic instance to handle SFTP subsystem requests on this SSH port.

###### shell?

[`VirtualShell`](VirtualShell.md) = `...`

Optional preconfigured virtual shell instance to reuse.
...
Promise resolved with bound listening port.
